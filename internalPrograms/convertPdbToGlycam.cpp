...
int main(int argc, char* argv[])
{
    ...
    gmml:
        log(__LINE__, __FILE__, gmml::INF, "BuildStructureByDistance()");
    }
    ...
    for (std::vector<Glycan::Monosaccharide*>::iterator mono_it = monos.begin(); mono_it != monos.end(); mono_it++)
    {
        if (local_debug > 0)
        {
            ...
            gmml::log(__LINE__, __FILE__, gmml::INF,
                      "InitiateDetectionOfCompleteSideGroupAtoms() for Mono " + std::to_string(monoNum));
            monoNum++;
        }
    ...
    for (std::vector<Glycan::Oligosaccharide*>::iterator oligo_it = oligos.begin(); oligo_it != oligos.end();
         oligo_it++)
    {
    ...
    gmml::GlycamResidueNamingMap res_map =
        assemblyA.ExtractResidueGlycamNamingMap(oligos, oligo_id_map, oligo_residue_map);
    ...
    for (unsigned int i = 0; i < oligos.size(); i++)
    {
        ...
        std::vector<std::map<MolecularModeling::Atom*, MolecularModeling::Atom*>>& all_isomorphisms =
            this_oligo_match_tracker->all_isomorphisms;
        ...
        if (all_isomorphisms.empty())
        {
            ...
            for (unsigned int j = 0; j < failures.size(); j++)
            {
                ...
                {
                    std::cout << "Failed atom: " << this_failure->failed_atom->GetResidue()->GetName() << "-"
                              << this_failure->failed_atom->GetName() << std::endl;
        ...
        else
        {
            ...
            for (std::map<MolecularModeling::Atom*, MolecularModeling::Atom*>::iterator mapit = first_match.begin();
                 mapit != first_match.end(); mapit++)
            {
