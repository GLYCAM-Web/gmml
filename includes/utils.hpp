...
namespace gmml
{
     ...
     * @param delim Series of delimiters (each delimiter character is followed by the next one in a single string
     * variable) in order to split the given string
     ...
     */
    template<typename T> inline T ConvertString(const std::string& str)
    {
        T val;
     ...
     */
    template<typename T> std::string ConvertT(const T& given_type)
    {
        std::stringstream ss;
        ss << given_type;
        return ss.str();
    ...
    /*! \fn
     * A function in order to look up the stereochemistry name of the sugar structure based on the given string version
     * of the chemical code structure
    ...
    /*! \fn
     * A function in order to search the stereochemistry lookup table and identify the closest match for the sugar
     * structure based on the given string version of the chemical code structure
     * @param code The string chemical code structure
    ...
    inline Glycan::SugarName ClosestMatchSugarStereoChemistryNameLookup(std::string code,
                                                                        std::vector<Glycan::SugarName>& closest_matches)
    {
                ...
                (target_code.find("F") != std::string::npos && code.find("F") != std::string::npos))
            {
                for (int j = 0; j < vocab_size; j++)
                {
                    ...
                    if (target_index != std::string::npos)
                    {
                        ...
                        {
                            if (pos.find(code[code_index - 1]) != std::string::npos ||
                                pos.find(target_code[target_index - 1]) != std::string::npos)
                            {
                            ...
                            {
                                if (stat.find(code[code_index + 1]) != std::string::npos ||
                                    stat.find(target_code[target_index + 1]) != std::string::npos)
                                {
                                    if (code[code_index + 1] != target_code[target_index + 1])
                                    {
                                        diff_score--;
        ...
        /// RULE2: if the input chemical code has 'd's and the matches don't have corresponding 'd's. for D sugars
        /// choose the match that has '^' instead of 'd' for that index and '_' for L sugars
                ...
                for (unsigned int i = 0; i < positions.size(); i++)
                {
                    if (positions.at(i) < name.chemical_code_string_.size())
                    {
                        ...
                        else
                        {
                            ...
                            if (current.compare("1") == 0)
                            { /// check for -1 and +1. in this case orientation is 2 positions back from the current
                              /// position
    ...
    /*! \fn
     * A function in order to look up the complex name of the sugar structure based on the given string version of the
     * chemical code structure
    ...
    //    /*! \fn
    //      * A function in order to write the information/warning/error messages produced by the program into a log
    //      file
    ...
    //    GEMSHOME=/path/to/gems\n    SH:     setenv GEMSHOME /path/to/gems\n"; 		char* gemshome_env_var =
    //    std::getenv("GEMSHOME");
    ...
    inline double** GenerateRotationMatrix(GeometryTopology::Coordinate* direction,
                                           GeometryTopology::Coordinate* parent, double angle)
    {
        ...
        rotation_matrix[0][3] = a * (v2 + w2) - u * (b * v + c * w) +
                                (u * (b * v + c * w) - a * (v2 + w2)) * cos_rotation_angle +
                                (b * w - c * v) * sin_rotation_angle;
        rotation_matrix[1][3] = b * (u2 + w2) - v * (a * u + c * w) +
                                (v * (a * u + c * w) - b * (u2 + w2)) * cos_rotation_angle +
                                (c * u - a * w) * sin_rotation_angle;
        rotation_matrix[2][3] = c * (u2 + v2) - w * (a * u + b * v) +
                                (w * (a * u + b * v) - c * (u2 + v2)) * cos_rotation_angle +
