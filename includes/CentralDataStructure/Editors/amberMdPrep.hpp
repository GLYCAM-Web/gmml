...
// We will want to process things that aren't pdb files. Also the name has changed from preprocessing to AmberMdPrep as of ~May 2023
namespace amberMdPrep
{
    bool checkForNonNaturalProteinResidues(std::vector<cds::Residue*> unknownResidues, const cds::Atom* cAtom, pdb::PreprocessorInformation &ppInfo);
}
