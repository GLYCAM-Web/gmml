...
private:
    ...
    inline Carbohydrate* GetGlycan()			 			        {return glycan_;}
    inline Residue* GetResidue() const							    {return residue_;}
    inline ResidueLinkage& GetProteinGlycanLinkage()                {return residueGlycanLinkage_;}
...
//    inline void SetProteinOverlap(double d)						{protein_overlap_ = d;}
    ...
    inline void SetInternalBondCount(unsigned long int i) {internalBondCount_ = i;}
    ...
    int CalculateOverlaps(Resolution resolutionLevel, const std::vector<Residue*> residuesA, const std::vector<Residue*> residuesB);
