...
namespace glygraph
{
    template<class T> class Node : public GenericGraphObject
    {
      public:
         ...
         *  LAMBDAS
         ***********************************************/
        // Implemented for sorting the vector of incoming edges by the source objects < operator. Uses a lambda
        // function.
      ...
      private:
         ...
         *  CONSTRUCTORS/DESTRUCTORS
         ***********************************************/
        // ToDo: Make all of these private and have T be friend. // Safeguards mistake like class prepAtom : public
        // Node<pdbAtom>;
    ...
    template<class T> inline Node<T>::~Node()
    {
    ...
    template<class T> inline std::vector<Edge<T>*> Node<T>::getOutEdges() const
    {
        std::vector<Edge<T>*> outEdgeVecToReturn;
        for (const std::unique_ptr<Edge<T>>& currOutEdge : this->outEdges_m)
        {
    ...
    template<class T> inline void Node<T>::addChild(std::string edgeName_t, T* const& childNode_t)
    {
        if (this->isNeighbor(childNode_t))
        {
            // badBehavior(__LINE__, __func__, "Trying to make create an edge between two nodes that are already
            // neighbors");
        }
    ...
    template<class T> inline bool Node<T>::isNeighbor(T* const& otherNode_t)
    {
        return this->isChildOf(otherNode_t) || this->isParentOf(otherNode_t);
    }

    template<class T> bool Node<T>::isChildOf(T* const& possibleParent_t)
    {
    ...
    template<class T> inline bool Node<T>::isParentOf(T* const& possibleChild_t)
    {
        for (const std::unique_ptr<Edge<T>>& currOutEdge : this->outEdges_m)
        {
    ...
    template<class T> inline std::vector<T*> Node<T>::getChildren() const
    {
    ...
    template<class T> inline void Node<T>::edgeConnectionUpdate()
    {
    ...
    template<class T> inline std::vector<T*> Node<T>::getParents() const
    {
    ...
    template<class T> inline T* Node<T>::getParent() const
    {
    ...
    template<class T> inline void Node<T>::removeInEdge(Edge<T>* edgeToRemove_t)
    {
        // lazyInfo(__LINE__, __func__,
    ...
    template<class T> inline Edge<T>* glygraph::Node<T>::getConnectingEdge(T* const& otherNode_t)
    {
        if (this->isNeighbor(otherNode_t))
        {
            if (this->isChildOf(otherNode_t))
            ...
            // badBehavior(__LINE__, __func__, "Found that two nodes were neihgbors but could not find the connecting
            // edge");
        }
        else
        {
