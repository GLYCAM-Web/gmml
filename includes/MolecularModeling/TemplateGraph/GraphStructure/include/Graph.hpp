...
namespace glygraph
{
    ...
    template<class T> class Graph
    {
      public:
        ...
        Node<T>* getNodeFromIndex(const unsigned int& queryIndex_t);
        /************************************************
         *  MUTATORS
         ***********************************************/
    ...
    template<class T> inline Graph<T>::Graph(Node<T>* const& initialNode_t)
    {
    ...
    template<class T> Graph<T>::Graph(T* const& initialNode_t)
    {
        this->allNodes_m.push_back(initialNode_t);
        this->dfsWalk(initialNode_t);
    ...
    template<class T> inline Graph<T>::Graph(const std::vector<Node<T>*>& nodeList_t)
    {
    ...
    template<class T> inline void Graph<T>::populateLookups()
    {
    ...
    template<class T> inline std::vector<Node<T>*> Graph<T>::getReachableNodes(Node<T>* const& startingNode_t)
    {
        std::unordered_set<Node<T>*> visitedNodes;
        // TODO: Please note that this current method does increase the size of our call stack a good bit due to the use
        // of
        // recursion. 			Depending on how large of graphs we are dealing with this could become an issue
        // and it may be a better 			call to use a different method.
    ...
    template<class T> inline unsigned int Graph<T>::getIndexFromNode(Node<T>* const& queryNode_t)
    {
        return this->indexLookup_m[queryNode_t];
    }

    template<class T> inline Node<T>* Graph<T>::getNodeFromIndex(const unsigned int& queryIndex_t)
    {
    ...
    //
    template<class T> inline void Graph<T>::lazyExpiredFixer()
    {
    ...
    template<class T> inline void Graph<T>::dfsWalk(Node<T>* const& startNode_t)
    {
        if (std::find(this->allNodes_m.begin(), this->allNodes_m.end(), startNode_t) != this->allNodes_m.end())
        {
            std::unordered_set<Node<T>*> visitedNodes;
            // TODO: Please note that this current method does increase the size of our call stack a good bit due to the
            // use of
            // recursion. 			Depending on how large of graphs we are dealing with this could become
            // an issue and it may be a better 			call to use a different method.
