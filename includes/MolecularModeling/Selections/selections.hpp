...
namespace selection
{
    ...
    std::vector<MolecularModeling::Residue*> FindResiduesWithChainID(MolecularModeling::Assembly& ass,
                                                                     std::string inputChainID);
    AtomVector AtomsWithinDistanceOf(Atom* query_atom, double distance, AtomVector atoms);
    void FindAtomsConnectingResidues(Atom* current_atom, Residue* second_residue, AtomVector* connecting_atoms,
                                     bool* found_neighbor);
    // void FindAtomsInPathToCycle(Atom *previous_atom, Atom *current_atom, AtomVector *atom_path, bool
    // *found_cycle_point, Atom *&cycle_point); void FindAtomsInPathToBackboneNAtom(Atom *previous_atom, Atom
    // *current_atom, AtomVector *atom_path, bool *found_N_atom); Pass pointer by reference in *&cycle_point, as I need
    // to modify the actual pointer and not a copy of it.
    bool FindCyclePoint(Atom* previous_atom, Atom* current_atom, AtomVector* atom_path, bool* found_cycle_point,
                        Atom*& cycle_point);
