...
namespace MolecularModeling
{
    ...
    class AtomNode;
    class Atom;
    typedef std::vector<MolecularModeling::Atom*>
        AtomVector; // Do this here so it can be used within the class, and is known
    ...
    class Atom : public MolecularDynamicAtom, public OligoSaccharideDetectionAtom
    {
      public:
        ...
        /*! \fn                                     //Added by ayush on 12/11/17 for molecules in assembly for atom type
         * like O,H An accessor function in order to access to the atom type
         ...
         * @param is_exocyclic_C_ The boolean value representing if the current atom object is in an exocyclic carbon or
         * not
         */
        ...
        std::multimap<int, Atom*>
        GetAtomicNumbersOfNeighbors(AtomVector& neighbors,
                                    AtomVector& visited_atoms); // A helper function for chirality determination
        int CompareBranches(std::vector<int>& vec1, std::vector<int>& vec2);
        std::vector<std::vector<int>>
        SortBranchesInDescendingOrder(std::vector<std::vector<int>>& originally_ordered_branches);
        std::vector<std::pair<Atom*, int>> SortPrimaryNeighborBranchesInDescendingOrder(
        ...
        std::vector<AtomVector> MakeNextLevelOfBranches(std::vector<AtomVector>& current_branches,
                                                        AtomVector& visited_atoms);
        void RecursivelyCompareBranches(std::map<std::vector<int>, std::vector<int>>& duplicate_higher_indices_map,
                                        std::vector<int>& ranks, AtomVector visited_atoms,
        ...
        void InitializeComparisonTracker(
            std::map<std::vector<int>, std::vector<int>>& duplicate_value_indices_versus_higher_rank_indices,
            AtomVector& visisted_atoms, std::map<Atom*, std::vector<AtomVector>>& comparison_progress_tracker);
      ...
      private:
        ...
        // MolecularModeling::AtomNode* node_;                   /*!< A Pointer to a node of the graph structure that
        // indicates this atom >*/
        std::vector<MolecularModeling::AtomNode*> nodes_;
        std::string id_; /*!< An identifier for an atom which is generated based on the type of the input file from
                  which the structure has to be built Mostly it is like "residue_name:atom_name" >*/
        bool is_ring_;   /*!< A boolean value which represents if an atom is involved in a sugar ring or not. This
                            attribute is set during the Sugar ID process >*/
        bool is_exocyclic_C_ = false; /*!< A boolean value which represents if an atom is an exocyclic carbon of a
                                         sugar. This attribute is set during the Sugar ID process >*/
        unsigned long long index_;    /*!< A unqiue index for each atom in an assembly >*/
        std::string atom_type_;
        /*!< List the atom type in an assembly >*/ // Added by ayush on 13/11/17 for molecules in assembly to set
                                                   // the atom type as an attribute like O,H
        float b_factor_;
        /*!< Gives the B Factor for the atom >*/ // Added by Dave on 03/23/18 for adding B Factor to ontology
        ...
        void SetAttributes(MolecularModeling::Residue* residue, std::string name,
                           GeometryTopology::CoordinateVector coordinates, std::string chemical_type,
                           std::string description, std::string element_symbol, std::vector<AtomNode*> atomnode,
                           std::string id, bool is_ring, std::string atom_type);
    };
