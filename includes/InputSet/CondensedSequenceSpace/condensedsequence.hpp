...
namespace CondensedSequenceSpace
{
    ...
    class CondensedSequence
    {
      public:
        ...
        enum class Reordering_Approach
        {
            PRESERVE_USER_INPUT,
            LOWEST_INDEX,
            LONGEST_CHAIN
        ...
        //                        FUNCTIONS                     //
        //////////////////////////////////////////////////////////
        int InsertNodeInCondensedSequenceResidueTree(CondensedSequenceResidue* condensed_residue, int parent_node_id,
                                                     int bond_id);
        int ReEvaluateParentIdentityUponAnomericAnomericLinkage();
        void RecursivelyCountNumberofDownstreamResiduesAndBranches(int parent_residue_index, int& num_residues,
                                                                   int& num_branches);
        void DetectAnomericAnomericLinkages();
        int
        InsertNodeInCondensedSequenceGlycam06ResidueTree(CondensedSequenceGlycam06Residue* condensed_glycam06_residue,
        ...
        void RecursivelyLabelCondensedSequence(
            int current_residue_index, int& current_resdiue_label_index, int& current_bond_label_index,
        ...
        void RecursivelyBuildLabeledCondensedSequence(
        ...
        std::string GetGlycam06ResidueCodeOfCondensedResidue(CondensedSequenceResidue* condensed_residue,
        ...
        std::string BuildLabeledCondensedSequence(Reordering_Approach labeling_approach,
                                                  Reordering_Approach reordering_approach, bool label);
        CondensedSequenceGlycam06Residue* GetCondensedSequenceDerivativeGlycam06Residue(std::string derivative_name,
                                                                                        int derivative_index);
        CondensedSequenceRotamersAndGlycosidicAnglesInfo
        GetCondensedSequenceRotamersAndGlycosidicAnglesInfo(CondensedSequenceResidueTree residue_tree);
        int CountAllPossibleSelectedRotamers(
            CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info);
        int CountAllPossible28LinkagesRotamers(
            CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info);
        std::vector<std::vector<int>> CreateBaseMapAllPossibleSelectedRotamers(
            CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info);
        IndexLinkageConfigurationMap CreateIndexLinkageConfigurationMap(
