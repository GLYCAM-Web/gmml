...
// Created to serve old gmml's setup. I don't want to pass out any classes like prep file or library file, they should
// only be used internally by a 3dTemplate class.
...
namespace parameters
{
    class Manager
    {
      public:
        ...
        inline std::map<std::string, std::string> GetAllResidueNameMap()
        {
            return residueNamesToSelfMap_;
        }

        inline std::map<std::string, std::vector<std::string>> GetResidueNamesToTheirAtomNamesMap()
        {
            return residueNamesToTheirAtomsNameMap_;
        }

        // Ok don't do this, figure out what they want and implement that query here:
        inline std::map<std::string, LibraryFileSpace::LibraryFileResidue*> GetLibraryResidueMap()
        {
            return libResMap_;
        }

        inline std::map<std::string, PrepFileSpace::PrepFileResidue*> GetPrepResidueMap()
        {
            return prepResMap_;
        }

        inline std::vector<std::string> GetGlycamResidueNames()
        {
            return glycamResidueNames_;
        }
