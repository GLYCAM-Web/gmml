...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //                PdbFileSpace::PdbFile::PdbPairVectorAtomNamePositionFlag residues =
        //                temp->GetAllResidueNames();
        //        for(PdbFileSpace::PdbFile::PdbPairVectorAtomNamePositionFlag::iterator it = residues.begin(); it !=
        //        residues.end(); it++)
        //        {
        ...
        //        \n"; PdbAtom* atom = new PdbAtom(line); temp->DeleteAtom(atom);
        ...
        //        PdbAtomCard::PdbAtomMap atom_card =
        //        (*(((((*(temp->GetModels()->GetModels()).begin()).second)->GetModelResidueSet())->GetAtoms()).begin()))->GetAtoms();
        //        int serial_number = 1;
        ...
        //            {
        //                new_atom_map[serial_number] = new PdbAtom(serial_number,atom->GetAtomName(), ' ',
        //                atom->GetAtomResidueName(), 'A', 34,
        //                                                          ' ', atom->GetAtomOrthogonalCoordinate(),
        //                                                          atom->GetAtomOccupancy(),
        //                                                          atom->GetAtomTempretureFactor(),
        //                                                          atom->GetAtomElementSymbol(),
        //                                                          atom->GetAtomCharge());
        //                serial_number++;
        //            }
        //        }
        ...
        //        std::cout << atom->GetAtomOrthogonalCoordinate().Distance(atom1->GetAtomOrthogonalCoordinate()) <<
        //        std::endl;
    }
    return 0;
}
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        std::vector<std::string> recognized_residue_names =
            pdb_preprocessor->GetRecognizedResidueNames(pdb_residue_names, dataset_residue_names);
        std::vector<std::string> unrecognized_residue_names =
            pdb_preprocessor->GetUnrecognizedResidueNames(pdb_residue_names, dataset_residue_names);

        std::cout << "********************* Recognized Residue Names ******************" << std::endl;
        for (std::vector<std::string>::iterator it = recognized_residue_names.begin();
             it != recognized_residue_names.end(); it++)
        {
            std::cout << *it << std::endl;
        }

        std::cout << "********************* Unrecognized Residue Names ******************" << std::endl;
        for (std::vector<std::string>::iterator it = unrecognized_residue_names.begin();
             it != unrecognized_residue_names.end(); it++)
        {
            std::cout << *it << std::endl;
        }
        ...
        std::vector<PdbResidue*> recognized_residues =
            pdb_preprocessor->GetRecognizedResidues(pdb_residues, recognized_residue_names);
        std::vector<PdbResidue*> unrecognized_residues =
            pdb_preprocessor->GetUnrecognizedResidues(pdb_residues, unrecognized_residue_names);
        ...
        for (std::vector<PdbResidue*>::iterator it = unrecognized_residues.begin(); it != unrecognized_residues.end();
             it++)
        {
            (*it)->Print(std::cout);
        }
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        PdbPreprocessor::PdbPreprocessorUnrecognizedResidueVector unrecognized_residues =
            pdb_preprocessor->GetUnrecognizedResidues();
        for (PdbPreprocessor::PdbPreprocessorUnrecognizedResidueVector::iterator it = unrecognized_residues.begin();
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        PdbPreprocessor::PdbPreprocessorRecognizedResidueVector recognized_residues =
            pdb_preprocessor->GetRecognizedResidues();
        for (PdbPreprocessor::PdbPreprocessorRecognizedResidueVector::iterator it = recognized_residues.begin();
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        for (PdbPreprocessor::PdbPreprocessorDisulfideBondVector::iterator it = cys_residues.begin();
             it != cys_residues.end(); it++)
        {
            (*it)->Print(std::cout);
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        for (PdbPreprocessor::PdbPreprocessorHistidineMappingVector::iterator it = his_residues.begin();
             it != his_residues.end(); it++)
        {
            (*it)->Print(std::cout);
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        PdbPreprocessor::PdbPreprocessorUnrecognizedHeavyAtomVector unknown_heavy_atoms =
            pdb_preprocessor->GetUnrecognizedHeavyAtoms();
        for (PdbPreprocessor::PdbPreprocessorUnrecognizedHeavyAtomVector::iterator it = unknown_heavy_atoms.begin();
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        PdbPreprocessor::PdbPreprocessorReplacedHydrogenVector removed_hydrogen_atoms =
            pdb_preprocessor->GetReplacedHydrogens();
        for (PdbPreprocessor::PdbPreprocessorReplacedHydrogenVector::iterator it = removed_hydrogen_atoms.begin();
             it != removed_hydrogen_atoms.end(); it++)
        {
            (*it)->Print(std::cout);
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        for (PdbPreprocessor::PdbPreprocessorChainTerminationVector::iterator it = chains.begin(); it != chains.end();
             it++)
        {
            (*it)->Print(std::cout);
        }
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        PdbPreprocessor::PdbPreprocessorAlternateResidueMap alternate_residues =
            pdb_preprocessor->GetAlternateResidueMap();
        for (PdbPreprocessor::PdbPreprocessorAlternateResidueMap::iterator it = alternate_residues.begin();
             it != alternate_residues.end(); it++)
        {
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        PdbFile* pdb_file = new PdbFile(pdb_file_path);
        pdb_preprocessor->UpdateGapsInAminoAcidChains(pdb_file, lib_files_paths,
                                                      pdb_preprocessor->GetMissingResidues());
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        for (int i = 0; i < number_of_prep_files; i++)
        {
            prep_files_paths.push_back(
                argv[6 + number_of_amino_lib_files + number_of_glycam_lib_files + number_of_other_lib_files + i]);
        }

        PdbFile* pdb_file                 = new PdbFile(pdb_file_path);
        PdbPreprocessor* pdb_preprocessor = new PdbPreprocessor();
        //        pdb_preprocessor->ExtractGapsInAminoAcidChains(pdb_file, lib_files_paths);
        pdb_preprocessor->Preprocess(pdb_file, amino_lib_files_paths, glycam_lib_files_paths, other_lib_files_paths,
                                     prep_files_paths);
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //        gmml::BuildingStructureOption building_type =
        //        gmml::ConvertString2AssemblyBuildingStructureOption(argv[3]); Assembly* temp = new Assembly(file_path,
        //        type);
        ...
        //        temp->BuildCoordinateFileStructureFromAssembly(); coordinate_file->Write("coordinate.text");
        ...
        //        temp->BuildTopologyFileStructureFromAssembly(argv[2]); topology_file->Write("top_file.parm7");
        //        CoordinateFileSpace::CoordinateFile* coordinate_file =
        //        temp->BuildCoordinateFileStructureFromAssembly(); coordinate_file->Write("crd_file.rst7");
        ...
        //        temp->BuildAssemblyFromPdbFile("pdb_file.pdb", std::vector<std::string>(), std::vector<std::string>(),
    ...
    else
    {
        ...
        if (temp->CheckCondensedSequenceSanity(argv[1], prep_residues))
        {
            ...
            CondensedSequenceSpace::CondensedSequence* condensed_sequence =
                new CondensedSequenceSpace::CondensedSequence(argv[1]);
            ...
            std::cout << condensed_sequence->CountAllPossible28LinkagesRotamers(rotamers_glycosidic_angles_info) <<
            std::endl;*/
            CondensedSequenceSpace::CondensedSequence::IndexNameMap names =
                CondensedSequenceSpace::CondensedSequence::IndexNameMap();
            Assembly::AssemblyVector structures = temp->BuildAllRotamersFromCondensedSequence(
                argv[1], argv[2], argv[3], rotamers_glycosidic_angles_info, names);
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //        assembly->Print(std::cout);
        //        Assembly::AtomVector atoms =
        //        assembly1->Select("1.*:#520,MAN,GAL@#3740-3750,^C:ALA@O1;1.1.2,1.1.3:NAG@O$");
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //        TopologyFileSpace::TopologyFile* top_file_ion = temp->BuildTopologyFileStructureFromAssembly(argv[3],
        //        argv[6]); top_file_ion->Write("top_file_ion.top"); LibraryFileSpace::LibraryFile* lib_file_ion =
        //        temp->BuildLibraryFileStructureFromAssembly(); lib_file_ion->Write("lib_file_ion.lib");
        ...
        //        TopologyFileSpace::TopologyFile* top_file_ion = temp->BuildTopologyFileStructureFromAssembly(argv[4],
        //        argv[7]); top_file_ion->Write("top_file_ion.top"); PrepFileSpace::PrepFile* prep_file_ion =
        //        temp->BuildPrepFileStructureFromAssembly(argv[4]); prep_file_ion->Write("prep_file_ion.prep");
        ...
        //        CoordinateFileSpace::CoordinateFile* crd_file_ion = temp->BuildCoordinateFileStructureFromAssembly();
        //        crd_file_ion->Write("crd_file_ion.crd");
        //        TopologyFileSpace::TopologyFile* top_file_ion = temp->BuildTopologyFileStructureFromAssembly(argv[2],
        //        argv[5]); top_file_ion->Write("top_file_ion.top"); LibraryFileSpace::LibraryFile* lib_file_ion =
        //        temp->BuildLibraryFileStructureFromAssembly(); lib_file_ion->Write("lib_file_ion.lib");
        ...
        //        TopologyFileSpace::TopologyFile* top_file = temp->BuildTopologyFileStructureFromAssembly(argv[2],
        //        argv[5]); top_file->Write("top_file.top"); LibraryFileSpace::LibraryFile* lib_file =
        //        temp->BuildLibraryFileStructureFromAssembly(); lib_file->Write("lib_file.lib");
        //        temp->Ionizing(argv[3], argv[4], argv[5], gmml::ConvertString<int>(argv[6]));
        //        PrepFileSpace::PrepFile* prep_file_ion = temp->BuildPrepFileStructureFromAssembly(argv[2]);
        //        prep_file_ion->Write("prep_file_ion.prep");
        //        PdbFileSpace::PdbFile* pdb_file_ion = temp->BuildPdbFileStructureFromAssembly();
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        //        temp->Print();
        //        std::cout << solvated->GetAssemblies().at(0)->GetId() << " " <<
        //        solvated->GetAssemblies().at(1)->GetId() << std::endl;
