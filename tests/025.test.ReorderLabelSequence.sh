...
if [[ "${GMML_ROOT_DIR}" != *"gmml" ]]; then
    echo -e "Test 009 failed, we think our GMML root directory is:\t${GMML_ROOT_DIR}\n"
    exit 1
fi
...
{
    ./"${Executable}" "${SequenceOne}"
    printf "\n***********************************************************************\n\n"
    ./"${Executable}" "${SequenceTwo}"
    printf "\n***********************************************************************\n\n"
    ./"${Executable}" "${SequenceThree}"
    printf "\n***********************************************************************\n\n"
    ./"${Executable}" "${SequenceFour}"
} >"${OutputFile}" 2>&1
