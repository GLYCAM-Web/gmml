...
export CCDHOME="/programs/repos/CCD/ligand-dict-v5"
...
run_PDB() {
	...
	# If the file is in the Updated PDB List (AKA the new files rsync copied since last time), then run script.
	# -q stops grep from writing output
	if grep -qi "$FILENAME" UpdatedPDBs.txt; then
		...
		# Run the pdb file through detect_sugars

		#python3 ${GEMSHOME}/testbin/cycle.py -amino_libs ${AMINO_LIBS} -pdb ${FILEDEST} &> ${REPDEST} & pid=$!
			
		${GEMSHOME}/gmml/tests/archiveRun_detect_sugars ${FILEDEST} &> ${REPDEST} 2>&1 && \
			rm -f ${FILEDEST} >> /dev/null 2>&1 & pid=$!
		
		# Some of the PDBs cause our code to get stuck in infinite loops.
