...
run_PDB() {
	...
	FILEPATH="$(echo "/home/installer/failedPDBs/PDBTestSet/$(basename ${FILEPATH})")"
	cp "${FILEPATH}" "${DEST}"
...
##	ZIPFILE="$(echo "${FILEPATH}" | cut -c${CUT_LENGTH}-100)"
...
##	FILENAME="$(echo "${FILENAME}" | tr [:lower:] [:upper:])"
	FILEDEST="$(echo "${PDBDEST}/${FILENAME}")"
...
##	mv "${ENTDEST}" "${FILEDEST}"
	...
	(sleep 120 && kill -HUP $pid ) 2>/dev/null & watcher=$!
