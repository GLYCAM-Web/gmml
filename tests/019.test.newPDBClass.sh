...
for filepath in $(ls tests/inputs/019.*.pdb); do
    ...
    if ! cmp output.txt tests/correct_outputs/"${file}"-output.txt >/dev/null 2>&1; then
        echo -e "Test FAILED!. output.txt different from tests/correct_outputs/${file}-output.txt\n Compare using diff\n"
        echo "Exit Code: 1"
        return 1
        exit 1
    elif ! cmp outputPdbFile.pdb tests/correct_outputs/"${file}"-output.pdb >/dev/null 2>&1; then
        echo -e "Test FAILED!. outputPdbFile.pdb different from tests/correct_outputs/${file}-output.pdb\n Compare using diff or VMD\n"
        echo "Exit Code: 1"
        return 1
        exit 1
    fi
