...
PdbPreprocessorResidueInfo::PdbPreprocessorResidueInfo(std::string residue_name, char chain_id, int sequence_number,
...
void PdbPreprocessorResidueInfo::Print(std::ostream& out)
{
    out << "Residue name: " << residue_name_ << ", Chain id: " << residue_chain_id_
        ...
        << ", Alternate location: " << residue_alternate_location_ << ", Residue charge: " << std::fixed
        << std::setprecision(2) << residue_charge_ << std::endl;
}
