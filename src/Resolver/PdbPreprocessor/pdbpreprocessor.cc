...
gmml::ResidueNameMap PdbPreprocessor::GetUnrecognizedResidueNamesMap(
    PdbFileSpace::PdbFile::PdbPairVectorAtomNamePositionFlag pdb_residue_names,
    gmml::ResidueNameMap dataset_residue_names)
{
    ...
    for (PdbFileSpace::PdbFile::PdbPairVectorAtomNamePositionFlag::iterator it = pdb_residue_names.begin();
         it != pdb_residue_names.end(); it++)
    {
...
gmml::ResidueNameMap PdbPreprocessor::GetRecognizedResidueNamesMap(
    PdbFileSpace::PdbFile::PdbPairVectorAtomNamePositionFlag pdb_residue_names,
    gmml::ResidueNameMap dataset_residue_names)
{
...
PdbFileSpace::PdbFile::PdbResidueVector
PdbPreprocessor::GetUnrecognizedResidues(PdbFileSpace::PdbFile::PdbResidueVector pdb_residues,
                                         gmml::ResidueNameMap unrecognized_residue_names)
{
...
PdbFileSpace::PdbFile::PdbResidueVector
PdbPreprocessor::GetRecognizedResidues(PdbFileSpace::PdbFile::PdbResidueVector pdb_residues,
                                       gmml::ResidueNameMap recognized_residue_names)
{
...
bool PdbPreprocessor::ExtractUnrecognizedResidues()
{
    ...
    std::map<std::string, std::string> unrecognized_residue_names =
        GetUnrecognizedResidueNamesMap(pdb_residue_names, dataset_residue_names);

    std::vector<PdbFileSpace::PdbResidue*> pdb_residues = this->GetPdbFile().GetAllResidues();
    std::vector<PdbFileSpace::PdbResidue*> unrecognized_residues =
        GetUnrecognizedResidues(pdb_residues, unrecognized_residue_names);
    ...
    for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = pdb_residues_atom_card.begin();
         it != pdb_residues_atom_card.end(); it++)
    {
    ...
    for (PdbPreprocessorChainIdResidueMap::iterator it = all_chain_map_residue.begin();
         it != all_chain_map_residue.end(); it++)
    {
        ...
        for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it1 = unrecognized_residues.begin();
             it1 != unrecognized_residues.end(); it1++)
        {
            PdbFileSpace::PdbResidue* pdb_residue                    = (*it1);
            PdbPreprocessorUnrecognizedResidue* unrecognized_residue = new PdbPreprocessorUnrecognizedResidue(
                pdb_residue->GetResidueName(), pdb_residue->GetResidueChainId(),
                pdb_residue->GetResidueSequenceNumber(), pdb_residue->GetResidueInsertionCode(),
                pdb_residue->GetResidueAlternateLocation(), false);
            ...
            {
                for (PdbPreprocessorChainIdResidueMap::iterator it2 = chain_map_residue.begin();
                     it2 != chain_map_residue.end(); it2++)
                {
                    ...
                    {
                        if (find(pdb_residues_in_chain.begin(), pdb_residues_in_chain.end(), pdb_residue) !=
                            pdb_residues_in_chain.end())
                        {
...
void PdbPreprocessor::RemoveUnrecognizedResiduesWithTheGivenModelNumber(
    PdbPreprocessorUnrecognizedResidueVector unrecognized_residues, int model_number)
{
    to_be_deleted_atoms_.clear();
    to_be_deleted_residues_.clear();
    for (PdbPreprocessorUnrecognizedResidueVector::iterator it = unrecognized_residues.begin();
         it != unrecognized_residues.end(); it++)
    {
        PdbPreprocessorUnrecognizedResidue* unrecognized_residue = (*it);
        PdbFileSpace::PdbResidue* pdb_residue                    = new PdbFileSpace::PdbResidue(
            unrecognized_residue->GetResidueName(), unrecognized_residue->GetResidueChainId(),
            unrecognized_residue->GetResidueSequenceNumber(), unrecognized_residue->GetResidueInsertionCode(),
            unrecognized_residue->GetResidueAlternateLocation());
...
PdbFileSpace::PdbFile::PdbResidueVector
PdbPreprocessor::GetAllCYSResidues(PdbFileSpace::PdbFile::PdbResidueVector pdbResidues)
{
...
double PdbPreprocessor::GetDistanceofCYS(PdbFileSpace::PdbResidue* first_residue,
                                         PdbFileSpace::PdbResidue* second_residue,
...
{
    double distance = 0.0;
    PdbFileSpace::PdbAtomCard* first_residue_sulfur_atom =
        this->GetPdbFile().GetAtomOfResidueByName(first_residue, "SG", residue_atom_map);
    PdbFileSpace::PdbAtomCard* second_residue_sulfur_atom =
        this->GetPdbFile().GetAtomOfResidueByName(second_residue, "SG", residue_atom_map);
    ...
    {
        distance = first_residue_sulfur_atom->GetAtomOrthogonalCoordinate().Distance(
            second_residue_sulfur_atom->GetAtomOrthogonalCoordinate());
    }
    return distance;
}
...
bool PdbPreprocessor::ExtractCYSResidues()
{
    ...
    for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = cys_residues.begin(); it != cys_residues.end(); it++)
    {
        ...
        for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it1 = it + 1; it1 != cys_residues.end(); it1++)
        {
            ...
            double distance = GetDistanceofCYS(first_residue, second_residue, residue_atom_map,
                                               first_sulfur_atom_serial_number, second_sulfur_atom_serial_number);
            if (distance < gmml::dSulfurCutoff)
            {
                PdbPreprocessorDisulfideBond* disulfide_bond = new PdbPreprocessorDisulfideBond(
                    first_residue->GetResidueChainId(), second_residue->GetResidueChainId(),
                    first_residue->GetResidueSequenceNumber(), second_residue->GetResidueSequenceNumber(), distance,
                    true, first_residue->GetResidueInsertionCode(), second_residue->GetResidueInsertionCode(),
...
void PdbPreprocessor::UpdateCYSResiduesWithTheGivenModelNumber(PdbPreprocessorDisulfideBondVector disulfide_bonds,
                                                               int model_number)
{
    to_be_deleted_atoms_.clear();
    to_be_deleted_residues_.clear();
    ...
    for (PdbPreprocessorDisulfideBondVector::iterator it = disulfide_bonds.begin(); it != disulfide_bonds.end(); it++)
    {
        ...
        {
            PdbFileSpace::PdbAtomCard* pdb_atom_1 = new PdbFileSpace::PdbAtomCard(
                disulfide_bond->GetResidueChainId1(), "HG", "CYS", disulfide_bond->GetResidueSequenceNumber1(),
                disulfide_bond->GetResidueInsertionCode1(), disulfide_bond->GetResidueAlternateLocation1());
            //            this->GetPdbFile().DeleteAtomWithTheGivenModelNumber(pdb_atom_1, model_number);
            to_be_deleted_atoms_.push_back(pdb_atom_1);
            PdbFileSpace::PdbAtomCard* pdb_atom_2 = new PdbFileSpace::PdbAtomCard(
                disulfide_bond->GetResidueChainId2(), "HG", "CYS", disulfide_bond->GetResidueSequenceNumber2(),
                disulfide_bond->GetResidueInsertionCode2(), disulfide_bond->GetResidueAlternateLocation2());
            ...
            ss_1 << "CYS"
                 << "_" << disulfide_bond->GetResidueChainId1() << "_" << disulfide_bond->GetResidueSequenceNumber1()
                 << "_" << disulfide_bond->GetResidueInsertionCode1() << "_"
                 << disulfide_bond->GetResidueAlternateLocation1();
            target_key1 = ss_1.str();
            std::string target_key2;
            std::stringstream ss_2;
            ss_2 << "CYS"
                 << "_" << disulfide_bond->GetResidueChainId2() << "_" << disulfide_bond->GetResidueSequenceNumber2()
                 << "_" << disulfide_bond->GetResidueInsertionCode2() << "_"
                 << disulfide_bond->GetResidueAlternateLocation2();
            target_key2  = ss_2.str();
            pdb_residues = this->GetPdbFile().GetAllResidues();
            for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it1 = pdb_residues.begin();
                 it1 != pdb_residues.end(); it1++)
            {
                PdbFileSpace::PdbResidue* pdb_residue = (*it1);
                ...
                if ((residue_name).compare("CYS") == 0)
                {
                    ...
                    ss << residue_name << "_" << pdb_residue->GetResidueChainId() << "_"
                       << pdb_residue->GetResidueSequenceNumber() << "_" << pdb_residue->GetResidueInsertionCode()
            ...
            PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap bonded_atoms_serial_numbers_map =
                pdb_connect_card->GetBondedAtomsSerialNumbers();
            if (bonded_atoms_serial_numbers_map.find(disulfide_bond->GetSulfurAtomSerialNumber1()) !=
                bonded_atoms_serial_numbers_map.end())
            {
                bonded_atoms_serial_numbers_map[disulfide_bond->GetSulfurAtomSerialNumber1()].push_back(
                    disulfide_bond->GetSulfurAtomSerialNumber2());
            }
            else
            {
            ...
            if (bonded_atoms_serial_numbers_map.find(disulfide_bond->GetSulfurAtomSerialNumber2()) !=
                bonded_atoms_serial_numbers_map.end())
            {
                bonded_atoms_serial_numbers_map[disulfide_bond->GetSulfurAtomSerialNumber2()].push_back(
                    disulfide_bond->GetSulfurAtomSerialNumber1());
            }
            else
            {
...
PdbFileSpace::PdbFile::PdbResidueVector
PdbPreprocessor::GetAllHISResidues(PdbFileSpace::PdbFile::PdbResidueVector pdb_residues)
{
...
bool PdbPreprocessor::ExtractHISResidues()
{
    ...
    for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = his_residues.begin(); it != his_residues.end(); it++)
    {
        ...
        // gmml::HIE residue
        if (this->GetPdbFile().GetAtomOfResidueByName(his_residue, "HE2") != NULL &&
            this->GetPdbFile().GetAtomOfResidueByName(his_residue, "HD1") == NULL)
        {
            PdbPreprocessorHistidineMapping* histidine_mapping = new PdbPreprocessorHistidineMapping(
                his_residue->GetResidueChainId(), his_residue->GetResidueSequenceNumber(), gmml::HIE,
                his_residue->GetResidueInsertionCode(), his_residue->GetResidueAlternateLocation());
            this->AddHistidineMapping(histidine_mapping);
        }
        // gmml::HID residue
        else if (this->GetPdbFile().GetAtomOfResidueByName(his_residue, "HE2") == NULL &&
                 this->GetPdbFile().GetAtomOfResidueByName(his_residue, "HD1") != NULL)
        {
            PdbPreprocessorHistidineMapping* histidine_mapping = new PdbPreprocessorHistidineMapping(
                his_residue->GetResidueChainId(), his_residue->GetResidueSequenceNumber(), gmml::HID,
                his_residue->GetResidueInsertionCode(), his_residue->GetResidueAlternateLocation());
            this->AddHistidineMapping(histidine_mapping);
        }
        // gmml::HIP residue
        ...
        {
            PdbPreprocessorHistidineMapping* histidine_mapping = new PdbPreprocessorHistidineMapping(
                his_residue->GetResidueChainId(), his_residue->GetResidueSequenceNumber(), gmml::HIP,
                his_residue->GetResidueInsertionCode(), his_residue->GetResidueAlternateLocation());
            this->AddHistidineMapping(histidine_mapping);
        }
        else if (this->GetPdbFile().GetAtomOfResidueByName(his_residue, "HE2") == NULL &&
...
void PdbPreprocessor::UpdateHISMappingWithTheGivenNumber(PdbPreprocessorHistidineMappingVector histidine_mappings,
                                                         int model_number)
{
    to_be_deleted_atoms_.clear();
    to_be_deleted_residues_.clear();
    PdbFileSpace::PdbFile::PdbResidueVector pdb_residues = this->GetPdbFile().GetAllResidues();
    for (PdbPreprocessorHistidineMappingVector::iterator it = histidine_mappings.begin();
         it != histidine_mappings.end(); it++)
    {
        PdbPreprocessorHistidineMapping* histidine_mapping = (*it);
        std::string target_key;
        std::stringstream ss;
        ss << "HIS"
           << "_" << histidine_mapping->GetResidueChainId() << "_" << histidine_mapping->GetResidueSequenceNumber()
           << "_" << histidine_mapping->GetResidueInsertionCode() << "_"
           << histidine_mapping->GetResidueAlternateLocation();
        target_key = ss.str();
        for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it1 = pdb_residues.begin(); it1 != pdb_residues.end();
             it1++)
        {
            PdbFileSpace::PdbResidue* pdb_residue = (*it1);
            ...
            if ((residue_name).compare("HIS") == 0)
            {
                ...
                std::string pdb_residue_key;
                ss1 << residue_name << "_" << pdb_residue->GetResidueChainId() << "_"
                    << pdb_residue->GetResidueSequenceNumber() << "_" << pdb_residue->GetResidueInsertionCode() << "_"
                ...
                if (pdb_residue_key.compare(target_key) == 0)
                {
                    ...
                    if (HE2 != NULL && HD1 == NULL)
                    {
                        ...
                        {
                            // Delete HE2
                            //                            this->GetPdbFile().DeleteAtomWithTheGivenModelNumber(HE2,
                            //                            model_number);
                    ...
                    if (HE2 == NULL && HD1 != NULL)
                    {
                        ...
                        {
                            // Delete HD1
                            //                            this->GetPdbFile().DeleteAtomWithTheGivenModelNumber(HD1,
                            //                            model_number);
                    ...
                    this->GetPdbFile().UpdateResidueNameWithTheGivenModelNumber(
                        pdb_residue, histidine_mapping->GetStringFormatOfSelectedMapping(), model_number);
                }
            }
        }
    }
...
PdbFileSpace::PdbFile::PdbAtomCardVector
PdbPreprocessor::GetUnknownHeavyAtomsOfResidue(PdbFileSpace::PdbFile::PdbAtomCardVector pdb_atoms,
                                               std::vector<std::string> dataset_atom_names_of_residue)
{
    ...
    for (PdbFileSpace::PdbFile::PdbAtomCardVector::iterator it = pdb_atoms.begin(); it != pdb_atoms.end(); it++)
    {
        ...
        if (!(pdb_atom_name.substr(0, 1).compare("H") == 0 ||
              (pdb_atom_name.substr(1, 1).compare("H") == 0 &&
               isdigit(gmml::ConvertString<char>(pdb_atom_name.substr(0, 1))))))
        {
            if (!(find(dataset_atom_names_of_residue.begin(), dataset_atom_names_of_residue.end(), pdb_atom_name) !=
                  dataset_atom_names_of_residue.end()))
            {
...
bool PdbPreprocessor::ExtractUnknownHeavyAtoms()
{
    ...
    std::vector<std::pair<std::string, std::string>> pdb_residue_names =
        this->GetPdbFile().GetAllResidueNames(); // It will be the residue name and then either S for start, E for end
                                                 // residue of each model in the PDB file, "" for nothing. Not great.
    gmml::ResidueNameMap recognized_residue_names =
        GetRecognizedResidueNamesMap(pdb_residue_names, dataset_residue_names);

    PdbFileSpace::PdbFile::PdbResidueVector pdb_residues = this->GetPdbFile().GetAllResidues();
    PdbFileSpace::PdbFile::PdbResidueVector recognized_residues =
        GetRecognizedResidues(pdb_residues, recognized_residue_names);

    PdbFileSpace::PdbFile::PdbResidueAtomsMap residue_atom_map = this->GetPdbFile().GetAllAtomsOfResidues();
    std::map<std::string, std::vector<std::string>> dataset_residue_atom_map =
        this->GetParameters().GetResidueNamesToTheirAtomNamesMap();
    ...
    for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = recognized_residues.begin();
         it != recognized_residues.end(); it++)
    {
        ...
        if (residue_name.compare("HIS") != 0)
        {
            ...
            if (find(pdb_residue_names.begin(), pdb_residue_names.end(), residue_sflag_pair) != pdb_residue_names.end())
            {
                ...
                for (std::vector<std::string>::iterator it1 = dataset_atom_names_of_head_residue.begin();
                     it1 != dataset_atom_names_of_head_residue.end(); it1++)
                {
                    std::string head_residue = (*it1);
                    dataset_atom_names_of_residue.push_back(head_residue);
                }
            }
            else if (find(pdb_residue_names.begin(), pdb_residue_names.end(), residue_eflag_pair) !=
                     pdb_residue_names.end())
            {
                std::stringstream ss1;
                ss1 << "C" << residue_name;
                dataset_atom_names_of_tail_residue = dataset_residue_atom_map[ss1.str()];
                for (std::vector<std::string>::iterator it2 = dataset_atom_names_of_tail_residue.begin();
                     it2 != dataset_atom_names_of_tail_residue.end(); it2++)
                {
            ...
            for (PdbFileSpace::PdbFile::PdbAtomCardVector::iterator it1 = unknown_heavy_atoms.begin();
                 it1 != unknown_heavy_atoms.end(); it1++)
            {
                PdbFileSpace::PdbAtomCard* heavy_atom                    = (*it1);
                PdbPreprocessorUnrecognizedHeavyAtom* unknown_heavy_atom = new PdbPreprocessorUnrecognizedHeavyAtom(
                    heavy_atom->GetAtomChainId(), heavy_atom->GetAtomSerialNumber(), heavy_atom->GetAtomName(),
                    heavy_atom->GetAtomResidueName(), heavy_atom->GetAtomResidueSequenceNumber(),
                    heavy_atom->GetAtomInsertionCode(), heavy_atom->GetAtomAlternateLocation());
...
void PdbPreprocessor::RemoveResiduesOfUnknownHeavyAtomsWithTheGivenModelNumber(
    PdbPreprocessorUnrecognizedHeavyAtomVector unknown_heavy_atoms, int model_number)
{
    to_be_deleted_atoms_.clear();
    to_be_deleted_residues_.clear();
    std::vector<std::string> removed_keys = std::vector<std::string>();
    for (PdbPreprocessorUnrecognizedHeavyAtomVector::iterator it = unknown_heavy_atoms.begin();
         it != unknown_heavy_atoms.end(); it++)
    {
        PdbPreprocessorUnrecognizedHeavyAtom* unknown_heavy_atom = (*it);
        std::stringstream ss;
        ss << unknown_heavy_atom->GetResidueInsertionCode() << "_" << unknown_heavy_atom->GetResidueChainId() << "_"
           << unknown_heavy_atom->GetResidueSequenceNumber() << "_" << unknown_heavy_atom->GetResidueInsertionCode()
           << "_" << unknown_heavy_atom->GetResidueAlternateLocation();
        std::string residue_key = ss.str();
        if (distance(removed_keys.begin(), find(removed_keys.begin(), removed_keys.end(), residue_key)) < 0 ||
            distance(removed_keys.begin(), find(removed_keys.begin(), removed_keys.end(), residue_key)) >=
                (int)removed_keys.size())
        {
            PdbFileSpace::PdbResidue* pdb_residue = new PdbFileSpace::PdbResidue(
                unknown_heavy_atom->GetResidueName(), unknown_heavy_atom->GetResidueChainId(),
                unknown_heavy_atom->GetResidueSequenceNumber(), unknown_heavy_atom->GetResidueInsertionCode(),
                unknown_heavy_atom->GetResidueAlternateLocation());
            //            this->GetPdbFile().DeleteResidueWithTheGivenModelNumber(pdb_residue , model_number);
            to_be_deleted_residues_.push_back(pdb_residue);
...
PdbFileSpace::PdbFile::PdbAtomCardVector
PdbPreprocessor::GetRemovedHydrogensOfResidue(PdbFileSpace::PdbFile::PdbAtomCardVector pdb_atoms,
                                              std::vector<std::string> dataset_atom_names_of_residue)
{
    ...
    for (PdbFileSpace::PdbFile::PdbAtomCardVector::iterator it = pdb_atoms.begin(); it != pdb_atoms.end(); it++)
    {
              ...
              isdigit(gmml::ConvertString<char>(pdb_atom_name.substr(0, 1))))))
        {
                ...
                dataset_atom_names_of_residue.end())
            {
                ...
                std::stringstream ss;
                ss << pdb_atom->GetAtomName() << "_" << pdb_atom->GetAtomResidueSequenceNumber() << "_"
                   << pdb_atom->GetAtomResidueName() << "_" << pdb_atom->GetAtomChainId();
...
bool PdbPreprocessor::ExtractRemovedHydrogens()
{
    gmml::ResidueNameMap dataset_residue_names = this->GetParameters().GetAllResidueNameMap();
    PdbFileSpace::PdbFile::PdbPairVectorAtomNamePositionFlag pdb_residue_names =
        this->GetPdbFile().GetAllResidueNames();
    gmml::ResidueNameMap recognized_residue_names =
        GetRecognizedResidueNamesMap(pdb_residue_names, dataset_residue_names);
    PdbFileSpace::PdbFile::PdbResidueVector pdb_residues = this->GetPdbFile().GetAllResidues();
    PdbFileSpace::PdbFile::PdbResidueVector recognized_residues =
         ...
         it != recognized_residues.end(); it++)
    {
        ...
        if (recognized_residue->GetResidueName().compare("HIS") != 0)
        {
            ...
            // OG JUST DUCT TAPING AT THIS POINT. Sometimes there will be atoms coming in like H1, H2, H3 on N terminus,
            ...
            dataset_atom_names_of_residue.insert(dataset_atom_names_of_residue.end(),
                                                 dataset_atom_names_of_Nresidue.begin(),
                                                 dataset_atom_names_of_Nresidue.end());
            dataset_atom_names_of_residue.insert(dataset_atom_names_of_residue.end(),
                                                 dataset_atom_names_of_Cresidue.begin(),
                                                 dataset_atom_names_of_Cresidue.end());
            // OG END DUCT TAPE.
            removed_hydrogens = GetRemovedHydrogensOfResidue(atoms_of_residue, dataset_atom_names_of_residue);
            for (PdbFileSpace::PdbFile::PdbAtomCardVector::iterator it1 = removed_hydrogens.begin();
                 it1 != removed_hydrogens.end(); it1++)
            {
                ...
                PdbPreprocessorReplacedHydrogen* removed_hydrogen_atom = new PdbPreprocessorReplacedHydrogen(
                    removed_hydrogen->GetAtomChainId(), removed_hydrogen->GetAtomSerialNumber(),
                    removed_hydrogen->GetAtomName(), removed_hydrogen->GetAtomResidueName(),
                    removed_hydrogen->GetAtomResidueSequenceNumber(), removed_hydrogen->GetAtomInsertionCode(),
                    removed_hydrogen->GetAtomAlternateLocation());
...
void PdbPreprocessor::RemoveRemovedHydrogensWithTheGivenModelNumber(
    PdbPreprocessorReplacedHydrogenVector replaced_hydrogens, int model_number)
{
    to_be_deleted_atoms_.clear();
    to_be_deleted_residues_.clear();
    for (PdbPreprocessorReplacedHydrogenVector::iterator it = replaced_hydrogens.begin();
         it != replaced_hydrogens.end(); it++)
    {
        PdbPreprocessorReplacedHydrogen* replaced_hydrogen = (*it);
        PdbFileSpace::PdbAtomCard* pdb_atom                = new PdbFileSpace::PdbAtomCard(
            replaced_hydrogen->GetResidueChainId(), replaced_hydrogen->GetAtomName(),
            replaced_hydrogen->GetResidueName(), replaced_hydrogen->GetResidueSequenceNumber(),
            replaced_hydrogen->GetResidueInsertionCode(), replaced_hydrogen->GetResidueAlternateLocation());
...
bool PdbPreprocessor::ExtractAminoAcidChains()
{
    try
    {
        ...
        for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = pdb_residues.begin(); it != pdb_residues.end();
             it++)
        {
            PdbFileSpace::PdbResidue* residue = *it;
            char chain_id                     = residue->GetResidueChainId();
        ...
        for (PdbPreprocessorChainIdSequenceNumbersMap::iterator it = chain_map_sequence_number.begin();
             it != chain_map_sequence_number.end(); it++)
        {
            ...
            std::vector<int>::iterator starting_sequence_number_iterator =
                min_element(sequence_numbers.begin(), sequence_numbers.end());
            std::vector<int>::iterator ending_sequence_number_iterator =
                max_element(sequence_numbers.begin(), sequence_numbers.end());
            ...
            PdbPreprocessorChainTermination* chain = new PdbPreprocessorChainTermination(
                chain_id, *starting_sequence_number_iterator, *ending_sequence_number_iterator,
...
void PdbPreprocessor::UpdateAminoAcidChainsWithTheGivenModelNumber(
    PdbPreprocessorChainTerminationVector chain_terminations, int model_number)
{
    // Before non-amino-acid residue
    for (PdbPreprocessor::PdbPreprocessorChainTerminationVector::iterator it1 = chain_terminations.begin();
         it1 != chain_terminations.end(); it1++)
    {
        PdbPreprocessorChainTermination* chain = (*it1);
        this->GetPdbFile().SplitAtomCardOfModelCard(chain->GetResidueChainId(),
                                                    chain->GetEndingResidueSequenceNumber() + 1);
        std::stringstream ss;
    ...
    gmml::log(__LINE__, __FILE__, gmml::INF,
              "Putting TER card after non-amino acid residues aka chain_terminations: Done");

    // Get all TER card positions and split
    std::vector<std::pair<char, int>> ter_card_positions =
        this->GetPdbFile().GetAllTerCardPositions(this->GetParameters().GetGlycamResidueNames());

    for (std::vector<std::pair<char, int>>::iterator it1 = ter_card_positions.begin(); it1 != ter_card_positions.end();
         it1++)
    {
    ...
    gmml::log(__LINE__, __FILE__, gmml::INF,
              "Putting TER card after residues with no tail or with at least 2 tails: Done");
         ...
         it1 != chain_terminations.end(); it1++)
    {
        ...
        if (chain->GetStringFormatOfSelectedNTermination().find("+") != std::string::npos ||
            chain->GetStringFormatOfSelectedNTermination().find("-") != std::string::npos)
        {
            gmml::log(__LINE__, __FILE__, gmml::INF,
                      "Doing nothing for N termination, which selected is " +
                          chain->GetStringFormatOfSelectedNTermination());
        }
        ...
        {    // Add n terminal residue at the beginning of the chain
            ...
            LibraryFileSpace::LibraryFileResidue* lib_file_residue =
                this->GetParameters().FindLibResidue(string_n_termination);
            if (lib_file_residue != NULL)
            {
                ...
                PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector atom_vector =
                    PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector();
                int serial_number = 0;
                for (LibraryFileSpace::LibraryFileResidue::AtomMap::iterator it2 = lib_atoms.begin();
                     it2 != lib_atoms.end(); it2++)
                {
                    LibraryFileSpace::LibraryFileAtom* lib_file_atom = (*it2).second;
                    PdbFileSpace::PdbAtomCard* pdb_atom              = new PdbFileSpace::PdbAtomCard(
                        serial_number, lib_file_atom->GetName(), ' ', string_n_termination, chain->GetResidueChainId(),
                        chain->GetStartingResidueSequenceNumber(), ' ', lib_file_atom->GetCoordinate(), gmml::dNotSet,
                        gmml::dNotSet, " ", "");
        ...
        if (chain->GetStringFormatOfSelectedCTermination().find("+") != std::string::npos ||
            chain->GetStringFormatOfSelectedCTermination().find("-") != std::string::npos)
        {
            ...
            gmml::log(__LINE__, __FILE__, gmml::INF,
                      "Doing nothing for C termination, which selected is " +
                          chain->GetStringFormatOfSelectedCTermination() +
                          ". Oliver maybe you need to add OXT if missing?");
            //            OLIVER HERE
            //            PdbFileSpace::PdbFile::PdbResidueVector pdbResidues =
            //            this->GetPdbFile().GetAllResiduesFromAtomSection(); PdbFileSpace::PdbResidue* firstRes =
            //            pdbResidues.front(); PdbFileSpace::PdbResidue* finalRes = pdbResidues.back();
            //            std::stringstream ss;
            //            ss << "firstRes:" << firstRes->GetResidueName() << firstRes->GetResidueSequenceNumber() << "."
            //            << firstRes->GetResidueChainId(); ss << "finalRes:" << finalRes->GetResidueName() <<
            //            finalRes->GetResidueSequenceNumber() << "." << finalRes->GetResidueChainId();
            //            gmml::log(__LINE__, __FILE__,  gmml::INF, ss.str() );
            //            LibraryFileSpace::LibraryFileResidue* lib_file_residue =
            //            this->GetParameters().FindLibResidue("C" + finalRes->GetResidueName());
            //            //PdbAtomCardVector GetAllAtomsOfResidue(PdbResidue* residue);
            //            std::vector<PdbFileSpace::PdbAtomCard*> finalResidueAtoms =
            //            this->GetPdbFile().GetAllAtomsOfResidue(finalRes);
            ...
            //                if (!found)
            //                {
            //                    gmml::log(__LINE__, __FILE__,  gmml::INF, "Creating a new atom called " +
            //                    libFileAtom->GetName());
            //                                                            int serial_number = 900000;
            //                                                            GeometryTopology::Coordinate coords(0.0, 0.0,
            //                                                            0.0);
            // GeometryTopology::Coordinate cCoordACE =
            // GeometryTopology::get_cartesian_point_from_internal_coords(cCoordProtein, caCoordProtein, nCoordProtein,
            // 120.0, -130.0, 1.4);
            //                    PdbAtomCard(
            //                    int atom_serial_number, std::string atom_name, char atom_alternate_location,
            //                    std::string residue_name, char chain_id,
            //                                        int residue_sequence_number, char insertion_code,
            //                                        GeometryTopology::Coordinate coordinate, double occupancy, double
            //                                        tempreture_factor, std::string element_symbol, std::string charge,
            //                                        std::vector<PdbAtomCard*> alternate_atom_locations ={});
        ...
        else
        {
            ...
            LibraryFileSpace::LibraryFileResidue* lib_file_residue =
                this->GetParameters().FindLibResidue(string_c_termination);
            if (lib_file_residue != NULL)
            {
                     ...
                     it2 != lib_atoms.end(); it2++)
                {
                    LibraryFileSpace::LibraryFileAtom* lib_file_atom = (*it2).second;
                    PdbFileSpace::PdbAtomCard* pdb_atom              = new PdbFileSpace::PdbAtomCard(
                        serial_number, lib_file_atom->GetName(), ' ', string_c_termination, chain->GetResidueChainId(),
                        chain->GetEndingResidueSequenceNumber(), ' ', lib_file_atom->GetCoordinate(), gmml::dNotSet,
                        gmml::dNotSet, " ", "");
                    atom_map[serial_number] = pdb_atom;
                    atom_vector.push_back(pdb_atom);
                    serial_number++;
...
bool PdbPreprocessor::ExtractGapsInAminoAcidChains()
{
    ...
    //        std::vector<std::string> amino_lib_residue_names = GetAllResidueNamesFromMultipleLibFiles(lib_files);
    // Advanced version
    gmml::ResidueNameMap amino_lib_residue_names_map =
        this->GetParameters().GetAllResidueNameMap(); // not exactly the same, returns more than just the amino.
    amino_lib_residue_names_map["HIS"]                                = "HIS";
    ...
    for (PdbPreprocessorChainIdResidueMap::iterator it = all_chain_map_amino_acid_residue.begin();
         it != all_chain_map_amino_acid_residue.end(); it++)
    {
        std::string chain_id                             = (*it).first;
        PdbFileSpace::PdbFile::PdbResidueVector residues = (*it).second;
        char c_id;
        ...
        for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it1 = residues.begin(); it1 != residues.end(); it1++)
        {
            ...
            if (dist != residues.size() - 1)
            {
                ...
                PdbFileSpace::PdbAtomCard* c_atom_of_residue =
                    this->GetPdbFile().GetAtomOfResidueByName(residue, "C", residue_atom_map);
                PdbFileSpace::PdbAtomCard* n_atom_of_next_residue =
                    this->GetPdbFile().GetAtomOfResidueByName(next_residue, "N", residue_atom_map);
                double distance = 0.0;
                if (c_atom_of_residue != NULL && n_atom_of_next_residue != NULL)
                {
                    ...
                    GeometryTopology::Coordinate n_atom_coordinate =
                        n_atom_of_next_residue->GetAtomOrthogonalCoordinate();
                ...
                if (distance > gmml::maxCutOff + 1.0)
                {
                    ...
                    PdbPreprocessorMissingResidue* missing_residues = new PdbPreprocessorMissingResidue(
                        c_id, *starting_sequence_number_iterator, *ending_sequence_number_iterator,
                        sequence_numbers.at(i), sequence_numbers.at(j), insertion_codes.at(i), insertion_codes.at(j));
...
void PdbPreprocessor::UpdateGapsInAminoAcidChainsWithTheGivenModelNumber(PdbPreprocessorMissingResidueVector gaps,
                                                                         int model_number)
{
    for (PdbPreprocessor::PdbPreprocessorMissingResidueVector::iterator it1 = gaps.begin(); it1 != gaps.end(); it1++)
    {
        ...
        gap->Print();
        this->GetPdbFile().SplitAtomCardOfModelCardWithTheGivenModelNumber(gap->GetResidueChainId(),
                                                                           gap->GetResidueAfterGap(), model_number);
        gap->Print();
    }
    LibraryFileSpace::LibraryFileResidue::AtomMap lib_atoms;
    for (PdbPreprocessor::PdbPreprocessorMissingResidueVector::iterator it1 = gaps.begin(); it1 != gaps.end(); it1++)
    {
        ...
        gap->Print();
        //        // Zwitterionic in n terminal
        //        if(gap->GetStringFormatOfSelectedNTermination().find("+") != std::string::npos ||
        //        gap->GetStringFormatOfSelectedNTermination().find("-") != std::string::npos)
        //        {
        //            // Zwitterionic in c terminal
        //            if(gap->GetStringFormatOfSelectedCTermination().find("+") != std::string::npos ||
        //            gap->GetStringFormatOfSelectedCTermination().find("-") != std::string::npos)
        //            {
        //
        //            }
        //            else
        ...
        //                LibraryFileSpace::LibraryFileResidue* lib_file_residue =
        //                this->GetParameters().FindLibResidue(string_c_termination); if(lib_file_residue != NULL)
        //                {
        //                    lib_atoms = lib_file_residue->GetAtoms();
        //                    PdbFileSpace::PdbAtomSection* pdb_atom_card = new PdbFileSpace::PdbAtomSection();
        //                    pdb_atom_card->SetRecordName("ATOM");
        //                    PdbFileSpace::PdbAtomSection::PdbAtomMap atom_map;
        //                    PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector atom_vector =
        //                    PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector(); int serial_number = 0;
        //                    for(LibraryFileSpace::LibraryFileResidue::AtomMap::iterator it2 = lib_atoms.begin(); it2
        //                    != lib_atoms.end(); it2++)
        //                    {
        //                        LibraryFileSpace::LibraryFileAtom* lib_file_atom = (*it2).second;
        //                        PdbFileSpace::PdbAtomCard* pdb_atom = new PdbFileSpace::PdbAtomCard(serial_number,
        //                        lib_file_atom->GetName(), ' ', string_c_termination, gap->GetResidueChainId(),
        //                                                                      gap->GetResidueBeforeGap(), ' ',
        //                                                                      lib_file_atom->GetCoordinate(),
        //                                                                      gmml::dNotSet, gmml::dNotSet, " ", "");
        //                        atom_map[serial_number] = pdb_atom;
        //                        atom_vector.push_back(pdb_atom);
        //                        serial_number++;
        //                    }
        ...
        //                LibraryFileSpace::LibraryFileResidue* lib_file_residue =
        //                this->GetParameters().FindLibResidue(string_n_termination); if(lib_file_residue != NULL)
        //                {
        //                    lib_atoms = lib_file_residue->GetAtoms();
        //                    PdbFileSpace::PdbAtomSection* pdb_atom_card = new PdbFileSpace::PdbAtomSection();
        //                    pdb_atom_card->SetRecordName("ATOM");
        ...
        //                        LibraryFileSpace::LibraryFileAtom* lib_file_atom = (*it2).second;
        //                        PdbFileSpace::PdbAtomCard* pdb_atom = new PdbFileSpace::PdbAtomCard(serial_number,
        //                        lib_file_atom->GetName(), ' ', string_n_termination, gap->GetResidueChainId(),
        //                                                                      gap->GetResidueAfterGap(), ' ',
        //                                                                      lib_file_atom->GetCoordinate(),
        //                                                                      gmml::dNotSet, gmml::dNotSet, " ", "");
        //                        atom_map[serial_number] = pdb_atom;
        //                        atom_vector.push_back(pdb_atom);
        ...
        //                    pdb_atom_card->SetAtomCards(atom_map);
        //                    pdb_atom_card->SetOrderedAtomCards(atom_vector);
        //                    this->GetPdbFile().InsertResidueBeforeWithTheGivenModelNumber(pdb_atom_card,
        //                    model_number); gap->Print(); if(it1 != --gaps.end())
        //                    {
        //                        std::stringstream bleh;
        //                        bleh << "NtermBOOM. Residue before gap: " << (*it1)->GetResidueBeforeGap() << " will
        //                        be incremeted +1 as it is same chain as current, this shouldn't happen anymore?";
        ...
        if (lib_file_residue != NULL)
        {
                 ...
                 it2 != lib_atoms.end(); it2++)
            {
                LibraryFileSpace::LibraryFileAtom* lib_file_atom = (*it2).second;
                PdbFileSpace::PdbAtomCard* pdb_atom              = new PdbFileSpace::PdbAtomCard(
                    serial_number, lib_file_atom->GetName(), ' ', string_n_termination, gap->GetResidueChainId(),
        ...
        gmml::PossibleCChainTermination c_termination = gap->GetSelectedCTermination();
        std::string string_c_termination              = gap->GetStringFormatOfCTermination(c_termination);
        LibraryFileSpace::LibraryFileResidue* lib_file_residue_from_c_termination =
            this->GetParameters().FindLibResidue(string_c_termination);
        if (lib_file_residue_from_c_termination != nullptr)
        {
            LibraryFileSpace::LibraryFileResidue::AtomMap lib_atoms_from_c_termination =
                lib_file_residue_from_c_termination->GetAtoms();
            ...
            PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector atom_vector_for_c_termination =
                PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector();
            int serial_number = 0;
            for (LibraryFileSpace::LibraryFileResidue::AtomMap::iterator it3 = lib_atoms_from_c_termination.begin();
                 it3 != lib_atoms_from_c_termination.end(); it3++)
            {
                LibraryFileSpace::LibraryFileAtom* lib_file_atom = (*it3).second;
                PdbFileSpace::PdbAtomCard* pdb_atom              = new PdbFileSpace::PdbAtomCard(
                    serial_number, lib_file_atom->GetName(), ' ', string_c_termination, gap->GetResidueChainId(),
            ...
            //            {
            //                std::stringstream bleh;
            //                bleh << "CtermBOOM. Residue before gap: " << (*it1)->GetResidueBeforeGap() << " will be
            //                incremeted +1 as it is same chain as current, this shouldn't happen anymore?";
            //                gmml::log(__LINE__, __FILE__, gmml::INF, bleh.str() );
            //                PdbPreprocessorMissingResidue* next_gap = (*(++it1));
            //                if(gap->GetResidueChainId() == next_gap->GetResidueChainId())
            //                {
...
bool PdbPreprocessor::ExtractAlternateResidue()
{
    ...
    for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = pdb_residues.begin(); it != pdb_residues.end(); it++)
    {
        ...
        std::stringstream ss;
        ss << target_residue_name << "_" << target_chain_id << "_" << target_sequence_number << "_"
        ...
        for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it1 = it + 1; it1 != pdb_residues.end(); it1++)
        {
            ...
            if (key.compare(target_key) == 0)
            {
                ...
                {
                    if (alternate_residue_map_.empty() ||
                        distance(alternate_residue_map_.begin(), alternate_residue_map_.find(key)) < 0 ||
                        distance(alternate_residue_map_.begin(), alternate_residue_map_.find(key)) >=
                            (int)alternate_residue_map_.size())
                    {
                        ...
                        alternate_residue_map_[target_key] = new PdbPreprocessorAlternateResidue(
                            residue_name, chain_id, sequence_number, insertion_code, alternate_locations, selected);
                    }
                    else
                    {
                        ...
                        if (distance(alternate_locations.begin(),
                                     find(alternate_locations.begin(), alternate_locations.end(), alternate_location)) <
                                0 ||
                            distance(alternate_locations.begin(),
                                     find(alternate_locations.begin(), alternate_locations.end(),
                                          alternate_location)) >= (int)alternate_locations.size())
                        {

                            alternate_locations.push_back(alternate_location);
                            alternate_residue->SetResidueAlternateLocation(alternate_locations);
                            std::vector<bool> selected_alternate_locations =
                                alternate_residue->GetSelectedAlternateLocation();
...
void PdbPreprocessor::RemoveUnselectedAlternateResiduesWithTheGivenModelNumber(
    PdbPreprocessorAlternateResidueMap alternate_residue_map /*, int model_number*/)
{
    to_be_deleted_atoms_.clear();
    to_be_deleted_residues_.clear();
    for (PdbPreprocessorAlternateResidueMap::iterator it = alternate_residue_map.begin();
         it != alternate_residue_map.end(); it++)
    {
        ...
        for (std::vector<bool>::iterator it1 = selected_alternate_locations.begin();
             it1 != selected_alternate_locations.end(); it1++)
        {
            bool selected_alternate_location = (*it1);
            char alternate_location          = alternate_residue->GetResidueAlternateLocation().at(
                distance(selected_alternate_locations.begin(), it1));
            if (!selected_alternate_location)
            {
                PdbFileSpace::PdbResidue* pdb_residue = new PdbFileSpace::PdbResidue(
                    alternate_residue->GetResidueName(), alternate_residue->GetResidueChainId(),
                    alternate_residue->GetResidueSequenceNumber(), alternate_residue->GetResidueInsertionCode(),
                    alternate_location);
                //                this->GetPdbFile().DeleteResidueWithTheGivenModelNumber(pdb_residue, model_number);
                to_be_deleted_residues_.push_back(pdb_residue);
            }
...
bool PdbPreprocessor::ExtractResidueInfo()
{
    ...
    for (PdbFileSpace::PdbFile::PdbResidueVector::iterator it = pdb_residues.begin(); it != pdb_residues.end(); it++)
    {
        ...
        if (lib_residues.find(residue->GetResidueName()) != lib_residues.end())
        {
            ...
            for (LibraryFileSpace::LibraryFileResidue::AtomMap::iterator it1 = lib_atoms.begin();
                 it1 != lib_atoms.end(); it1++)
            {
        ...
        std::stringstream ss;
        ss << residue->GetResidueName() << "_" << residue->GetResidueChainId() << "_"
           << residue->GetResidueSequenceNumber() << "_" << residue->GetResidueInsertionCode() << "_"
           << residue->GetResidueAlternateLocation();
        PdbPreprocessorResidueInfo* residue_info = new PdbPreprocessorResidueInfo(
            residue->GetResidueName(), residue->GetResidueChainId(), residue->GetResidueSequenceNumber(),
...
// void PdbPreprocessor::Preprocess(PdbFileSpace::PdbFile* pdb_file, std::vector<std::string> amino_lib_files_path,
// std::vector<std::string> glycam_lib_files_path, std::vector<std::string> other_lib_files_path,
// std::vector<std::string> prep_files_path)
...
void PdbPreprocessor::ApplyPreprocessingWithTheGivenModelNumber(int model_number)
{
    ...
    UpdateCYSResiduesWithTheGivenModelNumber(
        this->GetDisulfideBonds()); // OG this must be last as it uses index numbers to make CONECT cards. And it copies
                                    // the index numbers during extraction. Bleh.
