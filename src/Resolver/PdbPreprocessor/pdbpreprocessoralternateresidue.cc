...
PdbPreprocessorAlternateResidue::PdbPreprocessorAlternateResidue(std::string residue_name, char chain_id,
                                                                 int sequence_number, char residue_insertion_code,
                                                                 std::vector<char> residue_alternate_location,
      ...
      residue_insertion_code_(residue_insertion_code)
{
    ...
    for (std::vector<char>::iterator it = residue_alternate_location.begin(); it != residue_alternate_location.end();
         it++)
    {
        residue_alternate_location_.push_back(*it);
    }
    selected_alternate_location_ = std::vector<bool>();
    for (std::vector<bool>::iterator it = selected_alternate_location.begin(); it != selected_alternate_location.end();
         it++)
    {
...
{
    out << "Residue name: " << residue_name_ << ", Chain id: " << residue_chain_id_
        << ", Sequence number: " << residue_sequence_number_ << ", Insertion code: " << residue_insertion_code_
        << std::endl;
    for (unsigned int i = 0; i < residue_alternate_location_.size(); i++)
    {
        out << "\t Alternate Location: " << residue_alternate_location_[i] << " " << selected_alternate_location_[i]
            << std::endl;
    }
