...
/// This function reads the stream line-by-line and uses the whole structure of the file to point to the std::right
/// position for data extraction For more information check a sample parameter file to find out how the function works
...
void ParameterFile::ProcessAtomType(const std::string& line)
{
    ...
    /// atom_types_.insert(std::pair<std::string, ParameterFileAtom*>(type, new ParameterFileAtom(type, mass,
    /// polarizability, dscr)));
...
void ParameterFile::ProcessBond(const std::string& line)
{
    ...
    in >> std::setw(2) >> types[0] >> c >> std::setw(2) >> types[1]; /// Tokenize the bond atom types by '-'
    in >> std::setw(10) >> force_constant >> length; /// Tokenize the rest of the line into the corresponding variables

    if (in.fail())
    { /// Invalid template of the read line
        throw std::exception();
    }
    if (line.size() > 26)
    { /// Line has description
    ...
    bonds_[types] =
        new ParameterFileBond(types, force_constant, length, dscr); /// Create a new bond and insert into the bond list
}
...
void ParameterFile::ProcessAngle(const std::string& line)
{
    ...
    std::istringstream in(line); /// Create an stream from the read bond line
    in >> std::setw(2) >> types[0] >> c >> std::setw(2) >> types[1] >> c >> std::setw(2) >>
        types[2]; /// Tokenize the angle atom types by '-'
    in >> std::setw(10) >> force_constant >> std::setw(10) >>
        angle; /// Tokenize the rest of the line into the corresponding variables
    ...
    angles_[types] = new ParameterFileAngle(types, force_constant, angle,
                                            dscr); /// Create a new angle and insert into the angle list
}
...
void ParameterFile::ProcessDihedral(std::string& line, int& line_number, std::ifstream& in_file)
{
    ...
    std::istringstream in(line); /// Create an stream from the read bond line
    in >> std::setw(2) >> types[0] >> c >> std::setw(2) >> types[1] >> c >> std::setw(2) >> types[2] >> c >>
        std::setw(2) >> types[3]; /// Tokenize the angle atom types by '-'
    in >> std::setw(4) >> temp_factor >> std::setw(15) >> temp_force_constant >> std::setw(15) >> temp_phase >>
    ...
    while (dihedral->GetTerms().at(dihedral->GetTerms().size() - 1).GetPeriodicity() <
           0) /// Processing the following lines with the same dihedral;
              /// While the periodicity is negative the following lines are the same dihedrals with different attributes
    {
        ...
        std::istringstream in2(
            line.substr(11)); /// Skip the first column of the line which is the same dihedral atom types
        in2 >> std::setw(4) >> temp_factor >> std::setw(15) >> temp_force_constant >> std::setw(15) >> temp_phase >>
            std::setw(15) >> temp_periodicity; /// Tokenize the rest of the line into the corresponding variables
    ...
    if (dihedrals_.find(types) == dihedrals_.end() &&
        dihedrals_.find(inverse_types) == dihedrals_.end()) /// Duplicate checking
    {
        dihedral->SetIsImproper(false); /// Set is_improper_ attribute to false; improper dihedral doesn't have factor
        if (dihedral->GetTypes().at(0) == "X" || dihedral->GetTypes().at(1) == "X" ||
            dihedral->GetTypes().at(2) == "X" || dihedral->GetTypes().at(3) == "X") ///  Check for generic dihedral
        {
...
void ParameterFile::ProcessImproperDihedral(std::string& line, int& line_number, std::ifstream& in_file)
{
    ...
    in >> std::setw(2) >> types[0] >> c >> std::setw(2) >> types[1] >> c >> std::setw(2) >> types[2] >> c >>
        std::setw(2) >> types[3]; /// Tokenize the angle atom types by '-'
    in >> std::setw(15) >> temp_force_constant >> std::setw(15) >> temp_phase >> std::setw(15) >>
        temp_periodicity; /// Tokenize the rest of the line into the corresponding variables
              ...
              /// While the periodicity is negative the following lines are the same dihedrals with different attributes
    {
        ...
        std::istringstream in2(
            line.substr(11)); /// Skip the first column of the line which is the same dihedral atom types
        in2 >> std::setw(15) >> temp_force_constant >> std::setw(15) >> temp_phase >> std::setw(15) >>
            temp_periodicity; /// Tokenize the rest of the line into the corresponding variables
...
void ParameterFile::ProcessHydrogenBond(const std::string& line)
{
    ...
    std::istringstream in(line); /// Create an stream from the read bond line
    in >> types[0] >> types[1] >> coefficients[0] >>
        coefficients[1]; /// Tokenize the read line to the types and coefficients
    ...
    {
        bonds_[inverse_types]->SetHbondCoefficients(
            inverse_coefficients); /// Update hydrogen-bond attribute of the bond
    }
}
...
void ParameterFile::ProcessEquivalentSymbols(const std::string& line)
{
    ...
    std::string type, t;
    std::istringstream in(line);         // Create an stream from the read line
    in >> std::setw(4) >> type;     // Extract the first atom type from the line which the equivalent list is created
    for gmml::Trim(type); while (in >> std::setw(4) >> t && !gmml::Trim(t).empty())        // Read until the end of the
    line
    {
        if(atom_types_.find(type) != atom_types_.end())         // Check for existing atom type in the atom type map
            atom_types_[type] -> equivalent_list_.push_back(t); // Add the equivalent atom type to the corresponding
    list of the found atom
    }
    **/
    ...
    while (in >> std::setw(4) >> type && !gmml::Trim(type).empty()) /// Read until the end of the line
    {
        ...
        ; /// Add the read atom type into the temporary atom list
    }

    for (unsigned int i = 0; i < types.size(); i++) /// For each atom type in the list create a list of equivalent atom
                                                    /// types and update the corresponding attribute
    {
        if (distance(atom_types_.begin(), atom_types_.find(types[i])) >=
            0) /// Check for the existing atom type in the atom type map
        {
            ...
            atom_types_[types[i]]->SetEquivalentList(
                equivalent_types); /// Assign the equivalent list to the corresponding atom in the map
        }
    }
}
...
void ParameterFile::ProcessPotentialParameter(const std::string& line)
{
    ...
    {
        atom_types_[type] =
            new ParameterFileAtom(type, gmml::dNotSet, gmml::dNotSet, radius, depth,
                                  dscr); /// Create a new entry in the map for a non-existing atom type in the map
    }
    else
    {
    ...
    { /// Check if the atom type in the map has equivalent list
    ...
    for (it = equivalent_atoms.begin(); it != equivalent_atoms.end();
         ++it) /// For all atom types in the equivalent list of the atom set the corresponding attributes
    {
        if (atom_types_.find(*it) == atom_types_.end()) /// Check for the non-existing atom type in the map
        {
            atom_types_[*it] =
                new ParameterFileAtom(*it, gmml::dNotSet, gmml::dNotSet, radius, depth,
                                      dscr); /// Create a new entry in the map for the non-existing atom type in the map
        }
        else
        {
            ...
            atom_types_[*it]->SetMod4Dscr(
                dscr); /// Update mod4 description attribute of the existing atom type in the map
        }
    }
}
...
{
    double val;
    size_t pos = dscr.find(
        std::string(key + "=")); /// Find the starting position of the value of the given key in the given description
    ...
    switch (file_type_)
    {
        ...
        case gmml::MAIN:
            for (AtomTypeMap::iterator it = atom_types_.begin(); it != atom_types_.end(); it++)
            {
                ...
                stream << std::left << std::setw(2) << atom->GetType() << " " << std::right << std::setw(10)
                       << std::fixed << std::setprecision(2) << atom->GetMass() << " ";
                ...
                {
                    stream << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                           << atom->GetPolarizability();
                }
        ...
        case gmml::IONICMOD:
            ...
            {
                ParameterFileAtom* atom = (*it).second;
                stream << std::left << std::setw(3) << atom->GetType() << " " << std::right << std::setw(10)
                       << std::fixed << std::setprecision(2) << atom->GetMass() << " ";
                if (atom->GetPolarizability() == gmml::dNotSet)
                {
                    stream << std::right << std::setw(10) << " ";
    ...
    switch (file_type_)
    {
        ...
        case gmml::MAIN:
            for (BondMap::iterator it2 = bonds_.begin(); it2 != bonds_.end(); it2++)
            {
                ...
                stream << std::left << std::setw(2) << atom_types.at(0) << "-" << std::left << std::setw(2)
                       << atom_types.at(1) << " " << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                       << bond->GetForceConstant() << " " << std::right << std::setw(10) << std::fixed
                       << std::setprecision(2) << bond->GetLength() << " " << std::left << bond->GetDscr() << std::endl;
            }
            stream << std::endl;
            break;
    }
    ...
    switch (file_type_)
    {
        ...
        case gmml::MAIN:
            for (AngleMap::iterator it3 = angles_.begin(); it3 != angles_.end(); it3++)
            {
                ...
                stream << std::left << std::setw(2) << angle_types.at(0) << "-" << std::left << std::setw(2)
                       << angle_types.at(1) << "-" << std::left << std::setw(2) << angle_types.at(2) << " "
                       << std::right << std::setw(10) << std::fixed << std::setprecision(2) << angle->GetForceConstant()
                       << " " << std::right << std::setw(10) << std::fixed << std::setprecision(2) << angle->GetAngle()
        ...
        case gmml::MAIN:
            for (DihedralMap::iterator it4 = dihedrals_.begin(); it4 != dihedrals_.end(); it4++)
            {
                ...
                if (!dihedral->GetIsImproper())
                {
                    ...
                    for (std::vector<ParameterFileDihedralTerm>::iterator it5 = dihedral_terms.begin();
                         it5 != dihedral_terms.end(); it5++)
                    {
                        ParameterFileDihedralTerm dihedral_term = (*it5);
                        stream << std::left << std::setw(2) << dihedral_types.at(0) << "-" << std::left << std::setw(2)
                               << dihedral_types.at(1) << "-" << std::left << std::setw(2) << dihedral_types.at(2)
                               << "-" << std::left << std::setw(2) << dihedral_types.at(3) << " " << std::right
                               << std::setw(4) << dihedral_term.GetFactor() << " " << std::right << std::setw(10)
                               << std::fixed << std::setprecision(2) << dihedral_term.GetForceConstant() << " "
                               << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                               << dihedral_term.GetPhase() << " " << std::right << std::setw(10) << std::fixed
                               << std::setprecision(2) << dihedral_term.GetPeriodicity() << " " << std::left
                               << dihedral_term.GetDscr() << std::endl;
                    }
                }
            }
            stream << std::endl;
    ...
    switch (file_type_)
    {
        ...
        case gmml::MAIN:
            for (DihedralMap::iterator it6 = dihedrals_.begin(); it6 != dihedrals_.end(); it6++)
            {
                ...
                {
                    std::vector<ParameterFileDihedralTerm> dihedral_terms = dihedral->GetTerms();
                    for (std::vector<ParameterFileDihedralTerm>::iterator it7 = dihedral_terms.begin();
                         it7 != dihedral_terms.end(); it7++)
                    {
                        ParameterFileDihedralTerm dihedral_term = (*it7);
                        stream << std::left << std::setw(2) << dihedral_types.at(0) << "-" << std::left << std::setw(2)
                               << dihedral_types.at(1) << "-" << std::left << std::setw(2) << dihedral_types.at(2)
                               << "-" << std::left << std::setw(2) << dihedral_types.at(3) << " " << std::right
                               << std::setw(4) << " " << std::right << std::setw(10) << std::fixed
                               << std::setprecision(2) << dihedral_term.GetForceConstant() << " " << std::right
                               << std::setw(10) << std::fixed << std::setprecision(2) << dihedral_term.GetPhase() << " "
                               << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                               << dihedral_term.GetPeriodicity() << " " << std::left << dihedral_term.GetDscr()
                               << std::endl;
                    }
                }
            }
...
void ParameterFile::ResolveHydrogenBondSection(std::ofstream& stream)
{
    for (BondMap::iterator it8 = bonds_.begin(); it8 != bonds_.end(); it8++)
    {
        ...
        {
            stream << std::left << std::setw(2) << " " << std::left << std::setw(2) << atom_types.at(0) << std::left
                   << std::setw(2) << " " << std::left << std::setw(2) << atom_types.at(1) << std::left << std::setw(2)
                   << " " << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                   << bond->GetHbondCoefficients().at(0) << " " << std::right << std::setw(10) << std::fixed
                   << std::setprecision(2) << bond->GetHbondCoefficients().at(1) << std::endl;
        }
    }
    stream << std::endl;
}
    ...
    for (AtomTypeMap::iterator it10 = atom_types_.begin(); it10 != atom_types_.end(); it10++)
    {
        ...
        if (atom->GetRadius() != gmml::dNotSet || atom->GetWellDepth() != gmml::dNotSet)
        {
            switch (this->file_type_)
            {
                case gmml::MAIN:
                case gmml::MODIFIED:
                    stream << std::left << std::setw(2) << " " << std::left << std::setw(2) << atom->GetType()
                           << std::left << std::setw(6) << " " << std::left << std::setw(2) << " "
                           << " " << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                           << atom->GetRadius() << " " << std::right << std::setw(10) << std::fixed
                           << std::setprecision(2) << atom->GetWellDepth() << std::left << atom->GetMod4Dscr()
                           << std::endl;
                    break;
                case gmml::IONICMOD:
                    stream << std::left << std::setw(2) << " " << std::left << std::setw(3) << atom->GetType()
                           << std::left << std::setw(6) << " " << std::left << std::setw(2) << " "
                           << " " << std::right << std::setw(10) << std::fixed << std::setprecision(2)
                           << atom->GetRadius() << " " << std::right << std::setw(10) << std::fixed
                           << std::setprecision(2) << atom->GetWellDepth() << std::left << atom->GetMod4Dscr()
...
void ParameterFile::Print(std::ostream& out)
{
    ...
    out << std::setw(60) << "******************** Atom Types ********************" << std::endl;
    out << std::setw(6) << "Type" << std::setw(6) << "Mass" << std::setw(16) << "Polarizability" << std::setw(8)
        << "Radius" << std::setw(12) << "Well Depth" << std::setw(13) << "Hydrophilic" << std::setw(60) << "Description"
        << std::setw(60) << "MOD4 Description" << std::endl;
    for (ParameterFile::AtomTypeMap::iterator it = atom_types_.begin(); it != atom_types_.end(); it++)
    {
        it->second->Print(out);
    }
    out << std::setw(60) << "******************** Bonds ********************" << std::endl;
    out << std::setw(12) << "Bond Types" << std::setw(15) << "Force" << std::setw(10) << "Length" << std::setw(60)
        << "Description" << std::endl;
    for (ParameterFile::BondMap::iterator it = bonds_.begin(); it != bonds_.end(); it++)
    {
        it->second->Print(out);
    }
    out << std::setw(60) << "******************** Angles ********************" << std::endl;
    out << std::setw(16) << "Angle Types" << std::setw(15) << "Force" << std::setw(10) << "Angle" << std::setw(60)
        << "Description" << std::endl;
    for (ParameterFile::AngleMap::iterator it = angles_.begin(); it != angles_.end(); it++)
    {
        it->second->Print(out);
    }
    out << std::setw(60) << "******************** Dihedrals ********************" << std::endl;
    out << std::setw(22) << "Dihedral Types" << std::setw(10) << "Generic" << std::setw(10) << "Improper"
        << std::setw(6) << "SCEE" << std::setw(6) << "SCNB" << std::setw(10) << "Factor" << std::setw(10) << "Force"
        << std::setw(10) << "Phase" << std::setw(13) << "Periodicity" << std::setw(60) << "Description" << std::endl;
