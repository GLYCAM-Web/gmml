...
void LibraryFile::Read(std::ifstream& in_file)
{
    ...
    for (LibraryFile::ResidueMap::iterator it = residues_.begin(); it != residues_.end(); it++)
    {
        ...
        if (line.find("atoms") != std::string::npos)
        {
            ...
            while (line[0] !=
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
        ...
        if (line.find("atomspertinfo") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process atompertinfo section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
                {
        ...
        if (line.find("boundbox") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    ...
                    if (is_box_set < 0) /// If the number written in the first line of the section is negative then
                                        /// boundbox attributes have not been defined in the file
                    {
                        getline(in_file, line);
                    ...
                    else /// If the number written in the first line of the section is positive then set the attributes
                         /// of the boundbox section
                    {
                        getline(in_file, line);
                        double val;
        ...
        if (line.find("childsequence") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process childsequence section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    getline(in_file, line);
                }
                catch (...)
        ...
        if (line.find("hierarchy") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process hierarchy section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
        ...
        if (line.find("name") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process name section -> This section doesn't have any useful information -> Ignore the section
                    /// by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
                {
        ...
        if (line.find("residueconnect") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process residueconnect section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
        ...
        if (line.find("residues") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process residues section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
        ...
        if (line.find("residuesPdbSequenceNumber") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process residuesPdbSequenceNumber section -> This section doesn't have any useful information ->
                    /// Ignore the section by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
                {
        ...
        if (line.find("solventcap") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process solventcap section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    getline(in_file, line); /// Read the next line
                }
                catch (...)
        ...
        if (line.find("velocities") != std::string::npos)
        {
                   ...
                   '!') /// Iterate until to the next section that indicates by ! at the begining of the read line
            {
                try
                {
                    /// Process velocities section -> This section doesn't have any useful information -> Ignore the
                    /// section by reading the file until the next section
                    if (!getline(in_file, line))
                    {
...
LibraryFileSpace::LibraryFileAtom* LibraryFile::ProcessAtom(std::string& line)
{
    ...
    ss >> name >> type >> int_t >> residue_index >> int_t >> atom_index >> atomic_number >>
        charge; /// Split the line by space to extract attributes of the atom

    gmml::RemoveQuotes(name);
    gmml::RemoveQuotes(type);

    LibraryFileSpace::LibraryFileAtom* atom =
        new LibraryFileSpace::LibraryFileAtom(type, name, residue_index, atom_index, atomic_number, charge);
    return atom;
}
...
{
    stream << "!entry." << residue->GetName() << ".unit.connectivity table  int atom1x  int atom2x  int flags"
           << std::endl;
    LibraryFileSpace::LibraryFileResidue::AtomMap atoms = residue->GetAtoms();
    for (unsigned int i = 0; i < atoms.size(); i++)
    {
        ...
        for (std::vector<int>::iterator it = bonded_atoms_indices.begin(); it != bonded_atoms_indices.end(); it++)
        {
            ...
            {
                stream << atom->GetAtomIndex() << " " << bonded_atom_index << " "
                       << "1" << std::endl;
            }
        }
    }
...
{
    stream << "!entry." << residue->GetName()
           << ".unit.hierarchy table  str abovetype  int abovex  str belowtype  int belowx" << std::endl;
    stream << std::endl;
}
...
void LibraryFile::ResolvePositionSection(std::ofstream& stream, LibraryFileSpace::LibraryFileResidue* residue)
{
    ...
    for (unsigned int i = 0; i < atoms.size(); i++)
    {
        ...
        stream << std::fixed << coordinate.GetX() << " " << std::fixed << coordinate.GetY() << " " << std::fixed
               << coordinate.GetZ() << std::endl;
    }
}
...
void LibraryFile::ResolveResiduePdbSequenceNumberSection(std::ofstream& stream,
                                                         LibraryFileSpace::LibraryFileResidue* residue)
{
