...
void PrepFile::ReadOnlyQueryResidues(std::ifstream& in_file, std::vector<std::string>& query_residue_names)
{
    ...
    getline(in_file, header1);
    getline(in_file, header2);
    // Have to reach the line containing residue name to tell if this residue should be read. However, residue
    // processing starts from the residue title line, which is two lines above Sadly, for getline() there is no easy way
    // back. So, the position of the title line is set as a stream pointer. When reading a residue, make the stream
    // pointer go back to this checkpoint.
    ...
    {
        in_file.seekg(one_line_before_residue_title); // go back to one line before residue title, so the rest of the
    ...
    while (getline(in_file, line_str))
    {
        ...
        if (std::find(query_residue_names.begin(), query_residue_names.end(), resname) != query_residue_names.end())
        {
            in_file.seekg(one_line_before_residue_title); // go back to one line before residue title, so the rest of
                                                          // the codes works correctly
            residue                       = ProcessResidueSection(in_file);
            residues_[residue->name_]     = residue;
            one_line_before_residue_title = in_file.tellg();
        }
...
void PrepFile::BuildPrepFile(std::ofstream& stream)
{
    ...
    {
        PrepFileSpace::PrepFileResidue* residue = (*it).second;
        stream << residue->GetTitle() << std::endl
               << std::endl
               << std::left << std::setw(4) << residue->GetName() << " " << std::right << std::setw(3)
               << residue->GetStringFormatOfCoordinateType() << " " << std::setw(1) << residue->GetOutputFormat()
               << std::endl
               << residue->GetStringFormatOfGeometryType() << " " << residue->GetStringFormatOfDummyAtomOmission()
               << " " << residue->GetDummyAtomType() << " " << residue->GetStringFormatOfDummyAtomPosition()
               << std::endl
        ...
        for (PrepFileSpace::PrepFileAtomVector::iterator it1 = atoms.begin(); it1 != atoms.end(); it1++)
        {
            ...
            stream << std::right << std::setw(2) << atom->GetIndex() << " " << std::left << std::setw(4)
                   << atom->GetName() << " " << std::left << std::setw(3) << atom->GetType() << " " << std::setw(1)
                   << atom->GetStringFormatOfTopologicalType() << " " << std::right << std::setw(2)
                   << atom->GetBondIndex() << " " << std::right << std::setw(2) << atom->GetAngleIndex() << " "
        ...
        if (dihedrals.size() != 0)
        {
            ...
            for (PrepFileSpace::PrepFileResidue::DihedralVector::iterator it2 = dihedrals.begin();
                 it2 != dihedrals.end(); it2++)
            {
        ...
        if (loops.size() != 0)
        {
            ...
            for (PrepFileSpace::PrepFileResidue::Loop::iterator it3 = loops.begin(); it3 != loops.end(); it3++)
            {
                ...
                stream << residue->GetAtomNameByIndex(first_atom_index) << " "
                       << residue->GetAtomNameByIndex(second_atom_index) << std::endl;
            }
        }
