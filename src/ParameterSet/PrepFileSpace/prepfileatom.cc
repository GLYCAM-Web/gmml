...
PrepFileAtom::PrepFileAtom()
    : index_(0), name_(""), type_(""), topological_type_(gmml::kTopTypeM), bond_index_(0), angle_index_(0),
      dihedral_index_(0), bond_length_(gmml::dNotSet), angle_(gmml::dNotSet), dihedral_(gmml::dNotSet),
      charge_(gmml::dNotSet)
{}

PrepFileAtom::PrepFileAtom(int index, const std::string& name, const std::string& type,
                           gmml::TopologicalType topological_type, int bond_index, int angle_index, int dihedral_index,
                           double bond_length, double angle, double dihedral, double charge)
    : index_(index), name_(name), type_(type), topological_type_(topological_type), bond_index_(bond_index),
      angle_index_(angle_index), dihedral_index_(dihedral_index), bond_length_(bond_length), angle_(angle),
      dihedral_(dihedral), charge_(charge)
{}
...
PrepFileAtom::PrepFileAtom(std::string& line)
{
    ...
    ss >> index_ >> name_ >> type_;

    topological_type_ = ExtractAtomTopologicalType(ss);

    ss >> bond_index_ >> angle_index_ >> dihedral_index_ >> bond_length_ >> angle_ >> dihedral_ >> charge_;
}
...
{
    out << std::setw(3) << index_ << std::setw(6) << name_ << std::setw(6) << type_;
    ...
    out << std::setw(4) << bond_index_ << std::setw(4) << angle_index_ << std::setw(4) << dihedral_index_
        << std::setw(10) << bond_length_ << std::setw(10) << angle_ << std::setw(10) << dihedral_ << std::setw(10)
        << charge_;
    //        << endl;
}
