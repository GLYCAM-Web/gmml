...
PrepFileResidue::PrepFileResidue()
    : title_(""), name_(""), coordinate_type_(PrepFileSpace::kINT), output_format_(PrepFileSpace::kFormatted),
      geometry_type_(PrepFileSpace::kGeometryCorrect), dummy_atom_omission_(PrepFileSpace::kOmit),
      dummy_atom_type_("DU"), dummy_atom_position_(PrepFileSpace::kPositionBeg), charge_(0.0)
{
      ...
      for(PrepFileSpace::PrepFileAtomVector::iterator it = atoms_.begin(); it != atoms_.end(); it++)
      {
          ...
          if(!stack.empty())
          {
              ...
              if(atom->GetTopologicalType() == gmml::kTopTypeM)
              {
                  ...
                  if(top_of_stack_atom->GetTopologicalType() == gmml::kTopTypeS ||
      top_of_stack_atom->GetTopologicalType() == gmml::kTopTypeB
...
PrepFileSpace::PrepFileAtomVector PrepFileResidue::GetAtomsParentVector()
{
    ...
    for (PrepFileSpace::PrepFileAtomVector::iterator it = this->atoms_.begin(); it != this->atoms_.end(); it++)
    {
        ...
        //        }
        //        std::stringstream ss;
        if (stack.empty()) // This next bit fills the stack. If an atom has 4 neighbors, it will go onto the stack 4
                           // times, unless it has a loop neighbor.
        {
            switch (atom->GetTopologicalType())
            {
                ...
                case gmml::kTopType3: // Has 4 neighbors. So self in as parent 4 times for each child?
                    //                 ss << "Atom: " << atom->GetName() << " index: " << index << "type :3" << "with no
                    //                 stack\n";
                    if (neighbors.at(index) == 0)
                    { // This is to record where the LOOPs are fml. So no LOOP connection means once more on the stack.
                      // Seems wrong but ok.
                        stack.push_back(atom);
                    }
                    stack.push_back(atom);
                    ...
                    break;
                case gmml::kTopTypeB:
                    //                  ss << "Atom: " << atom->GetName() << " index: " <<  index << "type :B" << "with
                    //                  no stack\n";
                    if (neighbors.at(index) == 0)
                    {
                        stack.push_back(atom);
                    ...
                    break;
                case gmml::kTopTypeS:
                    //                  ss << "Atom: " << atom->GetName() << " index: " << index << "type :S" << "with
                    //                  no stack\n";
                    if (neighbors.at(index) == 0)
                    {
                        stack.push_back(atom);
                    ...
                    break;
                case gmml::kTopTypeE:
                    //                ss << "Atom: " << atom->GetName() << " index: " << index << "type :E" << "with no
                    //                stack\n";
                    if (neighbors.at(index) == 0)
                    {
                        stack.push_back(atom);
        ...
        else
        {
            ...
            {
                case gmml::kTopTypeM: // OG 2022Apr06 Here is the fix. It seems to be ok to have too many parents on the
                                      // stack for M type, but not too few. For TBT the M atom C2 had 4 children, so
                                      // having 3 by default for M was insufficient. Pray this doesn't break anything
                                      // and we move away from prep files before some poor soul has to read this again.
                    //                ss << "Atom: " << atom->GetName() << " index: " << index << " type: M" << " with
                    //                stack:" << stack.back()->GetName() << " and neighbors " << neighbors.at(index) <<
                    //                "\n";
                    parents.at(index) =
                        stack.at(stack.size() - 1); // last thing on the stack goes onto parents for this index.
                    stack.pop_back();
                    if (neighbors.at(index) == 0)
                    {
                        stack.push_back(atom);
                ...
                case gmml::kTopType3: // Ok so I'm of size 3, I will go on 3 times for each of my kids. Put take the
                                      // last parent off the stack and assign it as my parent.
                    //                 ss << "Atom: " << atom->GetName() << " index: " << index << " type: 3" << " with
                    //                 stack:" << stack.back()->GetName() << " and neighbors " << neighbors.at(index) <<
                    //                 "\n";
                    parents.at(index) =
                        stack.at(stack.size() - 1); // last thing on the stack goes onto parents for this index.
                ...
                case gmml::kTopTypeS:
                    ...
                    break;
                case gmml::kTopTypeE:
                    //                 ss << "Atom: " << atom->GetName() << " index: " << index << " type: E" << " with
                    //                 stack:" << stack.back()->GetName() << "\n";
                    parents.at(index) = stack.at(stack.size() - 1);
                    stack.pop_back();
                    break;
...
PrepFileResidue* PrepFileResidue::LoadFromStream(std::ifstream& in_file)
{
    ...
    if (gmml::Trim(line).find("STOP") != std::string::npos)
    { /// End of file
        return NULL;
    }
