...
void OffFileSpace::OffFile::PopulateOffFileResiduesFromAssembly(MolecularModeling::ResidueVector assembly_residues,
                                                                int CoordinateIndex)
{
    int ResidueIndex      = 0;
    ...
    for (MolecularModeling::ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
        ...
        for (MolecularModeling::AtomVector::iterator it = all_atoms_of_residue.begin();
             it != all_atoms_of_residue.end(); it++)
        {
            AtomIndex++;
            BoundingAtomIndex++;
            ...
            off_file_atom->SetAtomicNumber(
                MolecularMetadata::findElementAtomicNumber(residue_atom->GetElementSymbol()));
    ...
    // for(MolecularModeling::ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end();
    // it++)
    //{
    ...
    This whole loop seems to be unused
                for(MolecularModeling::AtomVector::iterator it2 = all_atoms_of_residue.begin(); it2 !=
    all_atoms_of_residue.end(); ++it2)
                {
    ...
    for (OffFileResidueVector::iterator it = off_file_residues.begin(); it != off_file_residues.end(); it++)
    {
        ...
        for (OffFileAtomVector::iterator it1 = residue_atoms.begin(); it1 != residue_atoms.end(); it1++)
        {
            ...
            stream << " \"" << atom->GetName() << "\" "
                   << "\"" << atom->GetType() << "\" "
                   << "0"
                   << " " << atom->GetResidueIndex() << " " << FLAG << " " << atom->GetAtomIndex() << " "
                   << atom->GetAtomicNumber() << " " << std::fixed << atom->GetCharge() << std::endl;
        }
    }
}
...
void OffFileSpace::OffFile::WriteConnectivitySection(std::ofstream& stream,
                                                     MolecularModeling::ResidueVector assembly_residues)
{
    ...
    for (MolecularModeling::ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
        MolecularModeling::Residue* residue                = (*it);
        MolecularModeling::AtomVector all_atoms_of_residue = residue->GetAtoms();
        for (MolecularModeling::AtomVector::iterator it2 = all_atoms_of_residue.begin();
             it2 != all_atoms_of_residue.end(); it2++)
        {
            MolecularModeling::Atom* atom = (*it2);
            ...
            for (MolecularModeling::AtomVector::iterator it3 = bonded_atoms.begin(); it3 != bonded_atoms.end(); it3++)
            {
                ...
                if (std::find(center_atoms_visited.begin(), center_atoms_visited.end(), bonded_atom) ==
                    center_atoms_visited.end())
                {
                    stream << " " << atom_bonding_map_[atom->GetIndex()] << " "
                           << atom_bonding_map_[bonded_atom->GetIndex()] << " " << 1 << std::endl;
                }
            }
        }
    }
...
{
    stream << "!entry." << unit_name_ << ".unit.hierarchy table  str abovetype  int abovex  str belowtype  int belowx"
           << std::endl;
    ...
    for (OffFileResidueVector::iterator it = off_file_residues.begin(); it != off_file_residues.end(); it++)
    {
        ...
        stream << " \""
               ...
               << "R"
               << "\""
               << " " << residue->GetListingIndex() << std::endl;
        ...
        for (OffFileAtomVector::iterator it = all_atoms_of_residue.begin(); it != all_atoms_of_residue.end(); it++)
        {
            ...
            stream << " \""
                   << "R"
                   << "\""
                   << " " << residue->GetListingIndex() << " "
                   << "\""
                   << "A"
                   << "\""
                   << " " << atom_counter << std::endl;
        }
    }
}
...
void OffFileSpace::OffFile::WritePositionSection(std::ofstream& stream,
                                                 MolecularModeling::ResidueVector assembly_residues,
                                                 int CoordinateIndex)
{
    ...
    for (MolecularModeling::ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
             ...
             it != all_atoms_of_residue.end(); it++)
        {
            ...
            stream << " " << std::fixed << coordinate.GetX() << " " << std::fixed << coordinate.GetY() << " "
                   << std::fixed << coordinate.GetZ() << std::endl;
        }
    }
}

void OffFileSpace::OffFile::WriteResidueConnectSection(std::ofstream& stream,
                                                       MolecularModeling::ResidueVector assembly_residues)
{
    stream << "!entry." << unit_name_
    ...
    for (MolecularModeling::ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
        ...
        int c1x_count = 0;
        int c2x_count = 0;
        for (MolecularModeling::AtomVector::iterator it1 = head_atoms_of_residue.begin();
             it1 != head_atoms_of_residue.end(); it1++)
        {
        ...
        for (MolecularModeling::AtomVector::iterator it2 = tail_atoms_of_residue.begin();
             it2 != tail_atoms_of_residue.end(); it2++)
        {
            MolecularModeling::Atom* atom = *it2;
        ...
        if (c1x.size() > 0 && c2x.size() > 0)
        { // Yao added this if statement guard. What if these vectors are empty? Then it segfaults. 03/24/2020
            stream << " " << c1x[0] << " " << c2x[0];
        }
...
void OffFileSpace::OffFile::WriteResiduesSection(std::ofstream& stream,
                                                 MolecularModeling::ResidueVector assembly_residues)
{
    std::string name;
    unsigned int seq = 0;
    ...
    for (MolecularModeling::ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
        ...
        // Yao: Don't determine starting atom index from head atoms. Instead, search for AtomIndex = 1 in an offfile
        // atom object, or use the index of an element in this ResidueVector. Since there are offfile residue/atom
        // objects populated, it might be better to rely on these objects, rather than assembly residues. In one
        // case,head_atoms_of_residue[0] is indeed the 2nd atom in residue.I'm talking about the residue ROH as
        // extracted from prep file.
        startatomx                            = atom_bonding_map_[current_atom->GetIndex()];
        restype                               = "?";
        imagingx                              = 0;

        stream << " \"" << name << "\""
               << " " << seq << " " << childseq << " " << startatomx << " "
               << "\"" << restype << "\""
               << " " << imagingx << std::endl;
    }
}
