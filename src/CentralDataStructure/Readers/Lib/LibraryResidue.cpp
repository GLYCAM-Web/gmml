...
LibraryResidue::LibraryResidue(std::stringstream& residueStream, const std::string& name)
{
    ...
    while (getline(residueStream, line))
    {
        ...
        //                       is_box_set << ss ;
        //                       if(is_box_set < 0)              /// If the number written in the first line of the
        //                       section is negative then boundbox attributes have not been defined in the file
        ...
        if (line.find("connectivity") != std::string::npos)
        {
            ...
            { // Process connectivity section
                ...
                {
                    std::stringstream ss;
                    ss << "Tried to access an atom in LibraryResidue constructor that doesn't exist. Positions were "
