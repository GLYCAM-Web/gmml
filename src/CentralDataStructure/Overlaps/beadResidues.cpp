...
{
    // sfat stands for sidechain fat atom. mfat stands for mainchain fat atom. fat atoms are now called beads, but the
    // name stayed. Different names allow for easy selection in VMD visualisation problem. They serve no other purpose.
    // The beads should completely envelope all atoms in the protein. I first did all CA atoms in the mainchain, then
    // visualized in VMD and manually selected sidechain atoms that would cover the rest of the protein. The code below
    // adds to CA atoms and then selected other atoms based on their name and sometimes also the residue name.
    std::vector<Atom*> protein_beads;
    std::vector<Residue*> proteinResidues =
        codeUtils::getElementsWithNames(assembly.getResidues(), biology::proteinResidueNames);
    for (auto& residue : proteinResidues)
    {
        for (auto& atom : residue->getAtoms())
        {
            ...
            {
                //				std::cout << "Adding CA bead to protein " << residue->getStringId() <<
                // std::endl;
                ...
                protein_beads.push_back(residue->getAtoms().back());
                     ...
                     (atom->getName().compare("CZ") == 0) || (atom->getName().compare("NE2") == 0) ||
                     (atom->getName().compare("OD1") == 0) || (atom->getName().compare("SD") == 0) ||
                     ((atom->getName().compare("CE2") == 0) && residue->getName().compare("TRP") == 0) ||
                     ((atom->getName().compare("CD1") == 0) &&
                      (residue->getName().compare("LEU") == 0 || residue->getName().compare("ILE") == 0)) ||
                     ((atom->getName().compare("CD") == 0) && residue->getName().compare("GLU") == 0))
            { // sfats should move when a chi1, chi2 is moved, so make sure they are connected to something for the
                // SetDihedral function to move them.
                //                std::cout << "Adding sidechain bead to protein " << residue->getStringId() <<
                //                std::endl;
...
std::vector<Atom*> beads::Add_Beads_To_Glycan(std::vector<Residue*> glycan_residues)
{
    ...
    {
        if (residue->getName().at(1) != 'S') // don't add a bead to a sialic acid in this section (see "else" below).
                                             // Middle character of resname is always S for sialic acid.
        {
        ...
        else // if it is sialic acid
        {
            for (auto& atom : residue->getAtoms())
            {
                if ((atom->getName().compare("C2") == 0) || (atom->getName().compare("N5") == 0) ||
                    (atom->getName().compare("C8") == 0))
                {
                    residue->addAtom(std::make_unique<Atom>("gfat", *(atom->getCoordinate())));
                    glycan_beads.push_back(residue->getAtoms().back());
                    Atom* any_atom = residue->getAtoms().front();
