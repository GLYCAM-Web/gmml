...
RotatableDihedral::RotatableDihedral(cds::Atom* atom1, cds::Atom* atom2, cds::Atom* atom3, cds::Atom* atom4, bool reverseAtomsThatMove)
{
    std::vector<cds::Atom*> atoms {atom1, atom2, atom3, atom4};
    this->Initialize(atoms, reverseAtomsThatMove);
}

RotatableDihedral::RotatableDihedral(cds::Atom* atom1, cds::Atom* atom2, cds::Atom* atom3, cds::Atom* atom4, std::vector<cds::Atom*> extraAtomsThatMove, bool reverseAtomsThatMove)
{
    std::vector<cds::Atom*> atoms {atom1, atom2, atom3, atom4};
    this->Initialize(atoms, reverseAtomsThatMove);
...
bool RotatableDihedral::SetSpecificShape(std::string dihedralName, std::string selectedRotamer)
{
    if (assigned_metadata_.empty())
    {
        std::string message = "Error in RotatableDihedral::SetSpecificAngleUsingMetadata; no metadata has been set for:\n"
                        + atom1_->getId() + " " + atom2_->getId() + " " + atom3_->getId() + " " + atom4_->getId();
        message += ".\nA rotatable dihedral named: " + dihedralName + " was requested to be set to the rotamer named: " + selectedRotamer;
...
void RotatableDihedral::WiggleUsingAllRotamers(std::vector<cds::Atom*>& overlapAtomSet1, std::vector<cds::Atom*> &overlapAtomSet2, const int &angleIncrement)
{
...
void RotatableDihedral::WiggleWithinCurrentRotamer(std::vector<cds::Atom*>& overlapAtomSet1, std::vector<cds::Atom*> &overlapAtomSet2, const int &angleIncrement)
{
...
// User requested gg, this prevents flipping into gt like the above would do.
void RotatableDihedral::WiggleWithinCurrentRotamer(std::vector<cds::Residue*>& overlapResidueSet1, std::vector<cds::Residue*> &overlapResidueSet2, const int &angleIncrement)
{
    double lowerBound = (this->GetCurrentMetaData()->default_angle_value_ - this->GetCurrentMetaData()->lower_deviation_);
    double upperBound = (this->GetCurrentMetaData()->default_angle_value_ + this->GetCurrentMetaData()->upper_deviation_);
    // Set to lowest deviation, work through to highest. Set best value and return it for reference.
    //this->WiggleWithinRanges(overlapAtomSet1, overlapAtomSet2, angleIncrement, lowerBound, upperBound);
    this->WiggleWithinRangesDistanceCheck(overlapResidueSet1, overlapResidueSet2, angleIncrement, lowerBound, upperBound);
    return;
}

double RotatableDihedral::WiggleWithinRanges(std::vector<cds::Atom*>& overlapAtomSet1, std::vector<cds::Atom*> &overlapAtomSet2, const int &angleIncrement, const double& lowerBound, const double& upperBound)
{
...
double RotatableDihedral::WiggleWithinRangesDistanceCheck(std::vector<cds::Atom*>& overlapAtomSet1, std::vector<cds::Atom*> &overlapAtomSet2, const int &angleIncrement, const double& lowerBound, const double& upperBound)
{
    this->SetDihedralAngle(lowerBound);
    double currentDihedral = lowerBound;
    double bestDihedral = lowerBound;
    double currentOverlap = cds::CountOverlappingAtoms(overlapAtomSet1, overlapAtomSet2);
    double lowestOverlap = currentOverlap;
//    std::cout << "Starting wiggle with overlap : " << currentOverlap << " . Current Angle: " << currentDihedral << " lowerBound: " << lowerBound << " upperBound: " << upperBound << "\n";
    while(currentDihedral < upperBound)
    {
...
double RotatableDihedral::WiggleWithinRangesDistanceCheck(std::vector<cds::Residue*>& overlapResidueSet1, std::vector<cds::Residue*> &overlapResidueSet2, const int &angleIncrement, const double& lowerBound, const double& upperBound)
{
    this->SetDihedralAngle(lowerBound);
    double currentDihedral = lowerBound;
    double bestDihedral = lowerBound;
    double currentOverlap = cds::CountOverlappingAtoms(overlapResidueSet1, overlapResidueSet2);
    double lowestOverlap = currentOverlap;
//    std::cout << "Starting wiggle with overlap : " << currentOverlap << " . Current Angle: " << currentDihedral << " lowerBound: " << lowerBound << " upperBound: " << upperBound << "\n";
    while(currentDihedral < upperBound)
    {
...
std::unique_ptr<cds::Atom> RotatableDihedral::CreateHydrogenAtomForPsiAngle()
{
    ...
    {
        std::stringstream ss;
        ss << "Error in CreateHydrogenAtomForPsiAngle. centralAtom neighbors size = " <<
