...
{
    void operator() (std::vector<cds::Atom*>::iterator currentPosition, std::vector<cds::Atom*>::iterator currentEnd, std::vector<cds::Atom*>::iterator  compareStart, std::vector<cds::Atom*>::iterator  compareEnd)
    { // Check every atom from current to end against every following atom.
        ...
        while (currentPosition != currentEnd)
        {
            ...
            while(comparePosition != compareEnd)
            {
                ...
                {
                    static_cast<void>(atomicBonds::bondAtomsIfClose(currentAtom, compareAtom)); // cast to ignore returned bool.
                }
                ++comparePosition;
            }
            ++currentPosition;
...
void cds::bondAtomsByDistance(std::vector<cds::Atom*> atoms)
{
    ...
    unsigned long const vectorLength = atoms.size();
    unsigned long const num_threads = codeUtils::calculateNumberOfThreads(vectorLength); // guesses from hardware, includes current load, not perfect.
    if (num_threads == 0)
    {
    ...
    for(unsigned long i = 0; i < (num_threads - 1); ++i)
    {
        ...
        //std::cout << "Thread " << i << " checking from: " << (*block_start)->getIndex() << "_" << (*block_start)->getNumber() << " to " << (*block_end)->getIndex() << std::endl;
