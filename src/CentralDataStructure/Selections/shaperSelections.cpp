...
bool cdsSelections::FindCyclePoint(cds::Atom* previous_atom, cds::Residue* residue, cds::Atom* current_atom, std::vector<cds::Atom*>* atom_path, bool* found_cycle_point, cds::Atom* &cycle_point)
...
//                    std::cout << "Going one deeper with found cycle set as " << std::boolalpha << *found_cycle_point << std::endl;
                    cdsSelections::FindCyclePoint(current_atom, residue, neighbor, atom_path, found_cycle_point, cycle_point);
                }
            }
        }
    }
...
bool cdsSelections::FindPathBetweenTwoAtoms(cds::Atom* current_atom, cds::Residue* currentResidue, cds::Atom* target_atom, cds::Residue* targetResidue, std::vector<cds::Atom*>* atom_path, bool* found)
{
    //atom_path->push_back(current_atom);
    ...
    for(std::vector<cds::Atom*>::iterator it1 = neighbors.begin(); it1 != neighbors.end(); ++it1)
    {
        ...
        {
            //std::cout << "STEEEPER\n";
            cdsSelections::FindPathBetweenTwoAtoms(neighbor, currentResidue, target_atom, targetResidue, atom_path, found);
        }
    }
...
//        std::cout << "\n";
        ...
        {
            cdsSelections::FindAtomsConnectingResidues(neighbor, currentResidue, otherResidue, connecting_atoms, found_neighbor);
        }
    }
    return;
}
...
cds::Atom* cdsSelections::FindCyclePointNeighbor(const std::vector<cds::Atom*> atom_path, cds::Atom* cycle_point, cds::Residue* cyclePointResidue)
{
    ...
    else if ( (cycle_point->getName().compare("C2")==0) && (std::find(atom_path.begin(), atom_path.end(), cyclePointResidue->FindAtom("C1")) == atom_path.end()) )
    {
        selected_neighbor = cyclePointResidue->FindAtom("C1");
    }
    else if (cycle_point->getName().compare("C1")==0)
    {
        selected_neighbor = cyclePointResidue->FindAtom("C2"); // We can always set phi to 180 this way, regardless of alpha/beta
    }
    else // This bit is overdone now, as I was looking for higher numbered atoms of C1, but now I know I always want C2, so put that above.
    {
        ...
        {
//            std::cout << "About to segfault in MolecularModeling/Selections/selections.cpp cdsSelections::FindCyclePointNeighbor" << std::endl;
        }
...
void cdsSelections::FindEndsOfBranchesFromLinkageAtom(cds::Atom* currentAtom, cds::Atom* previousAtom, cds::Residue* residue, Branch *branch)
{
    ...
    {
        if (neighbor->getLabel() != "VistedByFindEndsOfBranchesFromLinkageAtom"
