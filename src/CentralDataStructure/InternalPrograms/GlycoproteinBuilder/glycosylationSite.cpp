...
//     for(auto &atom : atoms)
//     {
//         atom->AddCoordinate(new GeometryTopology::Coordinate(atom->GetCoordinate())); // push back the currect
//         coordinates onto the end of the coordinates vector.
//     }
//     return;
// }
//
// void GlycosylationSite::SetStashedCoordinates() // When a lower overlap is found, the coords are pushed back. Last
// one pushed back is the lowest overlap.
//{
...
// GlycosylationSite::GetXClosestSitesWithinOverlapDistanceY(std::vector<GlycosylationSite> &glycosites, int
// maxNumberOfSitesToConsider)
//{
...
//     double maxLengthOfThisGlycan =
//     (selection::GetMaxDistanceBetweenAtoms(this->GetAttachedGlycan()->GetAllAtomsOfAssembly()) + 5); Atom
//     *this_cb_atom = this->GetResidue()->GetAtom("CB"); for(auto &glycosite : glycosites)
//     {
//         if (this->GetResidue()->getId().compare(glycosite.GetResidue()->getId())==0) // if not this site. Oly you
//         should overload the = operator?
//         {
//             continue; // I think this skips to next item in for loop.
//         }
//         double maxLengthOfThatGlycan =
//         (selection::GetMaxDistanceBetweenAtoms(glycosite.GetAttachedGlycan()->GetAllAtomsOfAssembly()) + 5); Atom
//         *that_cb_atom = glycosite.GetResidue()->GetAtom("CB"); double distanceBetweenCBAtoms =
//         this_cb_atom->GetDistanceToAtom(that_cb_atom); std::cout << distanceBetweenCBAtoms; if ( (
//         distanceBetweenCBAtoms ) <= (maxLengthOfThisGlycan + maxLengthOfThatGlycan) )
//         {
