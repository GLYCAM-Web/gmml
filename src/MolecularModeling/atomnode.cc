...
AtomNode::AtomNode(AtomNode& node)
{
    ...
    for (MolecularModeling::AtomVector::iterator it = intra_node_neighbors.begin(); it != intra_node_neighbors.end();
         it++)
    {
...
{
    for (MolecularModeling::AtomVector::iterator it = intra_node_neighbors_.begin(); it != intra_node_neighbors_.end();
         it++)
    {
        Atom* atom = (*it);
...
void AtomNode::Print(std::ostream& out)
{
    ...
    switch (number_of_bonds)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            //            out << "\t\t" << node_neighbors_.at(0)->GetResidue()->GetId() << ":" <<
            //            node_neighbors_.at(0)->GetName() << std::endl
            //                << "\t\t" << "  |  " << std::endl
            //                << "\t" << node_neighbors_.at(1)->GetResidue()->GetId() << ":" <<
            //                node_neighbors_.at(1)->GetName() << " -- "
            //                << atom_->GetResidue()->GetId() << ":" << atom_->GetName() << std::endl
            //                << "\t\t" << "  |  " << std::endl
            //                << "\t\t" <<  node_neighbors_.at(2)->GetResidue()->GetId() << ":" <<
            //                node_neighbors_.at(2)->GetName() << std::endl;
            out << "\t\t" << node_neighbors_.at(0)->GetId() << std::endl
                << "\t\t"
                << "  |  " << std::endl
                << "\t" << node_neighbors_.at(1)->GetId() << " -- " << atom_->GetId() << std::endl
                << "\t\t"
                << "  |  " << std::endl
        ...
        case 4:
            ...
            //                node_neighbors_.at(1)->GetName() << " -- "
            //                << atom_->GetResidue()->GetId() << ":" << atom_->GetName() << " -- "
            //                << node_neighbors_.at(2)->GetResidue()->GetId() << ":" << node_neighbors_.at(2)->GetName()
            //                << std::endl
            //                << "\t\t\t" << "  |  " << std::endl
            //                << "\t\t\t" << node_neighbors_.at(3)->GetResidue()->GetId() << ":" <<
            //                node_neighbors_.at(3)->GetName() << std::endl;
            out << "\t\t\t" << node_neighbors_.at(0)->GetId() << std::endl
                << "\t\t\t"
                << "  |  " << std::endl
                << "\t" << node_neighbors_.at(1)->GetId() << " -- " << atom_->GetId() << " -- "
                << node_neighbors_.at(2)->GetId() << std::endl
                << "\t\t\t"
                << "  |  " << std::endl
