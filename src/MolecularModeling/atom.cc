...
Atom::Atom()
{
    ...
    this->SetAttributes(NULL, "", GeometryTopology::CoordinateVector(), "", "", "", std::vector<AtomNode*>(), "", false,
                        "");
    this->SetBFactor(0);
    // Check if atom id is set. If so, change the index part (2nd token) from the index as in the PDB to the absolute
    // index
    if (this->id_.find("_") != std::string::npos)
    {
        std::vector<std::string> underscore_split_tokens = gmml::Split(this->id_, "_");
        if (underscore_split_tokens.size() >= 2)
        { // name_id_ etc.
            underscore_split_tokens[1] = std::to_string(this->index_);
        }
        std::string new_id;
        for (std::vector<std::string>::iterator it = underscore_split_tokens.begin();
             it != underscore_split_tokens.end(); it++)
        {
            new_id += *it;
            if (it != (underscore_split_tokens.end() - 1))
...
src/MolecularModeling/atom.cc:51:1: warning: base class 'class MolecularModeling::MolecularDynamicAtom' should be
explicitly initialized in the copy constructor [-Wextra] Atom::Atom(const Atom& atom)
 ^
*/
// Added by Ayush
Atom::Atom(const Atom& atom)
...
void Atom::SetCoordinates(GeometryTopology::CoordinateVector coordinates)
{
    ...
    for (GeometryTopology::CoordinateVector::iterator it = this->coordinates_.begin(); it != this->coordinates_.end();
         it++)
    {
...
{
    static unsigned long long s_AtomIndex =
        0;                // static keyword means it is created only once and persists beyond scope of code block.
    return s_AtomIndex++; // makes copy of s_AtomIndex, increments the real s_AtomIndex, then returns the value in the
                          // copy
...
std::string Atom::DetermineChirality() // Added by Yao 08/26/3019 Return values are R,S,A. A = achiral
{
    ...
    else if (primary_neighbors.size() == 4)
    {
        ...
        {
            // std::cout << "Atom " << primary_neighbors[std::distance(ranks.begin(), int_it)]->GetName() << " has rank
            // " << *int_it << std::endl;
    ...
    {
        bool sp2 = false;
        // Allow up to 11.47 deg from parallel. This cutoff is just slightly above side group N2 in prep file, which is
        // 0.98491, or 9.97 deg from ideal trigonal planar.
        if (std::abs(std::cos(GetDihedral(primary_neighbors[0], this, primary_neighbors[1], primary_neighbors[2]))) >
            0.98)
        {
            sp2 =
                true; // if improper torsion is close to 0 or 180 deg, then carbon neighbor is planar, hence sp2 carbon.
        }
        // std::cout << "Sp2 judgment torsion is: " << primary_neighbors[0]->GetName() << "-" <<  this->GetName() << "-"
        // <<  primary_neighbors[1]->GetName() << "-" <<  primary_neighbors[2]->GetName()
        //<< ": " << std::abs(std::cos(GetDihedral(primary_neighbors[0], this, primary_neighbors[1],
        // primary_neighbors[2]))) << std::endl;
        if (sp2)
        {
...
Atom Atom::PlaceFakeHydrogen()
{
    ...
    for (AtomVector::iterator atom_it = primary_neighbors.begin(); atom_it != primary_neighbors.end(); atom_it++)
    {
        ...
        // std::cout << "Coordinate" << (*atom_it)->GetCoordinates().at(0)->GetX() << "," <<
        // (*atom_it)->GetCoordinates().at(0)->GetY() << "," << (*atom_it)->GetCoordinates().at(0)->GetZ() << std::endl;
...
MolecularModeling::AtomVector Atom::GetRankedPrimaryNeighbors(std::vector<int>& ranks)
{
    std::map<std::vector<int>, std::vector<int>> duplicate_value_indices_versus_higher_rank_indices =
        this->ComparePrimaryNeighbors(ranks);
    AtomVector primary_neighbors = this->GetNode()->GetNodeNeighbors();
    ...
    else
    {
        ...
        for (std::map<std::vector<int>, std::vector<int>>::iterator mapit =
                 duplicate_value_indices_versus_higher_rank_indices.begin();
             mapit != duplicate_value_indices_versus_higher_rank_indices.end(); mapit++)
        {
            ...
            for (std::vector<int>::iterator int_it = duplicate_indices.begin(); int_it != duplicate_indices.end();
                 int_it++)
            {
                int index                   = *int_it;
             ...
             mapit != duplicate_value_indices_versus_higher_rank_indices.end(); mapit++)
        {
                 ...
                 int_it++)
            {
                ...
                {
                    // Although neighbors have identical ranking at the moment, if a neighbor has no downstream
                    // neighbor, it is smaller than other neighbors. std::cout << "Adjusting rank, which shouldn't
                    // happen." << std::endl;
                    ranks[index]++;
                    for (std::vector<int>::iterator int_it2 = higher_rank_indices.begin();
                         int_it2 != higher_rank_indices.end(); int_it2++)
                    {
                        ranks[*int_it2]++;
                    }
                    // dead_end = true;
        ...
        {
            // std::cout << "Test before recursion, corresponding atom is: " << primary_neighbors[s]->GetName() <<
            // std::endl; std::cout << "Test before recusion rank: " << ranks[s] << std::endl;
        }
        ...
        if (!dead_end && unique_ranks2.size() != ranks.size())
        { // If not at a dead end yet and ranking is not unique, need to start recursive comparison.
            ...
            this->InitializeComparisonTracker(duplicate_value_indices_versus_higher_rank_indices, visited_atoms,
                                              comparison_progress_tracker);
            this->RecursivelyCompareBranches(duplicate_value_indices_versus_higher_rank_indices, ranks, visited_atoms,
                                             comparison_progress_tracker);
        }
...
double Atom::GetDihedral(MolecularModeling::Atom* atom1, MolecularModeling::Atom* atom2, MolecularModeling::Atom* atom3,
                         MolecularModeling::Atom* atom4)
{
...
void Atom::InitializeComparisonTracker(
    std::map<std::vector<int>, std::vector<int>>& duplicate_value_indices_versus_higher_rank_indices,
    MolecularModeling::AtomVector& visited_atoms,
    std::map<Atom*, std::vector<MolecularModeling::AtomVector>>& comparison_progress_tracker)
{
         ...
         mapit != duplicate_value_indices_versus_higher_rank_indices.end(); mapit++)
    {
        ...
        for (std::vector<int>::iterator int_it = duplicate_indices.begin(); int_it != duplicate_indices.end(); int_it++)
        {
            ...
            for (AtomVector::iterator atomit = secondary_neighbors.begin(); atomit != secondary_neighbors.end();
                 atomit++)
            {
...
std::map<std::vector<int>, std::vector<int>> Atom::ComparePrimaryNeighbors(std::vector<int>& ranks)
{
    ...
    std::multimap<int, Atom*> primary_neighbor_atomic_number_map =
        GetAtomicNumbersOfNeighbors(primary_neighbors, one_time_use_vector);

    std::set<int>
        unique_element_numbers; // Utilize the unique property of a set to sort unique atomic numbers(key) in multimap.
    for (std::multimap<int, Atom*>::iterator mapit = primary_neighbor_atomic_number_map.begin();
         mapit != primary_neighbor_atomic_number_map.end(); mapit++)
    {
         ...
         mapit != primary_neighbor_atomic_number_map.end(); mapit++)
    {
        ...
        int rank =
            std::distance(unique_element_numbers.rbegin(),
                          std::find(unique_element_numbers.rbegin(), unique_element_numbers.rend(), atomic_number)) +
            1;
        // std::cout << "Rank is: " << rank << std::endl;
        int index    = std::distance(primary_neighbors.begin(),
                                     std::find(primary_neighbors.begin(), primary_neighbors.end(), primary_neighbor));
        ranks[index] = rank;
    }

    std::map<std::vector<int>, std::vector<int>> duplicate_value_indices_versus_higher_rank_indices =
        this->MakeDuplicateRanksHigherRankIndicesMap(ranks);
    return duplicate_value_indices_versus_higher_rank_indices;
}

void Atom::RecursivelyCompareBranches(std::map<std::vector<int>, std::vector<int>>& duplicate_higher_indices_map,
                                      std::vector<int>& ranks, MolecularModeling::AtomVector visited_atoms,
                                      std::map<Atom*, std::vector<AtomVector>>& comparison_progress_tracker)
{
    AtomVector primary_neighbors = this->GetNode()->GetNodeNeighbors();
    for (std::map<std::vector<int>, std::vector<int>>::iterator mapit = duplicate_higher_indices_map.begin();
         mapit != duplicate_higher_indices_map.end(); mapit++)
    {
        ...
        for (AtomVector::iterator atomit = primary_neighbors.begin(); atomit != primary_neighbors.end(); atomit++)
        {
            ...
            if (std::find(indices_with_identical_ranking.begin(), indices_with_identical_ranking.end(), index) !=
                indices_with_identical_ranking.end())
            {
        ...
        for (AtomVector::iterator atom_it = primary_neighbors_with_identical_ranking.begin();
             atom_it != primary_neighbors_with_identical_ranking.end(); atom_it++)
        {
            Atom* primary_neighbor           = *atom_it;
            std::vector<AtomVector> branches = comparison_progress_tracker[primary_neighbor];
            if (branches.empty())
            {
                std::cout << primary_neighbor->GetResidue()->GetName() << "-" << primary_neighbor->GetName()
                          << "is empty. Prevent previous recursion from visiting it." << std::endl;
                std::cout << "Primary neighbor neighbors size: "
                          << primary_neighbor->GetNode()->GetNodeNeighbors().size() << std::endl;
            }
            ...
            std::vector<std::vector<int>> descent_order_sorted_branch_info =
                this->SortBranchesInDescendingOrder(branch_info);
            // Add descent sorted branch info to comparison tracker. Descent sorted so that later the bigger brances are
            // compared first.
             ...
             atom_it != primary_neighbors_with_identical_ranking.end(); atom_it++)
        {
            Atom* primary_neighbor           = *atom_it;
            std::vector<AtomVector> branches = comparison_progress_tracker[primary_neighbor];
            for (std::vector<AtomVector>::iterator branch_it = branches.begin(); branch_it != branches.end();
                 branch_it++)
            {
            ...
            // Make new branches for next round of comparison. Branch expansion rule: each atom in the current branch is
            // replaced by its neighbors, which forms a new branch.
        ...
        std::vector<std::pair<Atom*, int>> descent_sorted_primary_neighbors =
            this->SortPrimaryNeighborBranchesInDescendingOrder(comparison_result_tracker);
        for (std::vector<std::pair<Atom*, int>>::iterator pair_it = descent_sorted_primary_neighbors.begin();
             pair_it != descent_sorted_primary_neighbors.end(); pair_it++)
        {
            // std::cout << "Testing relative rank after sorting: "  << pair_it->first->GetName() << "-" <<
        ...
        // Check if certain identically ranked primary neighbors and empty. If some, but not all, are empty, increase
        // their relative rank by 1.
        std::vector<std::pair<AtomVector, AtomVector>> duplicate_neighbors_higher_ranked_neighbors;
        for (std::set<int>::iterator setit = unique_relative_ranking.begin(); setit != unique_relative_ranking.end();
             setit++)
        {
            ...
            {
                duplicate_neighbors_higher_ranked_neighbors.push_back(
                    std::make_pair(duplicate_neighbors, higher_ranked_neighbors));
            }
        }

        for (std::vector<std::pair<AtomVector, AtomVector>>::iterator vec_it =
                 duplicate_neighbors_higher_ranked_neighbors.begin();
             vec_it != duplicate_neighbors_higher_ranked_neighbors.end(); vec_it++)
        {
            AtomVector duplicate_neighbors     = vec_it->first;
            AtomVector higher_ranked_neighbors = vec_it->second;
            bool all_empty                     = true;
            for (AtomVector::iterator atomit = duplicate_neighbors.begin(); atomit != duplicate_neighbors.end();
                 atomit++)
            {
                Atom* pri_neighbor                   = *atomit;
                std::vector<AtomVector> new_branches = comparison_progress_tracker[pri_neighbor];
                for (std::vector<AtomVector>::iterator branchit = new_branches.begin(); branchit != new_branches.end();
                     branchit++)
                {
                    if (!branchit->empty())
                    {
                 ...
                 atomit++)
            {
                ...
                if (empty && !all_empty)
                {
                    ...
                    for (std::vector<std::pair<Atom*, int>>::iterator pair_it =
                             descent_sorted_primary_neighbors.begin();
                         pair_it != descent_sorted_primary_neighbors.end(); pair_it++)
                    {
            ...
            if (!all_empty)
            {
                for (AtomVector::iterator atomit = higher_ranked_neighbors.begin();
                     atomit != higher_ranked_neighbors.end(); atomit++)
                {
                    Atom* pri_neighbor = *atomit;
                    for (std::vector<std::pair<Atom*, int>>::iterator pair_it =
                             descent_sorted_primary_neighbors.begin();
             ...
             pair_it != descent_sorted_primary_neighbors.end(); pair_it++)
        {
            ...
            int primary_neighbor_index =
                std::distance(primary_neighbors.begin(),
                              std::find(primary_neighbors.begin(), primary_neighbors.end(), primary_neighbor));
            // std::cout << "But neighbor according to index is: " <<
            // primary_neighbors[primary_neighbor_index]->GetName() << std::endl;
            int relative_rank             = pair_it->second;
            // std::cout << "relative rank is: " << relative_rank << " ," << "min rank is: " << min_relative_rank <<
            // std::endl; std::cout << "Rank before adjustment is: " << ranks[primary_neighbor_index] << std::endl;
        ...
        for (std::vector<int>::iterator int_it = indices_with_higher_ranking.begin();
             int_it != indices_with_higher_ranking.end(); int_it++)
        {
    ...
    std::map<std::vector<int>, std::vector<int>> child_duplicate_higher_indices_map =
        this->MakeDuplicateRanksHigherRankIndicesMap(ranks);
    int non_empty_primary_neighbor_count = 0;
    if (!child_duplicate_higher_indices_map.empty())
    {
        for (std::map<std::vector<int>, std::vector<int>>::iterator map_it = child_duplicate_higher_indices_map.begin();
             map_it != child_duplicate_higher_indices_map.end(); map_it++)
        {
    ...
    {
        this->RecursivelyCompareBranches(child_duplicate_higher_indices_map, ranks, visited_atoms,
                                         comparison_progress_tracker);
    }
}

std::vector<MolecularModeling::AtomVector>
Atom::MakeNextLevelOfBranches(std::vector<MolecularModeling::AtomVector>& current_branches,
                              MolecularModeling::AtomVector& visited_atoms)
{
    std::vector<AtomVector> new_branches = std::vector<AtomVector>();
    for (std::vector<AtomVector>::iterator branch_it = current_branches.begin(); branch_it != current_branches.end();
         branch_it++)
    {
        AtomVector individual_branch = *branch_it;
        for (AtomVector::iterator atom_it = individual_branch.begin(); atom_it != individual_branch.end(); atom_it++)
        {
            ...
            for (AtomVector::iterator atom_it = neighbors.begin(); atom_it != neighbors.end(); atom_it++)
            {
                ...
                if (std::find(visited_atoms.begin(), visited_atoms.end(), neighbor) == visited_atoms.end())
                {
                    ...
                    // std::cout << "Next level neighbor " << neighbor->GetName() << " is the offspring of " <<
                    // individual_atom->GetName() << std::endl;
...
std::vector<std::vector<int>> Atom::ObtainBranchInfo(std::vector<MolecularModeling::AtomVector>& branches,
                                                     MolecularModeling::AtomVector& visited_atoms)
{
    ...
    {
        AtomVector individual_branch = *branch_it;
        // TODO:Right now each neighbor is counted only once, regardless of the bond type. Need to find double/triple
        // bonded neighbors and count that neighbor multiple times.
        std::multimap<int, Atom*> neighbor_atomic_number_map =
            this->GetAtomicNumbersOfNeighbors(individual_branch, visited_atoms);

        for (std::multimap<int, Atom*>::iterator nanm_it = neighbor_atomic_number_map.begin();
             nanm_it != neighbor_atomic_number_map.end(); nanm_it++)
        {
        ...
        for (std::set<int>::reverse_iterator uan_rit = unique_atomic_numbers.rbegin();
             uan_rit != unique_atomic_numbers.rend(); uan_rit++)
        {
            int atomic_number = *uan_rit;
            // std::cout << "Atomic number: " << atomic_number << std::endl;
            std::pair<std::multimap<int, Atom*>::iterator, std::multimap<int, Atom*>::iterator> range =
                neighbor_atomic_number_map.equal_range(atomic_number);
...
std::vector<std::pair<Atom*, int>> Atom::SortPrimaryNeighborBranchesInDescendingOrder(
    std::map<Atom*, std::vector<std::vector<int>>>& comparison_result_tracker)
{
    // std::cout << "Compare primary neighbors: " << std::endl;
    /*for (std::map<Atom*, std::vector<std::vector<int> > >::iterator m = comparison_result_tracker.begin(); m !=
    comparison_result_tracker.end(); m++ ){ std::cout << m->first->GetName() << ",";
    }
    std::cout << std::endl;*/
    ...
    for (std::map<Atom*, std::vector<std::vector<int>>>::iterator mapit = begin_pos;
         mapit != comparison_result_tracker.end(); mapit++)
    {
        ...
        if (this->CompareTwoSetsOfBranches(secondary_branches,
                                           comparison_result_tracker[descent_sorted_primary_neighbor.front()]) != 1)
        ...
        else if (this->CompareTwoSetsOfBranches(secondary_branches,
                                                comparison_result_tracker[descent_sorted_primary_neighbor.back()]) != 0)
        { // incoming set <= existing set
            // std::cout << "S1 <= S2" << std::endl;
        ...
        else
        {
            ...
            for (AtomVector::iterator atom_it = descent_sorted_primary_neighbor.begin();
                 atom_it != descent_sorted_primary_neighbor.end() - 1; atom_it++)
            {
                ...
                if (this->CompareTwoSetsOfBranches(secondary_branches, comparison_result_tracker[existing_atom]) != 0 &&
                    this->CompareTwoSetsOfBranches(secondary_branches, comparison_result_tracker[next_existing_atom]) !=
                        1)
                {
    ...
    int relative_rank = 1;
    descent_sorted_descent_sorted_primary_neighbors_and_ranking.push_back(
        std::make_pair(descent_sorted_primary_neighbor.front(), relative_rank));
         ...
         atom_it != descent_sorted_primary_neighbor.end() - 1; atom_it++)
    {
        ...
        AtomVector::iterator next_atom_it = atom_it + 1;
        Atom* next_existing_atom          = *next_atom_it;
        if (this->CompareTwoSetsOfBranches(comparison_result_tracker[next_existing_atom],
                                           comparison_result_tracker[existing_atom]) == 1)
        {
            relative_rank++;
        }
        descent_sorted_descent_sorted_primary_neighbors_and_ranking.push_back(
            std::make_pair(next_existing_atom, relative_rank));
    }
    ...
    for (unsigned int i = 0; i < smaller_size; i++)
    {
        ...
        else
        {
            if (i == smaller_size - 1)
            {
                ...
                {
                    break; // Could've returned 2 here.But this will cause compilation warning that control reaches
                           // non-void function.So break out this for loop and return at the end.
                }
            }
        }
    }
...
Atom::SortBranchesInDescendingOrder(std::vector<std::vector<int>>& originally_ordered_branches)
{
    std::vector<std::vector<int>> descent_order_sorted_branch_info;
    std::vector<int> first_branch = originally_ordered_branches[0];
    descent_order_sorted_branch_info.push_back(first_branch);

    for (std::vector<std::vector<int>>::iterator bi_it = originally_ordered_branches.begin() + 1;
         bi_it != originally_ordered_branches.end(); bi_it++)
    {
        ...
        {
            for (std::vector<std::vector<int>>::iterator dosbi_it = descent_order_sorted_branch_info.begin();
                 dosbi_it != descent_order_sorted_branch_info.end() - 1; dosbi_it++)
            {
                std::vector<std::vector<int>>::iterator next_it = dosbi_it + 1;
                if (this->CompareBranches(branch_info, *dosbi_it) != 0 &&
                    this->CompareBranches(branch_info, *next_it) != 1)
                {
    ...
    for (unsigned int i = 0; i < smaller_size; i++)
    {
        ...
        else
        {
            if (i == smaller_size - 1)
            {
                ...
                {
                    break; // Could've returned 2 here,which cause compilation warning "control reaches end of non-void
                           // function". So break out of for loop and return at the end.
                }
            }
        }
    }
...
void Atom::SetAttributes(MolecularModeling::Residue* residue, std::string name,
                         GeometryTopology::CoordinateVector coordinates, std::string chemical_type,
                         std::string description, std::string element_symbol,
                         std::vector<MolecularModeling::AtomNode*> nodes, std::string id, bool is_ring,
