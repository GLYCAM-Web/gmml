...
//                       FUNCTIONS                      //
//////////////////////////////////////////////////////////
void Assembly::ExtractOntologyInfoByNameOfGlycan(std::string stereo_name, std::string stereo_condensed_name,
                                                 std::string name, std::string condensed_name,
                                                 std::string output_file_type)
{
    if (stereo_name.compare("") == 0 && stereo_condensed_name.compare("") == 0 && name.compare("") == 0 &&
        condensed_name.compare("") == 0)
    {
...
void Assembly::ExtractOntologyInfoByNamePartsOfGlycan(std::string isomer, std::string ring_type,
                                                      std::string configuration, std::string output_file_type)
{
    ...
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE
          << " ?pdb ?stereo_name ?stereo_condensed_name ?name ?condensed_name ?shape " << Ontology::WHERE_CLAUSE;
    query << "?pdb_file     :hasOligo   ?oligo.\n";
    query << "?oligo        :hasCore    ?mono.\n";
    query << "?mono         :hasSugarName   ?sugarName.\n";
    if (isomer.compare("") != 0)
...
void Assembly::ExtractOntologyInfoByPDBID(std::string pdb_id, std::string output_file_type)
{
    ...
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?oligo_sequence ?residue_links ?glycosidic_linkage "
          << Ontology::WHERE_CLAUSE;
...
void Assembly::ExtractOntologyInfoByStringChemicalCode(std::string chemical_code, std::string output_file_type)
{
    ...
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?pdb ?name ?short_name ?stereo_name ?stereo_short_name "
          << Ontology::WHERE_CLAUSE;
...
{
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?pdb ?residue_links ?glycosidic_linkage "
          << Ontology::WHERE_CLAUSE;
    ...
    /// To DO: std::string manipulation: split the oligo_name by _ and for each of them write the following to represent
    /// the names of the monos:
    //    query << "?oligo	:hasCore	?mono.\n";
    //    query << "?mono     :hasSugarName	?sn.\n";
    //    query << "?sn       :monosaccharideName 	?name.\n";
    //    query << "?sn       :monosaccharideShortName 	?short_name.\n";
...
void Assembly::ExtractOntologyInfoByOligosaccharideNameSequenceByRegex(std::string oligo_name_pattern,
                                                                       std::string output_file_type)
{
    ...
    {
        //        std::cout << "Please specify the input argument. (you can use up to two * in the name pattern)" <<
        //        std::endl;
        return;
    }
    if (count(oligo_name_pattern.begin(), oligo_name_pattern.end(), '*') > 3)
    {
        //        std::cout << "Wrong name pattern format. Please use only up tp three * in the input argument." <<
        //        std::endl;
        return;
    }

    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?pdb ?oligo_sequence ?residue_links ?glycosidic_linkage "
          << Ontology::WHERE_CLAUSE;
    query << "?oligo        :oligoName	?oligo_sequence.\n";
    ...
    if (count(oligo_name_pattern.begin(), oligo_name_pattern.end(), '*') == 0)
    { /// No *
    ...
    else if (count(oligo_name_pattern.begin(), oligo_name_pattern.end(), '*') == 1) /// Only one *
    {
        ...
        else if (first == oligo_name_pattern.size() - 1)
        { ///* at the end
            query << "FILTER regex(?oligo_sequence, \"^" << filter1 << "\", \"i\")\n";
        }
        else if (first < oligo_name_pattern.size() - 1 && first > 0)
        { ///* in the middle
    ...
    {
        if (first == 0 && last == oligo_name_pattern.size() - 1)
        { ///* at the beginning and end
            query << "FILTER regex(?oligo_sequence, \"" << filter2 << "\", \"i\")\n";
        }
        else if (first == 0 && last < oligo_name_pattern.size() - 1)
        { /// one * at the beginning another in the middle
            query << "FILTER regex(?oligo_sequence, \"" << filter2 << ".+" << filter3 << "$\", \"i\")\n";
        }
        else if (first > 0 && last == oligo_name_pattern.size() - 1)
        { /// one * in the middle another at the end
...
void Assembly::ExtractOntologyInfoByGlycanStructure(std::string ring_type, std::string anomeric_orientation,
                                                    std::string minus_one_orientation,
                                                    std::string index_two_orientation,
                                                    std::string index_three_orientation,
                                                    std::string index_four_orientation,
                                                    std::string plus_one_orientation, std::string output_file_type)
{
    if (ring_type.compare("") == 0)
    {
        //        std::cout << "Please specify the ring type which is the first argument of the function as either \"P\"
        //        or \"F\" " << std::endl;
        return;
    }
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE
          << " ?stereo_name ?stereo_condensed_name ?condensed_name ?name ?oligo_sequence ?residue_links "
             "?glycosidic_linkage "
          << Ontology::WHERE_CLAUSE;
    query << "?pdb_file     :hasOligo       ?oligo.\n";
    query << "?oligo	    :hasCore        ?mono.\n";
...
void Assembly::ExtractOntologyInfoByDerivativeModificationMap(std::string ring_type,
                                                              DerivativeModificationMap derivative_modification_map,
                                                              std::string output_file_type)
{
    if (ring_type.compare("") == 0)
    {
        //        std::cout << "Please specify the ring type as the first argument of the function as either \"P\" or
        //        \"F\" " << std::endl;
        return;
    }
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << "?pdb ?stereo_name ?stereo_condensed_name ?condensed_name "
          << Ontology::WHERE_CLAUSE;
    for (DerivativeModificationMap::iterator it = derivative_modification_map.begin();
         it != derivative_modification_map.end(); it++)
    {
...
void Assembly::ExtractOntologyInfoByAttachedGlycanStructures(AttachedGlycanStructuresVector attached_structures,
                                                             std::string output_file_type)
{
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE
          << " ?pdb ?linkage_indices ?stereo_short_name0 ?short_name0 ?stereo_short_name1 ?short_name1 ?oligo_sequence "
             "?residue_links "
          << Ontology::WHERE_CLAUSE;
    unsigned int i                  = 0;
    std::vector<std::string> oligos = std::vector<std::string>();
    for (AttachedGlycanStructuresVector::iterator it = attached_structures.begin(); it != attached_structures.end();
         it++)
    {
        std::vector<std::string> structure = (*it);
        if (structure.size() < 7)
        {
            //            std::cout << "Missing arguments! All should be set even as an empty value (for empty values
            //            set \"\")" << std::endl;
            return;
        }
        if (structure.at(0).compare("") == 0)
        {
            //            std::cout << "Please specify the ring type which is the first argument of the function as
            //            either \"P\" or \"F\" " << std::endl;
            return;
        }
...
void Assembly::ExtractOntologyInfoByNote(std::string pdb_id, std::string note_type, std::string note_category,
                                         std::string output_file_type)
{
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?pdb ?note_type ?note_category ?description "
          << Ontology::WHERE_CLAUSE;
...
void Assembly::ExtractAtomCoordinatesForTorsionAnglesFromOntologySlow(
    std::string disaccharide_pattern /*, std::string output_file_type*/)
{
    ...
    std::string parent_mono =
        disaccharide_pattern.substr(link_index + 2, disaccharide_pattern.size()); /// e.g DGalpb in DNeupNAca2-3DGalpb
    ...
    { /// Disaccharides with any of 1-6, 1-7, 2-6, and 2-7 linkages might have omega torsion angles
        omega = true;
    }

    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE
          << " ?pdb ?O5_crd ?C1_crd ?Ox_crd ?Cx ?Cx_crd ?Cx_neighbor ?Cx_neighbor_crd ?O5_prime_crd "
          << Ontology::WHERE_CLAUSE;
    ...
    curl << Ontology::DATA_STORE_ADDRESS << Ontology::QUERY_PREFIX << query.str() << Ontology::QUERY_POSTFIX
         << " \\>\\> result.txt"; // Has ewarning: unknown escape sequence: '\>' [enabled by default] so changed \>\> to
                                  // \\>\\> by Ayush on 06/22/2017
    std::string tmp  = curl.str();
    const char* cstr = tmp.c_str();
    system(cstr);
}

void Assembly::ExtractAtomCoordinatesForTorsionAnglesFromOntologyFast(
    std::string disaccharide_pattern /*, std::string output_file_type*/)
{
    int link_index = disaccharide_pattern.find_first_of("-");
    ...
    //    curl << Ontology::DATA_STORE_ADDRESS << Ontology::QUERY_PREFIX << query.str() << Ontology::QUERY_POSTFIX << "
    //    >> result.txt"; std::string tmp = curl.str(); const char* cstr = tmp.c_str(); system(cstr); std::cout <<
    //    std::endl;

    /*   int link_index = disaccharide_pattern.find_first_of("-");

       std::string child_mono = disaccharide_pattern.substr(0, link_index - 1); /// e.g DNeupNAca in DNeupNAca2-3DGalpb
       std::string parent_mono = disaccharide_pattern.substr(link_index + 2, disaccharide_pattern.size()); /// e.g
       DGalpb in DNeupNAca2-3DGalpb std::string linkage_indeces = disaccharide_pattern.substr(link_index - 1, 3); bool
       omega = false; if(linkage_indeces.find("6") != std::string::npos || linkage_indeces.find("7") !=
       std::string::npos
                || linkage_indeces.find("8") != std::string::npos || linkage_indeces.find("9") != std::string::npos)
           omega = true;

       std::stringstream query;
       query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?pdb ?RA1 ?RA1_crd ?SA1 ?SA1_crd ?RA2 ?RA2_crd ?SA2
       ?SA2_crd "<< Ontology::WHERE_CLAUSE; query << "?sn1           :monosaccharideShortName    \"" << child_mono <<
       "\".\n"; query << "?mono1         :hasSugarName    ?sn1.\n"; query << "?oligo1        :hasCore    ?mono1.\n";
       query << "?sn2           :monosaccharideShortName    \"" << parent_mono << "\".\n";
       query << "?mono2         :hasSugarName    ?sn2.\n";
       query << "?oligo2        :hasCore    ?mono2.\n";
...
void Assembly::ExtractTorsionAnglesFromSlowQueryResult()
{
    ...
    if(out_file_size == 0)
    {
        ...
        out_file << std::left << std::setw(15) << "PDB" << std::setw(15) << "Phi Angle" << std::setw(15) << "Psi Angle"
    << std::setw(15) << "Omega Angle" << std::endl;
    } */
    ...
    //    std::cout << std::left << std::setw(15) << "PDB" << std::setw(15) << "Phi Angle" << std::setw(15) << "Psi
    //    Angle" << std::setw(15) << "Omega Angle" << std::endl;
    ...
    while (getline(in, line))
    {
        if (line.find("\"pdb\",\"O5_crd\",\"C1_crd\",\"Ox_crd\",\"Cx\",\"Cx_crd\",\"Cx_neighbor\",\"Cx_neighbor_crd\","
                      "\"O5_prime_crd\"") != std::string::npos)
        {
            break;
        }
    }
    ...
    while (getline(in, line))
    {
        ...
        if (last_Cx.compare("") == 0 ||
            last_Cx.compare(line_tokens.at(10)) !=
                0) // If it is the first line or the line with info about a new oligosaccharide
        {
            ...
            Cx_id = gmml::Split(line_tokens.at(10), "#").at(1); /// e.g. spliting 5BO9_C3_4773_GAL_A_410_n_n_1 from
                                                                /// http://gmmo.uga.edu/#5BO9_C3_4773_GAL_A_410_n_n_1
        }
        ...
        if (gmml::Split(Cx_neighbor_id, "_").at(1).find("C") != std::string::npos) /// If the neighbor is a carbon
        {
            ...
            int Cx_index =
                gmml::ConvertString<int>(gmml::Split(gmml::Split(Cx_id, "_").at(1), "C*,\'")
                                             .at(0)); /// e.g. 5BO9_C3_4773_GAL_A_410_n_n_1 -split-> C3 -split-> 3
            int Cx_neighbor_index =
                gmml::ConvertString<int>(gmml::Split(gmml::Split(Cx_neighbor_id, "_").at(1), "C*,\'").at(0));
            ...
            if (Cx_index > Cx_neighbor_index) /// if the neighbor is Cx-1
            {
                ...
                phi_angle          = GeometryTopology::CalculateDihedralAngle(O5_crd, C1_crd, Ox_crd, Cx_crd,
                                                                              returnRadians); /// ϕ (O5′-C1′-Ox-Cx)
                psi_angle          = GeometryTopology::CalculateDihedralAngle(C1_crd, Ox_crd, Cx_crd, Cx_neighbor_crd,
                                                                              returnRadians); /// ψ (C1′-Ox-Cx-Cx−1)

                //                std::cout << std::left << std::setw(15) << gmml::Split(gmml::Split(line_tokens.at(0),
                //                "#").at(1), "\"").at(0)
                ...
                {
                    omega_angle = GeometryTopology::CalculateDihedralAngle(
                        Ox_crd, Cx_crd, Cx_neighbor_crd, O5_prime_crd, returnRadians); /// Ω (O1-C6′-C5′-O5′)
...
std::string Assembly::ExtractOntologyInfoByOligosaccharideNameSequenceGF(std::string oligo_name,
                                                                         std::string output_file_type)
{
    std::stringstream query;
...
std::string Assembly::ExtractOntologyInfoByOligosaccharideNameSequenceByRegexGF(std::string oligo_name_pattern,
                                                                                std::string url,
                                                                                std::string output_file_type)
{
    gmml::FindReplaceString(oligo_name_pattern, "[", "\\\\[");
    ...
    if (count(oligo_name_pattern.begin(), oligo_name_pattern.end(), '*') > 3)
    {
        //        std::cout << "Wrong name pattern format. Please use only up to three * in the input argument." <<
        //        std::endl;
        return "Wrong name pattern format. Please use only up to three * in the input argument.";
    }

    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?pdb ?oligo_sequence ?residue_links "
          << Ontology::WHERE_CLAUSE;
    query << "?oligo        :oligoName	?oligo_sequence.\n";
...
std::string Assembly::ExtractOntologyInfoByPDBIDGF(std::string pdb_id, std::string url, std::string output_file_type)
{
    ...
    std::stringstream query;
    query << Ontology::PREFIX << Ontology::SELECT_CLAUSE << " ?oligo_sequence ?residue_links "
          << Ontology::WHERE_CLAUSE;
    query << ":" << pdb_id << "    :hasOligo   ?oligo.\n";
    query << "?oligo    :oligoName 	?oligo_sequence.\n";
    query << "OPTIONAL { ?oligo	:oligoResidueLinks	?residue_links.}\n";
