...
void Assembly::AddIon(std::string ion_name, std::string lib_file, std::string parameter_file, int ion_count)
{
    if (ion_count == 0)
    {
        ...
        ParameterFileSpace::ParameterFile* param =
            new ParameterFileSpace::ParameterFile(parameter_file, gmml::IONICMOD);
        ...
        if (find(ion_list.begin(), ion_list.end(), ion_name) != ion_list.end())
        {
            ...
            else if (ion_charge > 0 && charge > 0)
            {
                gmml::log(__LINE__, __FILE__, gmml::ERR,
                          "The assembly and the given ion have positive charges, neutralizing process is aborted.");
                //                std::cout << "The assembly and the given ion have positive charges, neutralizing
                //                process is aborted." << std::endl;
                return;
            }
            else if (ion_charge < 0 && charge < 0)
            {
                gmml::log(__LINE__, __FILE__, gmml::ERR,
                          "The assembly and the given ion have positive charges, neutralizing process is aborted.");
                //                std::cout << "The assembly and the given ion have negative charges, neutralizing
                //                process is aborted." << std::endl;
                return;
            }
            else
            {
                int number_of_neutralizing_ion =
                    (int)(fabs(charge) + gmml::CHARGE_TOLERANCE) / (int)(fabs(ion_charge) + gmml::CHARGE_TOLERANCE);
                std::stringstream ss;
                ...
                if (minimum_boundary->GetX() == INFINITY || minimum_boundary->GetY() == INFINITY ||
                    minimum_boundary->GetZ() == INFINITY || maximum_boundary->GetX() == -INFINITY ||
                    maximum_boundary->GetY() == -INFINITY || maximum_boundary->GetZ() == -INFINITY)
                {
                    return;
                }
                ...
                {
                    GeometryTopology::Grid* grid =
                        new GeometryTopology::Grid(this, minimum_boundary, maximum_boundary, ion_radius, ion_charge);
                    ...
                    if (best_positions.size() == 0)
                    {
                        ...
                        //                        std::cout << "There is no optimum position to place the ion" <<
                        //                        std::endl;
                        return;
                    }
                    else
                    {
                        ...
                        GeometryTopology::Coordinate* best_position = new GeometryTopology::Coordinate(
                            best_positions.at(index)->GetX(), best_positions.at(index)->GetY(),
                            best_positions.at(index)->GetZ());
                        ...
                        {
                            if (best_position->GetX() + gmml::CRITICAL_RADIOUS * ion_radius + gmml::GRID_OFFSET >
                                    (*it)->GetCellCenter()->GetX() &&
                                best_position->GetY() + gmml::CRITICAL_RADIOUS * ion_radius + gmml::GRID_OFFSET >
                                    (*it)->GetCellCenter()->GetY() &&
                                best_position->GetZ() + gmml::CRITICAL_RADIOUS * ion_radius + gmml::GRID_OFFSET >
                                    (*it)->GetCellCenter()->GetZ() &&
                                best_position->GetX() - gmml::CRITICAL_RADIOUS * ion_radius - gmml::GRID_OFFSET <
                                    (*it)->GetCellCenter()->GetX() &&
                                best_position->GetY() - gmml::CRITICAL_RADIOUS * ion_radius - gmml::GRID_OFFSET <
                                    (*it)->GetCellCenter()->GetY() &&
                                best_position->GetZ() - gmml::CRITICAL_RADIOUS * ion_radius - gmml::GRID_OFFSET <
                        ...
                        residue_id << ion->GetName() << "_" << gmml::BLANK_SPACE << "_" << (i + 1) << "_"
                        ...
                        Atom* ion_atom = new Atom(ion, lib_ion_residue->GetLibraryAtomByAtomName(ion_name)->GetName(),
                                                  atom_coordinates);
                        ion_atom->MolecularDynamicAtom::SetAtomType(
                            lib_ion_residue->GetLibraryAtomByAtomName(ion_name)->GetType());
                        ion_atom->MolecularDynamicAtom::SetCharge(
                            lib_ion_residue->GetLibraryAtomByAtomName(ion_name)->GetCharge());
    ...
    {
        gmml::log(__LINE__, __FILE__, gmml::ERR,
                  "Please have a non-negative number as the number of ion(s) want to add");
