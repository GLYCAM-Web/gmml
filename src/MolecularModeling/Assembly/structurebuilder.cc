...
//                       FUNCTIONS                      //
//////////////////////////////////////////////////////////
void Assembly::BuildStructure(gmml::BuildingStructureOption building_option, std::vector<std::string> options,
    ...
    switch (building_option)
    {
        ...
        case gmml::DATABASE:
            ...
            if (types.size() == file_paths.size())
            {
                ...
                {
                    ss << "File type: " << types.at(i) << ";"
                       << "File path: " << file_paths.at(i) << ";";
                }
                description_ = ss.str();
...
{
    MolecularModeling::DistanceCalculationThreadArgument* arg =
        (MolecularModeling::DistanceCalculationThreadArgument*)args;
    ...
    for (MolecularModeling::AtomVector::iterator it = all_atoms_of_assembly.begin() + ti * (atoms_size / t);; it++)
    {
        ...
        for (MolecularModeling::AtomVector::iterator it1 = it + 1; it1 != all_atoms_of_assembly.end(); it1++)
        {
            ...
            // X distance
            if (atom->GetCoordinates().at(model_index)->GetX() -
                    neighbor_atom->GetCoordinates().at(model_index)->GetX() <
                cutoff)
            {
                // Y distance
                if (atom->GetCoordinates().at(model_index)->GetY() -
                        neighbor_atom->GetCoordinates().at(model_index)->GetY() <
                    cutoff)
                {
                    // Z distance
                    if (atom->GetCoordinates().at(model_index)->GetZ() -
                            neighbor_atom->GetCoordinates().at(model_index)->GetZ() <
                        cutoff)
                    {
                        if ((atom->GetCoordinates()
                                 .at(model_index)
                                 ->Distance(*(neighbor_atom->GetCoordinates().at(model_index)))) < cutoff)
                        {
    ...
    if (atoms_size > 0)
    {
        for (MolecularModeling::AtomVector::iterator it = all_atoms_of_assembly.begin() + begin_index;; it++)
        {
            ...
            for (MolecularModeling::AtomVector::iterator it1 = it + 1; it1 != all_atoms_of_assembly.end(); it1++)
            {
                ...
                std::pair<double, double> minAndMaxCutoffs =
                    atomicBonds::getBondLengthByAtomType(atom->GetElementSymbol(), neighbor_atom->GetElementSymbol());
                ...
                // X distance
                if (abs(atom->GetCoordinates().at(model_index)->GetX() -
                        neighbor_atom->GetCoordinates().at(model_index)->GetX()) < cutoff)
                {
                    // Y distance
                    if (abs(atom->GetCoordinates().at(model_index)->GetY() -
                            neighbor_atom->GetCoordinates().at(model_index)->GetY()) < cutoff)
                    {
                        // Z distance
                        if (abs(atom->GetCoordinates().at(model_index)->GetZ() -
                                neighbor_atom->GetCoordinates().at(model_index)->GetZ()) < cutoff)
                        {
                            double thisDistance = (atom->GetCoordinates()
                                                       .at(model_index)
                                                       ->Distance(*(neighbor_atom->GetCoordinates().at(model_index))));
                            if (thisDistance < cutoff)
                            {
                                ...
                                {
                                    std::stringstream log;
                                    log << "The atoms " << atom->GetId() << " and " << neighbor_atom->GetId()
                                        << " were too close together to bond";
...
{
    MolecularModeling::DistanceCalculationByMatrixThreadArgument* arg =
        (MolecularModeling::DistanceCalculationByMatrixThreadArgument*)args;
    int ti                                      = arg->thread_index;
    double cutoff                               = arg->cutoff;
    int model_index                             = arg->model_index;
...
//            std::cout << "thread=" << k << ", first chunk size = " << first_chunk->size() << ", starting from " << i *
(atoms_size/matrix_size);
//            std::cout << ", 2nd chunk size = " << second_chunk->size() << ", starting from " << (j *
((atoms_size/matrix_size) )) << std::endl; arg[k] = MolecularModeling::DistanceCalculationByMatrixThreadArgument(k,
model_index, cutoff, first_chunk, second_chunk); pthread_create(&threads[k], NULL,
&BuildStructureByDistanceByMatrixThread, &arg[k]); k++;
        }
    }
...
//        std::cout << "thread=" << k << ", first chunk size = " << first_chunk->size() << ", starting from " << i *
(atoms_size/matrix_size); i++; if(!no_atoms_for_second_chunk)
        {
            for(AtomVector::iterator it = all_atoms_of_assembly.begin() + (i * (atoms_size/matrix_size)); ; it++)
            {
                if(i + 1 < matrix_size)
        ...
        //std::cout << ", 2nd chunk size = " << second_chunk->size() << ", starting from " << (i *
(atoms_size/matrix_size)) << std::endl; arg[k] = MolecularModeling::DistanceCalculationByMatrixThreadArgument(k,
model_index, cutoff, first_chunk, second_chunk); pthread_create(&threads[k], NULL,
&BuildStructureByDistanceByMatrixDiameterThread, &arg[k]); k++;
    }
...
void Assembly::BuildStructureByDistance(int number_of_threads, double cutoff, int model_index)
{
    ...
    for (int i = 0; i < number_of_threads; i++)
    {
        ...
        //        pthread_create(&threads[i], NULL, &BuildStructureByDistanceThread, &arg[i]); ///First version.
        //        Workload of threads are not equal
        pthread_create(&threads[i], NULL, &BuildStructureByDistanceByOptimizedThread,
                       &arg[i]); /// Second version. Workload of threads are roughly equal.
    }
    for (int i = 0; i < number_of_threads; i++)
    {
        if (local_debug > 0)
    ...
    try
    {
        ...
        for (AtomVector::iterator it = all_atoms_of_assembly.begin(); it != all_atoms_of_assembly.end(); it++)
        {
            ...
            if (pdb_atom != NULL)
            {
                ...
                PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap bonded_atoms_map =
                    connectivities->GetBondedAtomsSerialNumbers();
                     ...
                     it1 != bonded_atoms_serial_number.end(); it1++)
                {
                    ...
                    PdbFileSpace::PdbAtomCard* pdb_bonded_atom =
                        pdb_file->GetAtomBySerialNumber(bonded_atom_serial_number);
                    std::stringstream sss;
                    sss << pdb_bonded_atom->GetAtomName() << "_" << pdb_bonded_atom->GetAtomSerialNumber() << "_"
                        << pdb_bonded_atom->GetAtomResidueName() << "_" << pdb_bonded_atom->GetAtomChainId() << "_"
                        << pdb_bonded_atom->GetAtomResidueSequenceNumber() << "_"
                        << pdb_bonded_atom->GetAtomInsertionCode() << "_"
                        << pdb_bonded_atom->GetAtomAlternateLocation();
                    std::string pdb_bonded_atom_key = sss.str();
                    for (AtomVector::iterator it2 = all_atoms_of_assembly.begin(); it2 != all_atoms_of_assembly.end();
                         it2++)
                    {
                        if (it != it2)
                        {
...
void Assembly::BuildStructureByLIBFileInformation()
{
    ...
    for (AtomVector::iterator it = all_atoms_of_assembly.begin(); it != all_atoms_of_assembly.end(); it++)
    {
        ...
        LibraryFileSpace::LibraryFileResidue* library_residue =
            library_file->GetLibraryResidueByResidueName(assembly_residue->GetName());
        if (library_residue != NULL)
        {
            LibraryFileSpace::LibraryFileAtom* library_atom =
                library_residue->GetAtomByOrder(gmml::ConvertString<int>(atom_id_tokens.at(1)));
            if (library_atom != NULL)
            {
                std::vector<int> library_bonded_atom_indices = library_atom->GetBondedAtomsIndices();
                for (std::vector<int>::iterator it1 = library_bonded_atom_indices.begin();
                     it1 != library_bonded_atom_indices.end(); it1++)
                {
                    int library_bonded_atom_index = (*it1);
                    LibraryFileSpace::LibraryFileAtom* library_atom =
                        library_residue->GetAtomByOrder(library_bonded_atom_index);
                    for (AtomVector::iterator it2 = all_atoms_of_assembly.begin(); it2 != all_atoms_of_assembly.end();
                         it2++)
                    {
                        MolecularModeling::Atom* assembly_atom = (*it2);
                        ...
                        ss << library_residue->GetName() << ":" << library_atom->GetName() << "("
                           << library_atom->GetAtomOrder() << ")";
                        std::string library_atom_id                      = ss.str();
                        std::vector<std::string> assembly_atom_id_tokens = gmml::Split(assembly_atom_id, "_");
                        std::stringstream sss;
                        sss << assembly_atom_id_tokens.at(2) << ":" << assembly_atom_id_tokens.at(0) << "("
                            << assembly_atom_id_tokens.at(1) << ")";
...
void Assembly::BuildStructureByPrepFileInformation()
{
    ...
    for (AtomVector::iterator it = all_atoms_of_assembly.begin(); it != all_atoms_of_assembly.end(); it++)
    {
        ...
        if (prep_residue != NULL)
        {
            ...
            if (prep_atom != NULL)
            {
                std::vector<int> bonded_atoms_index =
                    prep_residue->GetBondingsOfResidue()[prep_residue->GetAtomIndexByName(atom->GetName())];
                for (std::vector<int>::iterator it1 = bonded_atoms_index.begin(); it1 != bonded_atoms_index.end();
                     it1++)
                {
                    int bonded_atom_index = (*it1);
                    PrepFileSpace::PrepFileAtom* bonded_atom =
                        prep_residue->GetPrepAtomByName(prep_residue->GetAtomNameByIndex(bonded_atom_index));
                    std::stringstream ss;
...
void Assembly::BuildStructureByDatabaseFilesBondingInformation(std::vector<gmml::InputFileType> types,
                                                               std::vector<std::string> file_paths)
{
    ...
    for (AtomVector::iterator it = all_atoms_of_assembly.begin(); it != all_atoms_of_assembly.end(); it++)
    {
        ...
        for (unsigned int i = 0; i < types.size(); i++)
        {
            if (types.at(i) == gmml::LIB)
            {
                ...
                {
                    LibraryFileSpace::LibraryFileAtom* library_atom =
                        library_residue->GetLibraryAtomByAtomName(atom->GetName());
                    if (library_atom != NULL)
                    {
                        std::vector<int> library_bonded_atom_indices = library_atom->GetBondedAtomsIndices();
                        for (std::vector<int>::iterator it1 = library_bonded_atom_indices.begin();
                             it1 != library_bonded_atom_indices.end(); it1++)
                        {
                            int library_bonded_atom_index = (*it1);
                            LibraryFileSpace::LibraryFileAtom* library_atom =
                                library_residue->GetAtomByIndex(library_bonded_atom_index);
                            for (AtomVector::iterator it2 = all_atoms_of_assembly.begin();
                                 it2 != all_atoms_of_assembly.end(); it2++)
                            {
                                MolecularModeling::Atom* assembly_atom = (*it2);
                                std::string assembly_atom_id           = assembly_atom->GetId();
                                std::stringstream ss;
            ...
            if (types.at(i) == gmml::PREP)
            {
                ...
                if (prep_residue != NULL)
                {
                    ...
                    if (prep_atom != NULL)
                    {
                        std::vector<int> bonded_atoms_index =
                            prep_residue->GetBondingsOfResidue()[prep_residue->GetAtomIndexByName(atom->GetName())];
                        for (std::vector<int>::iterator it1 = bonded_atoms_index.begin();
                             it1 != bonded_atoms_index.end(); it1++)
                        {
                            int bonded_atom_index = (*it1);
                            PrepFileSpace::PrepFileAtom* bonded_atom =
                                prep_residue->GetPrepAtomByName(prep_residue->GetAtomNameByIndex(bonded_atom_index));
