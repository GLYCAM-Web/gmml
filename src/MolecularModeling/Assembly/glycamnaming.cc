...
void Assembly::PutAglyconeInNewResidueAndRearrangeGlycanResidues(std::vector<Glycan::Oligosaccharide*> oligosaccharides, std::map<Glycan::Oligosaccharide*, ResidueVector>& oligo_residue_map){
        ...
        oligo_id_map, std::map<Glycan::Oligosaccharide*, ResidueVector>& oligo_residue_map)
{
    ...
    for(std::vector<Glycan::Oligosaccharide*>::iterator it = oligosaccharides.begin(); it != oligosaccharides.end(); it++)
    {
        ...
        //Detect anomeric carbon and oxygen to determine aglycone atoms, which in regular PDB file is put in the same residue of the 1st sugar. For Glycam, this goes into a new residue, such as ROH. 
        MolecularModeling::Atom* anomeric_o = NULL;
        MolecularModeling::Atom* anomeric_c = NULL;
        if(oligo->root_->cycle_atoms_.at(0) != NULL){
            anomeric_c = oligo->root_->cycle_atoms_.at(0);
...
void Assembly::RemoveDownstreamMatches(Atom* target_atom, std::vector<std::map<Atom*, Atom*> >& target_template_vertex_match, 
	    ...
	    //Remove downstream matches
            for (std::vector<std::map<Atom*, Atom*> >::iterator vec_it = target_template_vertex_match.begin(); vec_it != target_template_vertex_match.end(); vec_it++){
                ...
                if (current_match.find(downstream_target) != current_match.end()){
                    ...
                    for (std::vector<std::map<Atom*, Atom*> >::iterator vec_it2 = template_target_vertex_match.begin(); vec_it2 != template_target_vertex_match.end(); vec_it2++){
        ...
        std::vector<Atom*>& target_insertion_order, Pdb2glycamMatchingTracker* pdb2glycam_matching_tracker, int previous_call_depth)
{
    ...
    if (this->AtomVertexMatch(target_atom, template_atom, target_atom_label_map, template_atom_label_map)){  
        ...
        mismatch_notice << "Current target " << target_atom_label_map[target_atom] << " does not match current template " << template_atom_label_map[template_atom];
        pdb2glycam_matching_tracker->failures.emplace_back(new Pdb2glycamMatchingFailInfo(pdb2glycam_matching_tracker, this_call_depth, target_atom, mismatch_notice.str()));
...
bool Assembly::CheckAndAcceptMatches(MolecularModeling::AtomVector& target_atoms, std::vector<std::map<Atom*, Atom*> >& target_template_vertex_match, std::vector<std::map<Atom*, Atom*> >& template_target_vertex_match, Pdb2glycamMatchingTracker* tracker){
    ...
    if (target_template_vertex_match.size() > 1){
        target_template_vertex_match.erase(target_template_vertex_match.begin() +1, target_template_vertex_match.end());
    }
    if (template_target_vertex_match.size() > 1){
        template_target_vertex_match.erase(template_target_vertex_match.begin() +1, template_target_vertex_match.end());
    }
