...
//                       FUNCTIONS                      //
//////////////////////////////////////////////////////////
void Assembly::AddSolvent(double extension, double closeness, Assembly* solvent_component_assembly,
                          std::string lib_file)
{
    ...
    GeometryTopology::Coordinate* center_of_box = new GeometryTopology::Coordinate(
        solute_min_boundary->GetX() + solute_length / 2, solute_min_boundary->GetY() + solute_width / 2,
        solute_min_boundary->GetZ() + solute_height / 2);
    // Creating the solvent cube around the center of solute
    GeometryTopology::Coordinate* solvent_box_min_boundary =
        new GeometryTopology::Coordinate(center_of_box->GetX(), center_of_box->GetY(), center_of_box->GetZ());
    solvent_box_min_boundary->operator+(-solvent_box_dimension);
    GeometryTopology::Coordinate* solvent_box_max_boundary =
        new GeometryTopology::Coordinate(center_of_box->GetX(), center_of_box->GetY(), center_of_box->GetZ());
    ...
    // Amount of shifting of the default water box along x, y and z axis to be placed in the min corner of the solvent
    // cube
            ...
            for (int k = 0; k < z_copy; k++)
            {
                ...
                for (AtomVector::iterator it = all_atoms_of_tip.begin(); it != all_atoms_of_tip.end(); it++)
                {
                    (*it)
                        ->GetCoordinates()
                        .at(model_index_)
                        ->Translate(shift_x + i * solvent_length, shift_y + j * solvent_width,
                                    shift_z + k * solvent_height);
                    (*it)->SetDescription("Het;");

                    // Check if the atom of the water box residue is outside the solvent cube and mark it as to be
                    // removed
                    ...
                    // Check if the atom of water box residue is overlaping the solute or is not far enough from the
                    // boundary of the solute and mark it as to be removed
                        ...
                        tip_atom_coords->GetZ() <= solute_max_boundary_with_extension->GetZ())
                    {
                        ...
                        bool flag                      = false;
                        for (AtomVector::iterator it1 = all_atoms_of_solute.begin(); it1 != all_atoms_of_solute.end();
                             it1++)
                        {
                            ...
                            if (((*it)
                                     ->GetCoordinates()
                                     .at(model_index_)
                                     ->Distance(*(solute_atom->GetCoordinates().at(model_index_)))) <= closeness)
                            {
                                removed_atom_id_list.push_back((*it)->GetId());
                                flag = true;
                                break;
                ...
                // Check if one atom of HOH is in to-be-removed list and add the other two atoms belonging to the same
                // HOH
                for (AtomVector::iterator it = all_atoms_of_tip.begin(); it != all_atoms_of_tip.end(); it++)
                {
                    Atom* tip_atom = *it;
                    if (find(removed_atom_id_list.begin(), removed_atom_id_list.end(), tip_atom->GetId()) !=
                        removed_atom_id_list.end())
                    {
                        ...
                        if (node != NULL)
                        {
                            ...
                            for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
                            {
                                ...
                                if (neighbor_node != NULL)
                                {
                                    ...
                                    for (AtomVector::iterator it2 = neighbors_of_neighbor.begin();
                ...
                for (AtomVector::iterator it = all_atoms_of_tip.begin(); it != all_atoms_of_tip.end(); it++)
                {
                        ...
                        removed_atom_id_list.end())
                    {
                        ...
                        std::string id = residue_name + "_" + gmml::BLANK_SPACE + "_" +
                                         gmml::ConvertT<int>(sequence_number) + "_" + gmml::BLANK_SPACE + "_" +
                                         gmml::BLANK_SPACE + "_" + this->GetId();
                        tip_residue->SetId(id);
                        std::string atom_id = tip_atom->GetName() + "_" + gmml::ConvertT<int>(serial_number) + "_" + id;
                        serial_number++;
                        tip_atom->SetId(atom_id);
                        tip_residue->SetIsResidueSolvent(
                            true); // Setting Residue as solvent uisng flag  --- edited by Ayush on 06/28/2017
                    }
                }
