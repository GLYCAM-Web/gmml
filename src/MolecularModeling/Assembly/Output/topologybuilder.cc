...
//                       FUNCTIONS                      //
//////////////////////////////////////////////////////////
TopologyFileSpace::TopologyFile* Assembly::BuildTopologyFileStructureFromAssembly(std::string parameter_file_path,
                                                                                  std::string ion_parameter_file_path)
{
    ...
    ParameterFileSpace::ParameterFile::AtomTypeMap ion_atom_types_map =
        ParameterFileSpace::ParameterFile::AtomTypeMap();
    ...
    for (ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
        ...
        std::vector<gmml::TopologicalType> residue_topological_types =
            GetAllTopologicalTypesOfAtomsOfResidue(assembly_atoms, loops, bond_index, 0);
        ...
        for (AtomVector::iterator it1 = assembly_atoms.begin(); it1 != assembly_atoms.end(); it1++)
        {
            ...
            //                std::cout << assembly_atom->GetAtomType() << " atom type is not found in parameter file"
            //                << std::endl;
            ...
            topology_atom->SetTreeChainClasification(
                gmml::ConvertTopologicalType2String(residue_topological_types.at(atom_index - 1)));
            ...
            if (atom_node != NULL)
            {
                ...
                for (AtomVector::iterator it2 = neighbors.begin(); it2 != neighbors.end(); it2++)
                {
                    ...
                    key2 << neighbor->GetId();
                    ExtractTopologyBondTypesFromAssembly(inserted_bond_types, assembly_atom, neighbor, bonds,
                                                         bond_type_counter, topology_file);
                    ExtractTopologyBondsFromAssembly(inserted_bonds, inserted_bond_types, assembly_atom, neighbor,
                                                     topology_file);
                    ...
                    if (find(excluded_atom_list.begin(), excluded_atom_list.end(), first_order_interaction.str()) ==
                            excluded_atom_list.end() &&
                        find(excluded_atom_list.begin(), excluded_atom_list.end(),
                             reverse_first_order_interaction.str()) == excluded_atom_list.end())
                    {
                        excluded_atom_list.push_back(first_order_interaction.str());
                        std::vector<std::string> key_tokens = gmml::Split(key2.str(), "_");
                        topology_atom->AddExcludedAtom(key_tokens.at(2) + "(" + key_tokens.at(4) +
                                                       "):" + key_tokens.at(0) + "(" + key_tokens.at(1) + ")");
                    }
                    ...
                    for (AtomVector::iterator it3 = neighbors_of_neighbor.begin(); it3 != neighbors_of_neighbor.end();
                         it3++)
                    {
                        ...
                        {
                            ExtractTopologyAngleTypesFromAssembly(assembly_atom, neighbor, neighbor_of_neighbor,
                                                                  inserted_angle_types, angle_type_counter,
                                                                  topology_file, angles);
                            ExtractTopologyAnglesFromAssembly(assembly_atom, neighbor, neighbor_of_neighbor,
                                                              inserted_angles, inserted_angle_types, topology_file);
                            ...
                            if (find(excluded_atom_list.begin(), excluded_atom_list.end(),
                                     second_order_interaction.str()) == excluded_atom_list.end() &&
                                find(excluded_atom_list.begin(), excluded_atom_list.end(),
                                     reverse_second_order_interaction.str()) == excluded_atom_list.end())
                            {
                            ...
                            for (AtomVector::iterator it4 = neighbors_of_neighbor_neighbor.begin();
                                 it4 != neighbors_of_neighbor_neighbor.end(); it4++)
                            {
                                ...
                                if (key2.str().compare(key4.str()) != 0)
                                {
                                    ExtractTopologyDihedralTypesFromAssembly(
                                        assembly_atom, neighbor, neighbor_of_neighbor, neighbor_of_neighbor_of_neighbor,
                                        inserted_dihedral_types, dihedral_type_counter, topology_file, dihedrals);
                                    ExtractTopologyDihedralsFromAssembly(
                                        assembly_atom, neighbor, neighbor_of_neighbor, neighbor_of_neighbor_of_neighbor,
                                    ...
                                    if (find(excluded_atom_list.begin(), excluded_atom_list.end(),
                                             third_order_interaction.str()) == excluded_atom_list.end() &&
                                        find(excluded_atom_list.begin(), excluded_atom_list.end(),
                                             reverse_third_order_interaction.str()) == excluded_atom_list.end())
                                    {
            ...
            topology_atom->GetExcludedAtoms().size() == 0
                ? number_of_excluded_atoms++
    ...
    for (AtomVector::iterator it = all_atoms.begin(); it != all_atoms.end(); it++)
    {
        ...
        for (AtomVector::iterator it2 = all_atoms.begin(); it2 != all_atoms.end(); it2++)
        {
                ...
                find(inserted_pairs.begin(), inserted_pairs.end(), reverse_sss.str()) == inserted_pairs.end())
            {
                ...
                if (atom_types_map.find(atom_type1) != atom_types_map.end() &&
                ...
                else if (!ion_atom_types_map.empty() &&
                         ion_atom_types_map.find(atom_type1) != ion_atom_types_map.end() &&
    ...
    topology_file->SetNumberOfDihedralsIncludingHydrogen(
        this->CountNumberOfDihedralsIncludingHydrogen(parameter_file_path));
    topology_file->SetNumberOfDihedralsExcludingHydrogen(
        this->CountNumberOfDihedralsExcludingHydrogen(parameter_file_path));
...
void Assembly::ExtractTopologyBondTypesFromAssembly(std::vector<std::vector<std::string>>& inserted_bond_types,
                                                    ...
                                                    TopologyFileSpace::TopologyFile* topology_file)
{
    ...
    if (find(inserted_bond_types.begin(), inserted_bond_types.end(), atom_pair_type) == inserted_bond_types.end() &&
        find(inserted_bond_types.begin(), inserted_bond_types.end(), reverse_atom_pair_type) ==
            inserted_bond_types.end())
    {
        ...
        {
            //            std::stringstream ss;
            //            ss << atom_pair_type.at(0) << "-" << atom_pair_type.at(1) << " bond type does not exist in the
            //            parameter files"; std::cout << ss.str() << std::endl; gmml::log(__LINE__, __FILE__, gmml::ERR,
            //            ss.str());
            return;
        }
...
void Assembly::ExtractTopologyBondsFromAssembly(std::vector<std::vector<std::string>>& inserted_bonds,
                                                std::vector<std::vector<std::string>>& inserted_bond_types,
                                                Atom* assembly_atom, Atom* neighbor,
                                                TopologyFileSpace::TopologyFile* topology_file)
{
    std::vector<std::string> atom_pair_type         = std::vector<std::string>();
    ...
    residue_names.push_back(assembly_atom->GetResidue()->GetName() + "(" +
                            gmml::Split(assembly_atom->GetResidue()->GetId(), "_").at(2) + ")");
    residue_names.push_back(neighbor->GetResidue()->GetName() + "(" +
                            gmml::Split(neighbor->GetResidue()->GetId(), "_").at(2) + ")");
        ...
        find(inserted_bonds.begin(), inserted_bonds.end(), reverse_bond) == inserted_bonds.end())
    {
        ...
        if ((assembly_atom->GetName().substr(0, 1).compare("H") == 0 ||
             (assembly_atom->GetName().substr(1, 1).compare("H") == 0 &&
              isdigit(gmml::ConvertString<char>(assembly_atom->GetName().substr(0, 1))))) ||
            (neighbor->GetName().substr(0, 1).compare("H") == 0 ||
             (neighbor->GetName().substr(1, 1).compare("H") == 0 &&
              isdigit(gmml::ConvertString<char>(neighbor->GetName().substr(0, 1))))))
        {
        ...
        if (find(inserted_bond_types.begin(), inserted_bond_types.end(), atom_pair_type) != inserted_bond_types.end())
        {
            index = distance(inserted_bond_types.begin(),
                             find(inserted_bond_types.begin(), inserted_bond_types.end(), atom_pair_type));
        }
        else if (find(inserted_bond_types.begin(), inserted_bond_types.end(), reverse_atom_pair_type) !=
                 inserted_bond_types.end())
        {
            index = distance(inserted_bond_types.begin(),
                             find(inserted_bond_types.begin(), inserted_bond_types.end(), reverse_atom_pair_type));
...
void Assembly::ExtractTopologyAngleTypesFromAssembly(Atom* assembly_atom, Atom* neighbor, Atom* neighbor_of_neighbor,
                                                     ...
                                                     ParameterFileSpace::ParameterFile::AngleMap& angles)
{
    ...
    if (find(inserted_angle_types.begin(), inserted_angle_types.end(), angle_type) == inserted_angle_types.end() &&
        find(inserted_angle_types.begin(), inserted_angle_types.end(), reverse_angle_type) ==
            inserted_angle_types.end())
    {
        ...
        {
            //            std::stringstream ss;
            //            ss << angle_type.at(0) << "-" << angle_type.at(1) << "-" << angle_type.at(2) << " angle type
            //            does not exist in the parameter files"; std::cout << ss.str() << std::endl;
...
void Assembly::ExtractTopologyAnglesFromAssembly(Atom* assembly_atom, Atom* neighbor, Atom* neighbor_of_neighbor,
                                                 ...
                                                 TopologyFileSpace::TopologyFile* topology_file)
{
    ...
    angle_atom_names.push_back(neighbor_of_neighbor->GetName() + "(" +
                               gmml::Split(neighbor_of_neighbor->GetId(), "_").at(1) + ")");
    ...
    residue_names.push_back(neighbor_of_neighbor->GetResidue()->GetName() + "(" +
                            gmml::Split(neighbor_of_neighbor->GetResidue()->GetId(), "_").at(2) + ")");
        ...
        find(inserted_angles.begin(), inserted_angles.end(), reverse_angle) == inserted_angles.end())
    {
        ...
        if ((assembly_atom->GetName().substr(0, 1).compare("H") == 0 ||
            ...
            (neighbor_of_neighbor->GetName().substr(0, 1).compare("H") == 0 ||
             (neighbor_of_neighbor->GetName().substr(1, 1).compare("H") == 0 &&
              isdigit(gmml::ConvertString<char>(neighbor_of_neighbor->GetName().substr(0, 1))))))
        {
        ...
        if (find(inserted_angle_types.begin(), inserted_angle_types.end(), angle_type) != inserted_angle_types.end())
        {
            index = distance(inserted_angle_types.begin(),
                             find(inserted_angle_types.begin(), inserted_angle_types.end(), angle_type));
        }
        else if (find(inserted_angle_types.begin(), inserted_angle_types.end(), reverse_angle_type) !=
                 inserted_angle_types.end())
        {
            index = distance(inserted_angle_types.begin(),
                             find(inserted_angle_types.begin(), inserted_angle_types.end(), reverse_angle_type));
...
void Assembly::ExtractTopologyDihedralTypesFromAssembly(Atom* assembly_atom, Atom* neighbor, Atom* neighbor_of_neighbor,
...
{
    std::vector<std::vector<std::string>> all_atom_type_permutations = CreateAllAtomTypePermutationsforDihedralType(
        assembly_atom->GetAtomType(), neighbor->GetAtomType(), neighbor_of_neighbor->GetAtomType(),
        neighbor_of_neighbor_of_neighbor->GetAtomType());
    bool is_found = false;
    for (std::vector<std::vector<std::string>>::iterator it = all_atom_type_permutations.begin();
         it != all_atom_type_permutations.end(); it++)
    {
        ...
        if (dihedrals[atom_types] != NULL)
        {
            ...
            if (find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(), ss.str()) ==
                inserted_dihedral_types.end())
            {
                ParameterFileSpace::ParameterFileDihedral* parameter_file_dihedral = dihedrals[atom_types];
                std::vector<ParameterFileSpace::ParameterFileDihedralTerm> dihedral_terms =
                    parameter_file_dihedral->GetTerms();
                for (std::vector<ParameterFileSpace::ParameterFileDihedralTerm>::iterator it1 = dihedral_terms.begin();
                     it1 != dihedral_terms.end(); it1++)
                {
                    inserted_dihedral_types.push_back(ss.str());
                    ParameterFileSpace::ParameterFileDihedralTerm parameter_file_dihedral_term = (*it1);
                    TopologyFileSpace::TopologyDihedralType* topology_dihedral_type =
                        new TopologyFileSpace::TopologyDihedralType();
    ...
    {
        //        std::stringstream ss;
        //        ss << all_atom_type_permutations.at(0).at(0) << "-" << all_atom_type_permutations.at(0).at(1) << "-"
        //        << all_atom_type_permutations.at(0).at(2) << "-"
        //           << all_atom_type_permutations.at(0).at(3) << " dihedral type (or any other permutation of it) does
        //           not exist in the parameter files";
    ...
    if (neighbors.size() == 3)
    {
        ...
        std::vector<std::vector<std::string>> all_improper_dihedrals_atom_type_permutations =
            CreateAllAtomTypePermutationsforImproperDihedralType(neighbor1->GetAtomType(), neighbor2->GetAtomType(),
                                                                 neighbor3->GetAtomType(),
                                                                 assembly_atom->GetAtomType());
        bool is_improper_found = false;
        for (std::vector<std::vector<std::string>>::iterator it = all_improper_dihedrals_atom_type_permutations.begin();
             it != all_improper_dihedrals_atom_type_permutations.end(); it++)
        {
            std::vector<std::string> improper_dihedral_permutation = (*it);
            if (dihedrals[improper_dihedral_permutation] != NULL)
            {
                std::stringstream ss;
                ss << improper_dihedral_permutation.at(0) << "_" << improper_dihedral_permutation.at(1) << "_"
                   << improper_dihedral_permutation.at(2) << "_" << improper_dihedral_permutation.at(3);
                if (find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(), ss.str()) ==
                    inserted_dihedral_types.end())
                {
                    ParameterFileSpace::ParameterFileDihedral* parameter_file_dihedral =
                        dihedrals[improper_dihedral_permutation];
                    std::vector<ParameterFileSpace::ParameterFileDihedralTerm> dihedral_terms =
                        parameter_file_dihedral->GetTerms();
                    for (std::vector<ParameterFileSpace::ParameterFileDihedralTerm>::iterator it1 =
                             dihedral_terms.begin();
                         it1 != dihedral_terms.end(); it1++)
                    {
                        inserted_dihedral_types.push_back(ss.str());
                        ParameterFileSpace::ParameterFileDihedralTerm parameter_file_dihedral_term = (*it1);
        ...
        {
            //            std::stringstream ss;
            //            ss << all_improper_dihedrals_atom_type_permutations.at(0).at(0) << "-" <<
            //            all_improper_dihedrals_atom_type_permutations.at(0).at(1) << "-"
            //               << all_improper_dihedrals_atom_type_permutations.at(0).at(2) << "-" <<
            //               all_improper_dihedrals_atom_type_permutations.at(0).at(3)
            //               << " improer dihedral type (or any other permutation of it) does not exist in the parameter
            //               files";
            //            std::cout << ss.str() << std::endl;
            //            gmml::log(__LINE__, __FILE__, gmml::ERR, ss.str());
        }
...
void Assembly::ExtractTopologyDihedralsFromAssembly(Atom* assembly_atom, Atom* neighbor, Atom* neighbor_of_neighbor,
                                                    Atom* neighbor_of_neighbor_of_neighbor,
                                                    ...
                                                    TopologyFileSpace::TopologyFile* topology_file)
{
         ...
         it != all_atom_type_permutations.end(); it++)
    {
        ...
        if (find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(), sss.str()) !=
            inserted_dihedral_types.end())
        {
            ...
            dihedral_atom_names.push_back(assembly_atom->GetName() + "(" +
                                          gmml::Split(assembly_atom->GetId(), "_").at(1) + ")");
            ...
            dihedral_atom_names.push_back(neighbor_of_neighbor_of_neighbor->GetName() + "(" +
                                          gmml::Split(neighbor_of_neighbor_of_neighbor->GetId(), "_").at(1) + ")");
            ...
            reverse_dihedral_atom_names.push_back(neighbor->GetName() + "(" +
                                                  gmml::Split(neighbor->GetId(), "_").at(1) + ")");
            ...
            residue_names.push_back(neighbor_of_neighbor_of_neighbor->GetResidue()->GetName() + "(" +
                                    gmml::Split(neighbor_of_neighbor_of_neighbor->GetResidue()->GetId(), "_").at(2) +
                                    ")");
            reverse_residue_names.push_back(
                neighbor_of_neighbor_of_neighbor->GetResidue()->GetName() + "(" +
                gmml::Split(neighbor_of_neighbor_of_neighbor->GetResidue()->GetId(), "_").at(2) + ")");
            reverse_residue_names.push_back(neighbor_of_neighbor->GetResidue()->GetName() + "(" +
                                            gmml::Split(neighbor_of_neighbor->GetResidue()->GetId(), "_").at(2) + ")");
            reverse_residue_names.push_back(neighbor->GetResidue()->GetName() + "(" +
            ...
            if (find(inserted_dihedrals.begin(), inserted_dihedrals.end(), dihedral) == inserted_dihedrals.end() &&
                find(inserted_dihedrals.begin(), inserted_dihedrals.end(), reverse_dihedral) ==
                    inserted_dihedrals.end())
            {
                ParameterFileSpace::ParameterFileDihedral* parameter_file_dihedral = dihedrals[atom_types];
                std::vector<ParameterFileSpace::ParameterFileDihedralTerm> dihedral_terms =
                    parameter_file_dihedral->GetTerms();
                     ...
                     it1 != dihedral_terms.end(); it1++)
                {
                    ...
                    if ((assembly_atom->GetName().substr(0, 1).compare("H") == 0 ||
                        ...
                        (neighbor_of_neighbor_of_neighbor->GetName().substr(0, 1).compare("H") == 0 ||
                         (neighbor_of_neighbor_of_neighbor->GetName().substr(1, 1).compare("H") == 0 &&
                          isdigit(
                              gmml::ConvertString<char>(neighbor_of_neighbor_of_neighbor->GetName().substr(0, 1))))))
                    {
                    ...
                    if ((atom_types.at(0).compare(assembly_atom->GetAtomType()) == 0) ||
                        ((atom_types.at(0).compare("X") == 0) &&
                         (atom_types.at(3).compare(neighbor_of_neighbor_of_neighbor->GetAtomType()) == 0)) ||
                        ((atom_types.at(0).compare("X") == 0 && atom_types.at(3).compare("X") == 0) &&
                         (atom_types.at(1).compare(neighbor->GetAtomType()) == 0)) ||
                        ((atom_types.at(0).compare("X") == 0 && atom_types.at(3).compare("X") == 0) &&
                         (atom_types.at(2).compare(neighbor_of_neighbor->GetAtomType())) == 0))
                    {
                    ...
                    {
                        index =
                            distance(inserted_dihedral_types.begin(),
                                     find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(), sss.str())) +
    ...
    if (neighbors.size() == 3)
    {
             ...
             it != all_improper_dihedrals_atom_type_permutations.end(); it++)
        {
            std::vector<std::string> improper_dihedral_permutation = (*it);
            std::stringstream sss;
            sss << improper_dihedral_permutation.at(0) << "_" << improper_dihedral_permutation.at(1) << "_"
                << improper_dihedral_permutation.at(2) << "_" << improper_dihedral_permutation.at(3);
            if (find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(), sss.str()) !=
                inserted_dihedral_types.end())
            {
                ...
                if (find(inserted_dihedrals.begin(), inserted_dihedrals.end(), dihedral1) == inserted_dihedrals.end() &&
                    ...
                    find(inserted_dihedrals.begin(), inserted_dihedrals.end(), reverse_dihedral1) ==
                        inserted_dihedrals.end() &&
                    find(inserted_dihedrals.begin(), inserted_dihedrals.end(), reverse_dihedral2) ==
                        inserted_dihedrals.end() &&
                    find(inserted_dihedrals.begin(), inserted_dihedrals.end(), reverse_dihedral3) ==
                        inserted_dihedrals.end())
                {
                    ...
                    for (std::vector<ParameterFileSpace::ParameterFileDihedralTerm>::iterator it1 =
                         ...
                         it1 != dihedral_terms.end(); it1++)
                    {
                        TopologyFileSpace::TopologyDihedral* topology_dihedral =
                            new TopologyFileSpace::TopologyDihedral();
                        ...
                        if ((assembly_atom->GetName().substr(0, 1).compare("H") == 0 ||
                            ...
                            (neighbor1->GetName().substr(0, 1).compare("H") == 0 ||
                             (neighbor1->GetName().substr(1, 1).compare("H") == 0 &&
                              isdigit(gmml::ConvertString<char>(neighbor1->GetName().substr(0, 1))))) ||
                            (neighbor2->GetName().substr(0, 1).compare("H") == 0 ||
                             (neighbor2->GetName().substr(1, 1).compare("H") == 0 &&
                              isdigit(gmml::ConvertString<char>(neighbor2->GetName().substr(0, 1))))) ||
                            (neighbor3->GetName().substr(0, 1).compare("H") == 0 ||
                             (neighbor3->GetName().substr(1, 1).compare("H") == 0 &&
                              isdigit(gmml::ConvertString<char>(neighbor3->GetName().substr(0, 1))))))
                        {
                            topology_dihedral->SetIncludingHydrogen(true);
                        }
                        else
                        ...
                        if (find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(), sss.str()) !=
                            inserted_dihedral_types.end())
                        {
                            index = distance(inserted_dihedral_types.begin(),
                                             find(inserted_dihedral_types.begin(), inserted_dihedral_types.end(),
                                                  sss.str())) +
                                    distance(dihedral_terms.begin(), it1);
                        }
                        topology_dihedral->SetDihedralType(topology_file->GetDihedralTypeByIndex(index));
...
int Assembly::CountNumberOfBondsIncludingHydrogen(std::string parameter_file_path)
{
    ...
    for (AtomVector::iterator it = atoms.begin(); it != atoms.end(); it++)
    {
        ...
        if (atom_node != NULL)
        {
            ...
            //                (atom_name.substr(1,1).compare("H") == 0 &&
            //                isdigit(gmml::ConvertString<char>(atom_name.substr(0,1))))))
            //            {
            ...
            for (AtomVector::iterator it1 = node_neighbors.begin(); it1 != node_neighbors.end(); it1++)
            {
                ...
                if ((atom_name.substr(0, 1).compare("H") == 0 ||
                     (atom_name.substr(1, 1).compare("H") == 0 &&
                      isdigit(gmml::ConvertString<char>(atom_name.substr(0, 1))))) ||
                    (node_neighbor_name.substr(0, 1).compare("H") == 0 ||
                     (node_neighbor_name.substr(1, 1).compare("H") == 0 &&
                      isdigit(gmml::ConvertString<char>(node_neighbor_name.substr(0, 1))))))
                {
                    std::vector<std::string> atom_pair_type         = std::vector<std::string>();
                    std::vector<std::string> reverse_atom_pair_type = std::vector<std::string>();
    ...
    for (AtomVector::iterator it = atoms.begin(); it != atoms.end(); it++)
    {
        ...
        if (atom_node != NULL)
        {
            ...
            for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
            {
                ...
                for (AtomVector::iterator it2 = neighbors_of_neighbor.begin(); it2 != neighbors_of_neighbor.end();
                     it2++)
                {
                    ...
                    if (ss.str().compare(ss1.str()) != 0)
                    {
                        ...
                        if ((atom_name.substr(0, 1).compare("H") == 0 ||
                             (atom_name.substr(1, 1).compare("H") == 0 &&
                              isdigit(gmml::ConvertString<char>(atom_name.substr(0, 1))))) ||
                            (neighbor_name.substr(0, 1).compare("H") == 0 ||
                             (neighbor_name.substr(1, 1).compare("H") == 0 &&
                              isdigit(gmml::ConvertString<char>(neighbor_name.substr(0, 1))))) ||
                            (neighbor_of_neighbor_name.substr(0, 1).compare("H") == 0 ||
                             (neighbor_of_neighbor_name.substr(1, 1).compare("H") == 0 &&
                              isdigit(gmml::ConvertString<char>(neighbor_of_neighbor_name.substr(0, 1))))))
                        {
                            std::vector<std::string> angle_type         = std::vector<std::string>();
                            std::vector<std::string> reverse_angle_type = std::vector<std::string>();
                            ...
                            if (angles.find(angle_type) != angles.end() ||
                                angles.find(reverse_angle_type) != angles.end())
                            {
                                counter++;
                            }
                        }
    ...
    for (AtomVector::iterator it = atoms.begin(); it != atoms.end(); it++)
    {
        ...
        if (atom_node != NULL)
        {
            ...
            for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
            {
                     ...
                     it2++)
                {
                    ...
                    if (ss.str().compare(ss2.str()) != 0)
                    {
                        ...
                        for (AtomVector::iterator it3 = neighbors_of_neighbor_of_neighbor.begin();
                             it3 != neighbors_of_neighbor_of_neighbor.end(); it3++)
                        {
                            ...
                            {
                                std::string neighbor_of_neighbor_of_neighbor_name =
                                    neighbor_of_neighbor_of_neighbor->GetName();
                                if ((atom_name.substr(0, 1).compare("H") == 0 ||
                                     (atom_name.substr(1, 1).compare("H") == 0 &&
                                      isdigit(gmml::ConvertString<char>(atom_name.substr(0, 1))))) ||
                                    (neighbor_name.substr(0, 1).compare("H") == 0 ||
                                    ...
                                    (neighbor_of_neighbor_name.substr(0, 1).compare("H") == 0 ||
                                     (neighbor_of_neighbor_name.substr(1, 1).compare("H") == 0 &&
                                      isdigit(gmml::ConvertString<char>(neighbor_of_neighbor_name.substr(0, 1))))) ||
                                    (neighbor_of_neighbor_of_neighbor_name.substr(0, 1).compare("H") == 0 ||
                                     (neighbor_of_neighbor_of_neighbor_name.substr(1, 1).compare("H") == 0 &&
                                      isdigit(gmml::ConvertString<char>(
                                          neighbor_of_neighbor_of_neighbor_name.substr(0, 1))))))
                                {

                                    std::vector<std::vector<std::string>> all_atom_type_permutations =
                                        CreateAllAtomTypePermutationsforDihedralType(
                                            atom->GetAtomType(), neighbor->GetAtomType(),
                                            neighbor_of_neighbor->GetAtomType(),
                                            neighbor_of_neighbor_of_neighbor->GetAtomType());
                                    ParameterFileSpace::ParameterFile::DihedralMap dihedrals =
                                        parameter_file->GetDihedrals();
                                    for (std::vector<std::vector<std::string>>::iterator it4 =
                                             all_atom_type_permutations.begin();
                                         it4 != all_atom_type_permutations.end(); it4++)
                                    {
                                        std::vector<std::string> atom_types = (*it4);
                                        if (dihedrals[atom_types] != NULL)
                                        {
                                            ParameterFileSpace::ParameterFileDihedral* parameter_file_dihedrals =
                                                dihedrals[atom_types];
            ...
            if (neighbors.size() == 3)
            {
                ...
                std::vector<std::vector<std::string>> all_improper_dihedrals_atom_type_permutations =
                    CreateAllAtomTypePermutationsforImproperDihedralType(neighbor1->GetAtomType(),
                                                                         neighbor2->GetAtomType(),
                                                                         neighbor3->GetAtomType(), atom->GetAtomType());

                ParameterFileSpace::ParameterFile::DihedralMap dihedrals = parameter_file->GetDihedrals();
                for (std::vector<std::vector<std::string>>::iterator it1 =
                         all_improper_dihedrals_atom_type_permutations.begin();
                     it1 != all_improper_dihedrals_atom_type_permutations.end(); it1++)
                {
                    std::vector<std::string> improper_dihedral_permutation = (*it1);
                    if ((atom_name.substr(0, 1).compare("H") == 0 ||
                         (atom_name.substr(1, 1).compare("H") == 0 &&
                          isdigit(gmml::ConvertString<char>(atom_name.substr(0, 1))))) ||
                        (neighbor1_name.substr(0, 1).compare("H") == 0 ||
                         (neighbor1_name.substr(1, 1).compare("H") == 0 &&
                          isdigit(gmml::ConvertString<char>(neighbor1_name.substr(0, 1))))) ||
                        (neighbor2_name.substr(0, 1).compare("H") == 0 ||
                         (neighbor2_name.substr(1, 1).compare("H") == 0 &&
                          isdigit(gmml::ConvertString<char>(neighbor2_name.substr(0, 1))))) ||
                        (neighbor3_name.substr(0, 1).compare("H") == 0 ||
                         (neighbor3_name.substr(1, 1).compare("H") == 0 &&
                          isdigit(gmml::ConvertString<char>(neighbor3_name.substr(0, 1))))))
                    {
                        if (dihedrals[improper_dihedral_permutation] != NULL)
                        {
                            ParameterFileSpace::ParameterFileDihedral* parameter_file_dihedrals =
                    ...
                    if (ss.str().compare(ss2.str()) != 0)
                    {
                             ...
                             it3 != neighbors_of_neighbor_of_neighbor.end(); it3++)
                        {
                            ...
                            if (ss1.str().compare(ss3.str()) != 0)
                            {
                                ...
                                {
                                    std::vector<std::string> atom_types = (*it4);
                                    //                                std::cout << atom_types.at(0) << atom_types.at(1)
                                    //                                << atom_types.at(2) << atom_types.at(3) <<
                                    //                                std::endl;
                                    if (dihedrals[atom_types] != NULL)
                                    {
                                        ParameterFileSpace::ParameterFileDihedral* parameter_file_dihedrals =
                            ...
                            if (ss1.str().compare(ss3.str()) != 0)
                            {
                                     ...
                                     it4 != all_atom_type_permutations.end(); it4++)
                                {
                                    ...
                                    if (dihedrals[atom_types] != NULL)
                                    {
                                        ...
                                        ss4 << atom_types.at(0) << "_" << atom_types.at(1) << "_" << atom_types.at(2)
                                            << "_" << atom_types.at(3);
                                        std::stringstream ss5;
                                        ss5 << atom_types.at(3) << "_" << atom_types.at(2) << "_" << atom_types.at(1)
                                            << "_" << atom_types.at(0);
                                    ...
                                    //                                        ss6 << atom->GetAtomType() << "_" <<
                                    //                                        neighbor->GetAtomType() << "_" <<
                                    //                                        neighbor_of_neighbor->GetAtomType() << "_"
                                    //                                        <<
                                    //                                        neighbor_of_neighbor_of_neighbor->GetAtomType();
                                    ...
                                    //                                        << neighbor->GetAtomType() << "_" <<
                                    //                                        atom->GetAtomType();
                                    //                                        if(find(type_list.begin(),
                                    //                                        type_list.end(), ss6.str()) ==
                                    //                                        type_list.end() &&
                                    //                                                find(type_list.begin(),
                                    //                                                type_list.end(), ss7.str()) ==
                                    //                                                type_list.end() )
                                    //                                        {
                                    //                                            type_list.push_back(ss7.str());
                                    //                                            ParameterFileSpace::ParameterFileDihedral*
                                    //                                            parameter_file_dihedrals =
                                    //                                            dihedrals[atom_types]; int terms_count
                                    //                                            =
                                    //                                            parameter_file_dihedrals->GetTerms().size();
                                    //                                            counter += terms_count;
                                    //                                        }
                                    //                                    }
                                    //                                    else
                     ...
                     it1 != all_improper_dihedrals_atom_type_permutations.end(); it1++)
                {
                    ...
                    if (dihedrals[improper_dihedral_permutation] != NULL)
                    {
                        ...
                        ss << improper_dihedral_permutation.at(0) << "_" << improper_dihedral_permutation.at(1) << "_"
                           << improper_dihedral_permutation.at(2) << "_" << improper_dihedral_permutation.at(3);
                        std::stringstream ss1;
                        ss1 << improper_dihedral_permutation.at(3) << "_" << improper_dihedral_permutation.at(2) << "_"
                            << improper_dihedral_permutation.at(1) << "_" << improper_dihedral_permutation.at(0);
...
std::vector<std::vector<std::string>> Assembly::CreateAllAtomTypePermutationsforDihedralType(std::string atom_type1,
...
int Assembly::CountNumberOfExcludedAtoms()
{
    AtomVector atoms                            = GetAllAtomsOfAssembly();
    std::vector<std::string> excluded_atom_list = std::vector<std::string>();
    std::map<std::string, std::vector<std::string>> excluded_atom_list_map =
    ...
    for (AtomVector::iterator it = atoms.begin(); it != atoms.end(); it++)
    {
        ...
        if (node != NULL)
        {
            ...
            for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
            {
                ...
                if (find(excluded_atom_list.begin(), excluded_atom_list.end(), first_order_interaction.str()) ==
                        excluded_atom_list.end() &&
                    find(excluded_atom_list.begin(), excluded_atom_list.end(), reverse_first_order_interaction.str()) ==
                        excluded_atom_list.end())
                {
                    excluded_atom_list.push_back(first_order_interaction.str());
                ...
                for (AtomVector::iterator it2 = neighbor_of_neighbors.begin(); it2 != neighbor_of_neighbors.end();
                     it2++)
                {
                    Atom* neighbor_of_neighbor = (*it2);
                    std::stringstream ss2;
                    ...
                    if (ss.str().compare(ss2.str()) != 0)
                    {
                        ...
                        for (AtomVector::iterator it3 = neighbor_of_neighbor_of_neighbors.begin();
                             it3 != neighbor_of_neighbor_of_neighbors.end(); it3++)
                        {
                            Atom* neighbor_of_neighbor_of_neighbor = (*it3);
                            std::stringstream ss3;
                            ss3 << neighbor_of_neighbor_of_neighbor->GetId();
                            if (ss1.str().compare(ss3.str()) != 0)
                            {
                                ...
                                {
                                    excluded_atom_list.push_back(third_order_interaction.str());
                                    excluded_atom_list_map[atom->GetId()].push_back(
                                        neighbor_of_neighbor_of_neighbor->GetId());
                                }
                            }
                        }
                    }
    ...
    int number_of_excluded_atoms = 0;
    for (std::map<std::string, std::vector<std::string>>::iterator it = excluded_atom_list_map.begin();
         it != excluded_atom_list_map.end(); it++)
    {
