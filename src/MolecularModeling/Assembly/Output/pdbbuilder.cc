...
PdbFileSpace::PdbFile* Assembly::BuildPdbFileStructureFromAssembly(int link_card_direction, int connect_card_existance,
                                                                   int model_index, bool useInputPDBResidueNumbers)
{
    int local_debug = -1;
    if (model_index == -1)
    { // -1 is the default set in header file, if that's not changed, then use model_index_. Can't pass in model_index_
      // as default for reasons.
    ...
    ExtractPdbModelSectionFromAssembly(residue_set, serial_number, sequence_number, model_index,
                                       useInputPDBResidueNumbers, assembly_to_sequence_number_map,
                                       assembly_to_serial_number_map);
    PdbFileSpace::PdbLinkSection* link_card = new PdbFileSpace::PdbLinkSection();
    // The follwing line might be commented out for my testing purpose, definitely shouldn'be committed/pushed. If you
    // see it commented out, please uncomment it.
    ...
    if (connect_card_existance ==
        1) // The CONECT card generated by this code cannot be read correctly by Chimera. Cannot find out the bug. --Yao
    {
...
void Assembly::ExtractPdbModelSectionFromAssembly(PdbFileSpace::PdbModelResidueSet* residue_set, int& serial_number,
                                                  ...
                                                  AssemblytoPdbSerialNumberMap& assembly_to_pdb_serial_number_map)
{
    for (AssemblyVector::iterator it = this->assemblies_.begin(); it != this->assemblies_.end(); it++)
    {
        ...
        {
            ExtractPdbModelSectionFromAssembly(residue_set, serial_number, sequence_number, model_number,
                                               useInputPDBResidueNumbers, assembly_to_pdb_sequence_number_map,
        ...
        PdbFileSpace::PdbHeterogenAtomSection::PdbHeterogenAtomCardMap het_atom_map =
            PdbFileSpace::PdbHeterogenAtomSection::PdbHeterogenAtomCardMap();
        PdbFileSpace::PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector het_atom_vector =
            PdbFileSpace::PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector();
        ...
        for (ResidueVector::iterator it1 = residues.begin(); it1 != residues.end(); it1++)
        {
            ...
            for (AtomVector::iterator it2 = atoms.begin(); it2 != atoms.end(); it2++)
            {
                ...
                PdbFileSpace::PdbAtomCard* pdb_atom = new PdbFileSpace::PdbAtomCard(
                    serial_number, atom->GetName(), ' ', residue_name, ' ', sequence_number, ' ',
                    *((atom->GetCoordinates()).at(model_number)), gmml::dNotSet, gmml::dNotSet,
                    atom->GetElementSymbol(), ""); // ss.str());
                ...
                // //Yao: must stop using id.at(1) for index.
                assembly_to_pdb_serial_number_map[atom->GetIndex()] =
                    serial_number; // Yao: must stop using id.at(1) for index.
            }
        }
    }
    PdbFileSpace::PdbAtomSection* atom_card              = new PdbFileSpace::PdbAtomSection();
    ...
    {
        Residue* residue = (*it1);
        // Oliver: It looks like there is functionality for sequence_number (residue number) and serial_number (starts
        // at 1) to coexist, but they are are both set to 1 up above. I'm going to try have sequence_number be the
        // actual sequence number
        if (useInputPDBResidueNumbers && !residue->GetNumber().empty() &&
            residue->GetNumber() != "\n") // if not an empty string or a newline
        {
            ...
            try
            {
                sequence_number = std::stoi(residue->GetNumber());
            }
            catch (...)
            {}; // Do nothing if can't convert, use the incremented sequence_number
        }
        AtomVector atoms = residue->GetAtoms();
        for (AtomVector::iterator it2 = atoms.begin(); it2 != atoms.end(); it2++)
        {
            Atom* atom                    = (*it2);
            // std::cout << "Working on atom: " << atom->GetName() << " with description: " << atom->GetDescription() <<
            // std::endl;
            std::vector<std::string> dscr = gmml::Split(atom->GetDescription(), ";");
            //            std::stringstream ss;
            //            ss << setw(2) << fixed << setprecision(1) << atom->MolecularDynamicAtom::GetCharge();
            ...
            PdbFileSpace::PdbAtomCard* pdb_atom =
                new PdbFileSpace::PdbAtomCard(serial_number, atom->GetName(), ' ', residue_name, ' ', sequence_number,
                                              ' ', *((atom->GetCoordinates()).at(model_number)), gmml::dNotSet,
                                              gmml::dNotSet, atom->GetElementSymbol(), ""); // ss.str());
            std::vector<std::string> atom_id_tokens = gmml::Split(atom->GetId(), "_");
            pdb_atom->SetAtomChainId(gmml::ConvertString<char>(atom_id_tokens.at(3)));
            pdb_atom->SetAtomInsertionCode(gmml::ConvertString<char>(atom_id_tokens.at(5)));
            pdb_atom->SetAtomAlternateLocation(gmml::ConvertString<char>(atom_id_tokens.at(6)));
            assembly_to_pdb_sequence_number_map[gmml::ConvertString<int>(atom_id_tokens.at(4))] = sequence_number;
            // assembly_to_pdb_serial_number_map[gmml::ConvertString<int>(atom_id_tokens.at(1))] = serial_number; //Yao:
            // must stop using id.at(1) for index.
...
void Assembly::ExtractPdbLinkSectionFromAssembly(PdbFileSpace::PdbLinkSection* link_card, int model_index,
                ...
                for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
                {
                    ...
                    if (find(visited_links.begin(), visited_links.end(), neighbor->GetId()) == visited_links.end())
                    {
                        if (residue->GetId().compare(neighbor_residue->GetId()) != 0)
                        {
                            ...
                            std::vector<std::string> NeighborResidueIdSplit =
                                gmml::Split(neighbor_residue->GetId(), "_");
                            std::string ResidueNumber         = ResidueIdSplit.at(2);
                            std::string NeighborResidueNumber = NeighborResidueIdSplit.at(2);

                            if (ResidueNumber.compare(NeighborResidueNumber) !=
                                0) // if this pair is not the two partial occupancies of the same residue
                            {      // my code
                                visited_links.push_back(atom->GetId());
                                visited_links.push_back(neighbor->GetId());
                                PdbFileSpace::PdbLinkCardResidue* link_residue1 =
                                    new PdbFileSpace::PdbLinkCardResidue();
                                std::vector<std::string> atom_id_tokens = gmml::Split(atom->GetId(), "_");
                                link_residue1->SetAtomName(atom->GetName());
                                link_residue1->SetResidueChainId(gmml::ConvertString<char>(atom_id_tokens.at(3)));
                                link_residue1->SetResidueSequenceNumber(
                                    assembly_to_pdb_sequence_number[gmml::ConvertString<int>(atom_id_tokens.at(4))]);
                                link_residue1->SetResidueInsertionCode(gmml::ConvertString<char>(atom_id_tokens.at(5)));
                                link_residue1->SetAlternateLocationIndicator(
                                    gmml::ConvertString<char>(atom_id_tokens.at(6)));
                                link_residue1->SetResidueName(residue->GetName());
                                PdbFileSpace::PdbLinkCardResidue* link_residue2 =
                                    new PdbFileSpace::PdbLinkCardResidue();
                                ...
                                link_residue2->SetResidueSequenceNumber(
                                    assembly_to_pdb_sequence_number[gmml::ConvertString<int>(
                                        neighbor_id_tokens.at(4))]);
                                link_residue2->SetResidueInsertionCode(
                                    gmml::ConvertString<char>(neighbor_id_tokens.at(5)));
                                link_residue2->SetAlternateLocationIndicator(
                                    gmml::ConvertString<char>(neighbor_id_tokens.at(6)));
                                ...
                                double distance = atom->GetCoordinates()
                                                      .at(model_index)
                                                      ->Distance(*(neighbor->GetCoordinates().at(model_index)));
...
void Assembly::ExtractPdbConnectSectionFromAssembly(PdbFileSpace::PdbConnectSection* connect_card,
                                                    AssemblytoPdbSerialNumberMap assembly_to_pdb_serial_number)
{
    PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap bonded_atoms_serial_number_map =
    ...
    for (AtomVector::iterator it = all_atoms.begin(); it != all_atoms.end(); it++)
    {
        ...
        if (node != NULL)
        {
            ...
            // //Yao: Must stop using id.at(1) as index
            ...
            for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
            {
                ...
                if (std::find(all_atoms.begin(), all_atoms.end(), neighbor) != all_atoms.end())
                {
                    ...
                    bonded_atoms_serial_number_map[atom_serial_number].push_back(
                        assembly_to_pdb_serial_number[neighbor->GetIndex()]);
                }
            }
        }
    }
