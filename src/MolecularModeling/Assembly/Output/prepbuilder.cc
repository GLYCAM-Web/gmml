    ...
    for (ResidueVector::iterator it = assembly_residues.begin(); it != assembly_residues.end(); it++)
    {
        ...
        for (int i = 0; i < gmml::DEFAULT_DUMMY_ATOMS; i++)
        {
            ...
            else if (i <= 1)
            {
                ...
                dummy_atom->SetAngle(0.0);
                dummy_atom->SetDihedral(0.0);
                cartesian_coordinate_list.push_back(
                    new GeometryTopology::Coordinate(gmml::maxCutOff, 0.0, 0.0)); // 1.522, 0.0, 0.0 ; -4.5, -5.0, -5.0
            }
            else
            {
                dummy_atom->SetBondLength(gmml::maxCutOff);
                ...
                dummy_atom->SetDihedral(0.0);
                cartesian_coordinate_list.push_back(new GeometryTopology::Coordinate(
                    gmml::maxCutOff, gmml::maxCutOff, 0.0)); // 2.43347, 1.34044, 0.0 ; -4.7, -4.2, -4.0
            }
        ...
        std::vector<gmml::TopologicalType> residue_topological_types =
            GetAllTopologicalTypesOfAtomsOfResidue(assembly_atoms, loops, bond_index);
        for (AtomVector::iterator it1 = assembly_atoms.begin(); it1 != assembly_atoms.end(); it1++)
        {
            ...
            internal_coordinate =
                internal_coordinate->GeometryTopology::Coordinate::ConvertCartesianCoordinate2InternalCoordinate(
            ...
            ExtractPrepImproperDihedralsFromAssembly(assembly_atom, inserted_improper_dihedrals,
                                                     inserted_improper_dihedral_types, dihedrals);
        }
...
void Assembly::ExtractPrepImproperDihedralTypesFromAssembly(Atom* assembly_atom,
        ...
        if (neighbors.size() == 3)
        {
                 ...
                 it != all_improper_dihedrals_atom_type_permutations.end(); it++)
            {
                ...
                if (dihedrals[improper_dihedral_permutation] != NULL)
                {
                    ...
                    if (find(inserted_improper_dihedral_types.begin(), inserted_improper_dihedral_types.end(),
                             ss.str()) == inserted_improper_dihedral_types.end())
                    {
...
void Assembly::ExtractPrepImproperDihedralsFromAssembly(
    Atom* assembly_atom, std::vector<std::vector<std::string>>& inserted_improper_dihedrals,
    std::vector<std::string> inserted_improper_dihedral_types,
    ParameterFileSpace::ParameterFile::DihedralMap& dihedrals)
{
    AtomNode* atom_node = assembly_atom->GetNode();
        ...
        if (neighbors.size() == 3)
        {
                 ...
                 it != all_improper_dihedrals_atom_type_permutations.end(); it++)
            {
                ...
                sss << improper_dihedral_permutation.at(0) << "_" << improper_dihedral_permutation.at(1) << "_"
                    << improper_dihedral_permutation.at(2) << "_" << improper_dihedral_permutation.at(3);
                if (find(inserted_improper_dihedral_types.begin(), inserted_improper_dihedral_types.end(), sss.str()) !=
                    inserted_improper_dihedral_types.end())
                {
                    ...
                    if (find(inserted_improper_dihedrals.begin(), inserted_improper_dihedrals.end(), dihedral1) ==
                            inserted_improper_dihedrals.end() &&
                        find(inserted_improper_dihedrals.begin(), inserted_improper_dihedrals.end(), dihedral2) ==
                            inserted_improper_dihedrals.end() &&
                        find(inserted_improper_dihedrals.begin(), inserted_improper_dihedrals.end(), dihedral3) ==
                            inserted_improper_dihedrals.end() &&
                        find(inserted_improper_dihedrals.begin(), inserted_improper_dihedrals.end(),
                             reverse_dihedral1) == inserted_improper_dihedrals.end() &&
                        find(inserted_improper_dihedrals.begin(), inserted_improper_dihedrals.end(),
                             reverse_dihedral2) == inserted_improper_dihedrals.end() &&
                        find(inserted_improper_dihedrals.begin(), inserted_improper_dihedrals.end(),
                             reverse_dihedral3) == inserted_improper_dihedrals.end())
                    {
...
Assembly::GetAllTopologicalTypesOfAtomsOfResidue(AtomVector assembly_atoms, PrepFileSpace::PrepFileResidue::Loop& loops,
                                                 std::vector<int>& bond_index, int dummy_atoms)
{
    ...
    for (AtomVector::iterator it = assembly_atoms.begin(); it != assembly_atoms.end(); it++)
    {
        ...
        else // Stack is not empty
        {
            ...
            if (find(neighbors_atom_index.begin(), neighbors_atom_index.end(), top_stack_atom_index) !=
                neighbors_atom_index.end())
            {
                    ...
                    case 2:
                        switch (number_of_visited_neighbors)
                        {
                            ...
                            case 2:
                                ...
                                for (unsigned int i = 0; i < neighbors_atom_index.size(); i++)
                                {
                                    if (neighbors_atom_index.at(i) != top_stack_atom_index &&
                                        visited.at(neighbors_atom_index.at(i)) == true)
                                    {
            ...
            else // Current atom is not a neighbor of the atom on top of the stack
            {
                ...
                {
                    if (find(neighbors_atom_index.begin(), neighbors_atom_index.end(), stack.at(i)) !=
                        neighbors_atom_index.end())
                    {
                        ...
                        case 2:
                            switch (number_of_visited_neighbors)
                            {
                                case 1:
                                    ...
                                    {
                                        if (stack_neighbor_index != 0 &&
                                            stack.at(stack_neighbor_index - 1) == stack_neighbor_atom_index)
                                        {
                                            topological_types.at(index) = gmml::kTopTypeS;
                                        }
                                        else
