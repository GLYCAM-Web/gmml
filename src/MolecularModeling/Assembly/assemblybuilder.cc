...
//                       FUNCTIONS                      //
//////////////////////////////////////////////////////////
bool Assembly::CheckCondensedSequenceSanity(
    std::string sequence,
    CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree& prep_residues)
{
    ...
    std::vector<char> badChars = {
        '\'', '(', ')'}; // Does sequence contain any characters it doesn't? Can expand this list as needed.
    for (auto badChar : badChars)
    {
        if (sequence.find(badChar) != std::string::npos)
        {
            //            std::cout << "The input sequence (" << sequence << ") is contains a bad character: " <<
            //            badChar << std::endl;
            return false;
        }
    }
    try
    {
        CondensedSequenceSpace::CondensedSequence* condensed_sequence =
            new CondensedSequenceSpace::CondensedSequence(sequence);
        prep_residues = condensed_sequence->GetCondensedSequenceGlycam06ResidueTree();
        for (CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree::iterator it =
                 prep_residues.begin();
             it != prep_residues.end(); it++)
        {
...
Assembly::TemplateAssembly* Assembly::BuildTemplateAssemblyFromPrepFile(
    ...
    PrepFileSpace::PrepFile* prep_file)
{
    ...
    // for (CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree::iterator it =
    // glycam06_residue_tree.begin(); it != glycam06_residue_tree.end(); it++)
    // {
    //     CondensedSequenceSpace::CondensedSequenceGlycam06Residue* glycam06_residue = *it;
...
Assembly::TemplateAssembly* Assembly::BuildTemplateAssemblyFromPrepFile(std::vector<std::string>& query_residue_names,
                                                                        PrepFileSpace::PrepFile* prep_file)
{
    ...
    for (PrepFileSpace::PrepFile::ResidueMap::iterator it = prep_residue_map.begin(); it != prep_residue_map.end();
         it++)
    {
        std::string prep_residue_name = it->first;
        all_prep_residue_names.push_back(prep_residue_name);
    }
    // Check each prep file residue, if their name match query residue name, build assembly residue from this prep
    // residue, and add assembly residue to a template assembly.
    for (std::set<std::string>::iterator it = query_residue_names_unique.begin();
         it != query_residue_names_unique.end(); it++)
    {
        if (std::find(all_prep_residue_names.begin(), all_prep_residue_names.end(), *it) !=
            all_prep_residue_names.end())
        {
        ...
        else
        {
            ...
            //            std::cout << "Either the prep file doesn't have it, or this is not a regular residue at all.
            //            For example,\"Deoxy\" means removing parent oxygen it attaches to. " << std::endl;
        }
    }
    ...
    for (ResidueVector::iterator it = template_assembly_residues.begin(); it != template_assembly_residues.end(); it++)
    {
        ...
        //      std::pair<std::multimap<std::string, std::string>::const_iterator, std::multimap<std::string,
        //      std::string>::const_iterator> key_range =
        //      gmml::MolecularMetadata::GLYCAM::Glycam06NamesToTypesLookupMap.equal_range(residue_name);
        //        if (key_range.first == key_range.second)
        //        {
        //            std::cout << "Warning: no match exists in metadata map for template residue " << residue_name <<
        //            std::endl; std::cout << "Cannot tag ring/sidechain atoms for this residue. This might affect
        //            accuracy of setting geometry." << std::endl;
        //        }
        //        else
        //        {
        //            std::vector<std::string> all_types = std::vector<std::string>();
        //            for (std::multimap<std::string, std::string>::const_iterator it = key_range.first; it !=
        //            key_range.second; it++)
        //            {
        ...
        if (all_types.empty())
        {
            //            std::cout << "Warning: no match exists in metadata map for template residue " << residue_name
            //            << "\n"
            //                      << "Cannot tag ring/sidechain atoms for this residue. This might affect accuracy of
            //                      setting geometry." << std::endl;
        }
        else
        {
            bool is_sugar             = false;
            ...
            if (std::find(all_types.begin(), all_types.end(), "aldose") != all_types.end() &&
                std::find(all_types.begin(), all_types.end(), "furanose") != all_types.end())
            {
            ...
            if (std::find(all_types.begin(), all_types.end(), "aldose") != all_types.end() &&
                std::find(all_types.begin(), all_types.end(), "pyranose") != all_types.end())
            {
    ...
    // testing
    /*
    for (ResidueVector::iterator it3 = template_assembly_residues.begin(); it3 != template_assembly_residues.end();
    it3++){ std:: cout << "Residues: " << (*it3)->GetName() <<std::endl; AtomVector atoms = (*it3)->GetAtoms(); for
    (AtomVector::iterator it4 = atoms.begin(); it4 != atoms.end();it4++){ if ((*it4) ->GetIsCycle()){ std::cout << "is
    cycle: " << (*it4)->GetName() << std::endl;
        }
...
Assembly::ConvertCondensedSequence2AssemblyResidues(
    CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree& glycam06_residue_tree,
    TemplateAssembly* template_assembly)
{
    ...
    std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*,
             CondensedSequenceSpace::CondensedSequenceGlycam06Residue*>
        condensed_sequence_child_parent_map = std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*,
                                                       CondensedSequenceSpace::CondensedSequenceGlycam06Residue*>();

    std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>
        index_condensed_sequence_assembly_residue_map =
            std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>();
        ...
        if (condensed_sequence_residue_name != "Deoxy")
        {
            ...
            for (unsigned int j = 0; j < all_template_residues.size(); j++)
            {
                ...
                if (condensed_sequence_residue_name == all_template_residues[j]->GetName())
                {
                    ...
                    assembly_residue->SetNode(
                        NULL); // When residue object is constructed, its node attribute is not set to NULL, causing it
                               // to contain garbage node address.So I need to do this.
                    ...
                    // Oliver Jul2020: Updating this to look like other IDs. Not sure what the template_residue->GetId()
                    // should be, it's blank in my tests
                    // residue_id_stream << template_residue->GetName() << "_" << residue_serial_number << "_" <<
                    // template_residue->GetId() << std::endl;
                    ...
                    // Oliver Jul2020
                    // For the serial_number + 1, it's set to 0 above and used in the
                    // index_condensed_sequence_assembly_residue_map below... But I can't have a 0 residue number Going
                    // to have the default chain be "A".
                    residue_id_stream << template_residue->GetName() << "_"
                                      << "A"
                                      << "_" << (residue_serial_number + 1) << "_" << gmml::BLANK_SPACE << "_"
                                      << gmml::BLANK_SPACE << "_" << gmml::BLANK_SPACE; // << template_residue_id;

                    // std::string residue_id = template_residue->GetName() + "_" + serial_number_stream.str() + "_" +
                    // template_residue->GetId();
                    assembly_residue->SetId(residue_id_stream.str());
                    index_condensed_sequence_assembly_residue_map[residue_serial_number].first =
                    ...
                    for (unsigned int k = 0; k < all_template_atoms.size(); k++)
                    {
                        ...
                        // Attention: SetAtomType()function is overloaded as Atom::SetAtomType() and
                        // MolecularModeling::MolecularDynamicAtom::SetAtomType(). You don't really know which one to
                        // use. Likiwise, GetAtomType() is also overloaded. In my situation, I called
                        // MolecularModeling::MolecularModelingAtom::SetAtomType(), but later called
                        // Atom::GetAtomType(). The result is empty. We need to talk about this later
                        template_atom_copy->MolecularDynamicAtom::SetAtomType(
                            template_atom->MolecularDynamicAtom::GetAtomType());
                        template_atom_copy->MolecularDynamicAtom::SetCharge(
                            template_atom->MolecularDynamicAtom::GetCharge());
                        // Create coordinate object for new atom
                        GeometryTopology::Coordinate* copy_coordinate =
                            new GeometryTopology::Coordinate(template_atom->GetCoordinates().at(0));
                        ...
                        // atom_id_stream << template_atom->GetName() << "_" << atom_serial_number << "_" <<
                        // template_residue->GetName() << "_" << "A" << "_" << " " << "_" << "?_" << "?_" << " " <<
                        // std::endl;
                        atom_id_stream << template_atom->GetName() << "_" << atom_serial_number << "_"
                                       << residue_id_stream.str();
                        template_atom_copy->SetId(atom_id_stream.str());
                        // Tag cycle/sidechain atoms
                        template_atom_copy->MolecularModeling::OligoSaccharideDetectionAtom::SetIsCycle(
                            template_atom->GetIsCycle());
                        template_atom_copy->MolecularModeling::OligoSaccharideDetectionAtom::SetIsSideChain(
                            template_atom->GetIsSideChain());

                        AtomVector template_head_atoms = template_residue->GetHeadAtoms();
                        if (std::find(template_head_atoms.begin(), template_head_atoms.end(), template_atom) !=
                            template_head_atoms.end())
                        {
                            assembly_residue->AddHeadAtom(template_atom_copy);
                        }
                        AtomVector template_tail_atoms = template_residue->GetTailAtoms();
                        if (std::find(template_tail_atoms.begin(), template_tail_atoms.end(), template_atom) !=
                            template_tail_atoms.end())
                        {
                    ...
                    // Copy atom nodes
                    for (std::map<Atom*, AtomNode*>::iterator it = new_atom_template_node_map.begin();
                         it != new_atom_template_node_map.end(); it++)
                    {
                        ...
                        for (AtomVector::iterator it2 = template_node_neighbors.begin();
                             it2 != template_node_neighbors.end(); it2++)
                        {
                            all_node_neighbor_names.push_back((*it2)->GetName());
                        }
                        AtomVector atoms_in_new_residue = new_atom->GetResidue()->GetAtoms();
                        for (AtomVector::iterator it2 = atoms_in_new_residue.begin(); it2 != atoms_in_new_residue.end();
                             it2++)
                        {
                            Atom* new_atom_in_residue = *it2;
                            std::string atom_name     = new_atom_in_residue->GetName();
                            if (std::find(all_node_neighbor_names.begin(), all_node_neighbor_names.end(), atom_name) !=
                                all_node_neighbor_names.end())
                            {
        ...
        if (glycam06_residue_tree[i]->GetName() == "Deoxy")
        {
            ...
            for (unsigned int j = 0; j < parent_atoms.size(); j++)
            {
                if (parent_atoms[j]->GetName() == parent_oxygen_name)
                {
                    ...
                    {
                        if (parent_neighbors[k]->GetIsCycle())
                        { // When the neighbor of the oxygen to be removed is the ring carbon
                            parent_neighbor_ring_carbon = parent_neighbors[k];
                            parent_neighbor_ring_carbon->GetNode()->AddNodeNeighbor(hydrogen_to_add);
                            parent_neighbor_ring_carbon->SetCharge(
                                parent_neighbor_ring_carbon->GetCharge() +
                                parent_oxygen_to_remove->GetCharge()); // Add oxygen charge to ring carbon
                        }
                        else
                        { // When the neighbor of the oxyben to be removed is the attached hydroxyl hydrogen
                            parent_neighbor_hydrogen_neighbor = parent_neighbors[k];
                            parent_neighbor_ring_carbon->SetCharge(
                                parent_neighbor_ring_carbon->GetCharge() +
                                parent_neighbor_hydrogen_neighbor
                                    ->GetCharge()); // Add side group H charge to ring carbon
                    ...
                    hydrogen_to_add->SetCharge(0.0000); // According to GLYCAM06, the charge of aliphatic oxygen is
                                                        // zero.
...
void Assembly::SetGlycam06ResidueBonding(
    std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>&
        index_condensed_sequence_assembly_residue_map)
{
    // index_condensed_sequence_assembly_residue_map : map <int ,pair <06 residue, assembly residue> >. The int key is
    // to maintain the order of residue in 06 residue tree.
    ...
    std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*,
        ...
        condensed_residue_chilren_map =
            std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*,
                     CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree>();

    std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*,
             CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree>
        condensed_residue_parent_map =
            std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*,
                     CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree>();
    ...
    for (std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>::iterator it =
             index_condensed_sequence_assembly_residue_map.begin();
         it != index_condensed_sequence_assembly_residue_map.end(); it++)
    {
        condensed_assembly_residue_map[it->second.first] = it->second.second;
        condensed_residue_chilren_map[it->second.first] =
            CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree();
        condensed_residue_parent_map[it->second.first] =
            CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree();
    }
    // Associating a condensed sequence residue with its children and/or parent, using two nested iterations through
    // index_condensed_sequence_assembly_residue_map;
    for (std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>::iterator it =
             index_condensed_sequence_assembly_residue_map.begin();
         it != index_condensed_sequence_assembly_residue_map.end(); it++)
    {
        ...
        for (std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>::iterator
                 it2 = index_condensed_sequence_assembly_residue_map.begin();
             it2 != index_condensed_sequence_assembly_residue_map.end(); it2++)
        {
            ...
            if (glycam_06_residue_II->GetParentId() == index1)
            { // If "glycam_06_residue_II" 's parent is "glycam_06_residue", or the latter is the child of the former
                condensed_residue_chilren_map[glycam_06_residue].push_back(glycam_06_residue_II);
            }
            if (glycam_06_residue->GetParentId() == index2)
            { // If "glycam_06_residue" 's parent is ""glycam_06_residue_II""
                condensed_residue_parent_map[glycam_06_residue].push_back(glycam_06_residue_II);
            }
        }
    }

    // Will set head and tail atoms based on child-parent relationship in the big for loop below.So now empty whatever
    // the default setting is. Moreover, prep file defaults can sometimes be WRONG!!!!!!!!!!!!!!!!!!!!!
    for (std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>::iterator map_it =
             condensed_assembly_residue_map.begin();
    ...
    for (std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>::iterator it =
             condensed_assembly_residue_map.begin();
         it != condensed_assembly_residue_map.end(); it++)
    {
        ...
        CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree condensed_residue_children =
            condensed_residue_chilren_map[condensed_residue];
        for (unsigned int i = 0; i < condensed_residue_children.size(); i++)
        {

            Residue* corresponding_assembly_residue_child =
                condensed_assembly_residue_map[condensed_residue_children[i]];
        ...
        CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree condensed_residue_parents =
            condensed_residue_parent_map[condensed_residue];
        for (unsigned int i = 0; i < condensed_residue_parents.size(); i++)
        {
            Residue* corresponding_assembly_residue_parent =
                condensed_assembly_residue_map[condensed_residue_parents[i]];
        ...
        ResidueNodeVector existing_assembly_residue_neighbor_nodes =
            corresponding_assembly_residue_node->GetResidueNodeNeighbors();
        ...
        for (unsigned int i = 0; i < corresponding_assembly_residue_neighbors.size(); i++)
        {
            ...
            if (std::find(existing_assembly_residue_neighbor_nodes.begin(),
                          existing_assembly_residue_neighbor_nodes.end(),
                          neighbor_node) == existing_assembly_residue_neighbor_nodes.end())
            {
            ...
            if (std::find(existing_neighbor_neighbor_nodes.begin(), existing_neighbor_neighbor_nodes.end(),
                          corresponding_assembly_residue_node) == existing_neighbor_neighbor_nodes.end())
            {
         ...
         it != condensed_assembly_residue_map.end(); it++)
    {
        ...
        for (unsigned int i = 0; i < condensed_residue_children.size(); i++)
        {
            CondensedSequenceSpace::CondensedSequenceGlycam06Residue* condensed_residue_child =
                condensed_residue_children[i];
        ...
        // Add all tail/head atoms to corresponding residue's tail/head atoms, as well as corresponding residueNode
        // connecting atoms
        for (std::map<Atom*, Atom*>::iterator it2 = parent_tail_child_head_map.begin();
             it2 != parent_tail_child_head_map.end(); it2++)
        {
    ...
    // The for loop above set parent residue tail atoms and child head atoms. But if a residue is either at reducing or
    // non-reducing terminal: If a residue is the reducing terminal/aglycon, at this point it doesn't have head atoms
    // set, since it has no parent. Set head atom equals tail atom. If a residue is the non-reducing terminal, at this
    // point it doesn't have tail atoms set, since it has no child. Set tail atom eqals head atom.
    for (std::map<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>::iterator mapit =
             condensed_assembly_residue_map.begin();
         mapit != condensed_assembly_residue_map.end(); mapit++)
    {
        ...
        if (head_atoms.empty() && tail_atoms.empty())
        {
            //            std::cout << "Warning: " << "Both head and tail atoms are not set(empty) for residue " <<
            //            residue->GetName() << std::endl;
        }
    }

} // SetGlycam06ResidueBonding

void Assembly::RecursivelyTagDihedrals(Residue* parent_residue,
                                       ...
                                       int& linkage_index)
{
    ...
    for (unsigned int i = 0; i < all_tail_atoms.size(); i++)
    {
        ...
        // For non-reducing terminal/derivative residues, tail atom equals head atom. In this case, the tail atoms
        // aren't really connecting to a child residue. If this is the case, stop recursion from further going down the
        // oligosaccharide tree through such tail atoms. This if statement below makes sure a tail atom is a head atom
        // at the same time.If so, skip any operations.
        if (parent_residue->GetIsAglycon() ||
            std::find(all_head_atoms.begin(), all_head_atoms.end(), tail_atom) == all_head_atoms.end())
        {
            ...
            for (unsigned int j = 0; j < tail_atom_neighbors.size(); j++)
            {
                ...
                // if a neighbor is outside of a parent residue, it must be the head atom of a child residue.There
                // should only exist one such atom, otherwise, something is amiss.
                if (std::find(all_atoms_in_residue.begin(), all_atoms_in_residue.end(), neighbor_atom) ==
                    all_atoms_in_residue.end())
                {
                    ...
                    // Set C-O(tail atom)-C(head atom ) angle to 120 deg. The first C is the neighbor of tail atom that
                    // is not the head atom && not a hydrogen The only exception is ROH, where you have to use that
                    // hydrogen Right now,rely on the first letter of atom name to determine element type (if hydrogen
                    // or not). Better solution is the rule class.
                    ...
                    for (unsigned int k = 0; k < tail_atom_neighbors.size(); k++)
                    {
                        ...
                        else if (tail_atom_neighbors[k] != head_atom_of_child_residue &&
                                 tail_atom_neighbors[k]->GetName().substr(0, 1) != "H")
                        {
                            non_hydrogen_tail_atom_neighbor = tail_atom_neighbors[k];
                        }
                    }

                    // If child is a derivative, set only one dihedral: HX-CX-OX(tail atom)-head atom. Set this to 0
                    // degree, making the derivative head eclipse the hydrogen. This is a general solution for bad
                    // derivative angles This torsion closely resembles the psi torsion for regular sugar-sugar
                    // connection.
                    if (child_residue->GetIsSugarDerivative())
                    {
                        //! \todo Add these cout statements to the debugging mechanism once the DebugLevel class (or
                        //! whatever) is implemented.
                        // std::cout << "Setting derivative psi torsion for residue: " << child_residue->GetName()
                        // <<std::endl;
                        ...
                        // Derivative atom 1 should be a exocyclic (normally hydrogen) neighbor of the neighbor of tail
                        // atom (neighbor of neighbor of tail oxygen), for example: H4-C4-O4-C1
                        for (unsigned int k = 0; k < derivative_atom_2_neighbors.size(); k++)
                        {
                            if (!derivative_atom_2_neighbors[k]->GetIsCycle() &&
                                derivative_atom_2_neighbors[k] != derivative_atom_3)
                            {
                                derivative_atom_1 = derivative_atom_2_neighbors[k];
                            }
                        }
                        if (derivative_atom_1 == NULL || derivative_atom_2 == NULL || derivative_atom_3 == NULL ||
                            derivative_atom_4 == NULL)
                        {
                        ...
                        else
                        {
                            //! \todo Add these cout statements to the debugging mechanism once the DebugLevel class (or
                            //! whatever) is implemented.
                            // std::cout << "Derivative psi: " << derivative_atom_1->GetName() << "-" <<
                            // derivative_atom_2->GetName() << "-" << derivative_atom_3->GetName() << "-" <<
                            // derivative_atom_4->GetName() <<std::endl;
                            ...
                            std::pair<int, std::pair<AtomVector*, std::string>> linkage_index_dihedral_type_pair =
                                std::make_pair(linkage_index, dihedral_type_pair);
                    ...
                    else
                    {
                        ...
                        std::string anomeric_carbon_index_str =
                            head_atom_of_child_residue->GetName().substr(1, 1); // The "1" in C1, the "2" in C2
                        std::stringstream s1;
                        ...
                        for (unsigned int k = 0; k < head_atom_neighbors.size(); k++)
                        {
                            ...
                            // Phi_atom_4 should be C anomeric plus 1. For example, if anomeric is C1, then C2. If
                            // anomeirc is C2, then C3. Set this dihedral to 180 deg. For now: If child residue is
                            // furanose,keto pyranose i.e. sialic acid etc, phi_atom_4 is C1. If is other
                            // pyronaoses,phi_atom_4 should be a hydrogen
                        ...
                        // Psi atom 1 should be a exocyclic (normally hydrogen) neighbor of the neighbor of tail atom
                        // (neighbor of neighbor of tail oxygen), for example: psi H4-C4-O4-C1
                        ...
                        // omega atom 3 should be a exocyclic non-hydrogen(probably carbon) atom that 's connects to the
                        // atoms that connects to tail atom (neighbor of neighbor of tail oxygen) If such an atom is
                        // already on the ring, then there is no omega angle. If it is exocyclic, then omega exists.
                        if (omega_atom_3->GetIsCycle())
                        {
                            // std::cout << "Tail atom is directly attached to ring atom. So omega does not exist. Skip
                            // setting omega." << std::endl;
                        }
                        else
                        {
                            // Choose omega atom 2 from the neighbors of omega atom 3. It can't be omega_atom_4, and it
                            // shouldn't be a hydrogen
                            AtomVector omega_atom_3_neighbors = omega_atom_3->GetNode()->GetNodeNeighbors();
                            for (AtomVector::iterator atom_it4 = omega_atom_3_neighbors.begin();
                                 atom_it4 != omega_atom_3_neighbors.end(); atom_it4++)
                            {
                        ...
                        // Once omega atom 2 is identified, get its non-hydrogen node neighbor, this should be omega
                        // atom 1.
                        if (omega_atom_2 != NULL)
                        { // if there is such an exocyclic atom, then omega atom 2 exists.
                            AtomVector omega_atom_2_neighbors = omega_atom_2->GetNode()->GetNodeNeighbors();
                            for (AtomVector::iterator atom_it5 = omega_atom_2_neighbors.begin();
                                 atom_it5 != omega_atom_2_neighbors.end(); atom_it5++)
                            {
                        ...
                        if (omega_atom_4 == NULL || omega_atom_3 == NULL || omega_atom_2 == NULL ||
                            omega_atom_1 == NULL)
                        {
                            // std::cout << "SetOmegaDihedral: cannot find all four omega atoms. Skipping." <<
                            // std::endl;
                        }
                        else
                        {
                            ...
                            std::pair<AtomVector*, std::string> dihedral_type_pair =
                                std::make_pair(omega_atoms, "omega");
                            std::pair<int, std::pair<AtomVector*, std::string>> linkage_index_dihedral_type_pair =
                                std::make_pair(linkage_index, dihedral_type_pair);
                            index_dihedral_map.insert(linkage_index_dihedral_type_pair);
                        }
            ...
            std::find(all_head_atoms.begin(), all_head_atoms.end(), tail_atom) == all_head_atoms.end())
        {
            ...
            for (unsigned int j = 0; j < tail_atom_neighbors.size(); j++)
            {
                    ...
                    all_atoms_in_residue.end())
                {
                    Atom* head_atom_of_child_residue = neighbor_atom;
                    Residue* child_residue           = head_atom_of_child_residue->GetResidue();
                    // Right now, all residues are at the position of the template residue. That is, they are all around
                    // the orgin and stacked upon each other. SetResidueResidueBondDistance function: takes a pair of
                    // parent tail/child head atoms as argument. This function keeps the parent residue intact,but finds
                    // out the new position of child head atom, and move atoms of child residue accordingly.(i.e.
                    // grafting)
                    ...
                    if (non_hydrogen_tail_atom_neighbor != NULL)
                    {
                        ...
                        this->SetAngle(non_hydrogen_tail_atom_neighbor, tail_atom, head_atom_of_child_residue,
                                       angle_to_set);
                    }
                    // Start new recursion
                    Residue* new_parent_residue = child_residue;
...
void Assembly::FigureOutResidueLinkagesInGlycan(Residue* from_this_residue1, Residue* to_this_residue2,
...
// OG. This is agnostic of it being generated from a condensed sequence. Could be any assembly that is a glycan with
// good connectivites and GLYCAM nomenclature for atoms and residues.
void Assembly::SetDihedralAngleGeometryWithMetadata()
{
    ...
    // Resovlving overlaps should be a separated function, but I don't want assembly to have a ResidueLinkageVector
    // member. Need new, seperate class.
    for (auto& linkage : all_residue_linkages)
    {
        ...
        linkage.SimpleWiggleCurrentRotamers(
            allAtomsOfAssembly, allAtomsOfAssembly,
            5); // In case a specific rotamer is being created, don't want to step outside those bounds.
    }
}

/* Oliver needs to clarify what he is doing here:
 * I don't currently have the ability to "select" which rotamers I will write out. I need to add default and
 overwritable indexes for residue_linkage class for that to work.
 * I want this "selection" to work at the level of AddMetadata. So I can only add relevant metadata while linkages or
 rotamers that are deselected are not used.
 *  Having flags active and inactive metadata in that class might be another option.
 * I've updated the pdb writer to allow me to pass in a model_index number X, which will trigger writing out of
 coordinate set X from coordinateVector in Atom.
 * I can create an Assembly::saveCurrentCoordinates function that uses Atom::AddCoordinate to push_back a coordinate set
 related to a rotamer.
 * Then I'll create a wrapper function that allows me to write out each coordinate set.... Either I can pass in a name
 (e.g. residue_linkage_index_ gg) or
 ...
 * In Residue_Linkage I have a simple wiggle function, I have implemented it here. The problem is that it explores all
 rotamers. That won't work.
                    ...
                    all_atoms_in_residue.end())
                {
                    ...
                    if (child_residue->GetIsSugarDerivative())
                    {
                            ...
                            derivative_atom_4 != NULL)
                        {
                            ...
                            this->SetDihedral(derivative_atom_1, derivative_atom_2, derivative_atom_3,
                                              derivative_atom_4, derivative_psi);
                        }
                    }
                    ...
                    else
                    {
                        ...
                        if (psi_atom_1 != NULL && psi_atom_2 != NULL && psi_atom_3 != NULL && psi_atom_4 != NULL)
                        {
                            ...
                            // The psi angle is ok for 2 bond linkages (e.g. 1-2, 1-4 etc) but for 3 bond and longer we
                            // use different atoms and different psi values. First I get the correct atom, and then I
                            // set dihedral to 180 instead of 0.
                            ...
                            if (std::regex_search(tail_atom->GetName(),
                                                  regex_query)) // if tail atom name matches "O[6-9]"
                            {
                                dihedral_psi = 180.0;
                                // Psi atom 1 should be a non-hydrogen neighbor of the neighbor of tail atom (neighbor
                                // of neighbor of tail oxygen), for example: psi C5-C6-O6-C1
                                for (unsigned int k = 0; k < psi_atom_2_neighbors.size(); k++)
                                { // If not a hydrogen and not psi_atom_3
                                    if ((psi_atom_2_neighbors[k]->GetName().substr(0, 1) != "H") &&
                                        (psi_atom_2_neighbors[k] != psi_atom_3))
                                    {
                                        psi_atom_1 = psi_atom_2_neighbors[k];
                                    }
                                }
...
ResidueVector Assembly::FindClashingResidues()
{
    ...
    for (AtomVector::iterator it = all_atoms_of_assembly.begin(); it != all_atoms_of_assembly.end(); it++)
    {
        ...
        unsigned int bond_by_distance_count =
            0; // How many bonds does a particular atom have, according to bond by distance
        ...
        // If bond by distance gives more bonds than the number of bonds in atom node, then this atom is clashing with
        // other atoms. This residue is a clashing residue
        if (bond_by_distance_count > actual_bond_count)
        {
            // If multiple atoms in a residue is clashing, prevent duplicate tagging of the corresponding residue as
            // clashing
            if (std::find(clashing_residues.begin(), clashing_residues.end(), current_atom->GetResidue()) ==
                clashing_residues.end())
            {
                clashing_residues.push_back(current_atom->GetResidue());
            }
        }
    }
    //! \todo Add these cout statements to the debugging mechanism once the DebugLevel class (or whatever) is
    //! implemented.
...
std::vector<ResidueVector> Assembly::FindPathToCommonAncestors(ResidueVector& all_clashing_residues)
{
    ...
    // Starting from each residue, construct a pathway until a branching point(a residue with multiple tail atoms), add
    // residue in this pathway to a ResidueVector
    for (ResidueVector::iterator it = all_clashing_residues.begin(); it != all_clashing_residues.end(); it++)
    {
        Residue* clashing_residue = *it;
        if (!clashing_residue->GetIsAglycon() &&
            std::find(visited_residues.begin(), visited_residues.end(), clashing_residue) == visited_residues.end())
        {
            Residue* current_residue = clashing_residue;
            ResidueVector path       = ResidueVector();
            // Going from the non-reducing end(starting from clashing residue) towards the reducing end using head atom
            // -> parent tail atom relationship
            while (true)
            {
                // If the code has reached the aglycon, or has reached a residue that's already visited. Then this
                // residue is the endpoint of a clashing pathway. It's either the aglycon, or a residue with multiple
                // branches. This is the last element is a clashing pathway residue vector, and is called "common
                // ancestor" in another function somewhere downstream. Termination condition is reached, break out of
                // while loop
                if (current_residue->GetIsAglycon() || std::find(visited_residues.begin(), visited_residues.end(),
                                                                 current_residue) != visited_residues.end())
                {
                ...
                else
                {
                    visited_residues.push_back(current_residue);
                    path.push_back(current_residue);
                    AtomVector head_atom_neighbors =
                        current_residue->GetHeadAtoms().at(0)->GetNode()->GetNodeNeighbors();
                              ...
                              std::multimap<int, std::pair<AtomVector*, std::string>>& index_dihedral_map)
{
    ...
    for (std::vector<ResidueVector>::iterator it = fused_clashing_paths.begin(); it != fused_clashing_paths.end(); it++)
    {
        ...
        if (!all_omega_dihedrals.empty())
        {
            ...
            GeometryTopology::CoordinateVector least_clash_coordinates_for_this_pathway =
                this->FindBestSetOfTorsions(all_omega_dihedrals);
            // For each atom in assembly, set coordinate according to the best set of coordiante found. This will
            // crudely resolve clashes.
...
Assembly::FindAllOmegaTorsionsInPathway(ResidueVector& pathway,
                                        std::multimap<int, std::pair<AtomVector*, std::string>>& index_dihedral_map)
{
    ...
    for (AtomVector::iterator it = all_tail_atoms_in_pathway.begin(); it != all_tail_atoms_in_pathway.end(); it++)
    {
        ...
        for (std::multimap<int, std::pair<AtomVector*, std::string>>::iterator it2 = index_dihedral_map.begin();
             it2 != index_dihedral_map.end(); it2++)
        {
            AtomVector* dihedral_atoms = it2->second.first;
            std::string dihedral_type  = it2->second.second;
            // If a dihedral contains a tail atom in the pathway, and has type "psi" or "omega", this is the dihedral we
            // want. Later perform grid search on these torsion to alleviate clashes.
            if (std::find(dihedral_atoms->begin(), dihedral_atoms->end(), tail_atom) != dihedral_atoms->end() &&
                (dihedral_type == "psi" || dihedral_type == "omega"))
            {
...
GeometryTopology::CoordinateVector Assembly::FindBestSetOfTorsions(std::vector<AtomVector*>& available_dihedrals)
{
    ...
    rotation_values.push_back(5.0);
    // Start generating a relationship betwewn each dihedral and all rotation values, in the form of a vector of pairs.
    // Each pair represents one dihedral Pair.first is an AtomVector* containing the four atoms of a dihedral.
    // Pair.second is a vector of double. In this function, it's always "rotation_values"
    std::vector<std::pair<AtomVector*, std::vector<double>>> all_dihedral_rotation_values =
        std::vector<std::pair<AtomVector*, std::vector<double>>>();
    ...
    // For the vector of pair generated above, go through each dihedral, expand their rotation angles in the form of
    // recursive for loops. Exhaustively list each possibility as a "combination"
    ...
    // For each combination, rotate coordinates accordingly, then compute clash score. If clash score is lower than the
    // current minimum, record the current set of coordinate as the least-clashing coordinate set Make a pair.
    // Pair.first is clash score, pair.second is coordinate set
    std::pair<double, GeometryTopology::CoordinateVector> least_clash_coordinate =
        std::pair<double, GeometryTopology::CoordinateVector>();
    AtomVector all_atoms_in_assembly = this->GetAllAtomsOfAssembly();
    // Compute rotation score using the originial coordinate. Initiate least_clash_coordinate to contain this original
    // state.
    ...
    for (unsigned int i = 0; i < all_combinations.size(); i++)
    {
        ...
        for (combination::iterator it = rotation_set.begin(); it != rotation_set.end(); it++)
        {
            ...
            rotation_axis->SetX(dihedral_atoms->at(2)->GetCoordinates().at(0)->GetX() -
                                dihedral_atoms->at(1)->GetCoordinates().at(0)->GetX());
            rotation_axis->SetY(dihedral_atoms->at(2)->GetCoordinates().at(0)->GetY() -
                                dihedral_atoms->at(1)->GetCoordinates().at(0)->GetY());
            rotation_axis->SetZ(dihedral_atoms->at(2)->GetCoordinates().at(0)->GetZ() -
                                dihedral_atoms->at(1)->GetCoordinates().at(0)->GetZ());
            ...
            GeometryTopology::CoordinateVector rotated_coordinates =
                rotation_operation.RotateCoordinates(pivot_point, rotation_axis, rotation_value, original_coordinates);
...
void Assembly::GenerateAllTorsionCombinations(
    std::vector<std::pair<AtomVector*, std::vector<double>>>& all_dihedral_rotation_values,
    ...
    std::vector<double>& angle_index_per_dihedral)
{
    ...
    {
        // Record the current rotation value of the current dihedral to the temporary container
        // "angle_index_per_dihedral"
        ...
        if (current_dihedral_index != all_dihedral_rotation_values.size() - 1)
        {
            ...
            this->GenerateAllTorsionCombinations(all_dihedral_rotation_values, next_dihedral_index,
                                                 container_for_combinations, angle_index_per_dihedral);
        }
        // If the current dihedral is already the last dihedral, the code has reached the "dead end" of nested for
        // loop.Now look at the temporary container for the rotation value at each dihedral. These become a new
        // combination.
        else
        {
            std::vector<std::pair<AtomVector*, double>> new_combination = std::vector<std::pair<AtomVector*, double>>();
            for (unsigned int dihedral_position = 0; dihedral_position < angle_index_per_dihedral.size();
                 dihedral_position++)
            {
...
void Assembly::BuildAssemblyFromCondensedSequence(std::string condensed_sequence, PrepFileSpace::PrepFile* prep_file)
{
    ...
    if (std::find(valid_aglycone_names.begin(), valid_aglycone_names.end(), aglycone_name) ==
        valid_aglycone_names.end())
    {
    ...
    CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree glycam06_residues_temp =
        sequence.GetCondensedSequenceGlycam06ResidueTree();
    // Here we are not really interested in the output reorganized sequence, but we are trying to rearrange the
    // condensed 06 residue tree acccording to labeling order. Harded to coded to reorder and label based on lowest
    // index. In the future, give this funcction arguments to specify this option. But fow now, changing this function
    // declaration I will have to change many other things.
    ...
    CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree glycam06_residues =
        sequence.GetCondensedSequenceGlycam06ResidueTree();

    //    CondensedSequenceSpace::CondensedSequence::CondensedSequenceResidueTree res_tree =
    //    sequence.GetCondensedSequenceResidueTree();
    //    CondensedSequenceSpace::CondensedSequence::CondensedSequenceRotamersAndGlycosidicAnglesInfo info =
    //    sequence.GetCondensedSequenceRotamersAndGlycosidicAnglesInfo(res_tree);

    Assembly::TemplateAssembly* template_assembly =
        this->BuildTemplateAssemblyFromPrepFile(glycam06_residues, prep_file);

    std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>
        glycam06_assembly_residue_map =
    ...
    std::multimap<int, std::pair<AtomVector*, std::string>> index_dihedral_map =
        std::multimap<int, std::pair<AtomVector*, std::string>>();
    for (std::map<int, std::pair<CondensedSequenceSpace::CondensedSequenceGlycam06Residue*, Residue*>>::iterator it =
             glycam06_assembly_residue_map.begin();
         it != glycam06_assembly_residue_map.end(); it++)
    {
        ...
        if (glycam_06_res->GetParentId() == gmml::iNotSet && glycam_06_res->GetName() != "Deoxy")
        {
            ...
            this->SetDihedralAngleGeometryWithMetadata(); // Removed for current push. Need to tag dihedrals. Also need
                                                          // to resolve clashes.
            // this->RecursivelySetDihedralAngleGeometry(root); // replaced by SetDihedralAngleGeometryWithMetadata. OG
            // 2019.09.
            //           The Recursive function below needs to number all dihedrals, so it needs to know the linkage
            //           index at the beginning. Linkage index is incremented inside function once a linkage has been
            //           processed.

            // OG 2019.09:
            // This next part should be replaced. The metadata contains the tags, and should be edited to include a
            // linkage_index However, the ResolveClashes function uses the index_dihedral_map, so that must be replaced
            // at the same time.
    ...
    //    std::vector<ResidueVector> clashing_residue_parent_paths = this ->
    //    FindPathToCommonAncestors(clashing_residues); this->ResolveClashes(clashing_residue_parent_paths,
    //    index_dihedral_map);
}

void Assembly::BuildAssemblyFromCondensedSequence(std::string sequence, std::string prep_file,
                                                  std::string parameter_file, bool structure)
{
    try
    {
        CondensedSequenceSpace::CondensedSequence* condensed_sequence =
        ...
        std::stringstream ss;
        for (CondensedSequenceSpace::CondensedSequence::CondensedSequenceGlycam06ResidueTree::iterator it =
                 glycam06_residues.begin();
             it != glycam06_residues.end(); ++it)
        {
            CondensedSequenceSpace::CondensedSequenceGlycam06Residue* glycam06_residue = *it;
            std::string glycam06_residue_name                                          = glycam06_residue->GetName();
            ...
            if (prep_residue_map.find(glycam06_residue_name) != prep_residue_map.end())
            {
                ...
                std::stringstream id;
                // Oliver Jul2020: Adding A as the default chain id. Should do this better, but this code should "go
                // away now please".
                id << prep_residue_name << "_"
                   << "A"
                   << "_" << sequence_number << "_" << gmml::BLANK_SPACE << "_" << gmml::BLANK_SPACE; // << "_" << id_;
                ...
                for (PrepFileSpace::PrepFileAtomVector::iterator it1 = prep_atoms.begin(); it1 != prep_atoms.end();
                     it1++)
                {
                    ...
                    if (parameter != NULL)
                    {
                        ...
                        {
                            ParameterFileSpace::ParameterFileAtom* parameter_atom =
                                atom_type_map[assembly_atom->GetAtomType()];
                    ...
                    if (prep_residue->GetCoordinateType() == PrepFileSpace::kINT)
                    {
                        ...
                        // std::vector<GeometryTopology::Coordinate*> coordinate_list =
                        // std::vector<GeometryTopology::Coordinate*>();
                        ...
                        if (index == 1)
                        {
                            ...
                            GeometryTopology::Coordinate* parent_coordinate =
                                cartesian_coordinate_list.at(parent_index);
                        ...
                        if (index == 2)
                        {
                            ...
                            int parent_index      = prep_atom->GetBondIndex() - 1;
                            GeometryTopology::Coordinate* grandparent_coordinate =
                                cartesian_coordinate_list.at(grandparent_index);
                            GeometryTopology::Coordinate* parent_coordinate =
                                cartesian_coordinate_list.at(parent_index);
                        ...
                        if (index > 2)
                        {
                            ...
                            GeometryTopology::Coordinate* great_grandparent_coordinate =
                                cartesian_coordinate_list.at(great_grandparent_index);
                            GeometryTopology::Coordinate* grandparent_coordinate =
                                cartesian_coordinate_list.at(grandparent_index);
                            GeometryTopology::Coordinate* parent_coordinate =
                                cartesian_coordinate_list.at(parent_index);
                    ...
                    {
                        assembly_atom->AddCoordinate(new GeometryTopology::Coordinate(
                            prep_atom->GetBondLength(), prep_atom->GetAngle(), prep_atom->GetDihedral()));
                    }
                ...
                if (glycam06_residue->GetParentId() != -1)
                {
                    ...
                    for (AtomVector::iterator it3 = parent_residue_atoms.begin(); it3 != parent_residue_atoms.end();
                         it3++)
                    {
            ...
            {
                //                std::cout << "Residue " << glycam06_residue_name << " has not been found in the
                //                database" << std::endl;
            }
        }
        ...
        if (structure)
        {
            ...
            for (ResidueVector::iterator it = parent_residues.begin(); it != parent_residues.end(); it++)
            {
                ...
                if (linkage_index >= 0)
                {
                    ...
                    linkage_name << assembly_residue->GetName()
                                 << assembly_residue->GetHeadAtoms().at(0)->GetName().at(1) << "-"
                                 << parent_residue->GetTailAtoms().at(branch_index)->GetName().at(1)
                                 << parent_residue->GetName();
                    if (linkage_name.str().find("0SA2-8") != std::string::npos ||
                        linkage_name.str().find("0SB2-8") != std::string::npos)
                    {
                        this->SetPhiTorsion(assembly_residue, parent_residue, branch_index,
                                            gmml::EXTERNAL28LINKAGEROTAMERS[0][0]);
                        this->SetPsiTorsion(assembly_residue, parent_residue, branch_index,
                                            gmml::EXTERNAL28LINKAGEROTAMERS[0][1], false);
                        this->SetOmegaTorsion(assembly_residue, parent_residue, branch_index,
                                              gmml::EXTERNAL28LINKAGEROTAMERS[0][2], 7);
                        this->SetOmegaTorsion(assembly_residue, parent_residue, branch_index,
                                              gmml::EXTERNAL28LINKAGEROTAMERS[0][3], 8);
                        this->SetOmegaTorsion(assembly_residue, parent_residue, branch_index,
                                              gmml::EXTERNAL28LINKAGEROTAMERS[0][4], 9);
                    }
                    if (linkage_name.str().find("0GL2-8") != std::string::npos)
                    {
                        this->SetPhiTorsion(assembly_residue, parent_residue, branch_index,
                                            gmml::INTERNAL28LINKAGEROTAMERS[0][0]);
                        this->SetPsiTorsion(assembly_residue, parent_residue, branch_index,
                                            gmml::INTERNAL28LINKAGEROTAMERS[0][1], false);
                        this->SetOmegaTorsion(assembly_residue, parent_residue, branch_index,
                                              gmml::INTERNAL28LINKAGEROTAMERS[0][2], 7);
                        this->SetOmegaTorsion(assembly_residue, parent_residue, branch_index,
                                              gmml::INTERNAL28LINKAGEROTAMERS[0][3], 8);
                        this->SetOmegaTorsion(assembly_residue, parent_residue, branch_index,
                                              gmml::INTERNAL28LINKAGEROTAMERS[0][4], 9);
                    }
                }
                linkage_index++;
            }
...
void Assembly::GenerateRotamersForCondensedSequence(
    ...
    std::multimap<int, std::pair<AtomVector*, std::string>>& index_dihedral_map)
{
    ...
    {
        CondensedSequenceSpace::RotamersAndGlycosidicAnglesInfo* linkage_info =
            rotamers_glycosidic_angles_info[i].second;
        int linkage_index = linkage_info->GetLinkageIndex();
        std::pair<index_torsion_map::iterator, index_torsion_map::iterator> current_linkage_dihedral_range =
            index_dihedral_map.equal_range(linkage_index);
        ...
        // Use a string to record what type of torsions are being set. For example, if phi and psi are explicitly set,
        // user_defined_angle_types = "phipsi"
        ...
        for (unsigned int j = 0; j < user_defined_angles.size(); j++)
        {
            ...
            for (index_torsion_map::iterator it = current_linkage_dihedral_range.first;
                 it != current_linkage_dihedral_range.second; it++)
            {
        ...
        std::vector<std::pair<std::string, std::vector<std::string>>> selected_rotamers =
            linkage_info->GetSelectedRotamers();
    ...
    std::vector<double> angle_index_per_dihedral =
        std::vector<double>(all_dihedral_rotation_values.size(), gmml::dNotSet);
    working_assembly->GenerateAllTorsionCombinations(all_dihedral_rotation_values, 0, all_rotation_combinations,
                                                     angle_index_per_dihedral);
    // Rotate according to each combination
    std::vector<GeometryTopology::CoordinateVector> rotamer_coordinate_sets =
        std::vector<GeometryTopology::CoordinateVector>();
    for (unsigned int i = 0; i < all_rotation_combinations.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < rotamer_rotation_set.size(); j++)
        {
            ...
            working_assembly->SetDihedral(dihedral_atoms->at(0), dihedral_atoms->at(1), dihedral_atoms->at(2),
                                          dihedral_atoms->at(3), rotation_value);
        }
...
void Assembly::BuildAssemblyFromPdbFile(std::string pdb_file_path, std::vector<std::string> amino_lib_files,
                                        std::vector<std::string> glycam_lib_files,
                                        std::vector<std::string> other_lib_files, std::vector<std::string> prep_files,
                                        std::string parameter_file)
{
    ...
    this->BuildAssemblyFromPdbFile(pdb_file, amino_lib_files, glycam_lib_files, other_lib_files, prep_files,
                                   parameter_file);
}
    ...
    catch (PdbFileSpace::PdbFileProcessingException& ex)
    {
        ...
        if (local_debug > 0)
        {
            gmml::log(__LINE__, __FILE__, gmml::ERR,
                      "Generating PdbFileSpace::PdbFile structure from stringstream failed.");
        }
...
void Assembly::BuildAssemblyFromPdbFile(PdbFileSpace::PdbFile* pdb_file, std::vector<std::string> amino_lib_files,
                                        std::vector<std::string> glycam_lib_files,
                                        std::vector<std::string> other_lib_files, std::vector<std::string> prep_files,
                                        std::string parameter_file)
{
    ...
    try
    {
        ...
        for (std::vector<std::string>::iterator it = key_order.begin(); it != key_order.end(); it++)
        {
            ...
            for (PdbFileSpace::PdbFile::PdbAtomCardVector::iterator it1 = atoms->begin(); it1 != atoms->end(); it1++)
            {
                ...
                // OG 2021Sep29 edit: residue ID needs to be the same for each atom in a residue, otherwise you get
                // weird residue node connectivites as it compares residue IDs when determining if a bonded atom is
                // within the same residue or not.
                ...
                if (!lib_residues.empty() || !prep_residues.empty())
                {
                    if (lib_residues.find(residue_name) != lib_residues.end())
                    {
                        ...
                        if (lib_atom != NULL)
                        {
                            ...
                            if (parameter != NULL)
                            {
                                if (atom_type_map.find(new_atom->MolecularDynamicAtom::GetAtomType()) !=
                                    atom_type_map.end())
                                {
                                    ParameterFileSpace::ParameterFileAtom* parameter_atom =
                                        atom_type_map[new_atom->GetAtomType()];
                ...
                {
                    for (PdbFileSpace::PdbModelSection::PdbModelCardMap::iterator it2 = model_maps.begin();
                         it2 != model_maps.end(); it2++)
                    {
                        ...
                        if (card_index.at(0).compare("ATOM") == 0)
                        {
                            PdbFileSpace::PdbAtomSection* atom_card =
                                atom_cards.at(gmml::ConvertString<int>(card_index.at(1)));
                            PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector atom_vector =
                                atom_card->GetOrderedAtomCards();
                            for (PdbFileSpace::PdbAtomSection::PdbAtomCardOrderVector::iterator it3 =
                                     atom_vector.begin();
                                 it3 != atom_vector.end(); it3++)
                            {
                                ...
                                sss << matching_residue_name << "_" << matching_chain_id << "_"
                                    << matching_sequence_number << "_" << matching_insertion_code << "_"
                                    << matching_alternate_location << "_" << id_;
                                std::string matching_key = sss.str();

                                if (key.compare(matching_key) == 0)
                                {
                                    GeometryTopology::Coordinate* coordinate =
                                        new GeometryTopology::Coordinate(matching_atom->GetAtomOrthogonalCoordinate());
                        ...
                        {
                            PdbFileSpace::PdbModelResidueSet::HeterogenAtomSectionVector heterogen_atom_cards =
                                residue_set->GetHeterogenAtomCards();
                            PdbFileSpace::PdbHeterogenAtomSection* heterogen_atom_card =
                                heterogen_atom_cards.at(gmml::ConvertString<int>(card_index.at(1)));
                            PdbFileSpace::PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector heterogen_atom_vector =
                                heterogen_atom_card->GetOrderedHeterogenAtomCards();
                            for (PdbFileSpace::PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector::iterator it3 =
                                     heterogen_atom_vector.begin();
                                 it3 != heterogen_atom_vector.end(); it3++)
                            {
                                PdbFileSpace::PdbAtomCard* matching_heterogen_atom = *it3;
                                std::string matching_heterogen_residue_name =
                                    matching_heterogen_atom->GetAtomResidueName();
                                char matching_heterogen_chain_id = matching_heterogen_atom->GetAtomChainId();
                                int matching_heterogen_sequence_number =
                                    matching_heterogen_atom->GetAtomResidueSequenceNumber();
                                char matching_heterogen_insertion_code =
                                    matching_heterogen_atom->GetAtomInsertionCode();
                                char matching_heterogen_alternate_location =
                                    matching_heterogen_atom->GetAtomAlternateLocation();
                                std::stringstream ssss;
                                ssss << matching_heterogen_residue_name << "_" << matching_heterogen_chain_id << "_"
                                     << matching_heterogen_sequence_number << "_" << matching_heterogen_insertion_code
                                     << "_" << matching_heterogen_alternate_location << "_" << id_;
                                std::string matching_heterogen_key = ssss.str();

                                if (key.compare(matching_heterogen_key) == 0)
                                {
                                    GeometryTopology::Coordinate* coordinate = new GeometryTopology::Coordinate(
                                        matching_heterogen_atom->GetAtomOrthogonalCoordinate());
                                    new_atom->AddCoordinate(coordinate);
                                    new_atom->SetDescription("Het;");
                                }
                            }
...
void Assembly::BuildAssemblyFromPdbqtFile(std::string pdbqt_file_path, std::string parameter_file)
{
    ...
    {
        //        std::cout << "Generating PdbqtFileSpace::PdbqtFile structure from " << pdbqt_file_path << "failed." <<
        //        std::endl;
    }
    ...
    try
    {
        ...
        for (std::vector<std::string>::iterator it = key_order.begin(); it != key_order.end(); it++)
        {
            ...
            for (PdbqtFileSpace::PdbqtFile::PdbqtAtomVector::iterator it1 = atoms->begin(); it1 != atoms->end(); it1++)
            {
                ...
                {
                    for (PdbqtFileSpace::PdbqtModelCard::PdbqtModelMap::iterator it2 = model_maps.begin();
                         it2 != model_maps.end(); it2++)
                    {
                        ...
                        char matching_alternate_location = matching_atom->GetAtomAlternateLocation();
                        std::stringstream sss;
                        sss << matching_residue_name << "_" << matching_chain_id << "_" << matching_sequence_number
                            << "_" << matching_insertion_code << "_" << matching_alternate_location << "_" << id_;
                        std::string matching_key = sss.str();
...
void Assembly::BuildAssemblyFromLibraryFile(std::string library_file_path, std::string parameter_file)
{
    ...
    gmml::log(__LINE__, __FILE__, gmml::INF,
              "Reading AMBER library/off file into LibraryFileSpace::LibraryFile structure ...");
    ...
    {
        //        std::cout << "Generating LibraryFileSpace::LibraryFile structure from " << library_file_path <<
        //        "failed." << std::endl;
    }
...
void Assembly::BuildAssemblyFromLibraryFile(LibraryFileSpace::LibraryFile* library_file, std::string parameter_file)
{
    ...
    int sequence_number = 0;
    int serial_number   = 0;
    for (LibraryFileSpace::LibraryFile::ResidueMap::iterator it = library_residues.begin();
         it != library_residues.end(); it++)
    {
        sequence_number++;
        Residue* assembly_residue = new Residue();
        assembly_residue->SetAssembly(this);
        ...
        std::map<LibraryFileSpace::LibraryFileAtom*, Atom*>
            library_assembly_atom_map; // Added by Yao 03/18/2020 to address bonding

        for (LibraryFileSpace::LibraryFileResidue::AtomMap::iterator it1 = library_atoms.begin();
             it1 != library_atoms.end(); it1++)
        {
            serial_number++;
            Atom* assembly_atom                             = new Atom();
...
void Assembly::BuildAssemblyFromPrepFile(std::string prep_file_path, std::string parameter_file)
{
    ...
    {
        //        std::cout << "Generating PrepFileSpace::PrepFile structure from " << prep_file_path << "failed." <<
        //        std::endl;
    }
        ...
        for (PrepFileSpace::PrepFileAtomVector::iterator it1 = prep_atoms.begin(); it1 != prep_atoms.end(); it1++)
        {
            ...
            if (prep_residue->GetCoordinateType() == PrepFileSpace::kINT)
            {
                ...
                if (index > 2)
                {
                    ...
                    GeometryTopology::Coordinate* great_grandparent_coordinate =
                        cartesian_coordinate_list.at(great_grabdparent_index);
                    GeometryTopology::Coordinate* grandparent_coordinate =
                        cartesian_coordinate_list.at(grandparent_index);
                    GeometryTopology::Coordinate* parent_coordinate = cartesian_coordinate_list.at(parent_index);
                    coordinate_list.push_back(great_grandparent_coordinate);
                ...
                double angle_value                       = prep_atom->GetAngle();
                double dihedral_value                    = prep_atom->GetDihedral();
                coordinate = coordinate->ConvertInternalCoordinate2CartesianCoordinate(coordinate_list, bond_length,
                                                                                       angle_value, dihedral_value);
                cartesian_coordinate_list.push_back(coordinate);
                // original
                // GeometryTopology::Coordinate* coordinate =
                // GeometryTopology::Coordinate::ConvertInternalCoordinate2CartesianCoordinate(coordinate_list,
                // prep_atom->GetBondLength(), prep_atom->GetAngle(), prep_atom->GetDihedral());
                // cartesian_coordinate_list.push_back(coordinate);
                //
            ...
            if (prep_atom->GetTopologicalType() == gmml::kTopTypeM &&
                prep_atom->GetType().compare(prep_residue->GetDummyAtomType()) != 0)
            {
