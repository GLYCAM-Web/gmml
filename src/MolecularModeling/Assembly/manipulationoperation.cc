...
{
    /// Translate all atoms of the attached residue to place them in proper position with respect to the tail atom of
    /// the parent residue/assembly
    this->SetAttachedResidueBond(residue, parent_residue, branch_index, parameter_file);

    /// Rotate all atoms of the attached residue to set the proper bond angle between the attached residue and the
    /// parent residue
...
void Assembly::AdjustCharge(Residue* residue, Residue* parent_residue, int branch_index)
{
    ...
    {
        parent_residue->GetTailAtoms()
            .at(branch_index)
            ->MolecularDynamicAtom::SetCharge(
