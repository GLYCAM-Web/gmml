...
{ // written by OG, so syntax a bit different from other functions.
    ...
    {
        Atom* atom = *it;
        if ((atom->GetName().compare("N") == 0) || (atom->GetName().compare("C") == 0) ||
            (atom->GetName().compare("O") == 0) || (atom->GetName().compare("CA") == 0))
        {
...
{ // written by OG, so syntax a bit different from other functions.
    ...
    // This is unintuitive, but GetAssemblies does not return "this" assembly, just additonal "sub-assemblies" contained
    // within this assembly. Horrific.
    ResidueVector residues = this->GetResidues();
    for (ResidueVector::iterator it1 = residues.begin(); it1 != residues.end(); it1++)
    {
        Residue* residue = (*it1);
...
GeometryTopology::CoordinateVector Assembly::GetAllCoordinates()
{
    ...
    for (AssemblyVector::iterator it = this->assemblies_.begin(); it != this->assemblies_.end(); it++)
    {
        ...
        {
            //            std::cout << "Central data structure is not complete in order for generating this type of
            //            file: Missing coordinate(s)" << std::endl;
            gmml::log(__LINE__, __FILE__, gmml::ERR,
                      "Central data structure is not complete in order for generating this type of file: Missing "
                      "coordinate(s)");
            return GeometryTopology::CoordinateVector();
        }
        for (GeometryTopology::CoordinateVector::iterator it1 = assembly_coordinate.begin();
             it1 != assembly_coordinate.end(); it1++)
        {
    ...
    for (ResidueVector::iterator it = this->residues_.begin(); it != this->residues_.end(); it++)
    {
        ...
        for (AtomVector::iterator it1 = residue_atoms.begin(); it1 != residue_atoms.end(); it1++)
        {
            ...
            {
                //                std::cout << "Central data structure is not complete in order for generating this type
                //                of file: Missing coordinate(s)" << std::endl;
                gmml::log(__LINE__, __FILE__, gmml::ERR,
                          "Central data structure is not complete in order for generating this type of file: Missing "
                          "coordinate(s)");
                return GeometryTopology::CoordinateVector();
...
void Assembly::RemoveAssembly(Assembly* assembly)
{
    ...
    // Following code added by Yao 08/20/2019. In addtion to removing assembly, also remove its residues& atoms from
    // current assembly.
...
void Assembly::UpdateIds(std::string new_id)
{
    for (AssemblyVector::iterator it = assemblies_.begin(); it != assemblies_.end(); it++)
    {
        ...
        std::stringstream ss;
        ss << new_id << "."
           << assembly->GetId().substr(assembly->GetId().find_first_of(this->id_) + this->id_.size() + 1);
        (*it)->UpdateIds(ss.str());
        (*it)->SetId(ss.str());
    }
...
void Assembly::RemoveResidue(Residue* residue) // Added back in by Oliver so that Glycoprotein builder will compile.
{
    ...
    //    newResidues.resize(residues_.size() - 1); // Resizing each push_back is inefficient, set size to be current
    //    - 1. for(ResidueVector::iterator it = residues_.begin(); it != residues_.end(); ++it)
    //    {
    //        Residue* r = *it;
    ...
    residues_.erase(std::remove(residues_.begin(), residues_.end(), residue),
                    residues_.end()); // Note need #include <algorithm>
}
...
void Assembly::MergeAssembly(
    Assembly* other) // Added by Oliver. He is unsure and this may well cause problems. Edit: it did.
{
...
Assembly::GetAllResiduesFromMultipleLibFilesMap(std::vector<std::string> lib_files)
{
...
PrepFileSpace::PrepFile::ResidueMap
Assembly::GetAllResiduesFromMultiplePrepFilesMap(std::vector<std::string> prep_files)
{
...
MolecularModeling::ResidueNodeVector Assembly::GenerateResidueNodesInAssembly()
{
    ...
    for (MolecularModeling::ResidueNodeVector::iterator it3 = residuenodes_.begin(); it3 != residuenodes_.end(); it3++)
    {
        ...
        for (MolecularModeling::AtomVector::iterator it4 = residue_connecting_atoms.begin();
             it4 != residue_connecting_atoms.end(); it4++)
        {
            Atom* connecting_atom = (*it4);

            AtomVector connecting_atom_neighbors = connecting_atom->GetNode()->GetNodeNeighbors();
            for (AtomVector::iterator it5 = connecting_atom_neighbors.begin(); it5 != connecting_atom_neighbors.end();
                 it5++)
            {
...
void Assembly::GenerateMoleculesDFSUtil(MolecularModeling::ResidueNode* DFSresiduenode)
{
    ...
    for (MolecularModeling::ResidueNodeVector::iterator it2 = residuenode_neighbors.begin();
         it2 != residuenode_neighbors.end(); it2++)
    {
...
MolecularModeling::AtomVector Assembly::GetAllBondedAtomsByStartDirection(MolecularModeling::Atom* start_atom,
                                                                          MolecularModeling::Atom* direction_atom,
                                                                          AtomVector ignore_list)
{
...
void Assembly::BondedAtomsByStartDirectionDFSUtil(MolecularModeling::Atom* DFSatom, AtomVector start_atom_neighbors,
                                                  AtomVector ignore_list)
{
    bool toConsider = false; // Check if the atom is part of the start atom neighor or ignore list and set toConsider
                             // accordingly. If toConsider is true, skip the atom.
...
GeometryTopology::CoordinateVector Assembly::GetCoordinatesFromAtomVector(AtomVector atomList, int CoordinateIndex)
{
    ...
    for (AtomVector::iterator it1 = atomList.begin(); it1 != atomList.end(); it1++)
    {
        ...
        {
            //          std::cout<<"Atom ID "<<atom->GetId()<<" does not have coordinate at index:
            //          "<<CoordinateIndex<<std::endl;
        }
    }
...
void Assembly::SetChargesAndAtomTypes()
{
    ...
    // MolecularModeling::Residue* selection::FindNeighborResidueConnectedViaSpecificAtom(MolecularModeling::Residue
    // *queryResidue, std::string queryAtomName)
