...
Assembly::CycleMap Assembly::DetectCyclesByExhaustiveRingPerception()
{
    ...
    std::vector<std::string> path_graph_labels =
        std::vector<std::string>(); /// The list of labels of edges in the molecular graph
    std::vector<std::string> cycless = std::vector<std::string>();

    /// Initializing the std::map
    std::map<std::string, Atom*> IdAtom =
        std::map<std::string, Atom*>(); /// A std::map from atom ID to Assembly atom object
    ...
    /// Whenever a walk a-b-c is found it should be reduced to a-c and the label should be changed from [a-b], [b-c] to
    /// [a-b-c]
    ...
    while (atoms.size() > 1 && path_graph_edges.size() != 0)
    {
        ...
        for (MolecularModeling::AtomVector::iterator it = atoms.begin(); it != atoms.end(); it++)
        {
            common_atom_it = it;
            int counter    = 0;
            for (std::vector<std::string>::iterator it1 = path_graph_edges.begin(); it1 != path_graph_edges.end();
                 it1++)
            {
                std::string edge = (*it1);
        ...
        ReducePathGraph(path_graph_edges, path_graph_labels, reduced_path_graph_edges, reduced_path_graph_labels,
                        (*common_atom_it)->GetId(), cycless);
    ...
    for (std::vector<std::string>::iterator it = cycless.begin(); it != cycless.end(); it++)
    {
        ...
        if (splitted_cycle.size() <= 7)
        {
            ...
            std::stringstream ss;
            for (std::vector<std::string>::iterator it1 = splitted_cycle.begin(); it1 != splitted_cycle.end() - 1;
                 it1++)
            {
    ...
    MolecularModeling::AtomVector atoms = GetAllAtomsOfAssemblyExceptProteinWaterResiduesAtoms();
    std::vector<std::string> path_graph_edges = std::vector<std::string> (); ///The list of edges in the molecular graph
    std::vector<std::string> path_graph_labels = std::vector<std::string> (); ///The list of labels of edges in the
    molecular graph std::vector<std::string> cycless = std::vector<std::string>();

    ///Initializing the std::map
    std::map<std::string, Atom*> IdAtom = std::map<std::string, Atom*>(); ///A std::map from atom ID to Assembly aTom
    object for(MolecularModeling::AtomVector::iterator it = atoms.begin(); it != atoms.end(); it++)
    {
        Atom* atom = (*it);
        IdAtom[atom->GetId()] = atom;
    }
    ...
    ///Reducing the path graph
    int neighbor_counter = 2;
    ///Whenever a walk a-b-c is found it should be reduced to a-c and the lable should be changed from [a-b], [b-c] to
    [a-b-c]
                               ...
                               std::vector<std::string>& cycles)
{
    ...
    for (std::vector<std::string>::iterator it = path_graph_edges.begin(); it != path_graph_edges.end() - 1; it++)
    {
        ...
        for (std::vector<std::string>::iterator it1 = it + 1; it1 != path_graph_edges.end(); it1++)
        {
            ...
            {
                if (source_edge_atoms.at(0).compare(source_edge_atoms.at(1)) != 0 &&
                    target_edge_atoms.at(0).compare(target_edge_atoms.at(1)) != 0)
                { /// if the edges a != b and b != c, so there might be a walk a_b_c
                    ...
                    if (source_edge_atoms.at(1).compare(target_edge_atoms.at(0)) == 0 &&
                        source_edge_atoms.at(1).compare(common_atom) ==
                            0) /// if there is a walk a_b_c in the graph (edges: a,b and b,c)
                    {
                        if (local_debug > 0)
                        {
                    ...
                    else if (source_edge_atoms.at(1).compare(target_edge_atoms.at(1)) == 0 &&
                             source_edge_atoms.at(1).compare(common_atom) ==
                                 0) /// if there is a walk a_b_c in the graph (edges: a,b and c,b)
                    {
                        if (local_debug > 0)
                        {
                    ...
                    else if (source_edge_atoms.at(0).compare(target_edge_atoms.at(0)) == 0 &&
                             source_edge_atoms.at(0).compare(common_atom) ==
                                 0) /// if there is a walk a_b_c in the graph (edges: b,a and b,c)
                    {
                        if (local_debug > 0)
                        {
                    ...
                    else if (source_edge_atoms.at(0).compare(target_edge_atoms.at(1)) == 0 &&
                             source_edge_atoms.at(0).compare(common_atom) ==
                                 0) /// if there is a walk a_b_c in the graph (edges: b,a and c,b)
                    {
                        if (local_debug > 0)
                        {
            ...
            if (walk_found)
            {
                ...
                else if (find(reduced_path_graph_labels.begin(), reduced_path_graph_labels.end(), new_label.str()) ==
                         reduced_path_graph_labels.end())
                {
                ...
                if (find(to_be_deleted_edges.begin(), to_be_deleted_edges.end(), source_index) ==
                    to_be_deleted_edges.end())
                {
                    to_be_deleted_edges.push_back(source_index);
                }
                if (find(to_be_deleted_edges.begin(), to_be_deleted_edges.end(), target_index) ==
                    to_be_deleted_edges.end())
                {
...
void Assembly::ConvertIntoPathGraph(std::vector<std::string>& path_graph_edges,
                                    std::vector<std::string>& path_graph_labels, MolecularModeling::AtomVector atoms)
        ...
        for (MolecularModeling::AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
        {
            ...
            if (find(atoms_id.begin(), atoms_id.end(), neighbor->GetId()) != atoms_id.end())
            {
                ...
                if ((find(path_graph_edges.begin(), path_graph_edges.end(), ss.str()) == path_graph_edges.end()) &&
                    (find(path_graph_edges.begin(), path_graph_edges.end(), reverse_ss.str()) ==
                     path_graph_edges.end())) /// path not existed before
                {
...
Assembly::CycleMap Assembly::DetectCyclesByDFS()
{
    ...
    MolecularModeling::AtomVector atoms = GetAllAtomsOfAssemblyExceptProteinWaterResiduesAtoms();
    // It's okay that all atoms in the map has this common parent, won't break algorithm. Moving new declaration out of
    // for loop avoids lots of memory leak.
...
void Assembly::DFSVisit(MolecularModeling::AtomVector atoms, AtomStatusMap& atom_status_map,
                        AtomIdAtomMap& atom_parent_map, Atom* atom, int& counter, AtomIdAtomMap& src_dest_map)
{
    ...
    for (MolecularModeling::AtomVector::iterator it = neighbors.begin(); it != neighbors.end(); it++)
    {
        ...
        if ((residue->GetName().compare("HOH") != 0) && (residue->CheckIfProtein() != true))
        {
            ...
            if (atom_status_map[neighbor->GetId()] == gmml::VISITED)
            {
                ...
                if (neighbor->GetId().compare(parent->GetId()) !=
                    0) /// making sure we are not tracking back to the previous atom which is the parent of neigbor
                       /// (current atom)
                {
                    counter++;
