...
// void Assembly::GetRingShapeBFMP_deprecatedExternalProgramCall(MolecularModeling::AtomVector cycle,
...
//
//        ///Preparing atom name and id for detect shape program. It doesn't work with atoms containing special
//        characters: C', C* etc
...
//         MolecularModeling::Atom* anomeric = Glycan::Monosaccharide::FindAnomericCarbon(anomeric_note,
//         anomeric_carbons_status, cycle_atoms, cycle_atoms_str);
//
//         if(anomeric != NULL)
//         {
...
//     std::cout << "\n" << "Detailed information of sorted cycles after discarding fused or oxygenless rings: " <<
//     "\n"; gmml::log(__LINE__, __FILE__,  gmml::INF, "Detailed information of sorted cycles after discarding fused or
//     oxygenless rings: ");
//
//     for(CycleMap::iterator it = cycles.begin(); it != cycles.end(); it++)
//     {
...
// MolecularModeling::Atom* Assembly::FindAnomericCarbon( Glycan::Note* anomeric_note, std::vector< std::string > &
// anomeric_carbons_status, MolecularModeling::AtomVector cycle, std::string cycle_atoms_str ) {
...
//
//       for( MolecularModeling::AtomVector::iterator it1 = o_neighbor1_neighbors.begin(); it1 !=
//       o_neighbor1_neighbors.end(); it1++ ) {///check if neighbor1 of oxygen has another oxygen or nitrogen neighbor
//         MolecularModeling::Atom* o_neighbor1_neighbor = ( *it1 );
//
//         if( cycle_atoms_str.find( o_neighbor1_neighbor->GetId() ) == std::string::npos ///if the neighbor is not one
//         of the cycle atoms
//                 && ( o_neighbor1_neighbor->GetName().substr( 0, 1 ).compare( "O" ) == 0 ||
//                 o_neighbor1_neighbor->GetName().substr( 0, 1 ).compare( "N" ) == 0 ) ) { ///if first element is "O"
//                 or "N"
//           //                        &&
//           isdigit(gmml::ConvertString<char>(neighbor1_neighbor->GetName().substr(1,1))))///if second element is a
//           digit anomeric_carbon = o_neighbor1; anomeric_carbons_status.push_back( "Anomeric carbon: "  );
...
//
//       for( MolecularModeling::AtomVector::iterator it2 = o_neighbor2_neighbors.begin(); it2 !=
//       o_neighbor2_neighbors.end(); it2++ ) {///check if neighbor2 of oxygen has another oxygen or nitrogen neighbor
//         MolecularModeling::Atom* o_neighbor2_neighbor = ( *it2 );
//
//         if( cycle_atoms_str.find( o_neighbor2_neighbor->GetId() ) == std::string::npos
//                 && ( o_neighbor2_neighbor->GetName().substr( 0, 1 ).compare( "O" ) == 0 ||
//                 o_neighbor2_neighbor->GetName().substr( 0, 1 ).compare( "N" ) == 0 ) ) {
...
//       }
//
//       ///Seems redundant to have this multiple times, if it is going to say the same thing and be generated for all
//       cases after the first logic check.
//       ///Specially if we ever want to change this Note to say something different. Like I am doing now. :)
//       std::stringstream ss;
//       ss << "Could not find glycosidic oxygen or nitrogen within " << gmml::dCutOff << " Angstroms of a ring carbon
//       bonded to the ring oxygen(" << node->GetAtom()->GetName() << ")."; anomeric_note->description_ = ss.str();
//
...
//         anomeric_carbons_status.push_back( "Anomeric carbon assigned based on its atom name index (" + ss1.str() +
...
//       for( MolecularModeling::AtomVector::iterator it1 = o_neighbor1_neighbors.begin(); it1 !=
//       o_neighbor1_neighbors.end(); it1++ ) {///check if neighbor1 of oxygen has non-ring carbon neighbor
//         MolecularModeling::Atom* neighbor1_neighbor = ( *it1 );
//         if( cycle_atoms_str.find( neighbor1_neighbor->GetId()) == std::string::npos ///if the neighbor is not one of
//         the cycle atoms
...
//       for( MolecularModeling::AtomVector::iterator it1 = o_neighbor2_neighbors.begin(); it1 !=
//       o_neighbor2_neighbors.end(); it1++ ) {///check if neighbor1 of oxygen has non-ring carbon neighbor
//         MolecularModeling::Atom* neighbor2_neighbor = ( *it1 );
//         if( cycle_atoms_str.find( neighbor2_neighbor->GetId()) == std::string::npos ///if the neighbor is not one of
//         the cycle atoms
...
//         anomeric_note->type_ = Glycan::WARNING;
//         anomeric_note->category_ = Glycan::ANOMERIC;
//         anomeric_carbons_status.push_back( "Anomeric carbon assigned to the ring carbon neighboring the ring oxygen
//         but is not attached to an exocyclic carbon (that is, assuming the monosaccharide is an aldose), Anomeric
//         Carbon is: " + anomeric_carbon->GetName() ); return o_neighbor2;
//       }
...
//         anomeric_carbons_status.push_back( "Anomeric carbon assigned to the ring carbon neighboring the ring oxygen
//         but is not attached to an exocyclic carbon (that is, assuming the monosaccharide is an aldose), Anomeric
//         Carbon is: " + anomeric_carbon->GetName() ); return o_neighbor1;
//       }
//       // @TODO August 8, 2017 Davis/Lachele - Once we get to a point where we read in mmcif definitions, we need to
//       use it to determine the Anomeric Carbon. anomeric_note->type_ = Glycan::WARNING; anomeric_note->category_ =
//       Glycan::ANOMERIC; anomeric_carbons_status.push_back( "Anomeric carbon assigned to the first ring carbon (" +
//       o_neighbor1->GetName() + "), Anomeric Carbon is: " + anomeric_carbon->GetName() ); return o_neighbor1;
//     }
//   }
//   return NULL;
// }
...
// std::vector<std::string> Assembly::GetSideGroupOrientations(Glycan::Monosaccharide* mono, std::string
// cycle_atoms_str)
// {
//   //9/14/18 Removed side atom initialization in this function and either moved it to detectSideGroups() or used Yao's
...
//
//     for(MolecularModeling::AtomVector::iterator it = mono->cycle_atoms_.begin(); it != mono->cycle_atoms_.end() - 1;
...
//           prev_atom = mono->cycle_atoms_.at(mono->cycle_atoms_.size() - 1); ///previous atom is the oxygen(last atom
//           of the sorted cycle)
//       else
...
//       GeometryTopology::Coordinate(*prev_atom->GetCoordinates().at(model_index_)); GeometryTopology::Coordinate
//       current_atom_coord = GeometryTopology::Coordinate(*current_atom->GetCoordinates().at(model_index_));
//       GeometryTopology::Coordinate next_atom_coord =
//       GeometryTopology::Coordinate(*next_atom->GetCoordinates().at(model_index_)); prev_atom_coord.operator
//       -(current_atom_coord) ; next_atom_coord.operator -(current_atom_coord) ; GeometryTopology::Plane plane =
//       GeometryTopology::Plane(); plane.SetV1(prev_atom_coord); plane.SetV2(next_atom_coord);
...
//             GeometryTopology::Coordinate(*neighbor->GetCoordinates().at(model_index_)); side_atom_coord.operator
//             -(current_atom_coord); side_atom_coord.Normalize(); double theta =
//             acos(normal_v.DotProduct(side_atom_coord));
//
//             if(index == 0 && neighbor_id.at(0) == 'C')///if anomeric atom has a non-ring carbon neighbor
//             {
//               if(orientations.at(index).compare("N") == 0) ///if the position of non-ring oxygen or nitrogen hasn't
//               been set yet
//               {
...
//               }
//             }
//             else if(neighbor_id.at(0) == 'O' || neighbor_id.at(0) == 'N')///if neighbor is a non-ring oxygen or
//             nitrogen
//             {
//               if(index == 0)///current atom is anomeric
//               {
//                 if(orientations.at(index).compare("N") == 0) ///if the position of non-ring carbon neighbor (if
//                 exist) hasn't been set yet
//                 {
//                   if(theta > (gmml::PI_RADIAN/2))
//                   {
...
//               }
//             }
//             else if(index == mono->cycle_atoms_.size() - 2 && neighbor_id.at(0) == 'C')///if the last ring carbon has
//             a non-ring carbon neighbor
//             {
...
//               int o_neighbors = 0;
//               int not_h_neighbors = 0;
//               for(MolecularModeling::AtomVector::iterator it2 = neighbors_of_neighbor.begin(); it2 !=
//               neighbors_of_neighbor.end(); it2++)
//               {
...
//                   ((neighbor_of_neighbor_id.at(0) == 'O') || (neighbor_of_neighbor_id.at(0) == 'N'))) ///if neighbor
//                   of neighbor is a non-ring oxygen or nitrogen
//                 {
//                   o_neighbors++;
//                 }
//                 if(cycle_atoms_str.find(neighbor_of_neighbor_id) == std::string::npos &&
//                   (neighbor_of_neighbor_id.at(0) != 'H'))///if neighbor of neighbor is any non-ring atom other than
//                   hydrogen
//                 {
//                   not_h_neighbors++;
//                 }
...
//     // {
//     //   for (std::vector<AtomVector>::iterator sideAtomVectorIT = side_atoms.begin(); sideAtomVectorIT !=
//     side_atoms.end(); sideAtomVectorIT ++)
//     //   {
//     //     MolecularModeling::AtomVector sideAtomVector = (*sideAtomVectorIT);
//     //     for (MolecularModeling::AtomVector::iterator sideAtomIT = sideAtomVector.begin(); sideAtomIT !=
//     sideAtomVector.end(); sideAtomIT++)
//     //     {
...
//
//         for (std::vector<AtomVector>::iterator it2 = mono->side_atoms_.begin(); it2 != mono->side_atoms_.end();
//         it2++){
...
//             }
//
//             for (MolecularModeling::AtomVector::iterator it4= all_plus_one_side_atoms.begin(); it4 !=
//             all_plus_one_side_atoms.end(); it4++ ){
//                 Atom* plus_one_side_atom = *it4;
//                 MolecularModeling::AtomVector visited_atoms = MolecularModeling::AtomVector();
//                 if ( this->CheckIfPlusOneSideAtomBelongsToCurrentMonosaccharide(SideAtomArm, cycle_atoms,
//                 plus_one_side_atom) ){
...
// }//InitiateDetectionOfCompleteSideGroupAtoms
//
// bool Assembly::CheckIfPlusOneSideAtomBelongsToCurrentMonosaccharide (MolecularModeling::AtomVector& SideAtomArm,
// MolecularModeling::AtomVector& cycle_atoms, Atom* working_atom )
// {
...
//         for (MolecularModeling::AtomVector::iterator it = working_node_neighbors.begin(); it !=
//         working_node_neighbors.end(); it++){
//             if (*it != NULL){
...
//             if (std::find(cycle_atoms.begin(),cycle_atoms.end(),attached_anomeric_carbons.at(0)) !=
//             cycle_atoms.end()){ //if the anomeric carbon is from the current monosaccharide
...
// 	            SideAtomArm.erase(std::find(SideAtomArm.begin(), SideAtomArm.end(), working_atom));	//Unless it's
// terminal, this oxygen belongs to the other monosaccharide. 		    belongs_to_current_monosaccharide = false;
// 	        }
//             }
//         }
...
//
//
// void Assembly::SetCompleteSideGroupAtoms(MolecularModeling::AtomVector& SideAtomArm, Atom* working_atom,
// MolecularModeling::AtomVector& cycle_atoms, MolecularModeling::AtomVector& visited_atoms)
// {
//     MolecularModeling::AtomVector working_node_neighbors = working_atom -> GetNode() -> GetNodeNeighbors();
//     for (MolecularModeling::AtomVector::iterator it = working_node_neighbors.begin(); it !=
...
// 		 !working_node_neighbor->GetResidue()->CheckIfProtein() ){	//If not part of another side chain &&
// not
// part of cycle && not visited && is not protein (NLN,LNK etc) 		AtomVector
// working_node_neighbor_neighbors = MolecularModeling::AtomVector();
//     		std::string working_neighbor_name = working_node_neighbor-> GetName();
//
// 	        if ( working_neighbor_name.substr(0,1) == "O" || working_neighbor_name.substr(0,1) == "N" ||
// working_neighbor_name.substr(0,1) == "S") { //if this neighbor is an O,N or S
// MolecularModeling::AtomVector attached_anomeric_carbons = MolecularModeling::AtomVector(); 		    for
// (MolecularModeling::AtomVector::iterator it2 = working_node_neighbor_neighbors.begin(); it2 !=
// working_node_neighbor_neighbors.end(); it2++){ 		        if (*it2 != NULL ){ 			    if
// ( (*it2)-> GetIsAnomericCarbon() ){ 		                attached_anomeric_carbons.push_back(*it2); //Check how
// many anomeric carbons this working neighbor is attacheed to, should be either 0,1,2.
// 			    }
// 		        }
// 	            }
//
...
// 		        if (attached_anomeric_carbons.front() != working_atom){  // if the working atom IS NOT the
// anomeric carbon the neighbor is attached to,the current side chain gets this oxygen.
// 			    SideAtomArm.push_back(working_node_neighbor);
// 		    	    working_node_neighbor-> SetIsSideChain(true);
// 		        }
// 			// if the workin atom IS INDEED the anomeric carbon the neighbor is attached to, the current
// side chain doesn't get this oxygen, unless this is a terminal hydroxyl.
//
// 			/*else {
...
// 			    CheckIfSideChainIsTerminal (working_node_neighbor, cycle_and_visited_atoms,
// side_chain_is_terminal); 			    if (side_chain_is_terminal){
// SideAtomArm.push_back(working_node_neighbor); 		    	        working_node_neighbor->
// SetIsSideChain(true);
// 			    }
//
// 			}*/
...
//
// 		else {  // if this neighbor is not oxygen,nitrogen or sulfur, the current side chain should get this
// atom 		    SideAtomArm.push_back(working_node_neighbor); 		    working_node_neighbor->
// SetIsSideChain(true);
// 		}
// 	    }//if not cycle
// 	}
...
//
//     for (std::vector<Atom*>::iterator it3 = working_node_neighbors.begin(); it3 != working_node_neighbors.end();
//     it3++){
// 	if (*it3 != NULL){
// 	    Atom* working_node_neighbor = *it3;
// 	    //if a node neighbor is not part of a ring,not visited, and not part on protein, change working atom to this
// neighbor, and start a new recursion call. 	    if ( working_node_neighbor != working_atom &&
...
//
// void Assembly::CheckIfSideChainIsTerminal(Atom* starting_atom, std::vector<Atom*> & cycle_and_visited_atoms, bool &
// is_terminal)
// {
...
//     for (std::vector<Atom*>::iterator it = starting_atom_node_neighbors.begin(); it !=
//     starting_atom_node_neighbors.end(); it++){
// 	if (*it != NULL){
...
// cycle_and_visited_atoms.end() ){ 	        if (current_atom_node_neighbor->GetIsCycle() ||
// current_atom_node_neighbor->GetResidue()->CheckIfProtein() ){ 		    is_terminal = false;
// 	        }
// 	    }
// 	}
//     }//for
...
//     if(mono->side_atoms_.at(mono->side_atoms_.size() - 1).at(0) != NULL)///if there exist a +1 carbon atom. in
//     side_atoms_ structure (std::vector<AtomVector>) the first index of the last element is dedicated to +1 atom
//     {
//         plus_sides.push_back(mono->side_atoms_.at(mono->side_atoms_.size() - 1).at(0));
//         MolecularModeling::AtomVector plus_one_atom_neighbors = mono->side_atoms_.at(mono->side_atoms_.size() -
//         1).at(0)->GetNode()->GetNodeNeighbors(); for(MolecularModeling::AtomVector::iterator it1 =
//         plus_one_atom_neighbors.begin(); it1 != plus_one_atom_neighbors.end(); it1++)
//         {
//             if((*it1)->GetName().at(0) == 'C' && mono->cycle_atoms_str_.find((*it1)->GetId()) ==
//             std::string::npos)///+2 carbon atom found
//             {
//                 MolecularModeling::Atom* plus_two = (*it1);
//                 plus_sides.push_back(plus_two);
//                 mono->side_atoms_.at(mono->side_atoms_.size() - 1).at(1) = plus_two;///in side_atoms_ structure
//                 (std::vector<AtomVector>) the second index of the last element is dedicated to +2 atom
//
//                 MolecularModeling::AtomVector plus_two_atom_neighbors = plus_two->GetNode()->GetNodeNeighbors();
//                 for(MolecularModeling::AtomVector::iterator it2 = plus_two_atom_neighbors.begin(); it2 !=
//                 plus_two_atom_neighbors.end(); it2++)
//                 {
//                     MolecularModeling::Atom* plus_three = (*it2);
//                     if(plus_three->GetName().at(0) == 'C' &&
//                     plus_three->GetId().compare(mono->side_atoms_.at(mono->side_atoms_.size() - 1).at(0)->GetId()) !=
//                     0)///+3 carbon atom found
//                     {
//                         plus_sides.push_back(plus_three);
//                         mono->side_atoms_.at(mono->side_atoms_.size() - 1).at(2) = plus_three;///in side_atoms_
//                         structure (std::vector<AtomVector>) the third index of the last element is dedicated to +3
//                         atom break;
//                     }
//                 }
//                 break;
...
//               gmml::log(__LINE__, __FILE__, gmml::INF, debugStr.str());
//             }
//             if(t_neighbor->GetName().at(0) == 'N' && mono->cycle_atoms_str_.find(t_neighbor->GetId()) ==
//             std::string::npos)///check formulas with nitrogen
//             {
//                 if((value = CheckxC_N(target, mono->cycle_atoms_str_/*, pattern_atoms*/)).compare("") != 0)///xCH-N
//                     break;
//                 if((value = CheckxC_NxO_CO_C(target, mono->cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
//                 0)///xC-N-C=OCH3
//                     break;
//                 if((value = CheckxC_NxO_CO_CO(target, mono->cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
//                 0)///xC-N-C=OCH2OH
//                     break;
//                 if((value = CheckxC_NxO_SO3(target, mono->cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
//                 0)///xC-N-SO3
//                     break;
//                 if((value = CheckxC_NxO_PO3(target, mono->cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
//                 0)///xC-N-PO3
//                     break;
//                 if((value = CheckxC_NxO_C(target, mono->cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
//                 0)///xC-N-CH3
//                     break;
//             }
//             if(t_neighbor->GetName().at(0) == 'O' && mono->cycle_atoms_str_.find(t_neighbor->GetId()) ==
//             std::string::npos)///check formulas with oxygen
//             {
//                 if((value = CheckxC_NxO_CO_C(target, mono->cycle_atoms_str_, 'O', pattern_atoms)).compare("") !=
//                 0)///xC-O-C=OCH3
//                     break;
//                 if((value = CheckxC_NxO_CO_CO(target, mono->cycle_atoms_str_, 'O', pattern_atoms)).compare("") !=
//                 0)///xC-O-C=OCH2OH
//                     break;
//                 if((value = CheckxC_NxO_SO3(target, mono->cycle_atoms_str_, 'O', pattern_atoms)).compare("") !=
//                 0)///xC-O-SO3
//                     break;
//                 if((value = CheckxC_NxO_PO3(target, mono->cycle_atoms_str_, 'O', pattern_atoms)).compare("") !=
//                 0)///xC-O-PO3
//                     break;
//                 if((value = CheckxC_NxO_C(target, mono->cycle_atoms_str_, 'O', pattern_atoms)).compare("") !=
//                 0)///xC-O-CH3
//                     break;
//                 if((value = CheckxCOO(target, mono->cycle_atoms_str_/*, pattern_atoms*/)).compare("") !=
//                 0)///xC-(O,O) and xC-(O,OH)
//                     break;
//             }
//         }
...
//         }
//     }
//     for(std::vector<AtomVector>::iterator it = mono->side_atoms_.begin(); it != mono->side_atoms_.end(); it++)
//     ///iterate on side atoms
//     {
...
//         //     if(sides.at(0) != NULL)
//         //     {
//         //         for(side_branch_last_carbon_index = sides.size() - 1; sides.at(side_branch_last_carbon_index) ==
//         NULL; side_branch_last_carbon_index-- )
//         //         {
...
//                 std::string::npos)///check formulas with nitrogen
//                 {
//                     if((value = CheckxC_N(target, mono->cycle_atoms_str_/*, pattern_atoms*/)).compare("") !=
//                     0)///xCH-N
//                         break;
//                     if((value = CheckxC_NxO_CO_C(target, mono->cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
//                     0)///xC-N-C=OCH3
...
// std::string Assembly::CheckxC_NxO_CH3C_COO(MolecularModeling::Atom *target, std::string cycle_atoms_str,
// MolecularModeling::AtomVector& pattern_atoms)
// {//isopropionic acid
// int local_debug = -1;
// std::stringstream pattern;
...
//         minus_one = true;
//     }
//     for(std::map<std::string, std::string>::iterator it1 = mono->derivatives_map_.begin(); it1 !=
//     mono->derivatives_map_.end(); it1++)
//     {
//         std::string key = (*it1).first;
//         std::string value = (*it1).second;
...
//             cond_name_pattern = "N";
//             pattern = "CH-N";
//             AddModificationRuleOneInfo(key, pattern, mono, long_name_pattern, cond_name_pattern, head, tail,
...
//             pattern = "C-O-C=OCH3";
//             AddDerivativeRuleInfo(key, pattern, mono, long_name_pattern, cond_name_pattern, head, minus_one,
//             in_bracket);
//         }
...
//             pattern = "C-(O,OH)";
//             AddModificationRuleTwoInfo(key, pattern, mono, long_name_pattern_at_minus_one,
//             long_name_pattern_at_plus_one, cond_name_pattern, tail, minus_one, in_bracket);
//         }
...
//
//         ///moving a, b or x to after the bracket: short-name + [...] + a/b/x and removing ", " from the end of
//         bracket stream int condensed_name_size = sn.size(); std::string condensed_name = sn; std::string
//         new_name_part1 = ""; char new_name_part2 = ' '; if(condensed_name_size > 0)
//         {
...
//     mono->sugar_name_.monosaccharide_short_name_.compare("") == 0)
//     {
...
//           if( ( index_it = find( mono->chemical_code_->right_up_.begin(), mono->chemical_code_->right_up_.end(), key
//           ) ) != mono->chemical_code_->right_up_.end() ) {
//               ( *index_it ) = code;
//           } else if( ( index_it = find( mono->chemical_code_->right_down_.begin(),
//           mono->chemical_code_->right_down_.end(), key ) ) != mono->chemical_code_->right_down_.end() ) {
//               ( *index_it ) = code;
//           }
//         } else if( ( key.compare( "2" ) == 0 ) || ( key.compare( "3" ) == 0 ) || ( key.compare( "4" ) == 0 ) ) {
//           if( ( index_it = find( mono->chemical_code_->left_up_.begin(), mono->chemical_code_->left_up_.end(), key )
//           ) != mono->chemical_code_->left_up_.end() ) {
//               ( *index_it ) = code;
//           } else if( ( index_it = find( mono->chemical_code_->left_down_.begin(),
//           mono->chemical_code_->left_down_.end(), key ) ) != mono->chemical_code_->left_down_.end() ) {
//               ( *index_it ) = code;
//           }
//         }
//       }
