...
//                       FUNCTIONS                      //
//////////////////////////////////////////////////////////
void Assembly::SetDerivativeAngle(MolecularModeling::Residue* residue, MolecularModeling::Residue* parent_residue,
                                  int branch_index)
{
...
void Assembly::SetAttachedResidueBond(MolecularModeling::Residue* residue, MolecularModeling::Residue* parent_residue,
                                      int branch_index, std::string parameter_file)
{
    ...
    for (AtomVector::iterator it = residue_head_atom_adjacent_atoms.begin();
         it != residue_head_atom_adjacent_atoms.end(); it++)
    {
        MolecularModeling::Atom* atom = *it;
        if (atom->GetId().compare(parent_target_atom->GetId()) != 0)
        {
            GeometryTopology::Coordinate* dist =
                new GeometryTopology::Coordinate(*residue_head_atom->GetCoordinates().at(model_index_));
    ...
    GeometryTopology::Coordinate* oxygen_position = new GeometryTopology::Coordinate(
        residue_direction->GetX(), residue_direction->GetY(), residue_direction->GetZ());
    GeometryTopology::Coordinate* offset =
        new GeometryTopology::Coordinate(*parent_target_atom->GetCoordinates().at(model_index_));
...
void Assembly::SetResidueResidueBondDistance(MolecularModeling::Atom* tail_atom,
                                             MolecularModeling::Atom* head_atom_of_child_residue)
{
    // Attention: SetAtomType()function is overloaded as MolecularModeling::Atom::SetAtomType() and
    // MolecularModeling::MolecularDynamicAtom::SetAtomType(). You don't really know which one to use. Likewise,
    // GetAtomType() is also overloaded. In my situation, I called
    // MolecularModeling::MolecularModelingAtom::SetAtomType(), but later called MolecularModeling::Atom::GetAtomType().
    // The result is empty. We need to talk about this later
    /*
    ...
    int GLYCAM06J1ATOMTYPESSIZE =
    sizeof(gmml::MolecularMetadata::GLYCAM::Glycam06j1AtomTypes)/sizeof(gmml::MolecularMetadata::GLYCAM::Glycam06j1AtomTypes[0]);
    for (int j = 0; j < GLYCAM06J1ATOMTYPESSIZE; j++){
        gmml::MolecularMetadata::GLYCAM::AmberAtomTypeInfo entry =
    gmml::MolecularMetadata::GLYCAM::Glycam06j1AtomTypes[j]; if (tail_atom_type == entry.type_){ tail_atom_hybridization
    = entry.hybridization_;
        }
    ...
    int GLYCAM06J1BONDLENGTHSSIZE =
    sizeof(gmml::MolecularMetadata::GLYCAM::Glycam06j1BondLengths)/sizeof(gmml::MolecularMetadata::GLYCAM::Glycam06j1BondLengths[0]);
    for (int j = 0; j < GLYCAM06J1BONDLENGTHSSIZE; j++){
        gmml::MolecularMetadata::GLYCAM::BondLengthByTypePair entry =
    gmml::MolecularMetadata::GLYCAM::Glycam06j1BondLengths[j];
        //Search bidirectionally e.g Cg-Os, Os-Cg
        if ( (tail_atom_type == entry.type1_ && head_atom_type == entry.type2_) || (tail_atom_type == entry.type2_ &&
    head_atom_type == entry.type1_) ){ head_tail_bond_length = entry.length_;
        }
    }
   */   //Above code replaced by Oliver 2018-10-26 with the code below that uses the new metadata classes:
    ...
    gmml::MolecularMetadata::GLYCAM::AmberAtomTypeInfo entry =
        amberAtomTypeInfoContainer.GetEntryWithAtomType(head_atom_type);
    ...
    // tail-head bond vector is the opposite of the sum. Normallize length of tail-head bond vector to bond length if
    // (tail_atom_hybridization == "sp3" || tail_atom_hybridization == "sp2"){
...
void Assembly::Grafting(MolecularModeling::Atom* tail_atom, MolecularModeling::Atom* head_atom_of_child_residue,
                        double head_tail_bond_length)
{
...
void Assembly::SetAttachedResidueAngle(MolecularModeling::Residue* residue, MolecularModeling::Residue* parent_residue,
                                       int branch_index, std::string parameter_file)
{
    ParameterFileSpace::ParameterFile* parameter               = new ParameterFileSpace::ParameterFile(parameter_file);
    ParameterFileSpace::ParameterFile::BondMap parameter_bonds = parameter->GetBonds();
    ...
    for (AtomVector::iterator it = parent_target_atom_adjacent_atoms.begin();
         it != parent_target_atom_adjacent_atoms.end(); it++)
    {
        MolecularModeling::Atom* atom = *it;
    ...
    GeometryTopology::Coordinate* head_target =
        new GeometryTopology::Coordinate(*residue_head_atom->GetCoordinates().at(model_index_));
    head_target->operator-(*parent_target_atom->GetCoordinates().at(model_index_));

    double angle          = acos((carbon_target->DotProduct(*head_target)) /
                                 (carbon_target->length() * head_target->length() + gmml::DIST_EPSILON));
    ...
    double** rotation_matrix =
        gmml::GenerateRotationMatrix(direction, parent_target_atom->GetCoordinates().at(model_index_), rotation_angle);
...
void Assembly::SetAttachedResidueTorsion(MolecularModeling::Residue* residue,
                                         MolecularModeling::Residue* parent_residue, int branch_index)
{
    MolecularModeling::Atom* residue_head_atom  = residue->GetHeadAtoms().at(0);
    MolecularModeling::Atom* parent_target_atom = parent_residue->GetTailAtoms().at(branch_index);

    MolecularModeling::Atom* carbon =
        residue_head_atom; /// The carbon atom of the new residue that is attached to the parent residue
    if (carbon != NULL)
    {
        MolecularModeling::Atom* oxygen =
            parent_target_atom; /// The oxygen atom of the parent residue that is attached to the new residue
        if (oxygen != NULL)
        {
...
void Assembly::SetPhiTorsion(MolecularModeling::Residue* residue, MolecularModeling::Residue* parent_residue,
                             int branch_index, double torsion)
{
    MolecularModeling::Atom* residue_head_atom  = residue->GetHeadAtoms().at(0);
    MolecularModeling::Atom* parent_target_atom = parent_residue->GetTailAtoms().at(branch_index);
...
void Assembly::SetPsiTorsion(MolecularModeling::Residue* residue, MolecularModeling::Residue* parent_residue,
                             int branch_index, double torsion, bool crystallographic_definition)
{
    MolecularModeling::Atom* residue_head_atom  = residue->GetHeadAtoms().at(0);
    MolecularModeling::Atom* parent_target_atom = parent_residue->GetTailAtoms().at(branch_index);
                ...
                for (AtomVector::iterator it = atom3_neighbors.begin(); it != atom3_neighbors.end(); it++)
                {
                    ...
                    if (neighbor->GetId().compare(oxygen->GetId()) != 0)
                    {
                        if (crystallographic_definition)
                        {
                            if (neighbor->GetName().at(0) == 'C' &&
                                (neighbor->GetName().size() > 1 && isdigit(neighbor->GetName().at(1)) &&
                                 gmml::ConvertString<int>(gmml::ConvertT<char>(neighbor->GetName().at(1))) ==
                                     oxygen_index + 1))
                            {
                                atom4 = neighbor;
                                break;
...
void Assembly::SetOmegaTorsion(MolecularModeling::Residue* residue, MolecularModeling::Residue* parent_residue,
                               int branch_index, double torsion, int type)
{
    MolecularModeling::Atom* residue_head_atom  = residue->GetHeadAtoms().at(0);
    MolecularModeling::Atom* parent_target_atom = parent_residue->GetTailAtoms().at(branch_index);
...
void Assembly::SetOmegaDerivativeTorsion(MolecularModeling::Residue* residue,
                                         MolecularModeling::Residue* parent_residue, int branch_index, double torsion)
{
...
// OG July 3rd 2018. This function leaks memory and I call it a lot during gp building. I've created a version that
// doesn't leak below.
// void Assembly::SetDihedral(MolecularModeling::Atom *atom1, MolecularModeling::Atom *atom2, MolecularModeling::Atom
// *atom3, MolecularModeling::Atom *atom4, double torsion)
//{
...
//    double** torsion_matrix = gmml::GenerateRotationMatrix(b4, a2, current_dihedral -
//    gmml::ConvertDegree2Radian(torsion));
...
double Assembly::GetDihedral(MolecularModeling::Atom* atom1, MolecularModeling::Atom* atom2,
                             MolecularModeling::Atom* atom3, MolecularModeling::Atom* atom4)
{
    double current_dihedral          = 0.0;
...
void Assembly::SetDihedral(MolecularModeling::Atom* atom1, MolecularModeling::Atom* atom2,
                           MolecularModeling::Atom* atom3, MolecularModeling::Atom* atom4, double torsion)
{
    double current_dihedral          = 0.0;
    GeometryTopology::Coordinate* a1 = atom1->GetCoordinates().at(model_index_);
    GeometryTopology::Coordinate* a2 = atom2->GetCoordinates().at(model_index_);
    ...
    double** torsion_matrix =
        gmml::GenerateRotationMatrix(&b4, a2, current_dihedral - gmml::ConvertDegree2Radian(torsion));
...
void Assembly::SetAngle(MolecularModeling::Atom* atom1, MolecularModeling::Atom* atom2, MolecularModeling::Atom* atom3,
                        double angle)
{
...
double Assembly::CalculateBondAngleByCoordinates(GeometryTopology::Coordinate* atom1_crd,
...
double Assembly::CalculateBondAngleByAtoms(MolecularModeling::Atom* atom1, MolecularModeling::Atom* atom2,
                                           MolecularModeling::Atom* atom3)
{
    GeometryTopology::Coordinate* a1 = atom1->GetCoordinates().at(model_index_);
    GeometryTopology::Coordinate* a2 = atom2->GetCoordinates().at(model_index_);
    GeometryTopology::Coordinate* a3 = atom3->GetCoordinates().at(model_index_);
...
// double Assembly::CalculateTorsionAngleByCoordinates(GeometryTopology::Coordinate* atom1_crd,
// GeometryTopology::Coordinate* atom2_crd, GeometryTopology::Coordinate* atom3_crd, GeometryTopology::Coordinate*
// atom4_crd) //This defines angles in radians!!!!!!!!
//{
//     double current_dihedral = 0.0;
...
double Assembly::CalculateTorsionAngleByAtoms(MolecularModeling::Atom* atom1, MolecularModeling::Atom* atom2,
                                              MolecularModeling::Atom* atom3, MolecularModeling::Atom* atom4)
{
...
void Assembly::GetCenterOfMass(GeometryTopology::Coordinate* center_of_mass)
{
    ...
    {
        MolecularModeling::Atom* atom = *it;
        center_of_mass->Translate(
            atom->MolecularDynamicAtom::GetMass() * atom->GetCoordinates().at(model_index_)->GetX(),
    ...
    center_of_mass->operator/(GeometryTopology::Coordinate(
        center_of_mass->GetX() / total_mass, center_of_mass->GetY() / total_mass, center_of_mass->GetZ() / total_mass));
}
...
void Assembly::GetBoundary(GeometryTopology::Coordinate* lower_left_back_corner,
                           GeometryTopology::Coordinate* upper_right_front_corner)
{
    ...
    for (AtomVector::iterator it = all_atoms_of_assembly.begin(); it != all_atoms_of_assembly.end(); it++)
    {
        ...
        {
            //            gmml::log(__LINE__, __FILE__,  gmml::ERR, "There is no information of the atom
            //            type/radius/charge of the atoms in the given library/parameter file"); cout << "There is no
            //            information of the atom type/radius/charge of the atoms in the given library/parameter file"
            //            << endl;
        ...
        double upper_right_front_x =
            atom->GetCoordinates().at(model_index_)->GetX() + atom->MolecularDynamicAtom::GetRadius();
        double lower_left_back_x =
            atom->GetCoordinates().at(model_index_)->GetX() - atom->MolecularDynamicAtom::GetRadius();
        ...
        double upper_right_front_y =
            atom->GetCoordinates().at(model_index_)->GetY() + atom->MolecularDynamicAtom::GetRadius();
        double lower_left_back_y =
            atom->GetCoordinates().at(model_index_)->GetY() - atom->MolecularDynamicAtom::GetRadius();
        ...
        double upper_right_front_z =
            atom->GetCoordinates().at(model_index_)->GetZ() + atom->MolecularDynamicAtom::GetRadius();
        double lower_left_back_z =
            atom->GetCoordinates().at(model_index_)->GetZ() - atom->MolecularDynamicAtom::GetRadius();
...
std::string Assembly::CalculateRSOrientations(MolecularModeling::Atom* prev_atom, MolecularModeling::Atom* target,
                                              MolecularModeling::Atom* next_atom)
{
    ...
    GeometryTopology::Coordinate current_atom_coord =
        GeometryTopology::Coordinate(*target->GetCoordinates().at(model_index_));
    ...
    for (AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); it1++)
    {
        ...
        if (neighbor->GetId().at(0) == 'O')
        {
            ...
            normal_v_x_side_atom.CrossProduct(side_atom_coord); /// cross product (perpendicular vector to plan's normal
                                                                /// vector and the normal vector of side atom oxygen)
            double sin_theta = normal_v_x_side_atom.length() / (normal_v.length() * side_atom_coord.length());

            if (sin_theta >= 0)
            { /// theta between plan's normal vector and the vector of side oxygen is between 0 to 180 degree
                orientation = "R";
            }
            else
            { /// theta between plan's normal vector and the vector of side oxygen is between grater than 180 degree
                orientation = "S";
            }
            break;
        }
