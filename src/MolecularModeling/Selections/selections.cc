...
MolecularModeling::AtomVector selection::AtomsWithinDistanceOf(MolecularModeling::Atom* query_atom, double distance,
                                                               MolecularModeling::AtomVector atoms)
{
...
void selection::FindAtomsConnectingResidues(MolecularModeling::Atom* current_atom,
                                            MolecularModeling::Residue* second_residue,
                                            MolecularModeling::AtomVector* connecting_atoms, bool* found_neighbor)
{
    ...
    for (MolecularModeling::AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); ++it1)
    {
        ...
        if (neighbor->GetResidue()->GetId().compare(second_residue->GetId()) ==
            0) // Should be using indices for Residues too.
        {
            *found_neighbor = true;
        ...
        else if ((neighbor->GetDescription().compare("VisitedByFindAtomsConnectingResidues") != 0) &&
                 (current_atom->GetResidue()->GetId().compare(neighbor->GetResidue()->GetId()) == 0))
        {
...
// atom.
bool selection::FindCyclePoint(MolecularModeling::Atom* previous_atom, MolecularModeling::Atom* current_atom,
                               MolecularModeling::AtomVector* atom_path, bool* found_cycle_point,
                               MolecularModeling::Atom*& cycle_point)
{
    // I wish there was a more solid way to check element type. But here we go, I definitely don't want cycles involving
    // hydrogens (they only every form one bond, unless bond by distance has bonded them).
    if (current_atom->GetName().at(0) != 'H')
    {
        ...
        for (MolecularModeling::AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); ++it1)
        {
            ...
            if ((neighbor->GetIndex() != previous_atom->GetIndex()) &&
                (current_atom->GetResidue()->GetId().compare(neighbor->GetResidue()->GetId()) == 0))
...
bool selection::FindRotationPointsForNonCycles(MolecularModeling::Atom* previous_atom,
                                               MolecularModeling::Atom* current_atom,
                                               MolecularModeling::AtomVector* rotation_points)
{
    ...
    //        {
    //            if ( (neighbor_neighbor->GetIndex() != current_atom->GetIndex()) &&
    //            (current_atom->GetResidue()->GetId().compare(neighbor_neighbor->GetResidue()->GetId())==0))
    //            {
    ...
    //        }
    //    }
    //    if ( found_rotation_point ) // Deadend Chitchatchoes. And we aren't right at the beginning looking down a
    //    deadend branch.
    //    {
    ...
    //        for (auto &neighbor : intra_node_neighbors)
    //        {
    //            // Only want one dead end for now, can't handle mulitple ones yet. Also, don't land on a dead end.
    //            Should not happen except at very start of recursion e.g(otherRes-C(=O)-CH3)
    //            //std::cout << neighbor->GetId() << "neighbor of\n" << current_atom->GetId() << "has " <<
    //            neighbor->GetNode()->GetIntraNodeNeighbors().size()  << " intranode neih\n"; if (
    //            (rotation_points->empty()) && (neighbor->GetNode()->GetNodeNeighbors().size() > 1) )
...
bool selection::FindPathBetweenTwoAtoms(MolecularModeling::Atom* current_atom, MolecularModeling::Atom* target_atom,
                                        MolecularModeling::AtomVector* atom_path, bool* found)
{
    // atom_path->push_back(current_atom);
    ...
    for (MolecularModeling::AtomVector::iterator it1 = neighbors.begin(); it1 != neighbors.end(); ++it1)
    {
        ...
        if ((*found == false) &&
            ...
            //(neighbor->GetDescription() != "VistedByFindPathBetweenTwoAtoms" )
            && ((current_atom->GetResidue() == neighbor->GetResidue()) ||
                (target_atom->GetResidue() == neighbor->GetResidue()))
            //&&  ( (current_atom->GetResidue()->GetId() == neighbor->GetResidue()->GetId()) ||
            //(target_atom->GetResidue()->GetId() == neighbor->GetResidue()->GetId()) )
        )
        {
            // std::cout << "STEEEPER\n";
...
// comparision
MolecularModeling::Atom* selection::FindCyclePointNeighbor(const MolecularModeling::AtomVector atom_path,
                                                           MolecularModeling::Atom* cycle_point)
{
    ...
    if (cycle_point->GetName().compare("CA") == 0) // This is a protein, and we always want the N atom.
    {
      ...
      // fructose 1-1)
             ...
             (std::find(atom_path.begin(), atom_path.end(), cycle_point->GetResidue()->GetAtom("C1")) ==
              atom_path.end()))
    {
    ...
    {
        selected_neighbor =
            cycle_point->GetResidue()->GetAtom("C2"); // We can always set phi to 180 this way, regardless of alpha/beta
    }
         ...
         // so put that above.
    {
        ...
        if (good_neighbors.size() == 0)
        {
            ...
            //            selection::FindCyclePointNeighbor" << std::endl;
        }
...
MolecularModeling::AtomVector selection::GetAtomsCommonToBothAtomVectors(MolecularModeling::AtomVector a,
                                                                         MolecularModeling::AtomVector b)
{
...
MolecularModeling::AtomVector selection::GetAtomsin_a_Notin_b_AtomVectors(MolecularModeling::AtomVector a,
                                                                          MolecularModeling::AtomVector b)
{
    MolecularModeling::AtomVector returned_selection;
    bool isCommon = false;
...
// MolecularModeling::Atom* selection::FindAtomNeighborThatMatchesQuery(MolecularModeling::Atom *atom, std::string
// query)
//{
...
// MolecularModeling::AtomVector selection::FindBondedAtomsThatMatchQuery(MolecularModeling::Atom *anomeric_carbon,
// std::vector<std::regex> atom_name_queries)
//{
//     // Jesus.
...
//    bool success = false;
//    int depth = 0;
//     selection::seek_neighbors_with_regex_query(anomeric_carbon, &found_atom_path, depth, atom_name_queries,
//     &success);
...
////        ++depth;
////        selection::seek_neighbors_with_regex_query(anomeric_carbon, &found_atom_path, depth, atom_name_queries,
///&success); /    } /    else /    { /        depth = 0;
// //       selection::seek_neighbors_with_regex_query(anomeric_carbon, &found_atom_path, depth, atom_name_queries,
// &success);
////    }
////    std::ptrdiff_t pos = std::distance(atom_name_queries.begin(), std::find(atom_name_queries.begin(),
/// atom_name_queries.end(), old_name_)); /    if(pos >= Names.size()) { /        //old_name_ not found /    }
//}

// MolecularModeling::Atom* selection::FindClosestNeighbor

// void selection::seek_neighbors_with_regex_query(MolecularModeling::Atom *current_atom, MolecularModeling::AtomVector
// *found_atom_path, int depth, std::vector<std::regex> atom_name_queries, bool &success)
//{
...
// Assumes query is in the format ?_222 or A_222, where 222 is the residue number and ?/A is the chain ID. ? if not
// specified.
...
MolecularModeling::ResidueVector selection::SortResidueNeighborsByAcendingConnectionAtomNumber(
    MolecularModeling::AtomVector atomsConnectedToOtherResidues)
{ // Sort neighbors by lowest index so Residue_linkage labeling matches front end.
    ...
    std::sort(atomsConnectedToOtherResidues.begin(), atomsConnectedToOtherResidues.end(),
              selection::compareAtomNumbers);

    // Now we want to get the residue of each atom connected to these atoms, but some of these will be in the same
    // residue and should be ignored.
    ...
    for (auto& sortedAtom : atomsConnectedToOtherResidues)
    {
        ...
        { // want to find residue of atoms that are connected to sortedAtom
            ...
            {
                //    std::cout << "Neighbor of that atom which is in another resiude is " <<
                //    neighboringAtom->GetResidue()->GetId() << "@" << neighboringAtom->GetId() << "\n";
...
void selection::FindEndsOfBranchesFromLinkageAtom(MolecularModeling::Atom* currentAtom,
                                                  MolecularModeling::Atom* previousAtom, Branch* branch)
{
    ...
    {
        if (neighbor->GetDescription() != "VistedByFindEndsOfBranchesFromLinkageAtom" && *neighbor != *previousAtom &&
...
std::vector<MolecularModeling::Residue*> selection::FindResiduesWithChainID(MolecularModeling::Assembly& ass,
                                                                            std::string inputChainID)
{
...
MolecularModeling::Residue*
selection::FindNeighborResidueConnectedViaSpecificAtom(MolecularModeling::Residue* queryResidue,
                                                       std::string queryAtomName)
{
    ...
    if (queryAtom == nullptr)
    {
        gmml::log(__LINE__, __FILE__, gmml::WAR,
                  "Warning: An atom named " + queryAtomName + " not found in residue: " + queryResidue->GetId());
        return nullptr;
    }
