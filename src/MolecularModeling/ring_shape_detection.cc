...
std::string glylib::CalculateRingShapeBFMP(GeometryTopology::CoordinateVector ring_coordinates, int cut_off)
{
    ...
    int list[60]        = {0, 1, 2, 3, 0, 1, 2, 4, 0, 1, 2, 5, 0, 1, 3, 4, 0, 1, 3, 5, 0, 1, 4, 5, 0, 2, 3, 4, 0, 2,
                           3, 5, 0, 2, 4, 5, 0, 3, 4, 5, 1, 2, 3, 4, 1, 2, 3, 5, 1, 2, 4, 5, 1, 3, 4, 5, 2, 3, 4, 5};
    ...
    for (i = 0; i < 15; i++)
    {
        temp_dihedral0 = calculateTorsionAngle(ring_coordinates.at(list[j]), ring_coordinates.at(list[j + 1]),
                                               ring_coordinates.at(list[j + 2]), ring_coordinates.at(list[j + 3]));
        temp_dihedral1 = calculateTorsionAngle(ring_coordinates.at(list[j + 1]), ring_coordinates.at(list[j + 2]),
                                               ring_coordinates.at(list[j + 3]), ring_coordinates.at(list[j]));
        temp_dihedral2 = calculateTorsionAngle(ring_coordinates.at(list[j + 2]), ring_coordinates.at(list[j + 3]),
                                               ring_coordinates.at(list[j]), ring_coordinates.at(list[j + 1]));
        temp_dihedral3 = calculateTorsionAngle(ring_coordinates.at(list[j + 3]), ring_coordinates.at(list[j]),
                                               ring_coordinates.at(list[j + 1]), ring_coordinates.at(list[j + 2]));
        // std::cout << temp_dihedral0 << ", " << temp_dihedral1 << ", " << temp_dihedral2 << ", " << temp_dihedral3 <<
        // "\n";
        fifteen_dihedrals[i] =
            (fabs(temp_dihedral0) + fabs(temp_dihedral1) + fabs(temp_dihedral2) + fabs(temp_dihedral3)) / 4;
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        dihedral = calculateTorsionAngle(ring_coordinates.at(list[j]), ring_coordinates.at(list[j + 1]),
                                         ring_coordinates.at(list[j + 2]), ring_coordinates.at(list[j + 3]));
        four.clear();
        ...
        // std::cout << "Plane: " << fifteen_planes.at(i).A << ", " << fifteen_planes.at(i).B << ", " <<
        // fifteen_planes.at(i).C << ", " << fifteen_planes.at(i).D << "\n";
        j = j + 4;
    }
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        dihedral         = calculateTorsionAngle(ring_coordinates.at(six_list[j]), ring_coordinates.at(six_list[j + 1]),
                                                 ring_coordinates.at(six_list[j + 2]), ring_coordinates.at(six_list[j + 3]));
   ...
   if(sixcheck !=6 ){
           for(i=0;i<1;i++){
                   ...
                   if((six_dihedrals[0] >=36.0 || six_dihedrals[0] <=-36.0) && (six_dihedrals[1]>=36.0 ||
six_dihedrals[1]<=-36.0)){ fivecheck++; for(i=0;i<2;i++){ fosize++;
                                   forty_planes=(int*)realloc(forty_planes,(fosize*sizeof(int)));
                                   forty_planes[fosize-1]=i;
                           }
                   }
                   //printf("%lf and %lf\n",six_dihedrals[1],six_dihedrals[2]);
                   if((six_dihedrals[1] >=36.0 || six_dihedrals[1] <=-36.0) && (six_dihedrals[2]>=36.0 ||
six_dihedrals[2]<=-36.0)){ fivecheck++; for(i=1;i<3;i++){ fosize++;
                                   forty_planes=(int*)realloc(forty_planes,(fosize*sizeof(int)));
                                   forty_planes[fosize-1]=i;
                           }
                   ...
                   if((six_dihedrals[2] >=36.0 || six_dihedrals[2] <=-36.0) && (six_dihedrals[3]>=36.0 ||
six_dihedrals[3]<=-36.0)){ fivecheck++; for(i=2;i<4;i++){ fosize++;
                                   forty_planes=(int*)realloc(forty_planes,(fosize*sizeof(int)));
                                   forty_planes[fosize-1]=i;
                           }
                   }
                   //printf("%lf and %lf\n",six_dihedrals[3],six_dihedrals[4]);
                   if((six_dihedrals[3] >=36.0 || six_dihedrals[3] <=-36.0) && (six_dihedrals[4]>=36.0 ||
six_dihedrals[4]<=-36.0)){ fivecheck++; for(i=3;i<5;i++){ fosize++;
                                   forty_planes=(int*)realloc(forty_planes,(fosize*sizeof(int)));
                                   forty_planes[fosize-1]=i;
                           }
                   ...
                   if((six_dihedrals[4] >=36.0 || six_dihedrals[4] <=-36.0) && (six_dihedrals[5]>=36.0 ||
six_dihedrals[5]<=-36.0)){ fivecheck++; for(i=4;i<6;i++){ fosize++;
                                   forty_planes=(int*)realloc(forty_planes,(fosize*sizeof(int)));
                                   forty_planes[fosize-1]=i;
                           }
                   }
                   //printf("%lf and %lf\n",six_dihedrals[5],six_dihedrals[0]);
                   if((six_dihedrals[5] >=36.0 || six_dihedrals[5] <=-36.0) && (six_dihedrals[0]>=36.0 ||
six_dihedrals[0]<=-36.0)){ fivecheck++; fosize++; forty_planes=(int*)realloc(forty_planes,(fosize*sizeof(int)));
   ...
   for(i=0;i<6;i++){
           if(i != forty_planes[0] && i!= forty_planes[1]){
                   if(i!=thirty_planes[0]){
                           //printf("inside if\n");
                           if((six_dihedrals[i]>=20.0 && six_dihedrals[i]<=30.0) || (six_dihedrals[i]<=-20.0 &&
six_dihedrals[i]>=-30.0)){ thcheck=1;
                           }
                   }
           }
   }
...
//printf("thcheck is %d\n",thcheck);*/
    ...
    if (envelope_check == 1)
    {
        ...
        {
            // printf("%lf and %lf\n",six_dihedrals[0],six_dihedrals[1]);
            if ((six_dihedrals[0] <= 12.0 && six_dihedrals[0] >= -12.0) &&
                (six_dihedrals[1] <= 12.0 && six_dihedrals[1] >= -12.0))
            {
                fivecheck2 = 1;
                ssize      = 0;
            ...
            // printf("%lf and %lf\n",six_dihedrals[1],six_dihedrals[2]);
            if ((six_dihedrals[1] <= 12.0 && six_dihedrals[1] >= -12.0) &&
                (six_dihedrals[2] <= 12.0 && six_dihedrals[2] >= -12.0))
            {
                fivecheck2 = 1;
                ssize      = 4;
                for (j = ssize; j < ssize + 8; j++)
            ...
            // printf("%lf and %lf\n",six_dihedrals[2],six_dihedrals[3]);
            if ((six_dihedrals[2] <= 12.0 && six_dihedrals[2] >= -12.0) &&
                (six_dihedrals[3] <= 12.0 && six_dihedrals[3] >= -12.0))
            {
                fivecheck2 = 1;
                ssize      = 8;
                for (j = ssize; j < ssize + 8; j++)
            ...
            // printf("%lf and %lf\n",six_dihedrals[3],six_dihedrals[4]);
            if ((six_dihedrals[3] <= 12.0 && six_dihedrals[3] >= -12.0) &&
                (six_dihedrals[4] <= 12.0 && six_dihedrals[4] >= -12.0))
            {
                fivecheck2 = 1;
                ssize      = 12;
                for (j = ssize; j < ssize + 8; j++)
            ...
            // printf("%lf and %lf\n",six_dihedrals[4],six_dihedrals[5]);
            if ((six_dihedrals[4] <= 12.0 && six_dihedrals[4] >= -12.0) &&
                (six_dihedrals[5] <= 12.0 && six_dihedrals[5] >= -12.0))
            {
                fivecheck2 = 1;
                ssize      = 16;
                for (j = ssize; j < ssize + 8; j++)
            ...
            // printf("%lf and %lf\n",six_dihedrals[5],six_dihedrals[0]);
            if ((six_dihedrals[5] <= 12.0 && six_dihedrals[5] >= -12.0) &&
                (six_dihedrals[0] <= 12.0 && six_dihedrals[0] >= -12.0))
            {
                fivecheck2 = 1;
                ssize      = 20;
   ...
   printf("%d\n",envelope_planes[i]);
    ...
    if (fivecheck2 == 0 && no_planes > 0)
    {
        ...
        for (i = 0; i < no_planes; i++)
        {
            ...
            d[0] = get_signed_distance_from_point_to_plane(fifteen_planes[sortedplanes[i]],
                                                           ring_coordinates.at(secondlist[sortedplanes[i] * 2]));
            d[1] = get_signed_distance_from_point_to_plane(fifteen_planes[sortedplanes[i]],
                                                           ring_coordinates.at(secondlist[sortedplanes[i] * 2 + 1]));
            // printf("%f and %f\n",d[0],d[1]);
            if (d[0] < 0.0 && d[1] > 0.0)
            {
                fprintf(Oset.F, "\t%d%c%d(%f)\t", secondlist[sortedplanes[i] * 2 + 1] + 1, conformers[sortedplanes[i]],
                        secondlist[sortedplanes[i] * 2] + 1, fifteen_dihedrals[sortedplanes[i]]);
                result << "\t" << secondlist[sortedplanes[i] * 2 + 1] + 1 << conformers[sortedplanes[i]]
                       << secondlist[sortedplanes[i] * 2] + 1 << "(" << fifteen_dihedrals[sortedplanes[i]] << ")"
                       << "\t";
            }
            if (d[0] > 0.0 && d[1] < 0.0)
            {
                fprintf(Oset.F, "\t%d%c%d(%f)\t", secondlist[sortedplanes[i] * 2] + 1, conformers[sortedplanes[i]],
                        secondlist[sortedplanes[i] * 2 + 1] + 1, fifteen_dihedrals[sortedplanes[i]]);
                result << "\t" << secondlist[sortedplanes[i] * 2] + 1 << conformers[sortedplanes[i]]
                       << secondlist[sortedplanes[i] * 2 + 1] + 1 << "(" << fifteen_dihedrals[sortedplanes[i]] << ")"
                       << "\t";
            }
            if (d[0] > 0.0 && d[1] > 0.0)
            {
                fprintf(Oset.F, "\t%d%d%c(%f)\t", secondlist[sortedplanes[i] * 2] + 1,
                        secondlist[sortedplanes[i] * 2 + 1] + 1, conformers[sortedplanes[i]],
                        fifteen_dihedrals[sortedplanes[i]]);
                result << "\t" << secondlist[sortedplanes[i] * 2] + 1 << secondlist[sortedplanes[i] * 2 + 1] + 1
                       << conformers[sortedplanes[i]] << "(" << fifteen_dihedrals[sortedplanes[i]] << ")"
                       << "\t";
            }
            if (d[0] < 0.0 && d[1] < 0.0)
            {
                fprintf(Oset.F, "\t%c%d%d(%f)\t", conformers[sortedplanes[i]], secondlist[sortedplanes[i] * 2] + 1,
                        secondlist[sortedplanes[i] * 2 + 1] + 1, fifteen_dihedrals[sortedplanes[i]]);
                result << "\t" << conformers[sortedplanes[i]] << secondlist[sortedplanes[i] * 2] + 1
                       << secondlist[sortedplanes[i] * 2 + 1] + 1 << "(" << fifteen_dihedrals[sortedplanes[i]] << ")"
                       << "\t";
            }
        }
...
double glylib::calculateTorsionAngle(GeometryTopology::Coordinate* coord1, GeometryTopology::Coordinate* coord2,
                                     GeometryTopology::Coordinate* coord3, GeometryTopology::Coordinate* coord4)
{
...
bool glylib::does_conformation_match(int jloops, int* sortedplanes, int* atoms, int atoms_size, int* list,
...
GeometryTopology::CoordinateVector gmml::GetCycleAtomCoordinates(Glycan::Monosaccharide* mono)
{
    ...
    for (MolecularModeling::AtomVector::iterator it1 = mono->cycle_atoms_.begin(); it1 != mono->cycle_atoms_.end();
         ++it1)
    {
        MolecularModeling::Atom* atom                       = (*it1);
        GeometryTopology::CoordinateVector atom_coordinates = atom->GetCoordinates();
        for (GeometryTopology::CoordinateVector::iterator it2 = atom_coordinates.begin(); it2 != atom_coordinates.end();
             ++it2)
        {
