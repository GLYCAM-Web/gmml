...
void Residue::BuildResidueFromPrepFileResidue(PrepFileSpace::PrepFileResidue* prep_residue)
{
    ...
    std::map<PrepFileSpace::PrepFileAtom*, Atom*> prep_assembly_atom_map =
        std::map<PrepFileSpace::PrepFileAtom*, Atom*>(); // associates a prep atom with corresponding assembly atom
    std::map<Atom*, PrepFileSpace::PrepFileAtom*> assembly_prep_parent_atom_map =
        std::map<Atom*,
                 PrepFileSpace::PrepFileAtom*>(); // associates an assembly atom with corresonding prep atom parent.
    ...
    for (PrepFileSpace::PrepFileAtomVector::iterator it1 = prep_atoms.begin(); it1 != prep_atoms.end(); it1++)
    {
        ...
        if (prep_atom->GetType().find("DU") == std::string::npos)
        {
            ...
            // MolecularModeling::Atom::GetAtomType(). The result is empty. We need to talk about this later
        ...
        if (prep_residue->GetCoordinateType() == PrepFileSpace::kINT)
        {
            ...
            if (index == 0)
            {
                //! \todo Add these cout statements to the debugging mechanism once the DebugLevel class (or whatever)
                //! is implemented.
                // std::cout << "Starting to get Cartesian coordinates from internal coords for serial number:  " <<
                // std::endl; std::cout << "    Index is 0 " << std::endl; std::cout << "    Serial number:  " <<
                // serial_number << " ;  Prep Name:  " << prep_atom->GetName() << " ;  Assembly Name:  " <<
                // assembly_atom->GetName() << std::endl;
            }
            if (index == 1)
            {
                ...
                // std::cout << "Starting to get Cartesian coordinates from internal coords for serial number:  " <<
                // std::endl; std::cout << "    Index is 1 " << std::endl; std::cout << "    Serial number:  " <<
                // serial_number << " ;  Prep Name:  " << prep_atom->GetName() << " ;  Assembly Name:  " <<
                // assembly_atom->GetName() << std::endl;
            }
            if (index == 2)
            {
                ...
                coordinate_list.push_back(parent_coordinate);
                // std::cout << "Starting to get Cartesian coordinates from internal coords for serial number:  " <<
                // std::endl; std::cout << "    Index is 2 " << std::endl; std::cout << "    Serial number:  " <<
                // serial_number << " ;  Prep Name:  " << prep_atom->GetName() << " ;  Assembly Name:  " <<
                // assembly_atom->GetName() << std::endl;
            }
            if (index > 2)
            {
                ...
                //                << "Parent Name & index :" << parent_atoms.at(index)->GetName() << "  " <<
                //                parent_index << "\n"; logss << "       X:   " << parent_coordinate->GetX() << "\n";
                //                logss << "       y:   " << parent_coordinate->GetY() << "\n";
                //                logss << "       Z:   " << parent_coordinate->GetZ() << "\n";
                //                logss << "Grand Parent Name  & index : " << parent_atoms.at(parent_index)->GetName()
                //                << "  " << grandparent_index << "\n"; logss << "       X:   " <<
                //                grandparent_coordinate->GetX() << "\n"; logss << "       y:   " <<
                //                grandparent_coordinate->GetY() << "\n"; logss << "       Z:   " <<
                //                grandparent_coordinate->GetZ() << "\n"; logss << "Great Grand Parent Name  & index : "
                //                << parent_atoms.at(grandparent_index)->GetName()  << "  " << great_grandparent_index
            ...
            if (index > 200000) // normally > 2
            {
                rawcoordinate = GeometryTopology::get_cartesian_point_from_internal_coords(
                    coordinate_list[0], coordinate_list[1], coordinate_list[2], dihedral_value, angle_value,
                    bond_length);
        ...
        if (prep_atom->GetTopologicalType() == gmml::TopologicalType::kTopTypeM &&
            prep_atom->GetType().find(prep_residue->GetDummyAtomType()) == std::string::npos)
        {
    ...
    for (PrepFileSpace::PrepFileResidue::Loop::iterator it = all_loops.begin(); it != all_loops.end(); it++)
    {
        ...
        if (from_prep_atom != NULL && to_prep_atom != NULL)
        {
            ...
            if (from_assembly_atom != NULL && to_assembly_atom != NULL)
            {
                ...
                if (std::find(from_assembly_atom_neighbors.begin(), from_assembly_atom_neighbors.end(),
                              to_assembly_atom) == from_assembly_atom_neighbors.end())
                {
                ...
                if (std::find(to_assembly_atom_neighbors.begin(), to_assembly_atom_neighbors.end(),
                              from_assembly_atom) == to_assembly_atom_neighbors.end())
                {
    ...
    for (unsigned int i = 0; i < all_atoms_added.size(); i++)
    {
        ...
        PrepFileSpace::PrepFileAtom* current_assembly_atom_prep_parent =
            assembly_prep_parent_atom_map[current_assembly_atom];
        if (current_assembly_atom_prep_parent !=
            NULL) // When parent atoms are dummy , this equals NULL, since dummy isn't added.
        {
        ...
        if (std::find(existing_assembly_atom_node_neighbors.begin(), existing_assembly_atom_node_neighbors.end(),
                      assembly_parent) == existing_assembly_atom_node_neighbors.end() &&
        ...
        if (assembly_parent != NULL)
        {
            ...
            if (std::find(existing_parent_node_neighbors.begin(), existing_parent_node_neighbors.end(),
                          current_assembly_atom) == existing_parent_node_neighbors.end())
            {
