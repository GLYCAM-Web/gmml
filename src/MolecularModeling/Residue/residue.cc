...
{ // OG Apr 2021: I have no idea what should make up the ID. Can't find any info on what the last three things are.
    ...
    if (chain == "default")
    {
        chain = "A"; // Leaving this commented out as it was the old default. Maybe changing it messes something up and
                     // we need to go back.
    ...
    id_stream << name << "_" << chain << "_" << number << "_" << gmml::BLANK_SPACE << "_" << gmml::BLANK_SPACE << "_"
              << gmml::BLANK_SPACE;
    ...
    for (AtomVector::iterator it = atoms_.begin(); it != atoms_.end(); it++)
    {
        ...
        if (a != atom)
        {
            if (remove_bonds)
            {
                ...
                for (std::vector<MolecularModeling::AtomNode*>::iterator it2 = leaving_atom_nodes.begin();
                     it2 != leaving_atom_nodes.end(); it2++)
                {
                    if ((*it2) != NULL)
                    {
...
// This is duct-tape for now. The PDBPreprocessor should write out an (e.g.) off file that contains all the info I'm
// trying to get here. And then GMML should read it in.
std::string Residue::GetTerminalCode()
{
    if ((this->GetName() == "NME") || (this->GetName() == "ACE"))
    {
        return ""; // Despite being on the ends, these caps don't get the CNME or NACE treatment like regular amino
                   // acids. Fun.
    }
...
bool Residue::GraphPredictionBasedElementLabeling()
{
    ...
    for (AtomVector::iterator it = atoms_.begin(); it != atoms_.end(); it++)
    {
        ...
        if (atom_node != NULL)
        {
            atom_node->SetElementLabel(isdigit(atom->GetName().at(0)) ? atom->GetName().at(1) + ""
                                                                      : atom->GetName().at(0) + "");
        }
        else
        {
...
{
    int local_debug = -1;
    if (std::find(gmml::PROTEINS, (gmml::PROTEINS + gmml::PROTEINSSIZE), this->GetName()) !=
        (gmml::PROTEINS + gmml::PROTEINSSIZE))
    {
        if (local_debug > 0)
        {
...
{
    int local_debug = -1;
    if (std::find(gmml::NUCLEICACIDS, (gmml::NUCLEICACIDS + gmml::NUCLEICACIDSSIZE), this->GetName()) !=
        (gmml::NUCLEICACIDS + gmml::NUCLEICACIDSSIZE))
    {
        if (local_debug > 0)
        {
...
{
    int local_debug = -1;
    if (std::find(gmml::SACCHARIDES, (gmml::SACCHARIDES + gmml::SACCHARIDESSIZE), this->GetName()) !=
        (gmml::SACCHARIDES + gmml::SACCHARIDESSIZE))
    {
        if (local_debug > 0)
        {
...
/*GeometryTopology::Coordinate Residue::GetRingCenter() // Disabled by OG; GetIsRing returns true for all atoms even
when IsRing wasn't set.
{
...
GeometryTopology::Coordinate Residue::GetGeometricCenter()
{
    ...
    if (atoms.size() == 0)
    {
        //        std::cout << "Problem in Residue::GetGeometricCenter(), the residue " << this->GetId() << " contains
        //        no atoms." << std::endl;
    }
...
MolecularModeling::Atom* Residue::GetAtom(std::string query_name)
{
    ...
    for (AtomVector::iterator it = atoms.begin(); it != atoms.end(); ++it)
    {
        if ((*it)->GetName() == query_name)
        {
            return_atom = (*it);
            return return_atom;
            // std::cout << "From Residue::GetAtom(), returning: " << return_atom->GetId() << " as a match for " <<
            // query_name << std::endl;
        }
    }
...
void Residue::PrettyPrintHet(std::ostream& out)
{
    ...
    for (AtomVector::iterator it = atoms_.begin(); it != atoms_.end(); it++)
    {
        ...
        out << " Coordinates"
            << " X: " << coords->GetX() << ", Y: " << coords->GetY() << ", Z: " << coords->GetZ() << std::endl;
        out << " Neighbors: ";
...
void Residue::PrintHetAtoms(std::ostream& out)
{
    ...
    {
        MolecularModeling::Atom* atom = *it;
        out << atom->GetId() << ";" << atom->GetName() << ";" << atom->GetAtomType() << ";" << atom->GetCharge() << ";"
            << atom->GetMass() << ";";
        GeometryTopology::Coordinate* coords = atom->GetCoordinates().at(0);
...
{
    static unsigned long long s_ResidueIndex =
        0;                   // static keyword means it is created only once and persists beyond scope of code block.
    return s_ResidueIndex++; // makes copy of s_AtomIndex, increments the real s_AtomIndex, then returns the value in
                             // the copy
