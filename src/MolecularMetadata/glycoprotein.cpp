...
std::vector<std::string> glycoproteinMetadata::GetTagsForSequence(const std::string firstRes, const std::string midRes,
                                                                  const std::string lastRes)
{
    ...
    // O-links // The midRes == "" is funky, but if I'm coming in here with more than just firstRes, I already have this
    // tag from the initial call.
...
std::string glycoproteinMetadata::GetSequenceContextAndDetermineTags(cds::Residue* residue,
                                                                     std::vector<std::string>& tags)
{
    ...
    if (firstPrecedingNeighbor)
    {
        ...
        {
            precedingContext =
                glycoproteinMetadata::LookupCodeForAminoAcidName(secondPrecedingNeighbor->getName()) + precedingContext;
        }
    }
    ...
    if (firstFollowingNeighbor)
    {
        ...
        if (secondFollowingNeighbor)
        {
            ...
            std::vector<std::string> contextTags =
                glycoproteinMetadata::GetTagsForSequence(conjugationResidueCode, midResCode, lastResCode);
