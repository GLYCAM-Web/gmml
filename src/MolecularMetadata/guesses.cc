...
bool Assembly::guessIfC_CDoubleBond(MolecularModeling::Atom* carbon1, MolecularModeling::Atom* carbon2)
{
    ...
    if ((carbon1neighbors.size() < 4) && (carbon2neighbors.size() < 4)) // If both have three or less neighbors
    {
        ...
        for (MolecularModeling::AtomVector::iterator it = carbon1neighbors.begin(); it != carbon1neighbors.end(); it++)
        {
            ...
            {
                for (MolecularModeling::AtomVector::iterator it2 = carbon2neighbors.begin();
                     it2 != carbon2neighbors.end(); it2++)
                {
                    ...
                    if (thisC2Neighbor->GetId() != carbon1->GetId())
                    {
                        if (local_debug > 0)
                        {
                            debugStr << thisC2Neighbor->GetId() << ", " << carbon1->GetId() << ", " << carbon2->GetId()
                                     << " have an angle of "
                                     << Assembly::CalculateBondAngleByAtoms(thisC2Neighbor, carbon2, carbon1);
                            gmml::log(__LINE__, __FILE__, gmml::INF, debugStr.str());
                            debugStr.str("");
                            debugStr << thisC1Neighbor->GetId() << ", " << carbon1->GetId() << ", " << carbon2->GetId()
                                     << " have an angle of "
                                     << Assembly::CalculateBondAngleByAtoms(thisC1Neighbor, carbon2, carbon1);
                            gmml::log(__LINE__, __FILE__, gmml::INF, debugStr.str());
                            debugStr.str("");
                        }
                        if ((Assembly::CalculateBondAngleByAtoms(thisC2Neighbor, carbon2, carbon1) < 130) &&
                            (Assembly::CalculateBondAngleByAtoms(thisC2Neighbor, carbon2, carbon1) > 110) &&
                            (Assembly::CalculateBondAngleByAtoms(thisC1Neighbor, carbon2, carbon1) < 130) &&
                            (Assembly::CalculateBondAngleByAtoms(thisC1Neighbor, carbon2, carbon1) > 110))
                        {
                            //  haveAll120angles = true;  // commenting out because not used
                            if (local_debug > 0)
                            {
                                debugStr << carbon1->GetId() << " and " << carbon2->GetId() << " and "
                                         << thisC2Neighbor->GetId() << " form an angle between 115 and 125 degrees\n";
                                gmml::log(__LINE__, __FILE__, gmml::INF, debugStr.str());
                                debugStr.str("");
                            }
                        }
                        ...
                        if (local_debug > 0)
                        {
                            debugStr << carbon1->GetId() << ", " << carbon2->GetId() << ", " << thisC2Neighbor->GetId()
                                     << " and " << thisC1Neighbor->GetId() << " have a torsion angle of "
                                     << Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2,
                                                                               thisC2Neighbor);
                            gmml::log(__LINE__, __FILE__, gmml::INF, debugStr.str());
                            debugStr.str("");
                        }
                        if ((Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2, thisC2Neighbor) >
                             6.2 /*greater than about 355 degrees as the function returns radians*/) &&
                            (Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2, thisC2Neighbor) <
                             0.09 /*less than about 5 degrees*/))
                        {
                            areTrigonalPlanar = true;
                            if (local_debug > 0)
                            {
                                debugStr << thisC1Neighbor->GetId() << ", " << carbon1->GetId() << ", "
                                         << carbon2->GetId() << ", and " << thisC2Neighbor->GetId() << " are planar\n";
                                gmml::log(__LINE__, __FILE__, gmml::INF, debugStr.str());
                                debugStr.str("");
                            }
                        }
                        else if ((Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2,
                                                                         thisC2Neighbor) <
                                  -6.2 /*less than about -355 degrees as the function returns radians*/) &&
                                 (Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2,
                                                                         thisC2Neighbor) >
                                  -0.09 /*greater than about -5 degrees*/))
                        {
                            areTrigonalPlanar = true;
                            if (local_debug > 0)
                            {
                                ...
                                debugStr.str("");
                                                                         ...
                                                                         thisC2Neighbor) >
                                  3.05 /*greater than about 175 degrees as the function returns radians*/) &&
                                 (Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2,
                                                                         thisC2Neighbor) <
                                  3.22 /*less than about 185 degrees*/))
                        {
                            areTrigonalPlanar = true;
                            if (local_debug > 0)
                            {
                                ...
                                debugStr.str("");
                                                                         ...
                                                                         thisC2Neighbor) <
                                  -3.05 /*less than about -175 degrees as the function returns radians*/) &&
                                 (Assembly::CalculateTorsionAngleByAtoms(thisC1Neighbor, carbon1, carbon2,
                                                                         thisC2Neighbor) >
                                  -3.22 /*greater than about -185 degrees*/))
                        {
                            areTrigonalPlanar = true;
                            if (local_debug > 0)
                            {
...
std::pair<double, double> Assembly::guessBondLengthByAtomType(MolecularModeling::Atom* atom1,
