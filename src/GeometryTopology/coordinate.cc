...
{
    double dist = (x_ - coordinate.x_) * (x_ - coordinate.x_) + (y_ - coordinate.y_) * (y_ - coordinate.y_) +
                  (z_ - coordinate.z_) * (z_ - coordinate.z_);
...
Coordinate* Coordinate::ConvertInternalCoordinate2CartesianCoordinate(CoordinateVector coordinate_list, double distance,
                                                                      double angle, double torsion)
{
    ...
    if (coordinate_list.size() == 2)
    {
        Coordinate* coordinate =
            new Coordinate(coordinate_list.at(1)->GetX() - cos(gmml::ConvertDegree2Radian(angle) * distance),
    ...
    else
    {
        ...
        //! implemented.
        // logss << "Internal to Cartesian.  distance, angle and torsion are:  " << distance << " " << angle << " " <<
        // torsion << "\n";
        if (torsion < 0.0)
        {
            torsion += 360.0;
        ...
        Coordinate great_grandparent_vector =
            Coordinate(coordinate_list.at(0)->GetX(), coordinate_list.at(0)->GetY(), coordinate_list.at(0)->GetZ());
        Coordinate grandparent_vector =
            Coordinate(coordinate_list.at(1)->GetX(), coordinate_list.at(1)->GetY(), coordinate_list.at(1)->GetZ());
        Coordinate parent_vector =
            Coordinate(coordinate_list.at(2)->GetX(), coordinate_list.at(2)->GetY(), coordinate_list.at(2)->GetZ());
        ...
        if ((fabs(v1.GetX() - v2.GetX()) < localEpsilon) && (fabs(v1.GetY() - v2.GetY()) < localEpsilon) &&
            (fabs(v1.GetZ() - v2.GetZ()) < localEpsilon))
        {
            ...
            //! \todo  Add these cout statmentes and associated error messaging to the errors/logging functions when
            //! created
...
Coordinate* Coordinate::ConvertCartesianCoordinate2InternalCoordinate(Coordinate* coordinate,
                                                                      CoordinateVector coordinate_list)
{
    ...
    if (coordinate_list.size() == 2)
    {
        ...
        double dist_current_parent_dot_grandparent_parent =
            dist_current_parent_vector.DotProduct(dist_grandparent_parent_vector);
        double angle = gmml::ConvertRadian2Degree(
            acos(dist_current_parent_dot_grandparent_parent / (dist_current_parent * dist_grandparent_parent)));
