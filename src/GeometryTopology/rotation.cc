...
GeometryTopology::Rotation::RotateCoordinates(GeometryTopology::Coordinate* pivot_point,
                                              GeometryTopology::Coordinate* direction_point, double rotation_angle,
                                              GeometryTopology::CoordinateVector coordinate_set)
{
    // Please refer
    // https://sites.google.com/site/glennmurray/Home/rotation-matrices-and-formulas/rotation-about-an-arbitrary-axis-in-3-dimensions
    // for the technique used in coordinate rotation with respect to arbitrary axis.
    ...
    else
    {
        ...
        for (GeometryTopology::CoordinateVector::iterator it = coordinate_set.begin(); it != coordinate_set.end(); it++)
        {
            ...
            result->SetX((a * (v2 + w2) - u * (b * v + c * w - u * x - v * y - w * z)) * oneMinusCosT + x * cosT +
                         (-c * v + b * w - w * y + v * z) * sinT);
            result->SetY((b * (u2 + w2) - v * (a * u + c * w - u * x - v * y - w * z)) * oneMinusCosT + y * cosT +
                         (c * u - a * w + w * x - u * z) * sinT);
            result->SetZ((c * (u2 + v2) - w * (a * u + b * v - u * x - v * y - w * z)) * oneMinusCosT + z * cosT +
