...
void Cell::CalculateCellCharge()
{
    ...
    for (MolecularModeling::AtomVector::iterator it = all_atoms.begin(); it != all_atoms.end(); it++)
    {
        ...
        if (atom->GetCoordinates().at(this->grid_->GetAssembly()->GetModelIndex())->GetX() <=
                this->GetMaxCorner()->GetX() &&
            atom->GetCoordinates().at(this->grid_->GetAssembly()->GetModelIndex())->GetY() <=
                this->GetMaxCorner()->GetY() &&
            atom->GetCoordinates().at(this->grid_->GetAssembly()->GetModelIndex())->GetZ() <=
                this->GetMaxCorner()->GetZ() &&
            atom->GetCoordinates().at(this->grid_->GetAssembly()->GetModelIndex())->GetX() >
                this->GetMinCorner()->GetX() &&
            atom->GetCoordinates().at(this->grid_->GetAssembly()->GetModelIndex())->GetY() >
                this->GetMinCorner()->GetY() &&
            atom->GetCoordinates().at(this->grid_->GetAssembly()->GetModelIndex())->GetZ() >
                this->GetMinCorner()->GetZ())
        {
            charge += (atom->MolecularDynamicAtom::GetCharge() != gmml::dNotSet)
                          ? atom->MolecularDynamicAtom::GetCharge()
                          : 0.0;
        }
    }
    cell_charge_ = charge;
...
void Cell::CalculateCellPotentialEnergy(double ion_radius)
{
    ...
    for (MolecularModeling::AtomVector::iterator it = all_atoms.begin(); it != all_atoms.end(); it++)
    {
        ...
        double radius = (atom->MolecularDynamicAtom::GetRadius() != gmml::dNotSet)
                            ? atom->MolecularDynamicAtom::GetRadius()
                            : gmml::MINIMUM_RADIUS;
        ...
        {
            potential_energy += ((atom->MolecularDynamicAtom::GetCharge() != gmml::dNotSet)
                                     ? atom->MolecularDynamicAtom::GetCharge() / dist
                                     : 0.0);
        }
    }
    cell_potential_energy_ = potential_energy;
