  ...
  for(std::vector<Glycan::Monosaccharide*>::iterator it = detected_monos.begin(); it != detected_monos.end(); it++)
  {
    ...
    else
    {
      ...
      for(std::vector<std::pair<Glycan::GlycosidicLinkage*, Glycan::Monosaccharide*> >::iterator monoNeighbor = this_mono->mono_neighbors_.begin(); monoNeighbor != this_mono->mono_neighbors_.end(); monoNeighbor++)
      {
        Glycan::GlycosidicLinkage* thisLinkage = (*monoNeighbor).first;
        // std::stringstream ss;
        // ss << this_mono->cycle_atoms_[0]->GetResidue()->GetId() << " is being compared to " << thisLinkage->non_reducing_mono_->cycle_atoms_[0]->GetResidue()->GetId() << " and the linkage type is " << thisLinkage->linkage_type_;
        // gmml::log(__LINE__, __FILE__,  gmml::INF, ss.str());
...
void Glycan::Oligosaccharide::traverseGraph(Glycan::Monosaccharide* thisMono, Glycan::Oligosaccharide* thisOligo)
{
  ...
  if(thisMono->is_root_)
  {
    ...
    if(anomeric_o == NULL)
    {
      ...
      {
        std::vector<MolecularModeling::Atom*> neighbors = thisMono->anomeric_carbon_pointer_->GetNode()->GetNodeNeighbors();
        if (neighbors.size() < 2)
        {
        ...
        {//2 neighbors in ring and 1 not
          for(std::vector<MolecularModeling::Atom*>::iterator it = neighbors.begin(); it != neighbors.end(); it++)
          {
        ...
        else if(neighbors.size() == 4)
        {
          ...
          {
            MolecularModeling::Atom* thisNeighbor = (*it);
            if ((thisMono->cycle_atoms_str_.find(thisNeighbor->GetId()) == std::string::npos) && thisNeighbor->GetElementSymbol() == "O")
            {//this neighbor not in ring
          ...
          if(possibleAnomericOs.size() == 0)
          {
            ...
            {
              MolecularModeling::Atom* thisNeighbor = (*it);
              if ((thisMono->cycle_atoms_str_.find(thisNeighbor->GetId()) == std::string::npos) && (thisMono->anomeric_carbon_pointer_->GetResidue()->GetId() !=  thisNeighbor->GetResidue()->GetId()))
          ...
          {//two non ring atoms attached
            if(possibleAnomericOs.at(0)->GetResidue()->GetId() != thisMono->anomeric_carbon_pointer_->GetResidue()->GetId())
            {
              if(possibleAnomericOs.at(1)->GetResidue()->GetId() == thisMono->anomeric_carbon_pointer_->GetResidue()->GetId())
  ...
  else if(thisMono->mono_neighbors_.size() == 1) //no branching
  {
    ...
    if(thisMono->is_root_)
    {
      ...
      if(!thisMono->mono_neighbors_[0].second->is_visited_)//temp fix to handle anomeric anomeric infinite loop
      {
        ...
        thisOligo->IUPAC_name_ = thisMono->mono_neighbors_[0].first->linkage_type_ +
                                 thisMonoName + thisOligo->IUPAC_name_;
        thisOligo->author_IUPAC_name_ = thisMono->mono_neighbors_[0].first->linkage_type_ +
  ...
  else if(thisMono->mono_neighbors_.size() == 2)
  {
    ...
    if(thisMono->is_root_) //BRANCHED
    {
      ...
      if(numEqualNeighbors == 0)
      {
        ...
        for(unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
        {
          ...
          {
            thisOligo->IUPAC_name_ = thisMono->mono_neighbors_[shortestBranchLocation].first->linkage_type_ + "]" +
                                    thisOligo->IUPAC_name_;
            thisOligo->author_IUPAC_name_ = thisMono->mono_neighbors_[shortestBranchLocation].first->linkage_type_ + "]" +
                                            thisOligo->author_IUPAC_name_;
          }
          ...
          if(!thisMono->mono_neighbors_[highestIndexLocation].second->is_visited_)
          {
            if(i < linkageStringVector.size() - 1)
            {
              thisOligo->IUPAC_name_ = thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ + "]" +
                                        thisOligo->IUPAC_name_;
              thisOligo->author_IUPAC_name_ = thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ + "]" +
                                              thisOligo->author_IUPAC_name_;
            }
            else
            {
              if(local_debug > 0)
              {
                gmml::log(__LINE__, __FILE__,  gmml::INF, thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_);
              }
    ...
    else //not root with 2 neighbors
    {
      ...
      if(!thisMonoNeighbor->is_visited_)
      {
        thisOligo->IUPAC_name_ = thisMono->mono_neighbors_[neighborNum].first->linkage_type_ +
                                thisMonoName + thisOligo->IUPAC_name_;
