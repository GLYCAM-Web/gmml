...
//                       CONSTRUCTOR                    //
//////////////////////////////////////////////////////////
Oligosaccharide::Oligosaccharide(std::vector<Glycan::Monosaccharide*> monos, gmml::ResidueNameMap dataset_residue_names,
                                 MolecularModeling::Assembly* assembly)
{
    ...
    createOligosaccharideGraphs(monos, dataset_residue_names, number_of_covalent_links,
...
void Glycan::Oligosaccharide::Print(std::ostream& out)
{
    ...
    if (terminal_.compare("") != 0)
    {
        if (local_debug > 0)
        {
            gmml::log(__LINE__, __FILE__, gmml::INF, IUPAC_name_);
            gmml::log(__LINE__, __FILE__, gmml::INF, terminal_);
        }
    ...
    bool is_cycle             = false;
    this->GenerateNameLinkage(oligosaccharide_name_, oligosaccharide_linkages_, linkage_index, root_->mono_id_,
                              is_cycle);
    ...
    // For each line of linkages the info of the first residue (e.g. RAE(402_A)) will be added to the
    // oligosaccharide_residue_linkages_.
    std::vector<std::string> oligo_linkages_tokens = gmml::Split(oligosaccharide_linkages_, "\n");
    std::vector<std::string> oligo_name_tokens =
        gmml::Split(oligosaccharide_name_, "-"); /// According to position of brackets in the name sequence,
    ...
    for (unsigned int i = 0; i < oligo_linkages_tokens.size() && i < oligo_name_tokens.size();
         i++) /// Processing linkages line by line
    {
        full_glycosidic_linkage  = oligo_linkages_tokens.at(i);
        link_tokens              = gmml::Split(full_glycosidic_linkage, "}-,");
        link_left_side           = link_tokens.at(1); /// Getting the first residue of linkage in each line. e.g.
                                                      /// {2}RAM(401_A)C1-RAE(402_A)C3, Glycosidic linkage: RAE(402_A)O3
        ...
        {
            link_right_side = link_tokens.at(2); /// Getting the second residue of linkage in the line. e.g.
                                                 /// {1}NAG(1521_A)C1-NAG(1520_A)C4, Glycosidic linkage: NAG(1520_A)O4
                                                 ///-> second_mono == NAG(1520_A)C4
            end_index       = link_right_side.find_last_of(")");
            second_residue_of_linkage =
                link_right_side.substr(0, end_index + 1); /// filtering out atom name. e.g. NAG(1520_A)
    ...
    if (mono_ring_atoms.at(0) != NULL)
    {
        ...
        if (oligo_linkages_tokens.size() == 0 && oligosaccharide_name_.compare("") != 0)
        {
            ...
            {
                residue_links_stream << root_residue_name << "(" << root_residue_number << "_"
                                     << /*root_->bfmp_ring_conformation_ <<*/ ")";
            }
            else
            {
                residue_links_stream << root_residue_name << "(" << root_residue_number << "_" << atom_id_tokens.at(3)
                                     << "_" << /*root_->bfmp_ring_conformation_ <<*/ ")";
            }
        }
    }
    ...
    if (terminal_.compare("") != 0)
    {
        ...
        if (root_anomeric_atom.at(0) != NULL || root_anomeric_atom.at(1) != NULL)
        {
            ...
            if (root_residue_name.compare(terminal_residue_name) != 0 &&
                root_residue_number.compare(terminal_residue_number) != 0)
            {
                ...
                // As we have underscore symbol at the end of terminal, we can remove everything after last _ in each
                // residue of oligo oligo-seq without BFMP would look like: FUL(404_D)-NAG(401_D)-ASN(80_D)
                if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
                {
    ...
    // I have commented out the lines that print to out, as we decided nothing should print that isn't an error.  This
    // function does other things so this was the quickest workaround. DM 1-15-20
 ...
 * @param oligosaccharide_linkages The linkages between sacchrides of the oligosacchride which has been generated by the
 * function so far
 ...
 * @param is_cycle The boolean variable which represnets whether the sacchrides involved in the oligosacchride forms a
 * cycle
 */
void Glycan::Oligosaccharide::GenerateNameLinkage(std::string& oligosaccharide_name,
                                                  std::string& oligosaccharide_linkages, int& i, int main_root_id,
                                                  bool& is_cycle)
{
    ...
    else if (child_oligos_.size() == 1)
    {
        ...
        {
            for (std::vector<MolecularModeling::Atom*>::iterator it = mono1_ring_atoms.begin();
                 it != mono1_ring_atoms.end(); it++)
            {
                ...
                if (atom != NULL && atom->GetId().compare(from) == 0)
                {
                    ...
                    if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
                    {
                        res_linkage1 << "-" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4) << "_" << mono1_bfmp
                                     << ")" << atom_id_tokens.at(0);
                    }
                    else
                    {
                        res_linkage1 << "-" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4) << "_"
                                     << atom_id_tokens.at(3) << "_" << mono1_bfmp << ")" << atom_id_tokens.at(0);
                    }

                    if (glycosidic_linkage_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
                    {
                        res_linkage1 << ", Glycosidic linkage: " << glycosidic_linkage_id_tokens.at(2) << "("
                                     << glycosidic_linkage_id_tokens.at(4) << ")" << glycosidic_linkage_id_tokens.at(0)
                                     << std::endl;
                    }
                    else
                    {
                        res_linkage1 << ", Glycosidic linkage: " << glycosidic_linkage_id_tokens.at(2) << "("
                                     << glycosidic_linkage_id_tokens.at(4) << "_" << glycosidic_linkage_id_tokens.at(3)
        ...
        {
            std::vector<MolecularModeling::Atom*> last_carbon_side_atoms =
                mono1_side_atoms.at(mono1_side_atoms.size() - 1);
            for (std::vector<MolecularModeling::Atom*>::iterator it = last_carbon_side_atoms.begin();
                 it != last_carbon_side_atoms.end(); it++)
            {
                MolecularModeling::Atom* atom = *it;
                if (atom != NULL && atom->GetId().compare(from) == 0)
                {
                    int from_index =
                        std::distance(last_carbon_side_atoms.begin(), it) + mono1_side_atoms.size() + mono1_start_index;
                    link1 << "-" << gmml::ConvertT(from_index);
                    std::vector<std::string> atom_id_tokens               = gmml::Split(from, "_");
                    std::vector<std::string> glycosidic_linkage_id_tokens = gmml::Split(tokens.at(1), "_");
                    if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
        ...
        {
            for (std::vector<MolecularModeling::Atom*>::iterator it = mono2_ring_atoms.begin();
                 it != mono2_ring_atoms.end(); it++)
            {
                MolecularModeling::Atom* atom = *it;
                if (atom != NULL && atom->GetId().compare(to) == 0)
                {
                    ...
                    if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
                    {
                        res_linkage << "{" << i + 1 << "}" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4) << "_"
                                    << mono2_bfmp << ")" << atom_id_tokens.at(0);
                    }
                    else
                    {
                        res_linkage << "{" << i + 1 << "}" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4) << "_"
                                    << atom_id_tokens.at(3) << "_" << mono2_bfmp << ")" << atom_id_tokens.at(0);
                    }
                    break;
                }
            }
        ...
        {
            std::vector<MolecularModeling::Atom*> last_carbon_side_atoms =
                mono2_side_atoms.at(mono2_side_atoms.size() - 1);
            for (std::vector<MolecularModeling::Atom*>::iterator it = last_carbon_side_atoms.begin();
                 it != last_carbon_side_atoms.end(); it++)
            {
                MolecularModeling::Atom* atom = *it;
                ...
                {
                    int to_index =
                        std::distance(last_carbon_side_atoms.begin(), it) + mono2_side_atoms.size() + mono2_start_index;
                    link << gmml::ConvertT(to_index);
                    std::vector<std::string> atom_id_tokens = gmml::Split(to, "_");
                    if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
                    {
    ...
    else
    {
        ...
        int mono1_start_index = 1;
        if (root_->side_atoms_.at(0).at(0) !=
            NULL) /// mono has a carbon at position -1 so the ring atom indices should start from 2
        {
            mono1_start_index = 2;
        }
        for (std::vector<std::string>::iterator it = child_oligos_linkages_.begin(); it != child_oligos_linkages_.end();
             it++)
        {
            ...
            std::string from =
                child_linkage_tokens.at(0); /// the atom id from current mono that is involved in the linkage

            if (root_->cycle_atoms_str_.find(from) != std::string::npos) /// if the atom that is involved in the linkage
                                                                         /// is one of the current mono cycle atoms
            {
                for (std::vector<MolecularModeling::Atom*>::iterator it = root_->cycle_atoms_.begin();
                     it != root_->cycle_atoms_.end(); it++)
                {
                    MolecularModeling::Atom* atom = *it;
                    if (atom != NULL && atom->GetId().compare(from) == 0)
                    {
                        int from_index = std::distance(root_->cycle_atoms_.begin(), it) +
                                         mono1_start_index; /// index position of the atom in the mono
            ...
            {
                std::vector<MolecularModeling::Atom*> last_carbon_side_atoms =
                    root_->side_atoms_.at(root_->side_atoms_.size() - 1);
                for (std::vector<MolecularModeling::Atom*>::iterator it = last_carbon_side_atoms.begin();
                     it != last_carbon_side_atoms.end(); it++)
                {
                    MolecularModeling::Atom* atom = *it;
                    if (atom != NULL && atom->GetId().compare(from) == 0)
                    {
                        int from_index = std::distance(last_carbon_side_atoms.begin(), it) + root_->side_atoms_.size() +
                                         mono1_start_index; /// index position of the side atom in the mono
                        root_child_index_map[linkage_index] = from_index;
                        break;
                    }
                }
        ...
        while (root_child_index_map.size() > 0)
        {
            ...
            for (std::map<int, int>::iterator it0 = root_child_index_map.begin(); it0 != root_child_index_map.end();
                 it0++)
            {
            ...
            if (mono1_ring_atoms_str.find(from) != std::string::npos)
            {
                     ...
                     it != mono1_ring_atoms.end(); it++)
                {
                    ...
                    if (atom != NULL && atom->GetId().compare(from) == 0)
                    {
                        ...
                        {
                            res_linkage1 << "-" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4) << "_"
                                         << mono1_bfmp << ")" << atom_id_tokens.at(0);
                        }
                        else
                        {
                            res_linkage1 << "-" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4) << "_"
                        ...
                        {
                            res_linkage1 << ", Glycosidic linkage: " << glycosidic_linkage_id_tokens.at(2) << "("
                                         << glycosidic_linkage_id_tokens.at(4) << ")"
                                         << glycosidic_linkage_id_tokens.at(0) << std::endl;
                        }
                        else
                        {
                            res_linkage1 << ", Glycosidic linkage: " << glycosidic_linkage_id_tokens.at(2) << "("
                                         << glycosidic_linkage_id_tokens.at(4) << "_"
                                         << glycosidic_linkage_id_tokens.at(3) << ")"
                                         << glycosidic_linkage_id_tokens.at(0) << std::endl;
                        }
                        break;
                    }
            ...
            else
            {
                     ...
                     it != last_carbon_side_atoms.end(); it++)
                {
                    ...
                    if (atom != NULL && atom->GetId().compare(from) == 0)
                    {
                        int from_index = std::distance(last_carbon_side_atoms.begin(), it) + mono1_side_atoms.size() +
                                         mono1_start_index;
                        link1 << "-" << gmml::ConvertT(from_index);
                        std::vector<std::string> atom_id_tokens               = gmml::Split(from, "_");
                        std::vector<std::string> glycosidic_linkage_id_tokens = gmml::Split(tokens.at(1), "_");
            ...
            if (mono2_ring_atoms_str.find(to) != std::string::npos)
            {
                     ...
                     it != mono2_ring_atoms.end(); it++)
                {
                    ...
                    if (atom != NULL && atom->GetId().compare(to) == 0)
                    {
                        ...
                        if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
                        {
                            res_linkage << "{" << i + 1 << "}" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4)
                                        << "_" << mono2_bfmp << ")" << atom_id_tokens.at(0);
                        }
                        else
                        {
                            res_linkage << "{" << i + 1 << "}" << atom_id_tokens.at(2) << "(" << atom_id_tokens.at(4)
                                        << "_" << atom_id_tokens.at(3) << "_" << mono2_bfmp << ")"
                                        << atom_id_tokens.at(0);
                        }
                        break;
                    }
            ...
            else
            {
                     ...
                     it != last_carbon_side_atoms.end(); it++)
                {
                    ...
                    if (atom != NULL && atom->GetId().compare(to) == 0)
                    {
                        int to_index = std::distance(last_carbon_side_atoms.begin(), it) + mono2_side_atoms.size() +
                                       mono2_start_index;
                        link << gmml::ConvertT(to_index);
                        std::vector<std::string> atom_id_tokens = gmml::Split(to, "_");
                        if (atom_id_tokens.at(3).at(0) == gmml::BLANK_SPACE)
...
//   for(std::vector<Glycan::Monosaccharide*>::iterator thisMono = detected_monos.begin(); thisMono !=
//   detected_monos.end(); thisMono++)
//   {
//     Glycan::Monosaccharide* thisMonosaccharide = (*thisMono);
//     if(thisMonosaccharide != NULL)
//     {
//       std::vector<MolecularModeling::Atom*> sideAtoms = (*thisMonosaccharide->side_atoms_.begin());//Checking
//       Anomeric Side atoms for(std::vector<MolecularModeling::Atom*>::iterator thisSideAtom = sideAtoms.begin();
//       thisSideAtom!= sideAtoms.end(); thisSideAtom++)
//       {
//         if(*thisSideAtom != NULL)
//         {
//           bool foundInOtherMono = false;
//           MolecularModeling::Atom* thisSideAtomPointer = (*thisSideAtom);
//           std::vector<MolecularModeling::Atom*> sideAtomNeighbors =
//           thisSideAtomPointer->GetNode()->GetNodeNeighbors(); for(std::vector<MolecularModeling::Atom*>::iterator
//           thisSideAtomNeighbor = sideAtomNeighbors.begin(); thisSideAtomNeighbor!= sideAtomNeighbors.end();
//           thisSideAtomNeighbor++)
//           {
//             MolecularModeling::Atom* thisNeighbor = *thisSideAtomNeighbor;
//             for(std::vector<Glycan::Monosaccharide*>::iterator otherMono = detected_monos.begin(); otherMono !=
//             detected_monos.end(); otherMono++)
//             {
//               if(thisMono != otherMono)///Cheking monos other than the current mono
//               {
//                 Glycan::Monosaccharide* otherMonosaccharide = (*otherMono);
//                 for(std::vector<std::vector<MolecularModeling::Atom*> >::iterator otherMonoSides =
//                 otherMonosaccharide->side_atoms_.begin(); otherMonoSides != otherMonosaccharide->side_atoms_.end();
//                 otherMonoSides++)
//                 {
//                   for(std::vector<MolecularModeling::Atom*>::iterator otherSideAtom = (*otherMonoSides).begin();
//                   otherSideAtom!= (*otherMonoSides).end(); otherSideAtom++)
//                   {
...
//                       std::vector<MolecularModeling::Atom*> otherMonoNeighbors =
//                       (*otherSideAtom)->GetNode()->GetNodeNeighbors(); for
//                       (std::vector<MolecularModeling::Atom*>::iterator otherMonoNeighbor =
//                       otherMonoNeighbors.begin(); otherMonoNeighbor != otherMonoNeighbors.end(); otherMonoNeighbor++)
//                       {
...
//                         }
//                       }
//                       Glycan::GlycosidicLinkage* thisLinkage = new Glycan::GlycosidicLinkage(thisMonosaccharide,
//                       otherMonosaccharide, anomericCarbonID, otherCarbonID);
//                       thisMonosaccharide->mono_neighbors_.insert(std::pair<Glycan::GlycosidicLinkage*,
//                       Glycan::Monosaccharide*>(thisLinkage, otherMonosaccharide)); std::stringstream logss; logss <<
//                       thisMonosaccharide->sugar_name_.monosaccharide_stereochemistry_short_name_ << " is connected to
//                       " << otherMonosaccharide->sugar_name_.monosaccharide_stereochemistry_short_name_ << " via " <<
//                       anomericCarbonID << "-" << otherCarbonID; gmml::log(__LINE__, __FILE__, gmml::INF,
//                       logss.str());
//                     }
//                   }
//                 }
...
Glycan::Oligosaccharide::createOligosaccharides(std::vector<Glycan::Monosaccharide*> detected_monos)
{
    ...
    for (std::vector<Glycan::Monosaccharide*>::iterator it = detected_monos.begin(); it != detected_monos.end(); it++)
    {
        ...
        else
        {
                 ...
                 monoNeighbor != this_mono->mono_neighbors_.end(); monoNeighbor++)
            {
                ...
                // thisLinkage->non_reducing_mono_->cycle_atoms_[0]->GetResidue()->GetId() << " and the linkage type is
                // " << thisLinkage->linkage_type_; gmml::log(__LINE__, __FILE__,  gmml::INF, ss.str());
...
void Glycan::Oligosaccharide::indexMonosaccharides()
{
    ...
    for (std::vector<Glycan::Monosaccharide*>::iterator mono = this->mono_nodes_.begin();
         mono != this->mono_nodes_.end(); mono++)
    {
    ...
    // Generate IUPAC Index (Longest chain, then closest branch to aglycon following IUPAC rules for equal branches &
    // branch ordering)
    for (std::vector<Glycan::Monosaccharide*>::iterator mono = this->mono_nodes_.begin();
         mono != this->mono_nodes_.end(); mono++)
    {
...
void Glycan::Oligosaccharide::indexMono(Glycan::Monosaccharide* thisMono, int thisIndex,
    ...
    if (!thisMono->is_indexed_)
    {
        ...
        else if (thisMono->mono_neighbors_.size() == 2)
        {
            ...
            if (thisMono->is_root_) // BRANCHED
            {
                ...
                for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                {
                    ...
                    if (std::find(equalNeighborsLengths.begin(), equalNeighborsLengths.end(), branchMaxLengths[i]) ==
                        equalNeighborsLengths.end())
                    { // if this length is not already considered
                        equalNeighborsLengths.push_back(branchMaxLengths[i]);
                        int branchesOfThisLength =
                            std::count(branchMaxLengths.begin(), branchMaxLengths.end(), branchMaxLengths[i]);
                ...
                if (numEqualNeighbors == 0)
                {
                    ...
                    unsigned int shortestBranchLocation = std::distance(
                        branchMaxLengths.begin(), std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                ...
                { // The 2 branches are the same length so we need to order by linkage
                    ...
                    for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                    {
                        linkageStringVector.push_back(thisMono->mono_neighbors_[i].first->linkage_type_);
                    }
                    // go through vector
                    // index lower linkage locant
                    // go to next linkage
                    // Getting highest linkage first as it is closer to root in IUPAC name
                    unsigned int highestIndexLocation =
                        std::distance(linkageStringVector.begin(),
                                      std::max_element(linkageStringVector.begin(), linkageStringVector.end()));
        ...
        else // more than 2 neighbors so branched
        {
            if (local_debug > 0)
            {
                ...
                for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                {
                    testLog << thisMono->mono_neighbors_[i].second->cycle_atoms_[0]->GetResidue()->GetId()
                            << " indexed bool is " << thisMono->mono_neighbors_[i].second->is_indexed_;
                    gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                    testLog.str(std::string()); // clear stringstream
                }
            }
            ...
            for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
            {
                ...
                if (std::find(equalNeighborsLengths.begin(), equalNeighborsLengths.end(), branchMaxLengths[i]) ==
                    equalNeighborsLengths.end()) // if this length is not already considered
                {
                    equalNeighborsLengths.push_back(branchMaxLengths[i]);
                    int branchesOfThisLength =
                        std::count(branchMaxLengths.begin(), branchMaxLengths.end(), branchMaxLengths[i]);
            ...
            if (numEqualNeighbors == 0)
            {
                ...
                int shortestBranchLocation = std::distance(
                    branchMaxLengths.begin(), std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                if (thisMono->mono_neighbors_[shortestBranchLocation].second->is_indexed_) // if is previous mono
                {
                    ...
                    shortestBranchLocation                   = std::distance(
                        branchMaxLengths.begin(), std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                }
                branchMaxLengths[shortestBranchLocation] =
                    999; // Already have the location and need to reset the max to the next highest for proper ordering
                for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                {
                    int nextShortestBranchLocation = std::distance(
                        branchMaxLengths.begin(), std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                    if (local_debug > 0)
                    {
                    ...
                    if ((branchMaxLengths[nextShortestBranchLocation] != 999) &&
                        (!thisMono->mono_neighbors_[nextShortestBranchLocation].second->is_indexed_))
                    {
                        if (local_debug > 0)
                        {
                            testLog << thisMono->mono_neighbors_[nextShortestBranchLocation]
                                           .second->cycle_atoms_[0]
                                           ->GetResidue()
                                           ->GetId();
                            gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                            testLog.str(std::string()); // clear stringstream
                        ...
                        this->number_branches_++;
                        thisMono->mono_neighbors_[nextShortestBranchLocation].second->oligo_branch_index_ =
                            this->number_branches_;
            ...
            else
            {
                ...
                unsigned int shortestBranchLocation = std::distance(
                    branchMaxLengths.begin(), std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                if ((std::count(equalNeighborsLengths.begin(), equalNeighborsLengths.end(),
                                branchMaxLengths[shortestBranchLocation]) == 0) &&
                    (branchMaxLengths[shortestBranchLocation] != 999)) // no other branches of this length
                {
                    branchMaxLengths[shortestBranchLocation] = 999; // Have location and need to get next shortest
                    for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                    {
                        int nextShortestBranchLocation =
                            std::distance(branchMaxLengths.begin(),
                                          std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                        if ((branchMaxLengths[nextShortestBranchLocation] != 999) &&
                            (!thisMono->mono_neighbors_[nextShortestBranchLocation].second->is_indexed_))
                        {
                            this->number_branches_++;
                ...
                { // equal branches, but not already considered
                    ...
                    for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                    {
                        ...
                        {
                            testLog << thisMono->mono_neighbors_[i].second->cycle_atoms_[0]->GetResidue()->GetId()
                                    << "This branch is length: " << branchMaxLengths[i];
                            gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                            testLog.str(std::string()); // clear stringstream
                        ...
                        if ((branchMaxLengths[i] == equalBranchLength) &&
                            (!thisMono->mono_neighbors_[i].second->is_indexed_))
                        { // This branch is not yet considered and is equal length
                            if (local_debug > 0)
                            {
                                testLog << thisMono->mono_neighbors_[i].second->cycle_atoms_[0]->GetResidue()->GetId()
                                        << " This branch is the same length as the shortest";
                                gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                                testLog.str(std::string()); // clear stringstream
                            }
                            linkageStringVector.push_back(thisMono->mono_neighbors_[i].first->linkage_type_);
                        ...
                        { // Has already been considered
                            ...
                            {
                                testLog << thisMono->mono_neighbors_[i].second->cycle_atoms_[0]->GetResidue()->GetId()
                                        << "This branch is indexed";
                                gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                                testLog.str(std::string()); // clear stringstream
                            }
                            linkageStringVector.push_back(
                                "-999"); // no linkage starts with -999; will be lower than all others and we traverse
                                         // highest -> lowest linkage
                        }
                ...
                //   lowestIndexLocation = std::distance(linkageStringVector.begin(),
                //   std::min_element(linkageStringVector.begin(), linkageStringVector.end()));
                // }
                linkageStringVector[highestIndexLocation] = "-999";
                for (unsigned int i = 0; i < linkageStringVector.size(); i++)
                {
                    int nextHighestIndexLocation =
                        std::distance(linkageStringVector.begin(),
                                      std::max_element(linkageStringVector.begin(), linkageStringVector.end()));
                    if ((!thisMono->mono_neighbors_[nextHighestIndexLocation].second->is_indexed_) &&
                        (linkageStringVector[nextHighestIndexLocation] !=
                         "-999")) // if isnt previous mono and not already considered
                    {
                        this->number_branches_++;
                        thisMono->mono_neighbors_[nextHighestIndexLocation].second->oligo_branch_index_ =
                            this->number_branches_;
...
void Glycan::Oligosaccharide::indexBranches(std::vector<Glycan::Monosaccharide*> branchedMonos)
{
    ...
    for (std::vector<Glycan::Monosaccharide*>::iterator mono = branchedMonos.begin(); mono != branchedMonos.end();
         mono++)
    {
        Glycan::Monosaccharide* thisMono = *mono;
...
std::vector<Glycan::Monosaccharide*>
Glycan::Oligosaccharide::IUPACindexMono(Glycan::Monosaccharide* thisMono, int thisIndex,
                                        std::vector<Glycan::Monosaccharide*>& branchedMonos)
{
    int local_debug = -1;
    std::stringstream testLog;
    ...
    if (!thisMono->is_indexed_)
    {
        ...
        else if (thisMono->mono_neighbors_.size() == 2)
        {
            ...
            if (thisMono->is_root_) // BRANCHED root
            {
                ...
                if (numEqualNeighbors == 0)
                {
                    unsigned int longestBranchLocation = std::distance(
                        branchMaxLengths.begin(), std::max_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                    unsigned int otherBranchLocation;
                    // only 2 neighbors so no need for loops
                    ...
                    // Longest branch is main chain
                    IUPACindexMono(thisMono->mono_neighbors_[longestBranchLocation].second, thisIndex + 1,
                                   branchedMonos);
                }
                else
                { // The 2 branches are the same length so we need to order by linkage
                    ...
                    // index lower linkage locant
                    // go to next linkage
                    unsigned int lowestIndexLocation =
                        std::distance(linkageStringVector.begin(),
                                      std::min_element(linkageStringVector.begin(), linkageStringVector.end()));
                    unsigned int otherIndexLocation;
        ...
        else // more than 2 neighbors so branched
        {
            ...
            if (numEqualNeighbors == 0)
            {
                ...
                int longestBranchLocation = std::distance(
                    branchMaxLengths.begin(), std::max_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                ...
                { // if is previous mono, clear and find next longest branch
                    ...
                    longestBranchLocation                   = std::distance(
                        branchMaxLengths.begin(), std::max_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                }
                branchMaxLengths[longestBranchLocation] =
                    -1; // Already have the location and need to reset the max to the next highest for proper ordering
                for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                {
                    int nextLongestBranchLocation = std::distance(
                        branchMaxLengths.begin(), std::max_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                    if (local_debug > 0)
                    {
                    ...
                    if ((branchMaxLengths[nextLongestBranchLocation] != -1) &&
                        (!thisMono->mono_neighbors_[nextLongestBranchLocation].second->is_indexed_))
                    {
                        if (local_debug > 0)
                        {
                            testLog << thisMono->mono_neighbors_[nextLongestBranchLocation]
                                           .second->cycle_atoms_[0]
                                           ->GetResidue()
                                           ->GetId();
                            gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                        ...
                        this->number_branches_++;
                        thisMono->mono_neighbors_[nextLongestBranchLocation].second->oligo_branch_index_ =
                            this->number_branches_;
            ...
            else
            {
                ...
                if ((std::count(equalNeighborsLengths.begin(), equalNeighborsLengths.end(),
                    ...
                    (branchMaxLengths[longestBranchLocation] !=
                     -1)) // no other branches of this length and this hasn't been considered
                {
                    branchMaxLengths[longestBranchLocation] = -1; // Have location and need to get next longest
                    for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                    {
                        int nextLongestBranchLocation =
                            std::distance(branchMaxLengths.begin(),
                                          std::max_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                        if ((branchMaxLengths[nextLongestBranchLocation] != -1) &&
                            (!thisMono->mono_neighbors_[nextLongestBranchLocation].second->is_indexed_))
                        {
                            if (local_debug > 0)
                ...
                { // equal branches, but not already considered
                    ...
                    for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                    {
                        ...
                        { // This branch is not yet considered and is equal length
                            ...
                            {
                                testLog << thisMono->mono_neighbors_[i].second->cycle_atoms_[0]->GetResidue()->GetId()
                                        << " This branch is the same length as the longest";
                                gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                                testLog.str(std::string()); // clear stringstream
                            }
                        ...
                        { // Already considered
                            ...
                            linkageStringVector.push_back("999"); // no linkage starts with 999; will be higher than all
                                                                  // others and we traverse lowest to highest linkage
                        }
                ...
                int lowestIndexLocation =
                    std::distance(linkageStringVector.begin(),
                                  std::min_element(linkageStringVector.begin(), linkageStringVector.end()));
                linkageStringVector[lowestIndexLocation] = "999";
                for (unsigned int i = 0; i < linkageStringVector.size(); i++)
                {
                    int nextLowestIndexLocation =
                        std::distance(linkageStringVector.begin(),
                                      std::min_element(linkageStringVector.begin(), linkageStringVector.end()));
                    if ((!thisMono->mono_neighbors_[nextLowestIndexLocation].second->is_indexed_) &&
                        (linkageStringVector[nextLowestIndexLocation] !=
                         "999")) // if isnt previous mono and not considered
                    {
                        this->number_branches_++;
                        thisMono->mono_neighbors_[nextLowestIndexLocation].second->oligo_branch_index_ =
                            this->number_branches_;
...
void Glycan::Oligosaccharide::IUPACindexBranches(std::vector<Glycan::Monosaccharide*> branchedMonos)
{
         ...
         mono++)
    {
        ...
        if (subBranchedMonos.size() > 0)
        {
            ...
            {
                std::stringstream testLog;
                testLog << "IUPACindexBranches Branch: " << (*mono)->cycle_atoms_[0]->GetResidue()->GetId() << " has "
                        << subBranchedMonos.size() << " subbranches";
            }
...
void Glycan::Oligosaccharide::traverseGraph(Glycan::Monosaccharide* thisMono, Glycan::Oligosaccharide* thisOligo)
{
    ...
    if (local_debug > 0)
    {
        testLog << thisMono->cycle_atoms_[0]->GetResidue()->GetId() << " has " << thisMono->mono_neighbors_.size()
                << " neighbors.";
        gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
        testLog.str(std::string()); // clear stringstream
    }
    ...
    if (thisMono->is_root_)
    {
        ...
        if (anomeric_o == NULL)
        {
            ...
            if (thisMono->anomeric_carbon_pointer_ != NULL)
            {
                std::vector<MolecularModeling::Atom*> neighbors =
                    thisMono->anomeric_carbon_pointer_->GetNode()->GetNodeNeighbors();
                if (neighbors.size() < 2)
                {
                ...
                { // 2 neighbors in ring and 1 not
                    for (std::vector<MolecularModeling::Atom*>::iterator it = neighbors.begin(); it != neighbors.end();
                         it++)
                    {
                        MolecularModeling::Atom* thisNeighbor = (*it);
                        if (thisMono->cycle_atoms_str_.find(thisNeighbor->GetId()) == std::string::npos)
                ...
                else if (neighbors.size() == 4)
                {
                    ...
                    {
                        for (std::vector<MolecularModeling::Atom*>::iterator it = neighbors.begin();
                             it != neighbors.end(); it++)
                        {
                            MolecularModeling::Atom* thisNeighbor = (*it);
                            if ((thisMono->cycle_atoms_str_.find(thisNeighbor->GetId()) == std::string::npos) &&
                                (thisMono->anomeric_carbon_pointer_->GetResidue()->GetId() !=
        ...
        if (anomeric_o != NULL)
        {
            ...
            if (local_debug > 0)
            {
                testLog << "Terminal for " << thisMono->cycle_atoms_[0]->GetResidue()->GetId() << " is " << terminal;
                gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
                testLog.str(std::string()); // clear stringstream
            }
            if (terminal == " Unknown")
        ...
        if (terminal == " Unknown")
        {
            if (thisMono->anomeric_carbon_pointer_ != NULL)
            {
                ...
                for (std::vector<std::string>::iterator it = thisMono->chemical_code_->right_down_.begin();
                     it != thisMono->chemical_code_->right_down_.end(); it++)
                {
                ...
                for (std::vector<std::string>::iterator it = thisMono->chemical_code_->right_up_.begin();
                     it != thisMono->chemical_code_->right_up_.end(); it++)
                {
                    if ((*it).find("a") != std::string::npos)
                    {
                        thisMono->unknown_derivatives_.push_back(std::make_pair("a", terminal));
                ...
                // thisMono->derivatives_map_.begin(); it != thisMono->derivatives_map_.end(); it++)
                // {
        ...
        if (thisMono->mono_neighbors_.size() == 0) // Just a monosaccharide
        {
            ...
            thisOligo->author_IUPAC_name_ = thisMonoAuthorName + terminal;
                 ...
                 (thisMono->mono_neighbors_[0].first->anomeric_anomeric_linkage_ == false))
        {
            if (local_debug > 0)
            {
    ...
    else if (thisMono->mono_neighbors_.size() == 1) // no branching
    {
        ...
        if (thisMono->is_root_)
        {
            ...
            if (!thisMono->mono_neighbors_[0].second->is_visited_) // temp fix to handle anomeric anomeric infinite loop
            {
                ...
                thisOligo->IUPAC_name_ =
                    thisMono->mono_neighbors_[0].first->linkage_type_ + thisMonoName + thisOligo->IUPAC_name_;
                thisOligo->author_IUPAC_name_ = thisMono->mono_neighbors_[0].first->linkage_type_ + thisMonoAuthorName +
                                                thisOligo->author_IUPAC_name_;
                thisMono->is_visited_ = true;
    ...
    else if (thisMono->mono_neighbors_.size() == 2)
    {
            ...
            if (numEqualNeighbors == 0)
            {
                ...
                for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                {
                    ...
                    if (i < thisMono->mono_neighbors_.size() -
                                1) // no countedBackwards flag as there is nothing backwards from root
                    {
                        thisOligo->IUPAC_name_ =
                            thisMono->mono_neighbors_[shortestBranchLocation].first->linkage_type_ + "]" +
                            thisOligo->IUPAC_name_;
                        thisOligo->author_IUPAC_name_ =
                            thisMono->mono_neighbors_[shortestBranchLocation].first->linkage_type_ + "]" +
                            thisOligo->author_IUPAC_name_;
            ...
            else
            {
                ...
                for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
                {
                    shortestBranchLocation = std::distance(
                        branchMaxLengths.begin(), std::min_element(branchMaxLengths.begin(), branchMaxLengths.end()));
                    if (std::count(equalNeighborsLengths.begin(), equalNeighborsLengths.end(),
                                   branchMaxLengths[shortestBranchLocation]) == 0) // no other branches of this length
                    {
                        linkageStringVector.push_back("0");
                        if ((i < thisMono->mono_neighbors_.size() - 1) &&
                            (!thisMono->mono_neighbors_[shortestBranchLocation].second->is_visited_))
                        {
                            thisOligo->IUPAC_name_ =
                                thisMono->mono_neighbors_[shortestBranchLocation].first->linkage_type_ + "]" +
                                thisOligo->IUPAC_name_;
                        ...
                        branchMaxLengths[shortestBranchLocation] =
                            9999; // arbitralily large number so it moves to the next shortest branch
                    }
                    else
                    {
                        int equalBranchLength = branchMaxLengths[shortestBranchLocation];
                        if ((branchMaxLengths[i] == equalBranchLength) &&
                            (!thisMono->mono_neighbors_[i].second->is_visited_))
                        {
                            if (local_debug > 0)
                            {
                                gmml::log(__LINE__, __FILE__, gmml::INF,
                                          thisMono->mono_neighbors_[i].second->cycle_atoms_[0]->GetResidue()->GetId());
                            }

                            linkageStringVector.push_back(thisMono->mono_neighbors_[i].first->linkage_type_);
                            // equalBranches.push_back(thisMono->mono_neighbors_[i]);
                        }
                        else if ((branchMaxLengths[i] == equalBranchLength) &&
                                 (thisMono->mono_neighbors_[i].second->is_visited_))
                        {
                            if (local_debug > 0)
                            {
                ...
                for (unsigned int i = 0; i < linkageStringVector.size(); i++)
                {
                    ...
                    if (!thisMono->mono_neighbors_[highestIndexLocation].second->is_visited_)
                    {
                        ...
                        {
                            thisOligo->IUPAC_name_ =
                                thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ + "]" +
                                thisOligo->IUPAC_name_;
                            thisOligo->author_IUPAC_name_ =
                                thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ + "]" +
                                thisOligo->author_IUPAC_name_;
                        ...
                        else
                        {
                            ...
                            thisOligo
                                ->IUPAC_name_ = /*thisMono->mono_neighbors_[highestIndexLocation].second->sugar_name_.monosaccharide_short_name_
                                                   +*/
                                thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ +
                                thisOligo->IUPAC_name_;
                            thisOligo
                                ->author_IUPAC_name_ = /*thisMono->mono_neighbors_[highestIndexLocation].second->sugar_name_.monosaccharide_short_name_
                                                          +*/
                                thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ +
                                thisOligo->author_IUPAC_name_;
                        }
                        ...
                        linkageStringVector[highestIndexLocation] =
                            "0"; // small number string so it moves to the next higest linkage
                    }
                }
            }
        }
        else // not root with 2 neighbors
        {
            ...
            {
                thisOligo->IUPAC_name_ =
                    thisMono->mono_neighbors_[neighborNum].first->linkage_type_ + thisMonoName + thisOligo->IUPAC_name_;
                thisOligo->author_IUPAC_name_ = thisMono->mono_neighbors_[neighborNum].first->linkage_type_ +
            ...
            for (unsigned int i = 0; i < thisMono->mono_neighbors_.size(); i++)
            {
                ...
                else
                {
                    ...
                    for (unsigned int j = 0; j < equalLinkageStringVector.size(); j++)
                    {
                        ...
                        int highestIndexLocation = std::distance(
                            equalLinkageStringVector.begin(),
                            std::max_element(equalLinkageStringVector.begin(), equalLinkageStringVector.end()));
                        if (local_debug > 0)
                        {
                        ...
                        equalLinkageStringVector[highestIndexLocation] =
                            "0"; // small number string so it moves to the next lowest linkage
                        i++;
                ...
                //   int highestIndexLocation = std::distance(linkageStringVector.begin(),
                //   std::max_element(linkageStringVector.begin(), linkageStringVector.end()));
                //   if(!thisMono->mono_neighbors_[highestIndexLocation].second->is_visited_)
                //   {
                //     if((i < thisMono->mono_neighbors_.size() - 1) && (countedBackwards))
                //     {
                //       // gmml::log(__LINE__, __FILE__,  gmml::INF, "Counted Backwards");
                //       thisOligo->IUPAC_name_ = thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ +
                //       "]" +
                //                                 thisOligo->IUPAC_name_;
                //       thisOligo->author_IUPAC_name_ =
                //       thisMono->mono_neighbors_[highestIndexLocation].first->linkage_type_ + "]" +
                ...
                //     countedBackwards = true;
                //   }
                //   linkageStringVector[highestIndexLocation] = "0"; //small number string so it moves to the next
                //   lowest linkage
            }
        }
    }
...
void Glycan::Oligosaccharide::getBranchMaxLengths(Glycan::Monosaccharide* this_mono, std::vector<int>& branchLengths)
{
    ...
    {
        std::stringstream testLog;
        testLog << this_mono->cycle_atoms_[0]->GetResidue()->GetId() << "Has "
                << std::to_string(this_mono->mono_neighbors_.size()) << " neighbors.";
        gmml::log(__LINE__, __FILE__, gmml::INF, testLog.str());
    }
    for (std::vector<std::pair<Glycan::GlycosidicLinkage*, Glycan::Monosaccharide*>>::iterator monoNeighbor =
             this_mono->mono_neighbors_.begin();
         monoNeighbor != this_mono->mono_neighbors_.end(); monoNeighbor++)
    {
        ...
        if (!this_mono_neighbor->is_visited_)
        {
            if (!this_mono_neighbor->is_counted_)
            {
                ...
                {
                    gmml::log(__LINE__, __FILE__, gmml::INF,
                              this_mono_neighbor->cycle_atoms_[0]->GetResidue()->GetId());
                }
                if (this_mono_neighbor->mono_neighbors_.size() > 1)
                {
                    while (
                        this_mono_neighbor->mono_neighbors_.size() >
                        1) // At least one more mono on the branch (if only 1, it's the previous mono so stop counting)
                    {
                        unsigned int countedNeighbors = 0;
                        if (local_debug > 0)
                        {
                            gmml::log(__LINE__, __FILE__, gmml::INF,
                                      std::to_string(this_mono_neighbor->mono_neighbors_.size()));
                        }
                        if (this_mono_neighbor->mono_neighbors_.size() == 2) // Not branched
                        {
                            ...
                            for (std::vector<std::pair<Glycan::GlycosidicLinkage*, Glycan::Monosaccharide*>>::iterator
                                     monoNeighborNeighbor = this_mono_neighbor->mono_neighbors_.begin();
                                 monoNeighborNeighbor != this_mono_neighbor->mono_neighbors_.end();
                                 monoNeighborNeighbor++)
                            {
                                std::pair<Glycan::GlycosidicLinkage*, Glycan::Monosaccharide*>
                                    monoNeighborNeighborPair                   = *monoNeighborNeighbor;
                                Glycan::Monosaccharide* this_neighbor_neighbor = monoNeighborNeighborPair.second;
                                if (local_debug > 0)
                                {
                                    std::stringstream ss;
                                    ss << this_mono_neighbor->cycle_atoms_[0]->GetResidue()->GetId() << " "
                                       << this_neighbor_neighbor->cycle_atoms_[0]->GetResidue()->GetId();
                                    gmml::log(__LINE__, __FILE__, gmml::INF, ss.str());
                                }
                                if (!this_neighbor_neighbor->is_counted_) // if it isn't the previous mono
                                {
                                    ...
                                    {
                                        std::stringstream ss;
                                        ss << this_mono_neighbor->sugar_name_.monosaccharide_short_name_ << " "
                                           << this_neighbor_neighbor->sugar_name_.monosaccharide_short_name_;
                                        gmml::log(__LINE__, __FILE__, gmml::INF, ss.str());
                                    }
                                    ...
                                    {
                                        std::stringstream ss;
                                        ss << "Branch " << branchIndex << " has " << branchLengths[branchIndex] + 1
                                           << " neighbors";
                                        gmml::log(__LINE__, __FILE__, gmml::INF, ss.str());
                                    }
                                    break;
                        ...
                        {
                            std::vector<int> subBranchLengths(
                                this_mono_neighbor->mono_neighbors_.size(),
                                1); // an array the size of the number of branches, with initial lengths as 1
                            if (local_debug > 0)
                            {
                                gmml::log(__LINE__, __FILE__, gmml::INF, " ");
                            }
                            ...
                            branchLengths[branchIndex] =
                                branchLengths[branchIndex] +
                                *std::max_element(subBranchLengths.begin(), subBranchLengths.end());
                            break;
                        }
                    }
                }
...
{
    int numR = 0;
    for (std::vector<Glycan::Monosaccharide*>::reverse_iterator rit = this_Oligo->mono_nodes_.rbegin();
         rit != this_Oligo->mono_nodes_.rend(); rit++)
    {
        ...
        if (thisMono->on_R_ > 0) // There is an R group
        {
            ...
            std::string logstatement =
                thisMono->sugar_name_.monosaccharide_short_name_ + std::to_string(thisMono->on_R_);
...
std::string Glycan::Oligosaccharide::CheckOMETerminal(MolecularModeling::Atom* target,
                                                      std::vector<MolecularModeling::Atom*>& terminal_atoms)
{
...
std::string Glycan::Oligosaccharide::CheckROHTerminal(MolecularModeling::Atom* target,
                                                      std::vector<MolecularModeling::Atom*>& terminal_atoms)
{
    terminal_atoms                                    = std::vector<MolecularModeling::Atom*>();
    std::vector<MolecularModeling::Atom*> o_neighbors = target->GetNode()->GetNodeNeighbors();
...
std::string Glycan::Oligosaccharide::CheckTBTTerminal(MolecularModeling::Atom* target,
                                                      std::vector<MolecularModeling::Atom*>& terminal_atoms)
{
    terminal_atoms                                = std::vector<MolecularModeling::Atom*>();
    std::vector<MolecularModeling::Atom*> atoms_1 = std::vector<MolecularModeling::Atom*>();
    ...
    if (C1 != NULL)
    {
        ...
        if (C1C1 != NULL && C1C2 != NULL && C1C3 != NULL)
        {
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = c1c1_neighbors.begin();
                 it != c1c1_neighbors.end(); it++)
            {
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = c1c2_neighbors.begin();
                 it != c1c2_neighbors.end(); it++)
            {
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = c1c3_neighbors.begin();
                 it != c1c3_neighbors.end(); it++)
            {
    ...
    if (C2 != NULL)
    {
        ...
        if (C2C1 != NULL && C2C2 != NULL && C2C3 != NULL)
        {
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = c2c1_neighbors.begin();
                 it != c2c1_neighbors.end(); it++)
            {
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = c2c2_neighbors.begin();
                 it != c2c2_neighbors.end(); it++)
            {
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = c2c3_neighbors.begin();
                 it != c2c3_neighbors.end(); it++)
            {
...
std::string Glycan::Oligosaccharide::CheckTerminals(MolecularModeling::Atom* target,
                                                    std::vector<MolecularModeling::Atom*>& terminal_atoms)
{
    int local_debug = -1;
    if (target != NULL)
    {
                                          ...
                                          // hydrogen.the sidechain connecting nitrogen contain 3 atoms
        {
            ...
            if ((o_neighbors.at(1)->GetResidue()->CheckIfProtein() == true))
            {
            ...
            if (target_o_neighbor != NULL)
            {
                ...
                for (std::vector<MolecularModeling::Residue*>::iterator it = residues.begin(); it != residues.end();
                     it++)
                {
    ...
    else
    {
        if (local_debug > 0)
        {
            gmml::log(__LINE__, __FILE__, gmml::ERR, "Target is NULL");
        }
        return "Unknown";
    }
}

void Glycan::Oligosaccharide::CheckLinkageNote(Glycan::Monosaccharide* mono1, Glycan::Monosaccharide* mono2,
                                               std::string linkage, std::vector<std::string>& checked_linkages)
{
...
void Glycan::Oligosaccharide::BuildOligosaccharideTreeStructure(
...
void Glycan::Oligosaccharide::SetGlycosylationBools()
{
    ...
    for (AtomVector::iterator it = anomeric_C_neighbors.begin(); it != anomeric_C_neighbors.end(); it++)
    {
        ...
        if (local_debug > 0)
        {
            ...
            {
                gmml::log(__LINE__, __FILE__, gmml::INF,
                          "Neighbor of anomeric C, " + neighbor->GetElementSymbol() + ", is in ring");
            }
            else
            {
                gmml::log(__LINE__, __FILE__, gmml::INF,
                          "Neighbor of anomeric C, " + neighbor->GetElementSymbol() + ", is not in ring");
            }
        }
        if (!neighbor->GetIsRing())
        {
    ...
    for (AtomVector::iterator it = glycosidic_O_neighbors.begin(); it != glycosidic_O_neighbors.end(); it++)
    {
        ...
        if (glycosidic_O_neighbor->GetResidue()->GetName() != anomeric_C_Residue)
        {
            ...
            std::string residue_name      = glycosidic_O_neighbor->GetResidue()->GetName();
            this->glycosylation_residue_  = residue_name;
            ...
            if (find(NGlycosylationResidues.begin(), NGlycosylationResidues.end(), residue_name) !=
                NGlycosylationResidues.end())
            {
            ...
            else if (find(OGlycosylationResidues.begin(), OGlycosylationResidues.end(), residue_name) !=
                     OGlycosylationResidues.end())
            {
            ...
            else if (find(SGlycosylationResidues.begin(), SGlycosylationResidues.end(), residue_name) !=
                     SGlycosylationResidues.end())
            {
            ...
            else if (find(CGlycosylationResidues.begin(), CGlycosylationResidues.end(), residue_name) !=
                     CGlycosylationResidues.end())
            {
