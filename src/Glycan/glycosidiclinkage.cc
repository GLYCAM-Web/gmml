...
//                       CONSTRUCTOR                    //
//////////////////////////////////////////////////////////
GlycosidicLinkage::GlycosidicLinkage(Monosaccharide* sourceMono, Monosaccharide* targetMono,
                                     std::string source_carbon_ID, std::string target_carbon_ID)
{
    int local_debug = -1;
    ...
    */
    for (std::vector<MolecularModeling::Atom*>::iterator it = sourceMono->cycle_atoms_.begin();
         it != sourceMono->cycle_atoms_.end(); it++)
    {
    ...
    for (std::vector<MolecularModeling::Atom*>::iterator it = targetMono->cycle_atoms_.begin();
         it != targetMono->cycle_atoms_.end(); it++)
    {
        MolecularModeling::Atom* thisAtom = *it;
    ...
    if (!anomeric_anomeric_linkage_)
    {
        if (non_reducing_mono_ == sourceMono)
        {
            ...
            std::vector<MolecularModeling::Atom*> targetMonoAtoms =
                targetMono->cycle_atoms_[0]->GetResidue()->GetAtoms();
            for (std::vector<MolecularModeling::Atom*>::iterator it = targetMonoAtoms.begin();
                 it != targetMonoAtoms.end(); it++)
            {
        ...
        else if (non_reducing_mono_ == targetMono)
        {
            ...
            std::vector<MolecularModeling::Atom*> sourceMonoAtoms =
                sourceMono->cycle_atoms_[0]->GetResidue()->GetAtoms();
            for (std::vector<MolecularModeling::Atom*>::iterator it = sourceMonoAtoms.begin();
                 it != sourceMonoAtoms.end(); it++)
            {
                MolecularModeling::Atom* thisMonoAtom = (*it);
    ...
    */
    ...
    if (non_reducing_mono_ != NULL)
    {
        ...
        */
        std::vector<MolecularModeling::Atom*> anomericNeighbors =
            non_reducing_mono_carbon_->GetNode()->GetNodeNeighbors();
        for (std::vector<MolecularModeling::Atom*>::iterator it = anomericNeighbors.begin();
             it != anomericNeighbors.end(); it++)
        {
            ...
            if (reducing_mono_ != NULL)
            {
                    ...
                    (thisAnomericNeighbor->GetIsCycle() == false))
                {
                    ...
                    { // Print Warning for now
                        ss.str("");
                        ss << "Glycosidic Oxygen not present. The atom present is "
                           << thisAnomericNeighbor->GetElementSymbol() << ".";
            ...
            { // The oxygen can belong to either sugar
                if ((thisAnomericNeighbor->GetElementSymbol() == "O") && (thisAnomericNeighbor->GetIsCycle() == false))
                { // Then this is the glycosidic oxygen
                    linkageAtoms.push_back(thisAnomericNeighbor);
                    glycosidic_oxygen_ = thisAnomericNeighbor;
                    setGlycosidicO     = true;
            ...
            else
            {
                gmml::log(__LINE__, __FILE__, gmml::ERR,
                          "Cannot identify both sugars for the linkage, something went horribly wrong!");
            }
        }
        ...
        { // Get atoms for calculations and calculate
            ...
            */
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it = CxNeighbors.begin(); it != CxNeighbors.end();
                 it++)
            {
                ...
                { // This is Cx-1
                    ...
                    */
                    if ((linkage_type_ == "1-6") || (inverse_linkage_type_ == "1-6") || (linkage_type_ == "2-6") ||
                        (inverse_linkage_type_ == "2-6"))
                    { // Get Cx-2
                        std::vector<MolecularModeling::Atom*> CxMinusOneNeighbors =
                            thisCxNeighbor->GetNode()->GetNodeNeighbors();
                        for (std::vector<MolecularModeling::Atom*>::iterator it2 = CxMinusOneNeighbors.begin();
                             it2 != CxMinusOneNeighbors.end(); it2++)
                        {
                            MolecularModeling::Atom* thisCxMinusOneNeighbor = (*it2);
                            int thisCxMinusOneNeighborNum = std::atoi(&thisCxMinusOneNeighbor->GetId()[1]);
                            if ((thisCxMinusOneNeighborNum = (x - 2)) &&
                                (thisCxMinusOneNeighbor->GetElementSymbol() == "C"))
                            { // This is Cx-2
                                // Add Cx-2
        ...
        { // Anomeric-anomeric linkage uses different atoms
            ...
            */
            for (std::vector<MolecularModeling::Atom*>::iterator it = non_reducing_mono_2_->cycle_atoms_.begin();
                 it != non_reducing_mono_2_->cycle_atoms_.end(); it++)
            {
                MolecularModeling::Atom* thisAtom = (*it);
        ...
        */
        ...
        { // check ring conformation and write logs
            if (((non_reducing_mono_->bfmp_ring_conformation_.find("1C4") != std::string::npos) ||
                 (non_reducing_mono_->bfmp_ring_conformation_.find("4C1") != std::string::npos)) &&
                ((reducing_mono_->bfmp_ring_conformation_.find("1C4") != std::string::npos) ||
                 (reducing_mono_->bfmp_ring_conformation_.find("4C1") != std::string::npos)))
        ...
        if ((!anomeric_anomeric_linkage_) && (non_reducing_mono_->sugar_name_.ring_type_ == "P") &&
...
double Glycan::GlycosidicLinkage::CalculatePhiAngle(std::vector<MolecularModeling::Atom*> linkageAtoms)
{
    ...
    // for (std::vector<MolecularModeling::Atom*>::iterator it = glycosidicO_neighbors.begin(); it !=
    glycosidicO_neighbors.end(); it++)
    // {
    //   MolecularModeling::Atom* this_neighbor = (*it);
    //   if (local_debug > 0)
    //   {
...
double Glycan::GlycosidicLinkage::CalculateOmegaAngle(std::vector<MolecularModeling::Atom*> linkageAtoms)
{
    ...
    //     gmml::log(__LINE__, __FILE__,  gmml::INF, ss.str());
    //     ss.clear();
    //     ss << " | " << this_cycle_atom->GetElementSymbol() << "_" << this_atom_number << " Neighbor ID: " <<
    this_cycle_atom->GetId();
    //     gmml::log(__LINE__, __FILE__,  gmml::INF, ss.str());
    //     ss.clear();
    //   }
    ...
    // for (std::vector<MolecularModeling::Atom*>::iterator it = C5prime_neighbors.begin(); it !=
    C5prime_neighbors.end(); it++)
    // {
    //   MolecularModeling::Atom* this_neighbor = (*it);
    //   if (local_debug > 0)
    //   {
    ...
    // for (std::vector<MolecularModeling::Atom*>::iterator it = C6prime_neighbors.begin(); it !=
    C6prime_neighbors.end(); it++)
    // {
    //   MolecularModeling::Atom* this_neighbor = (*it);
    //   if (local_debug > 0)
    //   {
    ...
    */
    ...
    Omega =
        GeometryTopology::CalculateDihedralAngle(linkageAtoms[2]->GetCoordinate(), linkageAtoms[3]->GetCoordinate(),
...
double Glycan::GlycosidicLinkage::CalculatePhiChiEnergy()
{
    ...
    */
    ...
    /*L-sugars note from paper
      ...
      relevant energy curve."
        ...
        According to Rob: "It only applies to the Phi angle, and so only applies to the non-reducing sugar in the
      linkage."
    */
    ...
    */
    ...
    { // Isomer unknown
        gmml::log(__LINE__, __FILE__, gmml::ERR,
                  "Unable to determine the isomer (D/L) of the non-reducing monosaccharide involved in the linkage.  "
                  "Exiting CalculatePhiChiEnergy()");
        return 0;
    }
    ...
    */
    ...
    std::list<std::string> ChiLinkTypes = {"1-1", "1-2", "1-3", "1-4", "1-6"};
    if ((find(ChiLinkTypes.begin(), ChiLinkTypes.end(), linkage_type_) != ChiLinkTypes.end()) ||
        (find(ChiLinkTypes.begin(), ChiLinkTypes.end(), inverse_linkage_type_) != ChiLinkTypes.end()))
    {
        ...
        {
            gmml::log(__LINE__, __FILE__, gmml::ERR,
                      "Unable to determine the configuration (α/β) of the linkage. Exiting CalculatePhiChiEnergy()");
            return 0;
        }
    }
    ...
    if (phi_alpha)
    {
        double LH = 2.97696467271672, Lc = -199.494365163839, LW = 677.808323900125;
        double RH = 102.253303636096, Rc = 170.599580473404, RW = 1696.7844369942;
        double aH = 10.7448005875571, ac = -105.313553566706, aW = 4724.58364072706;
        double bH = 3.67344580413578, bc = 6.20116671874232, bW = 1347.7205625156;
        double cH = 2.06094652655659, cc = 91.6553021324274, cW = 1500.02002601097;
        double dH = 6.19388683252667, dc = -22.9786969888816, dW = 2122.27783139301;
        double eH = -2.11153017593601, ec = 83.6019123356148, eW = 1254.13371108961;
        double fH = -98.0013005657107, fc = 170.012289132741, fW = 1598.73272567307;
    ...
    {
        double LH = 450.540038600828, Lc = -330.769995527134, LW = 4449.7622241787;
        double RH = 23.7118506901333, Rc = 304.624980492529, RW = 8375.1929028027;
        double aH = 5.93533323829663, ac = -152.080139620062, aW = 6049.77220005964;
        double bH = 22.467372096061, bc = -23.5159916173247, bW = 606.89715970453;
        double cH = 10.0360057033439, cc = 120.962836525241, cW = 4037.89330459304;
        double dH = -18.1406478565247, dc = -24.2677756921736, dW = 543.050986049266;
        double eH = 5.88226333077368, ec = 19.6321032903376, eW = 897.92664572344;
...
double Glycan::GlycosidicLinkage::CalculatePsiChiEnergy()
{
    ...
    */
    ...
    {
        double LH = 4.62366277694845, Lc = 5.045583934308, LW = 5005.75236060956;
        double RH = 4.61387450239844, Rc = 362.487847702007, RW = 2090.63190217702;
        double aH = 4.94191727813274, ac = 121.202321824468, aW = 2093.75214491931;
        double bH = 0.402901504292045, bc = 241.428583877882, bW = 456.828754790442;
        double cH = 0.798876573705798, cc = 68.425080241155, cW = 678.807178379645;
        double dH = 0.222992242737354, dc = 192.925748017071, dW = 347.244734136509;
    ...
    {
        double LH = 4.46811874171788, Lc = 1e-30, LW = 1279.58772056199;
        double RH = 4.38204018882823, Rc = 357.770654336205, RW = 6050.14162479438;
        double aH = 284.944948778136, ac = 146.644068129462, aW = 1551.75673776163;
        double bH = 4.76134025362478, bc = 220.683118921686, bW = 5892.94143218231;
        double cH = -169.197666368856, cc = 147.370828680332, cW = 1742.47541063603;
        double dH = -118.440552792375, dc = 146.05660843428, dW = 1359.82873591396;
    ...
    {
        double aH = 67.9431348410598, ac = -59.5393395706705, aW = 993.323581145538;
        double bH = 6.13421142432396, bc = 10.4786088782815, bW = 945.770771330812;
        double cH = 3.27628727235978, cc = 54.2960678151208, cW = 851.528141801851;
        double dH = 0.727486729062442, dc = 131.067737803489, dW = 1037.41211378392;
        double eH = 2.57362265878937, ec = 245.102891425541, eW = 2012.99451568206;
        double fH = 5.75995973448166, fc = 359.999988549478, fW = 1153.3974275673;
        double gH = 3.47492643928157, gc = 321.677942414686, gW = 2080.97053159226;
        double hH = -0.741000462200939, hc = 199.106903524814, hW = 522.180434119001;
    ...
    {
        double aH = 7.24858655753829, ac = 3.60600554520403, aW = 2459.23916629141;
        double bH = 1.9, bc = 96.5930821702371, bW = 2683.88656516991;
        double cH = 0.741022592342903, cc = 141.663521919709, cW = 1150.04756181103;
        double dH = 0.2, dc = 162, dW = 400;
        double eH = 0.287090039932611, ec = 228.171314273305, eW = 272.201363844744;
        double fH = 1.22591971967808, fc = 292.206221787048, fW = 1134.52455512381;
        double gH = 7.41063235334191, gc = 369.867701147817, gW = 3499.15994772992;
        double hH = -0.61489499584011, hc = 271.319024293053, hW = 532.437194483944;
        double iH = -0.35, ic = 183, iW = 100;
...
int Glycan::GlycosidicLinkage::pickPsiChiEnergyFunction()
{
    ...
    */
    ...
    {
        std::list<std::string> ChiLinkTypes = {"1-2", "1-3", "1-4", "1-6", "2-3", "2-6"};
        if (find(ChiLinkTypes.begin(), ChiLinkTypes.end(), linkage_type_) != ChiLinkTypes.end())
        {
        ...
        { // something is wrong with the numbering or weird with the ring
            ...
            { // atom not in ring and numbered wrong
                ...
                */
                ...
                std::vector<MolecularModeling::Atom*> neighborAtoms =
                    reducing_mono_carbon_->GetNode()->GetNodeNeighbors();
                for (std::vector<MolecularModeling::Atom*>::iterator it = neighborAtoms.begin();
                     it != neighborAtoms.end(); it++)
                {
                    ...
                    { // go to neighbor's neighbors
                        std::vector<MolecularModeling::Atom*> neighbor2Atoms =
                            thisNeighbor->GetNode()->GetNodeNeighbors();
                        for (std::vector<MolecularModeling::Atom*>::iterator it = neighbor2Atoms.begin();
                             it != neighbor2Atoms.end(); it++)
                        {
                            ...
                            {
                                std::vector<MolecularModeling::Atom*> neighbor3Atoms =
                                    thisN2neighbor->GetNode()->GetNodeNeighbors();
                                for (std::vector<MolecularModeling::Atom*>::iterator it = neighbor3Atoms.begin();
                                     it != neighbor3Atoms.end(); it++)
                                {
                                    MolecularModeling::Atom* thisN3neighbor = (*it);
                                    if (thisN3neighbor->GetResidue()->GetId() !=
                                        reducing_mono_carbon_->GetResidue()->GetId())
                                    { // skip
                                        continue;
                                    }
                                    if (reducing_mono_->cycle_atoms_str_.find(thisN3neighbor->GetId()) !=
                                        std::string::npos)
                                    { // This neighbor's neighbor's neighbor (Ugh I know) is in the ring
                                        otherCarbonNum = 8;
                                        if (reducing_mono_->sugar_name_.chemical_code_string_.find("-1") !=
                                            std::string::npos)
                                        {
                                            otherCarbonNum++;
                                        }
                                    ...
                                    { // In case of C9
                                        std::vector<MolecularModeling::Atom*> neighbor3Atoms =
                                            thisN2neighbor->GetNode()->GetNodeNeighbors();
                                        for (std::vector<MolecularModeling::Atom*>::iterator it =
                                                 neighbor3Atoms.begin();
                                             it != neighbor3Atoms.end(); it++)
                                        {
                                            MolecularModeling::Atom* thisN3neighbor = (*it);
                                            if (thisN3neighbor->GetResidue()->GetId() !=
            ...
            else
            {
                ...
                for (std::vector<MolecularModeling::Atom*>::iterator it = reducing_mono_->cycle_atoms_.begin();
                     it != reducing_mono_->cycle_atoms_.end() - 1; it++)
