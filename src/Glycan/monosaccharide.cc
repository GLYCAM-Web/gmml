...
Monosaccharide::Monosaccharide(std::string* cycle_atoms_str, std::vector<MolecularModeling::Atom*>& cycle_atoms,
                               MolecularModeling::Assembly* this_assembly, std::string CCD_Path)
{
    int local_debug = -1;
    std::stringstream ss;
    ...
    anomeric_carbon_pointer_ =
        FindAnomericCarbon(anomeric_note, anomeric_carbons_status, cycle_atoms, *cycle_atoms_str);
    ...
    */
    ...
    std::vector<std::string> orientations =
        GetSideGroupOrientations(this_assembly); // This also sets the first side atom for each ring position
    ...
    int num_atoms        = 0;
    for (std::vector<std::vector<MolecularModeling::Atom*>>::iterator it1 = side_atoms_.begin();
         it1 != side_atoms_.end(); it1++)
    {
    ...
    { // No match in lookup table, check if its from Deoxy groups
        ...
        {
            if (((new_code.right_up_[i].substr(0, 1) == "+") || (new_code.right_up_[i].substr(0, 1) == "-")) &&
        ...
        {
            if (((new_code.right_down_[i].substr(0, 1) == "+") || (new_code.right_down_[i].substr(0, 1) == "-")) &&
        ...
        // for(std::vector<std::string>::iterator it = chemical_code_->left_down_.begin(); it !=
        // chemical_code_->left_down_.end(); it++)
        // {
        //     std::cout << "left down: " << (*it) << "\n";
        // }
        // for(std::vector<std::string>::iterator it = chemical_code_->left_up_.begin(); it !=
        // chemical_code_->left_up_.end(); it++)
        // {
        //     std::cout << "left up: " << (*it) << "\n";
        // }
        // for(std::vector<std::string>::iterator it = chemical_code_->right_down_.begin(); it !=
        // chemical_code_->right_down_.end(); it++)
        // {
        //     std::cout << "right down: " << (*it) << "\n";
        // }
        // for(std::vector<std::string>::iterator it = chemical_code_->right_up_.begin(); it !=
        // chemical_code_->right_up_.end(); it++)
        // {
        ...
        if ((updated_name.monosaccharide_name_ != "") &&
            (updated_name.monosaccharide_name_ != sugar_name_.monosaccharide_name_))
        {
        ...
        // this will create the correct name for the sugar, but the chemical code will not match, and will be incorrect,
        // as it will have ^n at the deoxy locations.
        //  sugar_name_ = base_name;
    }
    if ((sugar_name_.monosaccharide_stereochemistry_name_.compare("") != 0) &&
        (sugar_name_.monosaccharide_name_.compare("") == 0))
    {
    ...
    if (pdb_codes.size() > 0)
    {
        ...
        if (!found_code)
        {
            ...
            else
            {
                ...
                res_ss << "Residue name, " << original_residue << " (" << original_residue_id
                       << "), in input PDB file for " << sugar_name_.monosaccharide_short_name_
                       << " does not match GlyFinder residue code: " << sugar_name_.pdb_code_;
            }
...
{ // Added for the PDB to create their own SNFGs
    ...
    if (monoSNFGName.size() > 0)
    {
        ...
        // if(((monoSNFGName[monoSNFGName.length()-1] == sugar_name_.configuration_[0]) &&
        // (monoSNFGName[monoSNFGName.length()-2] != 'N'))
        //   ||
        //   || (monoSNFGName[monoSNFGName.length()-1] == 'x'))
        // {
        //   monoSNFGName.erase(monoSNFGName.length()-1,1);
        // }
        if ((monoSNFGName[monoSNFGName.length() - 1] == 'x') || (monoSNFGName[monoSNFGName.length() - 1] == 'a') ||
...
{ // Added for the PDB to create their own SNFGs
    ...
    if (monoSNFGName.size() > 0)
    {
        ...
        if (((monoSNFGName[monoSNFGName.length() - 1] == author_sugar_name_.configuration_[0]) &&
             (monoSNFGName[monoSNFGName.length() - 2] != 'N')) ||
            (monoSNFGName[monoSNFGName.length() - 1] == 'x'))
        {
            monoSNFGName.erase(monoSNFGName.length() - 1, 1);
...
MolecularModeling::Atom* Glycan::Monosaccharide::FindAnomericCarbon(Glycan::Note* anomeric_note,
                                                                    std::vector<std::string>& anomeric_carbons_status,
                                                                    std::vector<MolecularModeling::Atom*> cycle,
                                                                    std::string cycle_atoms_str)
{
    ...
    for (std::vector<MolecularModeling::Atom*>::iterator it = cycle.begin(); it != cycle.end(); it++)
    {
        ...
        if ((cycle_atom->GetElementSymbol() == "O"))
        { /// find oxygen in ring
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it1 = o_neighbor1_neighbors.begin();
                 it1 != o_neighbor1_neighbors.end(); it1++)
            { /// check if neighbor1 of oxygen has another oxygen or nitrogen neighbor
                MolecularModeling::Atom* o_neighbor1_neighbor = (*it1);

                if (cycle_atoms_str.find(o_neighbor1_neighbor->GetId()) ==
                        std::string::npos /// if the neighbor is not one of the cycle atoms
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it2 = o_neighbor2_neighbors.begin();
                 it2 != o_neighbor2_neighbors.end(); it2++)
            { /// check if neighbor2 of oxygen has another oxygen or nitrogen neighbor
            ...
            /// Seems redundant to have this multiple times, if it is going to say the same thing and be generated for
            /// all cases after the first logic check. Specially if we ever want to change this Note to say something
            /// different. Like I am doing now. :)
            std::stringstream ss;
            ss << "Could not find glycosidic oxygen or nitrogen within " << gmml::maxCutOff
               << " Angstroms of a ring carbon bonded to the ring oxygen(" << node->GetAtom()->GetName() << ").";
            ...
            if (gmml::ConvertString<int>(ss1.str()) < gmml::ConvertString<int>(ss2.str()))
            {
                ...
                anomeric_carbons_status.push_back("Anomeric carbon assigned based on its atom name index (" +
                                                  ss1.str() + "), Anomeric Carbon is: " + anomeric_carbon->GetName());
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it1 = o_neighbor1_neighbors.begin();
                 it1 != o_neighbor1_neighbors.end(); it1++)
            { /// check if neighbor1 of oxygen has non-ring carbon neighbor
                MolecularModeling::Atom* neighbor1_neighbor = (*it1);
                if (cycle_atoms_str.find(neighbor1_neighbor->GetId()) ==
                        std::string::npos /// if the neighbor is not one of the cycle atoms
                    && (neighbor1_neighbor->GetName().substr(0, 1).compare("C") == 0))
            ...
            for (std::vector<MolecularModeling::Atom*>::iterator it1 = o_neighbor2_neighbors.begin();
                 it1 != o_neighbor2_neighbors.end(); it1++)
            { /// check if neighbor1 of oxygen has non-ring carbon neighbor
                MolecularModeling::Atom* neighbor2_neighbor = (*it1);
                if (cycle_atoms_str.find(neighbor2_neighbor->GetId()) ==
                        std::string::npos /// if the neighbor is not one of the cycle atoms
                    && (neighbor2_neighbor->GetName().substr(0, 1).compare("C") == 0))
                { /// if first element is "C"
            ...
            if (!neighbor1_is_anomeric)
            {
                anomeric_note->type_     = Glycan::WARNING;
                anomeric_note->category_ = Glycan::ANOMERIC;
                anomeric_carbons_status.push_back(
                    "Anomeric carbon assigned to the ring carbon neighboring the ring oxygen but is not attached to an "
                    "exocyclic carbon (that is, assuming the monosaccharide is an aldose), Anomeric Carbon is: " +
                    anomeric_carbon->GetName());
                if (o_neighbor2->GetElementSymbol() != "H")
                {
                    return o_neighbor2;
                }
            ...
            // @TODO August 8, 2017 Davis/Lachele - Once we get to a point where we read in mmcif definitions, we need
            // to use it to determine the Anomeric Carbon.
            anomeric_note->type_     = Glycan::WARNING;
            anomeric_note->category_ = Glycan::ANOMERIC;
            anomeric_carbons_status.push_back("Anomeric carbon assigned to the first ring carbon (" +
                                              o_neighbor1->GetName() +
                                              "), Anomeric Carbon is: " + anomeric_carbon->GetName());
            if (o_neighbor2->GetElementSymbol() != "H")
            {
                return o_neighbor1;
            }
...
std::vector<std::string> Glycan::Monosaccharide::GetSideGroupOrientations(MolecularModeling::Assembly* this_assembly)
{
    ...
    // 9/14/18 Removed side atom initialization in this function and moved it to Yao's
    ...
    if (!cycle_atoms_.empty())
    {
        ...
        std::vector<std::vector<MolecularModeling::Atom*>> side_atoms =
            std::vector<std::vector<MolecularModeling::Atom*>>();
        std::vector<MolecularModeling::Atom*> default_atom_vector = std::vector<MolecularModeling::Atom*>(3, NULL);
        for (std::vector<MolecularModeling::Atom*>::iterator it = cycle_atoms_.begin(); it != cycle_atoms_.end() - 1;
             it++) /// iterate on cycle atoms except the oxygen in the ring
        {
            ...
            {
                prev_atom = cycle_atoms_.at(cycle_atoms_.size() -
                                            1); /// previous atom is the oxygen(last atom of the sorted cycle)
            }
            else
            {
            ...
            GeometryTopology::Coordinate prev_atom_coord =
                GeometryTopology::Coordinate(*prev_atom->GetCoordinates().at(model_index_));
            GeometryTopology::Coordinate current_atom_coord =
                GeometryTopology::Coordinate(*current_atom->GetCoordinates().at(model_index_));
            GeometryTopology::Coordinate next_atom_coord =
                GeometryTopology::Coordinate(*next_atom->GetCoordinates().at(model_index_));
            ...
            {
                for (std::vector<MolecularModeling::Atom*>::iterator it1 = neighbors.begin(); it1 != neighbors.end();
                     it1++)
                {
                    MolecularModeling::Atom* neighbor = (*it1);
                    std::string neighbor_id           = neighbor->GetId();
                    if (cycle_atoms_str_.find(neighbor_id) == std::string::npos) /// if not one of the cycle atoms
                    {
                        // if(neighbor->GetName().at(0) != 'H') ///deoxy check
                        //     not_h_neighbors++;
                        GeometryTopology::Coordinate side_atom_coord =
                            GeometryTopology::Coordinate(*neighbor->GetCoordinates().at(model_index_));
                        ...
                        if (index == 0 &&
                            neighbor->GetElementSymbol() ==
                                "C") /// if anomeric atom has a non-ring carbon neighbor that isn't a C-linked sugar
                        {
                            if (neighbor->GetResidue()->CheckIfProtein() == false)
                            {
                                ...
                                if (orientations.at(index).compare("N") ==
                                    0) /// if the position of non-ring oxygen or nitrogen hasn't been set yet
                                {
                        ...
                        // else if(neighbor_id.at(0) == 'O' || neighbor_id.at(0) == 'N')///if neighbor is a non-ring
                        // oxygen or nitrogen Yao: After talking to Dave. Replace search based on atom id to element
                        // symbol
                        else if (neighbor->GetElementSymbol() == "O" || neighbor->GetElementSymbol() == "N" ||
                                 neighbor->GetElementSymbol() == "S")
                        {
                            if (index == 0) /// current atom is anomeric
                            {
                                if (orientations.at(index).compare("N") ==
                                    0) /// if the position of non-ring carbon neighbor (if exist) hasn't been set yet
                                {
                                    if (theta > (gmml::PI_RADIAN / 2))
                                    {
                                        orientations.at(index)     = "D";
                        ...
                        else if (index == cycle_atoms_.size() - 2 &&
                                 neighbor->GetElementSymbol() ==
                                     "C") /// if the last ring carbon has a non-ring carbon neighbor
                        {
                            neighbor->SetIsExocyclicCarbon(true);

                            /// Check if neighbor of neighbor is oxygen or nitrogen
                            MolecularModeling::AtomNode* neighbor_node = neighbor->GetNode();
                            std::vector<MolecularModeling::Atom*> neighbors_of_neighbor =
                                neighbor_node->GetNodeNeighbors();
                            int o_neighbors     = 0;
                            int not_h_neighbors = 0;
                            for (std::vector<MolecularModeling::Atom*>::iterator it2 = neighbors_of_neighbor.begin();
                                 it2 != neighbors_of_neighbor.end(); it2++)
                            {
                                ...
                                if ((cycle_atoms_str_.find(neighbor_of_neighbor_id) == std::string::npos) &&
                                    //((neighbor->GetElementSymbol() == "O") || (neighbor->GetElementSymbol() == "N")))
                                    /////if neighbor of neighbor is a non-ring oxygen or nitrogen  Yao: should be
                                    //"neighbor_of_neighbor"
                                    ((neighbor_of_neighbor->GetElementSymbol() == "O") ||
                                     (neighbor_of_neighbor->GetElementSymbol() ==
                                      "N"))) /// if neighbor of neighbor is a non-ring oxygen or nitrogen  Yao:
                                             /// Correction
                                {
                                    o_neighbors++;
                                }
                                if (cycle_atoms_str_.find(neighbor_of_neighbor_id) == std::string::npos &&
                                    //(neighbor->GetElementSymbol() != "H"))///if neighbor of neighbor is any non-ring
                                    // atom other than hydrogen  Yao: should be "neighbor_of_neighbor"
                                    (neighbor_of_neighbor->GetElementSymbol() !=
                                     "H")) /// if neighbor of neighbor is any non-ring atom other than hydrogen
                                {
                                    not_h_neighbors++;
                                }
                            }
        ...
        // {
        //   for (std::vector<std::vector<MolecularModeling::Atom*>>::iterator
        //   sidestd::vector<MolecularModeling::Atom*>IT = side_atoms.begin();
        //   sidestd::vector<MolecularModeling::Atom*>IT != side_atoms.end();
        //   sidestd::vector<MolecularModeling::Atom*>IT ++)
        //   {
        //     std::vector<MolecularModeling::Atom*> sidestd::vector<MolecularModeling::Atom*> =
        //     (*sidestd::vector<MolecularModeling::Atom*>IT); for (std::vector<MolecularModeling::Atom*>::iterator
        //     sideAtomIT = sidestd::vector<MolecularModeling::Atom*>.begin(); sideAtomIT !=
        //     sidestd::vector<MolecularModeling::Atom*>.end(); sideAtomIT++)
        //     {
...
void Glycan::Monosaccharide::InitiateDetectionOfCompleteSideGroupAtoms()
{
    ...
    for (std::vector<std::vector<MolecularModeling::Atom*>>::iterator it = side_atoms_.begin(); it != side_atoms_.end();
         it++)
    {
        ...
        for (std::vector<MolecularModeling::Atom*>::iterator it2 = SideAtomArm.begin(); it2 != SideAtomArm.end(); it2++)
        {
            ...
            if (side_atom != NULL)
            {
                ...
                for (MolecularModeling::AtomVector::iterator atom_it = side_atom_neighbors.begin();
                     atom_it != side_atom_neighbors.end(); atom_it++)
                {
        ...
        for (std::vector<MolecularModeling::Atom*>::iterator it3 = all_plus_one_side_atoms.begin();
             it3 != all_plus_one_side_atoms.end(); it3++)
        {
            ...
            if (this->CheckIfPlusOneSideAtomBelongsToCurrentMonosaccharide(SideAtomArm, cycle_atoms_,
                                                                           plus_one_side_atom))
            {
...
bool Glycan::Monosaccharide::CheckIfPlusOneSideAtomBelongsToCurrentMonosaccharide(
    std::vector<MolecularModeling::Atom*>& SideAtomArm, std::vector<MolecularModeling::Atom*>& cycle_atoms,
    MolecularModeling::Atom* working_atom)
{
    ...
    for (std::vector<MolecularModeling::Atom*>::iterator it = working_node_neighbors.begin();
         it != working_node_neighbors.end(); it++)
    {
        if (*it != NULL)
        {
    ...
    if (attached_anomeric_carbons.size() == 1)
    {
        ...
        { // if the anomeric carbon is from the current monosaccharide
            ...
            {
                SideAtomArm.erase(
                    std::find(SideAtomArm.begin(), SideAtomArm.end(),
                              working_atom)); // Unless it's terminal, this oxygen belongs to the other monosaccharide.
                belongs_to_current_monosaccharide = false;
            }
        }
    }
    // If number of attached anomeric carbon is zero, then this atom belongs to current side chain, and it is already.
    // No need to do anything!
    return belongs_to_current_monosaccharide;
}

void Glycan::Monosaccharide::SetCompleteSideGroupAtoms(std::vector<MolecularModeling::Atom*>& SideAtomArm,
                                                       MolecularModeling::Atom* working_atom,
                                                       std::vector<MolecularModeling::Atom*>& cycle_atoms,
                                                       std::vector<MolecularModeling::Atom*>& visited_atoms)
{
    std::vector<MolecularModeling::Atom*> working_node_neighbors = working_atom->GetNode()->GetNodeNeighbors();
    if (working_atom->GetResidue() == cycle_atoms[0]->GetResidue() && !working_atom->GetIsCycle() &&
        std::find(visited_atoms.begin(), visited_atoms.end(), working_atom) == visited_atoms.end())
    {
    ...
    /*for (std::vector<MolecularModeling::Atom*>::iterator it = working_node_neighbors.begin(); it !=
    working_node_neighbors.end(); it++) {  //Detect atoms that should be added to the current side chain arm.
      MolecularModeling::Atom* working_node_neighbor = *it;
      if (working_node_neighbor != NULL)
      {
        ...
        {//If not part of another side chain && not part of cycle && not visited && is not protein (NLN,LNK etc)
          std::vector<MolecularModeling::Atom*> working_node_neighbor_neighbors =
    std::vector<MolecularModeling::Atom*>(); std::string working_neighbor_name = working_node_neighbor-> GetName(); if
    (working_neighbor_name.substr(0,1) == "O" || working_neighbor_name.substr(0,1) == "N" ||
              working_neighbor_name.substr(0,1) == "S")
          { //if this neighbor is an O,N or S
            std::vector<MolecularModeling::Atom*> attached_anomeric_carbons = std::vector<MolecularModeling::Atom*>();
            for (std::vector<MolecularModeling::Atom*>::iterator it2 = working_node_neighbor_neighbors.begin(); it2 !=
    working_node_neighbor_neighbors.end(); it2++)
            {
              if (*it2 != NULL )
              {
                if ( (*it2)-> GetIsAnomericCarbon() )
                {
                  attached_anomeric_carbons.push_back(*it2); //Check how many anomeric carbons this working neighbor is
    attacheed to, should be either 0,1,2.
                }
              }
            }
            if (attached_anomeric_carbons.size() == 1)
            {  // if attached to only one anomeric carbon
              ...
              {  // if the working atom IS NOT the anomeric carbon the neighbor is attached to,the current side chain
    gets this oxygen. SideAtomArm.push_back(working_node_neighbor); working_node_neighbor-> SetIsSideChain(true);
              }
              // if the workin atom IS INDEED the anomeric carbon the neighbor is attached to, the current side chain
    doesn't get this oxygen, unless this is a terminal hydroxyl. else
              {
    ...
    for (std::vector<MolecularModeling::Atom*>::iterator it3 = working_node_neighbors.begin();
         it3 != working_node_neighbors.end(); it3++)
    {
        if (*it3 != NULL)
        {
            MolecularModeling::Atom* working_node_neighbor = *it3;
            // if a node neighbor is not part of a ring,not visited, and not part on protein, change working atom to
            // this neighbor, and start a new recursion call.
...
void Glycan::Monosaccharide::CheckIfSideChainIsTerminal(MolecularModeling::Atom* starting_atom,
                                                        std::vector<MolecularModeling::Atom*>& cycle_and_visited_atoms,
                                                        bool& is_terminal)
{
    ...
    for (std::vector<MolecularModeling::Atom*>::iterator it = starting_atom_node_neighbors.begin();
         it != starting_atom_node_neighbors.end(); it++)
    {
        if (*it != NULL)
        {
            MolecularModeling::Atom* current_atom_node_neighbor = *it;
            if (std::find(cycle_and_visited_atoms.begin(), cycle_and_visited_atoms.end(), current_atom_node_neighbor) ==
                cycle_and_visited_atoms.end())
            {
                if (current_atom_node_neighbor->GetIsCycle() ||
                    current_atom_node_neighbor->GetResidue()->CheckIfProtein())
                {
                    is_terminal = false;
                }
                if (std::find(cycle_and_visited_atoms.begin(), cycle_and_visited_atoms.end(), *it) ==
                        cycle_and_visited_atoms.end() &&
...
void Glycan::Monosaccharide::ExtractDerivatives(MolecularModeling::Assembly* this_assembly)
{
    ...
    for (std::vector<MolecularModeling::Atom*>::iterator it = cycle_atoms_.begin(); it != cycle_atoms_.end() - 1;
         it++) /// iterate on cycle atoms except the oxygen of the ring
    {
        ...
        for (std::vector<MolecularModeling::Atom*>::iterator it1 = t_neighbors.begin(); it1 != t_neighbors.end(); it1++)
        {
            ...
            if (t_neighbor->GetName().at(0) == 'N' &&
                cycle_atoms_str_.find(t_neighbor->GetId()) == std::string::npos) /// check formulas with nitrogen
            {
                if ((value = this_assembly->CheckxC_N(target, cycle_atoms_str_ /*, pattern_atoms*/)).compare("") != 0)
                { /// xCH-N
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_CO_C(target, cycle_atoms_str_, 'N', pattern_atoms))
                        .compare("") != 0)
                { /// xC-N-C=OCH3
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_CO_CO(target, cycle_atoms_str_, 'N', pattern_atoms))
                        .compare("") != 0)
                { /// xC-N-C=OCH2OH
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_SO3(target, cycle_atoms_str_, 'N', pattern_atoms))
                        .compare("") != 0)
                { /// xC-N-SO3
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_PO3(target, cycle_atoms_str_, 'N', pattern_atoms))
                        .compare("") != 0)
                { /// xC-N-PO3
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_C(target, cycle_atoms_str_, 'N', pattern_atoms)).compare("") !=
                    0)
                { /// xC-N-CH3
                    break;
                }
            ...
            if (t_neighbor->GetName().at(0) == 'O' &&
                cycle_atoms_str_.find(t_neighbor->GetId()) == std::string::npos) /// check formulas with oxygen
            {
                if ((value = this_assembly->CheckxC_NxO_CO_C(target, cycle_atoms_str_, 'O', pattern_atoms))
                        .compare("") != 0)
                { /// xC-O-C=OCH3
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_CO_CO(target, cycle_atoms_str_, 'O', pattern_atoms))
                        .compare("") != 0)
                { /// xC-O-C=OCH2OH
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_SO3(target, cycle_atoms_str_, 'O', pattern_atoms))
                        .compare("") != 0)
                { /// xC-O-SO3
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_PO3(target, cycle_atoms_str_, 'O', pattern_atoms))
                        .compare("") != 0)
                { /// xC-O-PO3
                    break;
                }
                if ((value = this_assembly->CheckxC_NxO_C(target, cycle_atoms_str_, 'O', pattern_atoms)).compare("") !=
                    0)
                { /// xC-O-CH3
                    break;
                }
                if ((value = this_assembly->CheckxCOO(target, cycle_atoms_str_ /*, pattern_atoms*/)).compare("") != 0)
                { /// xC-(O,O) and xC-(O,OH)
                    break;
                }
            }
         ...
         it++) /// iterate on side atoms
    {
        ...
        // {
        //     if(sides.at(0) != NULL)
        //         target = sides.at(0);///first index of each side is for carbon atoms in the
        ...
        if (it == side_atoms_.end() - 1) // side atoms of last carbon of the ring
        {
            if (sides.at(0) != NULL)
            {
                for (side_branch_last_carbon_index = sides.size() - 1; sides.at(side_branch_last_carbon_index) == NULL;
                     side_branch_last_carbon_index--)
                {
        ...
        {
            std::vector<MolecularModeling::Atom*> t_neighbors = target->GetNode()->GetNodeNeighbors();
            for (std::vector<MolecularModeling::Atom*>::iterator it1 = t_neighbors.begin(); it1 != t_neighbors.end();
                 it1++)
            {
                std::vector<MolecularModeling::Atom*> pattern_atoms = std::vector<MolecularModeling::Atom*>();
                MolecularModeling::Atom* t_neighbor                 = (*it1);
                ...
                if (t_neighbor->GetName().at(0) == 'N' &&
                    cycle_atoms_str_.find(t_neighbor->GetId()) == std::string::npos) /// check formulas with nitrogen
                {
                    if ((value = this_assembly->CheckxC_N(target, cycle_atoms_str_ /*, pattern_atoms*/)).compare("") !=
                        0)
                    { /// xCH-N
                        break;
                    }
                    ...
                    if ((value = this_assembly->CheckxC_NxO_C(target, cycle_atoms_str_, 'N', pattern_atoms))
                            .compare("") != 0)
                    { /// xC-N-CH3
                        break;
                    }
                    // if((value = this_assembly->CheckxC_NxO_CH3C_COO(target, cycle_atoms_str_, 'N',
                    // pattern_atoms)).compare("") != 0)///xC-N-CH3C-C-(O,O/OH)
                    //   break;
                }
                if (t_neighbor->GetName().at(0) == 'O' &&
                    cycle_atoms_str_.find(t_neighbor->GetId()) == std::string::npos) /// check formulas with oxygen
                {
                    ...
                    if ((value = this_assembly->CheckxC_NxO_C(target, cycle_atoms_str_, 'O', pattern_atoms))
                            .compare("") != 0)
                    { /// xC-O-CH3
                        break;
                    }
                    if ((value = this_assembly->CheckxCOO(target, cycle_atoms_str_ /*, pattern_atoms*/)).compare("") !=
                        0)
                    { /// xC-(O,O) and xC-(O,OH)
                        break;
                    }
...
std::string Glycan::Monosaccharide::GetFormula(MolecularModeling::Atom* target) //
{
    ...
    {
        MolecularModeling::Atom* t_neighbor = (*it1);
        if ((cycle_atoms_str_.find(t_neighbor->GetId()) == std::string::npos) &&
            (t_neighbor->GetElementSymbol() != "H")) // this neighbor isn't in the ring or the hydrogen
        {
...
void Glycan::Monosaccharide::CountElements(MolecularModeling::Atom* thisAtom,
                                           std::vector<std::pair<std::string, int>>& elementVector)
{
    int local_debug = -1;
    if (local_debug > 0)
    ...
    {
        for (std::vector<MolecularModeling::Atom*>::iterator it = thisAtomNeighbors.begin();
             it != thisAtomNeighbors.end(); it++)
        {
            MolecularModeling::Atom* thisNeighbor = (*it);
            if (!thisNeighbor->GetNode()->GetIsVisited() &&
                !thisNeighbor
                     ->GetIsCycle()) /* && (cycle_atoms_str_.find(thisNeighbor->GetId()) == std::string::npos)*/ // this
                                                                                                                 // is
                                                                                                                 // what
                                                                                                                 // breaks
                                                                                                                 // it
...
std::vector<MolecularModeling::Atom*> Glycan::Monosaccharide::ExtractAdditionalSideAtoms()
{
    ...
    if (side_atoms_.at(side_atoms_.size() - 1).at(0) !=
        NULL) /// if there exist a +1 carbon atom. in side_atoms_ structure
              /// (std::vector<std::vector<MolecularModeling::Atom*>>) the first index of the last element is dedicated
              /// to +1 atom
    {
        plus_sides.push_back(side_atoms_.at(side_atoms_.size() - 1).at(0));
        std::vector<MolecularModeling::Atom*> plus_one_atom_neighbors =
            side_atoms_.at(side_atoms_.size() - 1).at(0)->GetNode()->GetNodeNeighbors();
        for (std::vector<MolecularModeling::Atom*>::iterator it1 = plus_one_atom_neighbors.begin();
             it1 != plus_one_atom_neighbors.end(); it1++)
        {
            if ((*it1)->GetName().at(0) == 'C' &&
                cycle_atoms_str_.find((*it1)->GetId()) == std::string::npos) ///+2 carbon atom found
            {
                MolecularModeling::Atom* plus_two = (*it1);
                plus_two->SetIsExocyclicCarbon(true);
                plus_sides.push_back(plus_two);
                side_atoms_.at(side_atoms_.size() - 1).at(1) =
                    plus_two; /// in side_atoms_ structure (std::vector<std::vector<MolecularModeling::Atom*>>) the
                              /// second index of the last element is dedicated to +2 atom

                std::vector<MolecularModeling::Atom*> plus_two_atom_neighbors = plus_two->GetNode()->GetNodeNeighbors();
                for (std::vector<MolecularModeling::Atom*>::iterator it2 = plus_two_atom_neighbors.begin();
                     it2 != plus_two_atom_neighbors.end(); it2++)
                {
                    MolecularModeling::Atom* plus_three = (*it2);
                    if (plus_three->GetName().at(0) == 'C' &&
                        plus_three->GetId().compare(side_atoms_.at(side_atoms_.size() - 1).at(0)->GetId()) !=
                            0) ///+3 carbon atom found
                    {
                        plus_three->SetIsExocyclicCarbon(true);
                        plus_sides.push_back(plus_three);
                        side_atoms_.at(side_atoms_.size() - 1).at(2) =
                            plus_three; /// in side_atoms_ structure
                                        /// (std::vector<std::vector<MolecularModeling::Atom*>>) the third index of the
                                        /// last element is dedicated to +3 atom
                        break;
                    }
                }
                break;
...
void Glycan::Monosaccharide::GenerateCompleteName(std::vector<MolecularModeling::Atom*>& plus_sides,
                                                  Glycan::Monosaccharide* this_mono,
                                                  MolecularModeling::Assembly* this_assembly)
{
    ...
    else
    {
        if (plus_sides.size() == 3)
        {
            ...
            if ((index_it = find(this_mono->chemical_code_->right_up_.begin(),
                                 this_mono->chemical_code_->right_up_.end(), "+1")) !=
                this_mono->chemical_code_->right_up_.end())
            {
                /// CHECKING R or S
                std::stringstream plus_one;
                std::string orientation = this_assembly->CalculateRSOrientations(
                    this_mono->cycle_atoms_.at(this_mono->cycle_atoms_.size() - 2), plus_sides.at(0), plus_sides.at(1));
                plus_one << "+1" << orientation;
                (*index_it) = plus_one.str();
                std::stringstream plus_two;
                orientation =
                    this_assembly->CalculateRSOrientations(plus_sides.at(0), plus_sides.at(1), plus_sides.at(2));
            ...
            else if ((index_it = find(this_mono->chemical_code_->right_down_.begin(),
                                      this_mono->chemical_code_->right_down_.end(), "+1")) !=
                     this_mono->chemical_code_->right_down_.end())
            {
                /// CHECKING R or S
                std::stringstream plus_one;
                std::string orientation = this_assembly->CalculateRSOrientations(
        ...
        // for( std::vector< std::vector<MolecularModeling::Atom*> >::iterator it1 = this_mono->side_atoms_.begin(); it1
        // != this_mono->side_atoms_.end(); it1++ )
        // {
...
void Glycan::Monosaccharide::GenerateCompleteSugarName(MolecularModeling::Assembly* this_assembly)
{
    ...
    //   [](const std::pair<std::string, std::string>& element){ return element.first == "-1";} ) ==
    //   derivatives_map_.end())
    // if(derivatives_map_.find("-1") != derivatives_map_.end())
    for (std::vector<std::string>::iterator it = chemical_code_->right_up_.begin();
         it != chemical_code_->right_up_.end(); it++)
    {
        std::string key = (*it);
    ...
    for (std::vector<std::string>::iterator it = chemical_code_->right_down_.begin();
         it != chemical_code_->right_down_.end(); it++)
    {
        std::string key = (*it);
    ...
    for (std::vector<std::pair<std::string, std::string>>::iterator it1 = derivatives_map_.begin();
         it1 != derivatives_map_.end(); it1++)
    {
        ...
        for (std::vector<std::pair<std::string, std::string>>::iterator it = this->unknown_derivatives_.begin();
             it != this->unknown_derivatives_.end(); it++)
        {
        ...
        if (((this->is_root_) && (key != "1")) || (this->is_root_ == false))
        {
            if (value.compare("xCH-N") == 0)
            {
                ...
                cond_name_pattern = "N";
                pattern           = "CH-N";
                this_assembly->AddModificationRuleOneInfo(key, pattern, this, long_name_pattern, cond_name_pattern,
                                                          head, tail, minus_one, in_bracket);
            }
            ...
            else if (value.compare("xC-O-C=OCH3") == 0)
            {
                ...
                pattern           = "C-O-C=OCH3";
                this_assembly->AddDerivativeRuleInfo(key, pattern, this, long_name_pattern, cond_name_pattern, head,
                                                     minus_one, in_bracket);
            }
            ...
            else if (value.compare("xC-(O,OH)") == 0)
            {
                ...
                pattern                        = "C-(O,OH)";
                this_assembly->AddModificationRuleTwoInfo(key, pattern, this, long_name_pattern_at_minus_one,
                                                          long_name_pattern_at_plus_one, cond_name_pattern, tail,
                                                          minus_one, in_bracket);
            }
            ...
            else if (unknownDerivativePattern != "")
            {
                ...
                if (key != "a")
                {
                    this_assembly->AddUnknownDerivativeRuleInfo(key, pattern, this, long_name_pattern,
                                                                cond_name_pattern, head, minus_one, in_bracket);
                }
                else
                {
                    this_assembly->AddDerivativeRuleInfo(key, pattern, this, long_name_pattern, cond_name_pattern, head,
    ...
    if (in_bracket.str().size() != 0)
    {
        ...
        /// moving a, b or x to after the bracket: short-name + [...] + a/b/x and removing ", " from the end of bracket
        /// stream
        ...
        short_name << new_name_part1 << "[" << in_bracket.str().substr(0, in_bracket.str().size() - 1) << "]"
                   << new_name_part2;

        sugar_name_.monosaccharide_short_name_ = short_name.str();
    }
    else if (sugar_name_.monosaccharide_stereochemistry_short_name_.compare("") != 0 &&
             sugar_name_.monosaccharide_short_name_.compare("") == 0)
    {
    ...
    if (sugar_name_.monosaccharide_stereochemistry_name_.compare("") != 0)
    {
        if ((sugar_name_.monosaccharide_stereochemistry_name_.substr(
                 sugar_name_.monosaccharide_stereochemistry_name_.size() - 3) == "ose") &&
            (tail.str() == "-osamine"))
        {
            long_name << head.str();
            long_name << sugar_name_.monosaccharide_stereochemistry_name_.substr(
                0, sugar_name_.monosaccharide_stereochemistry_name_.size() - 3);
...
void Glycan::Monosaccharide::UpdateComplexSugarChemicalCode()
{
         ...
         it1 != derivatives_map_.end(); it1++)
    {
        ...
        if (value.compare("") != 0)
        {
            ...
            // }
            std::vector<std::string>::iterator index_it;
            if ((key.compare("a") == 0) || (key.compare("-1") == 0) || (key.compare("+1") == 0) ||
                (key.compare("+2") == 0) || (key.compare("+3") == 0))
            {
                if ((index_it = find(chemical_code_->right_up_.begin(), chemical_code_->right_up_.end(), key)) !=
                    chemical_code_->right_up_.end())
                {
                    (*index_it) = code;
                }
                else if ((index_it = find(chemical_code_->right_down_.begin(), chemical_code_->right_down_.end(),
                                          key)) != chemical_code_->right_down_.end())
                {
                    (*index_it) = code;
                }
            }
            ...
            {
                if ((index_it = find(chemical_code_->left_up_.begin(), chemical_code_->left_up_.end(), key)) !=
                    chemical_code_->left_up_.end())
                {
                    (*index_it) = code;
                }
                else if ((index_it = find(chemical_code_->left_down_.begin(), chemical_code_->left_down_.end(), key)) !=
                         chemical_code_->left_down_.end())
                {
                    (*index_it) = code;
                }
            }
...
void Glycan::Monosaccharide::CheckMonoNaming(std::string original_residue, std::string original_residue_id)
{
    if (sugar_name_.monosaccharide_stereochemistry_name_.compare("") == 0 &&
        sugar_name_.monosaccharide_name_.compare("") == 0)
    {
        // //Rob wanted closest match removed; this is a quick and dirty fix, TODO we still would like to report close
        // matches.
        // //
        ...
        //
        // std::stringstream ss;
        // ss << "No match found for "; //<<  THIS NEEDS TO BE RESIDUE NAME
        // sugar_name_.monosaccharide_stereochemistry_short_name_ ss << original_residue_id; ss << ". close matches: ";
        // for( std::vector< Glycan::SugarName >::iterator ite = closest_matches.begin(); ite != closest_matches.end();
        // ite++ )
        // {
        ...
        // else
        // {
        //   std::cout << "No exact match found for the chemical code, the following information comes from one of the
        //   closest matches:" << std::endl;
        // }
    }
}

// void Glycan::Monosaccharide::addDeoxyToName(Glycan::SugarName base_name, Glycan::ChemicalCode chemical_code,
// std::vector<int> deoxy_locations)
// {
//   //Figure out how to modify all parts of the sugar name (base_name) correctly for the deoxy locations specified in
//   deoxy_locations
//
