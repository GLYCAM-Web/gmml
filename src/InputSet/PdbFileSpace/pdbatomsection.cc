...
PdbAtomSection::PdbAtomSection(std::stringstream& stream_block, std::string index)
{
    ...
    // OG Jan 22 edits so that it reads the first set of alternative residues, whether it starts with A or B or
    // whatever.
    ...
    while (!gmml::Trim(temp).empty())
    {
        ...
        {
            //
            //          for(std::vector<PdbAtomCard*>::reverse_iterator rit = ordered_atom_cards_.rbegin(); rit !=
            //          ordered_atom_cards_.rend(); rit++)
            //          {
            //            PdbAtomCard* thisAtomCard = *rit;
            //            // gmml::log(__LINE__, __FILE__, gmml::INF, thisAtomCard->GetAtomName() + " " +
            //            atom_card->GetAtomName()); if(thisAtomCard->GetAtomName() == atom_card->GetAtomName())
            //            {
            //              // gmml::log(__LINE__, __FILE__, gmml::INF, thisAtomCard->GetAtomResidueName() + " " +
            //              atom_card->GetAtomResidueName()); if(thisAtomCard->GetAtomResidueName() ==
            //              atom_card->GetAtomResidueName())
            //              {
            //                // gmml::log(__LINE__, __FILE__, gmml::INF,thisAtomCard->GetAtomChainId()  + " " +
            //                atom_card->GetAtomChainId());
            ...
            //                " " + atom_card->GetAtomResidueSequenceNumber());
...
{
    std::vector<PdbAtomCard*>::iterator positionOfRef =
        std::find(orderedAtomCards_.begin(), orderedAtomCards_.begin(), referenceCard);
    orderedAtomCards_.insert(positionOfRef, insertionCard); // Default is end, if not found. That's ok ref card defaults
                                                            // to nullptr so if not passed you get it on the end.
...
{
    out << "Record Name: " << recordName_ << std::endl << "_________________ Atoms _______________" << std::endl;
