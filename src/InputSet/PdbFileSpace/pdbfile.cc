...
PdbFile::PdbPairVectorAtomNamePositionFlag PdbFile::GetAllResidueNames()
{
    ...
    for (PdbModelResidueSet::HeterogenAtomSectionVector::iterator it1 = heterogen_atom_cards.begin();
         it1 != heterogen_atom_cards.end(); it1++)
    {
        PdbHeterogenAtomSection* heterogen_atom_card = (*it1);
        PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector ordered_heterogen_atoms =
            heterogen_atom_card->GetOrderedHeterogenAtomCards();
        for (PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector::iterator it2 = ordered_heterogen_atoms.begin();
             it2 != ordered_heterogen_atoms.end(); it2++)
        {
            PdbFileSpace::PdbAtomCard* atom                           = (*it2);
            std::string atom_residue_name                             = atom->GetAtomResidueName();
            std::pair<std::string, std::string> pair_residue_position = std::make_pair(atom_residue_name, " ");
...
PdbFile::PdbResidueVector PdbFile::GetAllResidues()
{
    ...
    for (PdbModelResidueSet::PdbAtomSectionVector::iterator it1 = atom_cards.begin(); it1 != atom_cards.end(); it1++)
    {
        ...
        for (PdbAtomSection::PdbAtomCardOrderVector::iterator it2 = atoms.begin(); it2 != atoms.end(); it2++)
        {
            ...
            {
                PdbResidue* res =
                    new PdbResidue(residue_name, chain_id, sequence_number, insertion_code, alternate_location);
...
PdbFile::PdbAtomCardVector PdbFile::GetAllAtomsOfResidue(PdbResidue* residue)
{
    ...
    std::stringstream ss;
    ss << target_residue_name << "_" << target_residue_chain_id << "_" << target_residue_sequence_number << "_"
       << target_residue_insertion_code << "_" << target_residue_alternate_location;
    ...
    for (PdbModelResidueSet::PdbAtomSectionVector::iterator it1 = atom_cards.begin(); it1 != atom_cards.end(); it1++)
    {
        ...
        for (PdbAtomSection::PdbAtomCardOrderVector::iterator it2 = atoms.begin(); it2 != atoms.end(); it2++)
        {
            ...
            sss << residue_name << "_" << chain_id << "_" << sequence_number << "_" << insertion_code << "_"
                << alternate_location;
...
PdbFileSpace::PdbAtomCard* PdbFile::GetAtomOfResidueByAtomKey(std::string atom_key)
{
    ...
    PdbModelResidueSet* residue_set                       = model->GetModelResidueSet();
    PdbModelResidueSet::PdbAtomSectionVector atom_cards   = residue_set->GetAtomCards();
    for (PdbModelResidueSet::PdbAtomSectionVector::iterator it = atom_cards.begin(); it != atom_cards.end(); it++)
    {
    ...
    PdbModelResidueSet::HeterogenAtomSectionVector heterogen_atom_cards = residue_set->GetHeterogenAtomCards();
    for (PdbModelResidueSet::HeterogenAtomSectionVector::iterator it = heterogen_atom_cards.begin();
         it != heterogen_atom_cards.end(); it++)
    {
        PdbHeterogenAtomSection* heterogen_atom_card = (*it);
        PdbHeterogenAtomSection::PdbHeterogenAtomCardMap heterogen_atom_map =
            heterogen_atom_card->GetHeterogenAtomCards();
...
PdbFile::PdbPairVectorTerCardPositions PdbFile::GetAllTerCardPositions(std::vector<std::string> glycam_residue_names)
{
    ...
    for (PdbFile::PdbResidueVector::iterator it = residues.begin(); it != residues.end(); it++)
    {
        ...
        if (dist != residues.size() - 1)
        {
            ...
            if (find(glycam_residue_names.begin(), glycam_residue_names.end(), residue_name) !=
                glycam_residue_names.end())
            {
                // No tail || has more than or equal two tails
                if (residue_name[0] == '0' || isalpha(residue_name[0]))
                {
                    ter_card_positions.push_back(
                        std::make_pair(residue->GetResidueChainId(), residue->GetResidueSequenceNumber() + 1));
                }
            }
        }
    }
...
void PdbFile::SetSequenceNumberMapping(PdbSequenceNumberMapping sequence_number_mapping)
{
    ...
    for (PdbSequenceNumberMapping::iterator it = sequence_number_mapping.begin(); it != sequence_number_mapping.end();
         it++)
    {
...
void PdbFile::UpdateConnectCard()
{
    ...
    {
        PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap bondedAtomsSerialNumbersMap =
            connectivities_->GetBondedAtomsSerialNumbers();
        PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap new_bonded_atoms_serial_numbers_map =
            PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap();
        for (PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap::iterator it =
                 bondedAtomsSerialNumbersMap.begin();
             it != bondedAtomsSerialNumbersMap.end(); it++)
        {
            ...
            if (serial_number_mapping_.find(source_serial_number) != serial_number_mapping_.end())
            {
                ...
                for (std::vector<int>::iterator it1 = bonded_serial_numbers.begin(); it1 != bonded_serial_numbers.end();
                     it1++)
                {
                    ...
                    if (serial_number_mapping_.find(bonded_serial_number) != serial_number_mapping_.end())
                    {
                        ...
                        new_bonded_atoms_serial_numbers_map[new_source_serial_number].push_back(
                            new_bonded_serial_number);
                    }
                }
            }
        }
...
bool PdbFile::ParseCards(std::ifstream& in_stream)
{
    ...
    record_name = line.substr(0, 6);
    record_name = gmml::Trim(record_name);
    if (record_name == "USER") // OG Jan22. This should just run through the USER section (created by Ambertool's
                               // reduce) and do nothing.
    {
...
bool PdbFile::ParseDatabaseReferenceSection(std::ifstream& stream, std::string& line)
{
    ...
    while (record_name.compare("DBREF") == 0 || record_name.compare("DBREF1") == 0 ||
           record_name.compare("DBREF2") == 0)
    {
        stream_block << line << std::endl;
        if (getline(stream, line))
        {
...
bool PdbFile::ParseModelSection(std::ifstream& stream, std::string& line)
{
    ...
    while (record_name.compare("MODEL") == 0 || record_name.compare("ATOM") == 0 ||
           record_name.compare("ANISOU") == 0 || record_name.compare("TER") == 0 ||
           record_name.compare("HETATM") == 0 || record_name.compare("ENDMDL") == 0)
    {
        stream_block << line << std::endl;
        if (getline(stream, line))
        {
...
{
    stream << std::left << std::setw(6) << header_->GetRecordName() << std::left << std::setw(4) << " " << std::left
           << std::setw(40) << header_->GetClassification() << std::left << std::setw(9) << header_->GetDepositionDate()
           << std::left << std::setw(3) << " " << std::right << std::setw(4) << header_->GetIdentifierCode()
           << std::left << std::setw(14) << " " << std::endl;
}

void PdbFile::ResolveObsoleteCards(std::ostream& stream)
{
    stream << std::left << std::setw(6) << obsolete_->GetRecordName() << std::left << std::setw(2) << " " << std::left
           << std::setw(2) << obsolete_->GetContinuation() << std::left << std::setw(1) << " " << std::left
           << std::setw(9) << obsolete_->GetReplacementDate() << std::left << std::setw(1) << " ";
    std::vector<std::string> identifier_codes = obsolete_->GetIdentifierCodes();
    for (unsigned int i = 0; i < identifier_codes.size(); i++)
    {
        stream << std::left << std::setw(4) << identifier_codes[i] << std::left << std::setw(5) << "      ";
    }
    stream << std::left << std::setw(14) << " " << std::endl;
}

void PdbFile::ResolveTitleCards(std::ostream& stream)
{
    const int MAX_TITLE_LENGTH_IN_LINE = 70;
    stream << std::left << std::setw(6) << title_->GetRecordName() << std::left << std::setw(2) << " ";
    if ((int)title_->GetTitle().length() > MAX_TITLE_LENGTH_IN_LINE)
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70)
               << title_->GetTitle().substr(0, MAX_TITLE_LENGTH_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << title_->GetRecordName() << std::left << std::setw(2) << " "
                       ...
                       << title_->GetTitle().substr(MAX_TITLE_LENGTH_IN_LINE * (i - 1), MAX_TITLE_LENGTH_IN_LINE)
                       << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << title_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << title_->GetTitle().substr(MAX_TITLE_LENGTH_IN_LINE * (i - 1),
                                                    title_->GetTitle().length() - MAX_TITLE_LENGTH_IN_LINE * (i - 1))
                       << std::endl;
            }
        }
    }
    ...
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70) << title_->GetTitle() << std::endl;
    }
}
...
void PdbFile::ResolveSplitCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << split_->GetRecordName() << std::left << std::setw(2) << " ";
    if ((int)split_->GetSplit().length() > MAX_SPLIT_ID_IN_LINE)
    {
        stream << std::left << std::setw(79) << split_->GetSplit().substr(0, MAX_SPLIT_ID_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            ...
            {
                stream << std::left << std::setw(6) << split_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(79)
                       << split_->GetSplit().substr(MAX_SPLIT_ID_IN_LINE * (i - 1), MAX_SPLIT_ID_IN_LINE) << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << split_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(79)
                       << split_->GetSplit().substr(MAX_SPLIT_ID_IN_LINE * (i - 1),
                                                    split_->GetSplit().length() - MAX_SPLIT_ID_IN_LINE * (i - 1))
                       << std::endl;
            }
        }
    }
    ...
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(79) << split_->GetSplit() << std::endl;
    }
}
...
void PdbFile::ResolveCaveatCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << caveat_->GetRecordName() << std::left << std::setw(2) << " " << std::left
           << std::setw(4) << header_->GetIdentifierCode();
    if ((int)caveat_->GetCaveat().length() > MAX_CAVEAT_LENGTH_IN_LINE)
    {
        stream << std::left << std::setw(70) << caveat_->GetCaveat().substr(0, MAX_CAVEAT_LENGTH_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << caveat_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << caveat_->GetCaveat().substr(MAX_CAVEAT_LENGTH_IN_LINE * (i - 1), MAX_CAVEAT_LENGTH_IN_LINE)
                       << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << caveat_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << caveat_->GetCaveat().substr(MAX_CAVEAT_LENGTH_IN_LINE * (i - 1),
                                                      caveat_->GetCaveat().length() -
                                                          MAX_CAVEAT_LENGTH_IN_LINE * (i - 1))
                       << std::endl;
            }
        }
    }
    ...
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70) << caveat_->GetCaveat() << std::endl;
    }
}
...
void PdbFile::ResolveCompoundCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1) << " " << std::right
           << std::setw(3) << " ";

    PdbFileSpace::PdbCompoundSection::PdbCompoundSpecificationMap compound_specification_map =
        compound_->GetCompoundSpecifications();
    ...
    bool first  = true;
    int counter = 2;
    for (PdbFileSpace::PdbCompoundSection::PdbCompoundSpecificationMap::iterator it =
             compound_specification_map.begin();
         it != compound_specification_map.end(); it++)
    {
        ...
        if (!first)
        {
            if (compound_specification->GetMoleculeId() != "")
            {
                ...
                stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1) << " "
                       << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str() << std::endl;
                counter++;
            }
            else
            {
        ...
        if (compound_specification->GetMoleculeName() != "")
        {
            ...
            else
            {
                ...
                for (int i = 1; i <= number_of_lines; i++)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << molecule_name.str().substr((i - 1) * (MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE),
                                                         MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE);
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
                        counter++;
                    }
                    else
                    {
                        std::stringstream ss;
                        ss << molecule_name.str().substr((i - 1) * (MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE),
                                                         length - (i - 1) * (MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE));
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
                        counter++;
        ...
        if (compound_specification->GetChainIds().size() > 0)
        {
            ...
            else
            {
                ...
                for (int i = 1; i <= number_of_lines; i++)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << chain_id.str().substr((i - 1) * MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE,
                                                    MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE);
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
        ...
        if (compound_specification->GetFragment() != "")
        {
            ...
            else
            {
                ...
                for (int i = 1; i <= number_of_lines; i++)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << ss.str().substr((i - 1) * MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE,
                                              MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE);
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
        ...
        if (compound_specification->GetMoleculeSynonyms().size() > 0)
        {
            ...
            synonyms << " SYNONYM: ";
            for (std::vector<std::string>::iterator it1 = molecule_synonyms.begin(); it1 != molecule_synonyms.end();
                 it1++)
            {
            ...
            else
            {
                ...
                for (int i = 1; i <= number_of_lines; i++)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << synonyms.str().substr((i - 1) * MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE,
                                                    MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE);
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
        ...
        if (compound_specification->GetEnzymeCommissionNumbers().size() > 0)
        {
            ...
            for (std::vector<std::string>::iterator it1 = enzyme_commission_numbers.begin();
                 it1 != enzyme_commission_numbers.end(); it1++)
            {
            ...
            else
            {
                ...
                for (int i = 1; i <= number_of_lines; i++)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << commission_numbers.str().substr((i - 1) * MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE,
                                                              MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE);
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
        ...
        if (compound_specification->GetComments() != "")
        {
            ...
            else
            {
                ...
                for (int i = 1; i <= number_of_lines; i++)
                {
                    ...
                    {
                        std::stringstream ss;
                        ss << comments.str().substr((i - 1) * MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE,
                                                    MAX_LENGTH_OF_COMPOUND_SPEC_IN_LINE);
                        stream << std::left << std::setw(6) << compound_->GetRecordName() << std::left << std::setw(1)
                               << " " << std::right << std::setw(3) << counter << std::left << std::setw(70) << ss.str()
                               << std::endl;
...
void PdbFile::ResolveSourceCards(std::ostream& stream)
{
    ...
    for (SourceCardVector::iterator it = source_cards.begin(); it != source_cards.end(); it++)
    {
        if (SOURCE_COUNT == 1)
        {
            stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(4) << " "
                   << std::left << std::setw(69) << (*it)->GetToken() + ":" + (*it)->GetValue() << std::endl;
            SOURCE_COUNT++;
        }
        else
        {
            stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::right << std::setw(4) << SOURCE_COUNT
                   << std::left << std::setw(68) << (*it)->GetToken() + ":" + (*it)->GetValue() << std::endl;
            SOURCE_COUNT++;
        }
    }
}
...
void PdbFile::ResolveKeywordCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << keywords_->GetRecordName() << std::left << std::setw(2) << " ";
    if ((int)keywords_->GetKeywords().length() > MAX_KEYWORDS_LENGTH_IN_LINE)
    {
        stream << std::left << std::setw(2) << " " << std::left << std::setw(70)
               << keywords_->GetKeywords().substr(0, MAX_KEYWORDS_LENGTH_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << keywords_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << keywords_->GetKeywords().substr(MAX_KEYWORDS_LENGTH_IN_LINE * (i - 1),
                                                          MAX_KEYWORDS_LENGTH_IN_LINE)
                       << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << keywords_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << keywords_->GetKeywords().substr(MAX_KEYWORDS_LENGTH_IN_LINE * (i - 1),
                                                          keywords_->GetKeywords().length() -
                                                              MAX_KEYWORDS_LENGTH_IN_LINE * (i - 1))
                       << std::endl;
            }
        }
    }
    ...
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70) << keywords_->GetKeywords()
               << std::endl;
    }
}
...
void PdbFile::ResolveExperimentalDataCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << experimental_data_->GetRecordName() << std::left << std::setw(2) << " ";
    if ((int)experimental_data_->GetExperimentalData().length() > MAX_EXPDTA_LENGTH_IN_LINE)
    {
        stream << std::left << std::setw(70)
               << experimental_data_->GetExperimentalData().substr(0, MAX_EXPDTA_LENGTH_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << experimental_data_->GetRecordName() << std::left << std::setw(2)
                       << " " << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << experimental_data_->GetExperimentalData().substr(MAX_EXPDTA_LENGTH_IN_LINE * (i - 1),
                                                                           MAX_EXPDTA_LENGTH_IN_LINE)
                       << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << experimental_data_->GetRecordName() << std::left << std::setw(2)
                       << " " << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << experimental_data_->GetExperimentalData().substr(
                              MAX_EXPDTA_LENGTH_IN_LINE * (i - 1),
                              experimental_data_->GetExperimentalData().length() - MAX_EXPDTA_LENGTH_IN_LINE * (i - 1))
                       << std::endl;
            }
        }
    }
    ...
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70)
               << experimental_data_->GetExperimentalData() << std::endl;
    }
}

void PdbFile::ResolveNumModelCard(std::ostream& stream)
{
    stream << std::left << std::setw(6) << number_of_models_->GetRecordName() << std::left << std::setw(4) << " ";
    ...
    stream << std::left << std::setw(66) << " " << std::endl;
}

void PdbFile::ResolveModelTypeCards(std::ostream& stream)
{
    stream << std::left << std::setw(6) << model_type_->GetRecordName() << std::left << std::setw(2) << " ";
    std::stringstream ss;
    for (std::vector<std::string>::iterator it = model_type_->GetComments().begin();
         it != model_type_->GetComments().end(); it++)
    {
    ...
    if (ss.str().length() > 70)
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70) << ss.str().substr(0, 70)
               << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << model_type_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << ss.str().substr(70 * (i - 1), 70) << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << model_type_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << ss.str().substr(70 * (i - 1), ss.str().length() - (i - 1) * 70) << std::endl;
            }
        }
    }
    else
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70) << ss.str() << std::endl;
    }
}
...
void PdbFile::ResolveAuthorCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << author_->GetRecordName() << std::left << std::setw(2) << " ";
    if ((int)author_->GetAuthor().length() > MAX_AUTHOR_LENGTH_IN_LINE)
    {
        stream << std::left << std::setw(70) << author_->GetAuthor().substr(0, MAX_AUTHOR_LENGTH_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << author_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << author_->GetAuthor().substr(MAX_AUTHOR_LENGTH_IN_LINE * (i - 1), MAX_AUTHOR_LENGTH_IN_LINE)
                       << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << author_->GetRecordName() << std::left << std::setw(2) << " "
                       << std::right << std::setw(2) << i << std::left << std::setw(70)
                       << author_->GetAuthor().substr(MAX_AUTHOR_LENGTH_IN_LINE * (i - 1),
                                                      author_->GetAuthor().length() -
    ...
    {
        stream << std::right << std::setw(2) << " " << std::left << std::setw(70) << author_->GetAuthor() << std::endl;
    }
}
...
void PdbFile::ResolveRevisionDataCards(std::ostream& stream)
{
    ...
    for (RevisionDataCardVector::iterator it = revision_data_cards.begin(); it != revision_data_cards.end(); it++)
    {
        ...
        if (REVDAT_COUNT == 1)
        {
            stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(1) << " "
                   << std::right << std::setw(3) << (*it)->GetModificationNumber() << std::right << std::setw(3) << " "
                   << std::left << std::setw(9) << (*it)->GetModificationDate() << std::left << std::setw(1) << " "
                   << std::left << std::setw(4) << (*it)->GetModificationID() << std::left << std::setw(4) << " "
                   << std::left << std::setw(1) << (*it)->GetModificationType() << std::left << std::setw(7) << " "
                   << std::left << std::setw(27) << (*it)->GetModificationDetails() << std::endl;
        }
        else if (REVDAT_NUM == (*it)->GetModificationNumber())
        {
            stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(1) << " "
                   << std::right << std::setw(3) << (*it)->GetModificationNumber() << std::right << std::setw(2)
                   << REVDAT_COUNT << std::left << std::setw(1) << " " << std::left << std::setw(9)
                   << (*it)->GetModificationDate() << std::left << std::setw(1) << " " << std::left << std::setw(4)
                   << (*it)->GetModificationID() << std::left << std::setw(4) << " " << std::left << std::setw(1)
                   << (*it)->GetModificationType() << std::left << std::setw(7) << " " << std::left << std::setw(27)
                   << (*it)->GetModificationDetails() << std::endl;
            REVDAT_COUNT++;
        }
...
void PdbFile::ResolveJournalCards(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(6) << journal_->GetRecordName() << std::left << std::setw(6) << " ";
    if ((int)journal_->GetText().length() > MAX_JOURNAL_LENGTH_IN_LINE)
    {
        stream << std::left << std::setw(67) << journal_->GetText().substr(0, MAX_JOURNAL_LENGTH_IN_LINE) << std::endl;
        ...
        for (int i = 2; i <= counter; i++)
        {
            if (i != counter)
            {
                stream << std::left << std::setw(6) << journal_->GetRecordName() << std::left << std::setw(6) << " "
                       ...
                       << journal_->GetText().substr(MAX_JOURNAL_LENGTH_IN_LINE * (i - 1), MAX_JOURNAL_LENGTH_IN_LINE)
                       << std::endl;
            }
            else
            {
                stream << std::left << std::setw(6) << journal_->GetRecordName() << std::left << std::setw(6) << " "
                       << std::left << std::setw(67)
                       << journal_->GetText().substr(MAX_JOURNAL_LENGTH_IN_LINE * (i - 1),
                                                     journal_->GetText().length() -
    ...
    {
        stream << std::right << std::setw(6) << " " << std::left << std::setw(67) << journal_->GetText() << std::endl;
    }
}
...
void PdbFile::ResolveDatabaseReferenceCards(std::ostream& stream)
{
    ...
    for (DatabaseReferenceVector::iterator it = database_reference_cards.begin(); it != database_reference_cards.end();
         it++)
    {
        if ((*it)->GetRecordName() == "DBREF ")
        {
            stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(1) << " "
                   << std::left << std::setw(4) << (*it)->GetIDCode() << std::left << std::setw(1) << " " << std::left
                   << std::setw(1) << (*it)->GetChainID() << std::left << std::setw(1) << " " << std::right
                   << std::setw(4) << (*it)->GetSeqBegin() << std::right << std::setw(1) << (*it)->GetInsertBegin()
                   << std::left << std::setw(1) << " " << std::right << std::setw(4) << (*it)->GetSeqEnd() << std::right
                   << std::setw(1) << (*it)->GetInsertEnd() << std::left << std::setw(1) << " " << std::left
                   << std::setw(6) << (*it)->GetDatabase() << std::left << std::setw(1) << " " << std::left
                   << std::setw(8) << (*it)->GetDatabaseAccession() << std::left << std::setw(1) << " " << std::left
                   << std::setw(12) << (*it)->GetDatabaseIDCode() << std::left << std::setw(1) << " " << std::right
                   << std::setw(5) << (*it)->GetDatabaseSeqBegin() << std::right << std::setw(1)
                   << (*it)->GetDatabaseInsBegin() << std::left << std::setw(1) << " " << std::right << std::setw(5)
                   << (*it)->GetDatabaseSeqEnd() << std::right << std::setw(1) << (*it)->GetDatabaseInsEnd()
                   << std::endl;
        }
        else if (((*it)->GetRecordName() == "DBREF1"))
        {
            stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(1) << " "
                   ...
                   << std::left << std::setw(1) << " " << std::right << std::setw(4) << (*it)->GetSeqEnd() << std::right
                   << std::setw(1) << (*it)->GetInsertEnd() << std::left << std::setw(1) << " " << std::left
                   << std::setw(6) << (*it)->GetDatabase() << std::left << std::setw(16) << " " << std::left
                   << std::setw(15) << (*it)->GetDatabaseIDCode() << std::endl
                   << std::left << std::setw(6) << "DBREF2" << std::left << std::setw(1) << " " << std::left
                   << std::setw(4) << (*it)->GetIDCode() << std::left << std::setw(1) << " " << std::left
                   << std::setw(1) << (*it)->GetChainID() << std::left << std::setw(6) << " " << std::left
                   << std::setw(22) << (*it)->GetDatabaseAccession() << std::left << std::setw(5) << " " << std::right
                   << std::setw(10) << (*it)->GetDatabaseSeqBegin() << std::left << std::setw(2) << " " << std::right
                   << std::setw(10) << (*it)->GetDatabaseSeqEnd() << std::endl;
        }
    }
}
...
void PdbFile::ResolveSequenceAdvancedCards(std::ostream& stream)
{
    ...
    for (SequenceAdvancedCardVector::iterator it = sequence_advanced_cards.begin(); it != sequence_advanced_cards.end();
         it++)
    {
        stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(1) << " " << std::right
               << std::setw(3) << (*it)->GetIdentifierCode() << std::left << std::setw(1) << " " << std::left
               << std::setw(3) << (*it)->GetResidueName() << std::left << std::setw(1) << " " << std::left
               << std::setw(1) << (*it)->GetChainId() << std::left << std::setw(1) << " " << std::right << std::setw(4)
               << (*it)->GetSequenceNumber() << std::left << std::setw(1) << (*it)->GetInsertionCode() << std::left
               << std::setw(1) << " " << std::left << std::setw(4) << (*it)->GetDatabase() << std::left << std::setw(1)
               << " " << std::left << std::setw(9) << (*it)->GetDatabaseAccession() << std::left << std::setw(1) << " "
               << std::right << std::setw(3) << (*it)->GetDatabaseResidue() << std::left << std::setw(1) << " "
               << std::right << std::setw(5) << (*it)->GetDatabaseSequence() << std::left << std::setw(1) << " "
               << std::left << std::setw(30) << (*it)->GetConflict() << std::endl;
    }
}

void PdbFile::ResolveSequenceResidueCards(std::ostream& stream)
{
    PdbFileSpace::PdbResidueSequenceSection::ResidueSequenceCardMap residue_sequence_map =
        residues_sequence_->GetResidueSequenceChain();
    for (PdbFileSpace::PdbResidueSequenceSection::ResidueSequenceCardMap::iterator it = residue_sequence_map.begin();
         it != residue_sequence_map.end(); it++)
    {
        ...
        if (residue_sequence->GetNumberOfResidues() <= MAX_RESIDUE_IN_SINGLE_LINE)
        {
            ...
            {
                ss << std::right << std::setw(1) << " " << std::right << std::setw(3) << (*it1);
            }
            for (int i = 0; i < MAX_RESIDUE_IN_SINGLE_LINE - residue_sequence->GetNumberOfResidues(); i++)
            {
                ss << std::right << std::setw(1) << " " << std::right << std::setw(3) << " ";
            }
            stream << std::left << std::setw(6) << residues_sequence_->GetRecordName() << std::left << std::setw(1)
                   << " " << std::right << std::setw(3) << serial_number << std::right << std::setw(1) << " "
                   << std::right << std::setw(1) << residue_sequence->GetChainId() << std::right << std::setw(1) << " ";
            ...
            stream << std::right << std::setw(1) << " " << std::right << std::setw(52) << ss.str() << std::right
            ...
            for (int i = 0; i < number_of_lines; i++)
            {
                ...
                if (i != number_of_lines - 1)
                {
                    for (std::vector<std::string>::iterator it1 =
                             residue_names.begin() + i * MAX_RESIDUE_IN_SINGLE_LINE;
                ...
                else
                {
                    ...
                    for (int i = 0; i < MAX_RESIDUE_IN_SINGLE_LINE -
                                            residue_sequence->GetNumberOfResidues() % MAX_RESIDUE_IN_SINGLE_LINE;
                         i++)
                    {
                        ss << std::right << std::setw(1) << " " << std::right << std::setw(3) << " ";
                    }
...
{
    PdbFileSpace::PdbResidueModificationSection::ResidueModificationCardMap residue_modification_cards_map =
        residue_modification_cards_->GetResidueModificationCards();
    for (PdbFileSpace::PdbResidueModificationSection::ResidueModificationCardMap::iterator it =
             residue_modification_cards_map.begin();
         it != residue_modification_cards_map.end(); it++)
    {
        PdbResidueModificationCard* residue_modification_cards = (*it).second;
        stream << std::left << std::setw(6) << residue_modification_cards_->GetRecordName() << std::left << std::setw(1)
               << " " << std::right << std::setw(4) << residue_modification_cards->GetIdCode() << std::left
               << std::setw(1) << " " << std::right << std::setw(3) << residue_modification_cards->GetResidueName()
               << std::left << std::setw(1) << " " << std::right << std::setw(1)
               << residue_modification_cards->GetChainId() << std::left << std::setw(1) << " ";
        ...
        stream << std::right << std::setw(1) << residue_modification_cards->GetInsertionCode() << std::left
               << std::setw(1) << " " << std::right << std::setw(3)
               << residue_modification_cards->GetStandardResidueName() << std::left << std::setw(2) << " " << std::left
               << std::setw(41) << residue_modification_cards->GetDscr() << std::left << std::setw(10) << " "
               << std::endl;
    }
}
...
void PdbFile::ResolveHeterogenCards(std::ostream& stream)
{
    ...
    for (PdbFileSpace::PdbHeterogenSection::HeterogenCardMap::iterator it = heterogen_map.begin();
         it != heterogen_map.end(); it++)
    {

        PdbHeterogenCard* heterogen = (*it).second;
        stream << std::left << std::setw(6) << heterogen_cards_->GetRecordName() << std::left << std::setw(1) << " "
               << std::right << std::setw(3) << heterogen->GetHeterogenId() << std::left << std::setw(2) << " "
        ...
        stream << std::right << std::setw(1) << heterogen->GetInsertionCode() << std::left << std::setw(2) << " ";
        ...
        stream << std::left << std::setw(5) << " " << std::left << std::setw(40) << heterogen->GetDscr() << std::left
               << std::setw(10) << " " << std::endl;
    }
}
...
{
    PdbFileSpace::PdbHeterogenNameSection::HeterogenNameCardMap heterogen_name_map =
        heterogen_name_cards_->GetHeterogenNameCards();
    for (PdbFileSpace::PdbHeterogenNameSection::HeterogenNameCardMap::iterator it = heterogen_name_map.begin();
         it != heterogen_name_map.end(); it++)
    {
        ...
        {
            stream << std::left << std::setw(6) << heterogen_name_cards_->GetRecordName() << std::left << std::setw(2)
                   << " " << std::right << std::setw(2) << " " << std::left << std::setw(1) << " " << std::right
                   << std::setw(3) << heterogen_name->GetHeterogenIdentifier() << std::left << std::setw(1) << " "
                   << std::left << std::setw(55)
                   << heterogen_name->GetHeterogenName().substr(0, MAX_NAME_LENGTH_IN_LINE) << std::left
                   << std::setw(10) << " " << std::endl;
            ...
            for (int i = 2; i <= counter; i++)
            {
                if (i != counter)
                {
                    stream << std::left << std::setw(6) << heterogen_name_cards_->GetRecordName() << std::left
                           << std::setw(2) << " " << std::right << std::setw(2) << i << std::left << std::setw(1) << " "
                           << std::right << std::setw(3) << heterogen_name->GetHeterogenIdentifier() << std::left
                           << std::setw(1) << " " << std::left << std::setw(55)
                           << heterogen_name->GetHeterogenName().substr(MAX_NAME_LENGTH_IN_LINE * (i - 1),
                                                                        MAX_NAME_LENGTH_IN_LINE)
                ...
                else
                {
                    stream << std::left << std::setw(6) << heterogen_name_cards_->GetRecordName() << std::left
                           ...
                           << std::setw(1) << " " << std::left << std::setw(55)
                           << heterogen_name->GetHeterogenName().substr(MAX_NAME_LENGTH_IN_LINE * (i - 1),
                                                                        heterogen_name->GetHeterogenName().length() -
                                                                            MAX_NAME_LENGTH_IN_LINE * (i - 1))
                           << std::left << std::setw(10) << " " << std::endl;
                }
            }
        }
        else
        {
            stream << std::left << std::setw(6) << heterogen_name_cards_->GetRecordName() << std::left << std::setw(2)
                   << " " << std::right << std::setw(2) << " " << std::left << std::setw(1) << " " << std::right
                   << std::setw(3) << heterogen_name->GetHeterogenIdentifier() << std::left << std::setw(1) << " "
                   << std::left << std::setw(55) << heterogen_name->GetHeterogenName() << std::left << std::setw(10)
                   << " " << std::endl;
        }
    }
}
...
{
    PdbFileSpace::PdbHeterogenSynonymSection::HeterogenSynonymCardMap heterogen_synonym_map =
        heterogen_synonym_cards_->GetHeterogensSynonymCards();
    for (PdbFileSpace::PdbHeterogenSynonymSection::HeterogenSynonymCardMap::iterator it = heterogen_synonym_map.begin();
         it != heterogen_synonym_map.end(); it++)
    {
        ...
        {
            stream << std::left << std::setw(6) << heterogen_synonym_cards_->GetRecordName() << std::left
                   << std::setw(2) << " " << std::right << std::setw(2) << " " << std::left << std::setw(1) << " "
                   << std::right << std::setw(3) << heterogen_synonym_card->GetHeterogenIdentifier() << std::left
                   << std::setw(1) << " " << std::left << std::setw(55)
                   << ss.str().substr(0, MAX_SYNONYM_LENGTH_IN_LINE) << std::left << std::setw(10) << " " << std::endl;
            ...
            for (int i = 2; i <= counter; i++)
            {
                if (i != counter)
                {
                    stream << std::left << std::setw(6) << heterogen_synonym_cards_->GetRecordName() << std::left
                           ...
                           << std::left << std::setw(1) << " " << std::left << std::setw(55)
                           << ss.str().substr(MAX_SYNONYM_LENGTH_IN_LINE * (i - 1), MAX_SYNONYM_LENGTH_IN_LINE)
                           << std::left << std::setw(10) << " " << std::endl;
                }
                else
                {
                    stream << std::left << std::setw(6) << heterogen_synonym_cards_->GetRecordName() << std::left
                           ...
                           << std::right << std::setw(3) << heterogen_synonym_card->GetHeterogenIdentifier()
                           << std::left << std::setw(1) << " " << std::left << std::setw(55)
                           << ss.str().substr(MAX_SYNONYM_LENGTH_IN_LINE * (i - 1),
                                              ss.str().length() - MAX_SYNONYM_LENGTH_IN_LINE * (i - 1))
                           << std::left << std::setw(10) << " " << std::endl;
                }
            }
        }
        else
        {
            stream << std::left << std::setw(6) << heterogen_synonym_cards_->GetRecordName() << std::left
                   << std::setw(2) << " " << std::right << std::setw(2) << " " << std::left << std::setw(1) << " "
                   << std::right << std::setw(3) << heterogen_synonym_card->GetHeterogenIdentifier() << std::left
                   << std::setw(1) << " " << std::left << std::setw(55) << ss.str() << std::left << std::setw(10) << " "
                   << std::endl;
        }
    }
}
...
void PdbFile::ResolveFormulaCards(std::ostream& stream)
{
    ...
    for (PdbFileSpace::PdbFormulaSection::FormulaCardMap::iterator it = formula_map.begin(); it != formula_map.end();
         it++)
    {
        ...
        {
            stream << std::left << std::setw(6) << formulas_->GetRecordName() << std::left << std::setw(2) << " ";
            ...
            stream << std::left << std::setw(2) << " " << std::right << std::setw(3)
                   << formula->GetHeterogenIdentifier() << std::left << std::setw(1) << " " << std::right
                   << std::setw(2) << " " << std::left << std::setw(1) << " " << std::left << std::setw(51)
                   << formula->GetChemicalFormula().substr(0, MAX_NAME_LENGTH_IN_LINE) << std::left << std::setw(10)
                   << " " << std::endl;
            ...
            for (int i = 2; i <= counter; i++)
            {
                if (i != counter)
                {
                    ...
                    stream << std::left << std::setw(2) << " " << std::right << std::setw(3)
                           << formula->GetHeterogenIdentifier() << std::left << std::setw(1) << " " << std::right
                           << std::setw(2) << i << std::left << std::setw(1) << " " << std::left << std::setw(51)
                           << formula->GetChemicalFormula().substr(MAX_NAME_LENGTH_IN_LINE * (i - 1),
                                                                   MAX_NAME_LENGTH_IN_LINE)
                           << std::left << std::setw(10) << " " << std::endl;
                }
                else
                {
                    ...
                    stream << std::left << std::setw(2) << " " << std::right << std::setw(3)
                           ...
                           << std::setw(2) << i << std::left << std::setw(1) << " " << std::left << std::setw(51)
                           << formula->GetChemicalFormula().substr(MAX_NAME_LENGTH_IN_LINE * (i - 1),
                                                                   formula->GetChemicalFormula().length() -
                                                                       MAX_NAME_LENGTH_IN_LINE * (i - 1))
                           << std::left << std::setw(10) << " " << std::endl;
                }
            }
...
void PdbFile::ResolveHelixCards(std::ostream& stream)
{
    ...
    while (serial_number <= counter)
    {
        for (PdbFileSpace::PdbHelixSection::HelixCardMap::iterator it = helix_map.begin(); it != helix_map.end(); it++)
        {
            ...
            {
                stream << std::left << std::setw(6) << helix_cards_->GetRecordName() << std::left << std::setw(1)
                       << " ";
                ...
                stream << std::left << std::setw(1) << " " << std::right << std::setw(3) << helix->GetHelixId()
                       << std::left << std::setw(1) << " " << std::right << std::setw(3)
                       << helix_residues.at(0)->GetResidueName() << std::left << std::setw(1) << " " << std::right
                       << std::setw(1) << helix_residues.at(0)->GetResidueChainId() << std::left << std::setw(1) << " ";
                ...
                stream << std::right << std::setw(1) << helix_residues.at(0)->GetResidueInsertionCode() << std::left
                       << std::setw(1) << " " << std::right << std::setw(3) << helix_residues.at(1)->GetResidueName()
                       << std::left << std::setw(1) << " " << std::right << std::setw(1)
                       << helix_residues.at(1)->GetResidueChainId() << std::left << std::setw(1) << " ";
                ...
                stream << std::left << std::setw(30) << helix->GetComment() << std::left << std::setw(1) << " ";
                ...
                stream << std::left << std::setw(4) << " " << std::endl;
                break;
            }
        }
        serial_number++;
...
void PdbFile::ResolveSheetCards(std::ostream& stream)
{
    ...
    for (PdbFileSpace::PdbSheetSection::SheetCardMap::iterator it = sheet_map.begin(); it != sheet_map.end(); it++)
    {
        ...
        for (PdbSheetCard::SheetStrandVector::iterator it1 = strands.begin(); it1 != strands.end(); it1++)
        {
            ...
            {
                stream << std::left << std::setw(6) << sheet_cards_->GetRecordName() << std::left << std::setw(1) << " "
                       << std::right << std::setw(3) << serial_number << std::left << std::setw(1) << " " << std::right
                       << std::setw(3) << sheet->GetSheetId();
                ...
                stream << std::left << std::setw(1) << " " << std::right << std::setw(3)
                       << strand_residues.at(0)->GetResidueName() << std::left << std::setw(1) << " " << std::right
                       << std::setw(1) << strand_residues.at(0)->GetResidueChainId();
                ...
                stream << std::right << std::setw(1) << strand_residues.at(0)->GetResidueInsertionCode() << std::left
                       << std::setw(1) << " " << std::right << std::setw(3) << strand_residues.at(1)->GetResidueName()
                       << std::left << std::setw(1) << " " << std::right << std::setw(1)
                       << strand_residues.at(1)->GetResidueChainId();
                ...
                stream << std::left << std::setw(2) << " " << std::left << std::setw(3) << strand->GetCurrentAtom()
                       << std::right << std::setw(3) << strand_residues.at(2)->GetResidueName() << std::left
                       << std::setw(1) << " " << std::right << std::setw(1)
                       << strand_residues.at(2)->GetResidueChainId();
                ...
                stream << std::right << std::setw(1) << strand_residues.at(2)->GetResidueInsertionCode() << std::left
                       << std::setw(1) << " " << std::left << std::setw(4) << strand->GetPreviousAtom() << std::right
                       << std::setw(3) << strand_residues.at(3)->GetResidueName() << std::left << std::setw(1) << " "
                ...
                stream << std::right << std::setw(1) << strand_residues.at(3)->GetResidueInsertionCode() << std::left
                       << std::setw(10) << " " << std::endl;
            }
            else
            {
                ...
                stream << std::left << std::setw(40) << " " << std::endl;
            }
            serial_number++;
        }
    }
...
{
    PdbFileSpace::PdbDisulfideBondSection::DisulfideResidueBondMap disulfide_bond_map =
        disulfide_bonds_->GetDisulfideResidueBonds();
    for (PdbFileSpace::PdbDisulfideBondSection::DisulfideResidueBondMap::iterator it = disulfide_bond_map.begin();
         it != disulfide_bond_map.end(); it++)
    {
        ...
        stream << std::left << std::setw(6) << disulfide_bonds_->GetRecordName() << std::left << std::setw(1) << " ";
        ...
        stream << std::left << std::setw(1) << " " << std::right << std::setw(3)
               << disulfide_bonds_residues.at(0)->GetResidueName() << std::left << std::setw(1) << " " << std::right
               << std::setw(1) << disulfide_bonds_residues.at(0)->GetResidueChainId() << std::left << std::setw(1)
               << " ";
        ...
        stream << std::right << std::setw(1) << disulfide_bonds_residues.at(0)->GetResidueInsertionCode() << std::left
               << std::setw(3) << " " << std::right << std::setw(3) << disulfide_bonds_residues.at(1)->GetResidueName()
               << std::left << std::setw(1) << " " << std::right << std::setw(1)
               << disulfide_bonds_residues.at(1)->GetResidueChainId() << std::left << std::setw(1) << " ";
        ...
        stream << std::right << std::setw(1) << disulfide_bonds_residues.at(1)->GetResidueInsertionCode() << std::left
               << std::setw(23) << " ";
        ...
        {
            stream << std::right << std::setw(5) << std::fixed << std::setprecision(2)
                   << disulfide_bonds->GetBondLength();
        }
        else
        {
            stream << std::right << std::setw(5) << " ";
        }
        stream << std::left << std::setw(2) << " " << std::endl;
    }
}
...
void PdbFile::ResolveLinkCards(std::ostream& stream)
{
    ...
    for (PdbFileSpace::PdbLinkSection::LinkCardVector::iterator it = links.begin(); it != links.end(); it++)
    {
        ...
        stream << std::left << std::setw(6) << link_cards_->GetRecordName() << std::left << std::setw(6) << " "
        ...
        stream << std::right << std::setw(3) << link_residues.at(0)->GetResidueName() << std::left << std::setw(1)
               << " ";
        ...
        stream << std::left << std::setw(15) << " " << std::left << std::setw(4) << link_residues.at(1)->GetAtomName();
        ...
        stream << std::right << std::setw(3) << link_residues.at(1)->GetResidueName() << std::left << std::setw(1)
               << " ";
...
void PdbFile::ResolveCISPeptideCards(std::ostream& stream)
{
    ...
    {
        stream << std::left << std::setw(6) << (*it)->GetRecordName() << std::left << std::setw(1) << " " << std::right
               << std::setw(3) << (*it)->GetSerialNumber() << std::left << std::setw(1) << " " << std::left
               << std::setw(3) << (*it)->GetPeptide1ResidueName() << std::left << std::setw(1) << " " << std::left
               << std::setw(1) << (*it)->GetPeptide1ChainId() << std::left << std::setw(1) << " " << std::right
               << std::setw(4) << (*it)->GetPeptide1SequenceNumber() << std::left << std::setw(1)
               << (*it)->GetPeptide1InsertionCode() << std::left << std::setw(3) << " " << std::left << std::setw(3)
               << (*it)->GetPeptide2ResidueName() << std::left << std::setw(1) << " " << std::left << std::setw(1)
               << (*it)->GetPeptide2ChainId() << std::left << std::setw(1) << " " << std::right << std::setw(4)
               << (*it)->GetPeptide2SequenceNumber() << std::left << std::setw(1) << (*it)->GetPeptide2InsertionCode()
               << std::left << std::setw(7) << " " << std::right << std::setw(3) << (*it)->GetModelNumber() << std::left
               << std::setw(7) << " " << std::right << std::setw(6) << (*it)->GetMeasure() << std::endl;
    }
}
    ...
    for (PdbFileSpace::PdbSiteSection::PdbSiteCardMap::iterator it = site_map.begin(); it != site_map.end(); it++)
    {
        ...
        if (number_of_residues > MAX_RESIDUE_IN_LINE)
        {
            ...
            while (sequence_number <= number_of_lines)
            {
                ...
                {
                    std::stringstream ss;
                    for (PdbSiteCard::SiteResidueVector::iterator it1 =
                             site_residues.begin() + (sequence_number - 1) * MAX_RESIDUE_IN_LINE;
                         it1 != site_residues.begin() + (sequence_number)*MAX_RESIDUE_IN_LINE; it1++)
                    {
                        PdbSiteResidue* residue = (*it1);
                        ss << std::left << std::setw(1) << " " << std::right << std::setw(3)
                           << residue->GetResidueName() << std::left << std::setw(1) << " " << std::right
                           << std::setw(1) << residue->GetResidueChainId();
                    ...
                    stream << std::left << std::setw(6) << site_cards_->GetRecordName() << std::left << std::setw(1)
                           << " " << std::right << std::setw(3) << sequence_number << std::left << std::setw(1) << " "
                           << std::right << std::setw(3) << site->GetSiteId() << std::left << std::setw(1) << " ";
                    ...
                    stream << std::left << std::setw(63) << ss.str() << std::endl;
                }
                else
                {
                    std::stringstream ss;
                    ...
                    {
                        ss << std::left
                           << std::setw((sequence_number * MAX_RESIDUE_IN_LINE - number_of_residues) *
                                        RESIDUE_LENGHT_IN_LINE)
                           << " ";
                    }
                    ss << std::left << std::setw(19) << " ";
                    stream << std::left << std::setw(6) << site_cards_->GetRecordName() << std::left << std::setw(1)
        ...
        {
            std::stringstream ss;
            for (PdbSiteCard::SiteResidueVector::iterator it1 = site_residues.begin(); it1 != site_residues.end();
                 it1++)
            {
                PdbSiteResidue* residue = (*it1);
                ss << std::left << std::setw(1) << " " << std::right << std::setw(3) << residue->GetResidueName()
                   << std::left << std::setw(1) << " " << std::right << std::setw(1) << residue->GetResidueChainId();
                if (residue->GetResidueSequenceNumber() != gmml::iNotSet)
                {
                    ss << std::right << std::setw(4) << residue->GetResidueSequenceNumber();
                }
            ...
            stream << std::left << std::setw(6) << site_cards_->GetRecordName() << std::left << std::setw(1) << " "
                   << std::right << std::setw(3) << sequence_number << std::left << std::setw(1) << " " << std::right
                   << std::setw(3) << site->GetSiteId() << std::left << std::setw(1) << " ";
            if (site->GetNumberOfResidues() != gmml::iNotSet)
            {
                stream << std::right << std::setw(2) << site->GetNumberOfResidues();
            }
...
void PdbFile::ResolveCrystallographyCard(std::ostream& stream)
{
    ...
    stream << std::left << std::setw(1) << " " << std::left << std::setw(11) << crystallography_->GetSpaceGroup();
    ...
    stream << std::left << std::setw(10) << " " << std::endl;
}
...
void PdbFile::ResolveOriginCard(std::ostream& stream)
{
    ...
    for (PdbFileSpace::PdbOriginXnSection::OriginXnCardVector::iterator it = origins.begin(); it != origins.end(); it++)
    {
        ...
        stream << std::left << std::setw(6) << ss.str() << std::left << std::setw(4) << " ";
        if (origin->GetOrigin().CompareTo(GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) ==
            false)
        {
        ...
        {
            stream << std::right << std::setw(10) << " " << std::right << std::setw(10) << " " << std::right
                   << std::setw(10) << " ";
        }
        ...
        stream << std::left << std::setw(25) << " " << std::endl;
    }
}
...
void PdbFile::ResolveScaleCard(std::ostream& stream)
{
    ...
    for (PdbFileSpace::PdbScaleNSection::ScaleNCardVector::iterator it = scales.begin(); it != scales.end(); it++)
    {
        ...
        stream << std::left << std::setw(6) << ss.str() << std::left << std::setw(4) << " ";
        if (scale->GetScaleVector().CompareTo(
                GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) == false)
        {
            stream << std::right << std::setw(10) << std::fixed << std::setprecision(6)
                   << scale->GetScaleVector().GetX() << std::right << std::setw(10) << std::fixed
                   << std::setprecision(6) << scale->GetScaleVector().GetY() << std::right << std::setw(10)
                   << std::fixed << std::setprecision(6) << scale->GetScaleVector().GetZ();
        }
        else
        {
            stream << std::right << std::setw(10) << " " << std::right << std::setw(10) << " " << std::right
...
void PdbFile::ResolveMatrixCards(std::ostream& stream)
{
    ...
    for (int i = 0; i < number_of_matrix_entries; i++)
    {
        for (unsigned int j = 0; j < 3; j++)
        {
            ...
            stream << std::left << std::setw(6) << ss.str() << std::left << std::setw(1) << " ";
            ...
            if (matrix->GetTransformationVector().CompareTo(
                    GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) == false)
            {
                stream << std::right << std::setw(10) << std::fixed << std::setprecision(6)
                       << matrix->GetTransformationVector().GetX() << std::right << std::setw(10) << std::fixed
                       << std::setprecision(6) << matrix->GetTransformationVector().GetY() << std::right
                       << std::setw(10) << std::fixed << std::setprecision(6)
                       << matrix->GetTransformationVector().GetZ();
            }
            else
            {
                stream << std::right << std::setw(10) << " " << std::right << std::setw(10) << " " << std::right
            ...
            stream << std::left << std::setw(20) << " " << std::endl;
        }
    }
}
...
void PdbFile::ResolveModelCards(std::ostream& stream)
{
    ...
    if (number_of_models == 1)
    {
        for (PdbFileSpace::PdbModelSection::PdbModelCardMap::iterator it = models.begin(); it != models.end(); it++)
        {
            ...
            PdbModelResidueSet* residue_set                     = model->GetModelResidueSet();
            PdbModelResidueSet::PdbAtomSectionVector atom_cards = residue_set->GetAtomCards();
            for (PdbModelResidueSet::PdbAtomSectionVector::iterator it1 = atom_cards.begin(); it1 != atom_cards.end();
                 it1++)
            {
                PdbAtomSection* atom_card                            = (*it1);
                int serial_number                                    = 0;
                ...
                for (PdbAtomSection::PdbAtomCardOrderVector::iterator it2 = ordered_atoms.begin();
                     it2 != ordered_atoms.end(); it2++)
                {
                    PdbFileSpace::PdbAtomCard* atom = (*it2);
                    ...
                    stream << std::left << std::setw(1) << " " << std::left << std::setw(4) << atom->GetAtomName();
                    ...
                    stream << std::right << std::setw(3) << atom->GetAtomResidueName() << std::left << std::setw(1)
                           << " ";
                    ...
                    if (atom->GetAtomOrthogonalCoordinate().CompareTo(
                            GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) == false)
                    {
                        stream << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                               << atom->GetAtomOrthogonalCoordinate().GetX() << std::right << std::setw(8) << std::fixed
                               << std::setprecision(3) << atom->GetAtomOrthogonalCoordinate().GetY() << std::right
                               << std::setw(8) << std::fixed << std::setprecision(3)
                               << atom->GetAtomOrthogonalCoordinate().GetZ();
                    }
                    else
                    {
                        stream << std::right << std::setw(8) << " " << std::right << std::setw(8) << " " << std::right
                               << std::setw(8) << " ";
                    }
                    if (atom->GetAtomOccupancy() != gmml::dNotSet)
                    {
                        stream << std::right << std::setw(6) << std::fixed << std::setprecision(2)
                               << atom->GetAtomOccupancy();
                    }
                    else
                    {
                        stream << std::right << std::setw(6) << " ";
                    ...
                    {
                        stream << std::right << std::setw(6) << std::fixed << std::setprecision(2)
                               << atom->GetAtomTempretureFactor();
                    }
                    else
                    {
                        stream << std::right << std::setw(6) << " ";
                    }
                    stream << std::left << std::setw(10) << " " << std::right << std::setw(2)
                           << atom->GetAtomElementSymbol() << std::left << std::setw(2) << atom->GetAtomCharge()
                           << std::endl;
                ...
                if (atoms_size != 0)
                {
                    ...
                    stream << std::left << std::setw(6) << " " << std::right << std::setw(3) << residue_name
                           << std::left << std::setw(1) << " ";
                    ...
                    stream << std::left << std::setw(1) << insertion_code << std::left << std::setw(53) << " "
                           << std::endl;
                }
            }
            PdbModelResidueSet::HeterogenAtomSectionVector heterogen_atom_cards = residue_set->GetHeterogenAtomCards();
                 ...
                 it1 != heterogen_atom_cards.end(); it1++)
            {
                ...
                PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector ordered_heterogen_atoms =
                    heterogen_atom_card->GetOrderedHeterogenAtomCards();
                for (PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector::iterator it2 =
                         ordered_heterogen_atoms.begin();
                     it2 != ordered_heterogen_atoms.end(); it2++)
                {
                    ...
                    stream << std::left << std::setw(1) << " " << std::left << std::setw(4)
                           << heterogen_atom->GetAtomName();
                    // stream << std::left << std::setw(2) << " "
                    //<< std::left << std::setw(3) << heterogen_atom->GetAtomName(); //Temporarily commenting out the
                    // correct line to write out an incorrect file, which ADT needs.
                    ...
                    stream << std::right << std::setw(3) << heterogen_atom->GetAtomResidueName() << std::left
                           << std::setw(1) << " ";
                    ...
                    stream << std::left << std::setw(3) << " ";
                    if (heterogen_atom->GetAtomOrthogonalCoordinate().CompareTo(
                            GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) == false)
                    {
                        stream << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                               << heterogen_atom->GetAtomOrthogonalCoordinate().GetX() << std::right << std::setw(8)
                               << std::fixed << std::setprecision(3)
                               << heterogen_atom->GetAtomOrthogonalCoordinate().GetY() << std::right << std::setw(8)
                               << std::fixed << std::setprecision(3)
                               << heterogen_atom->GetAtomOrthogonalCoordinate().GetZ();
                    }
                    else
                    {
                        stream << std::right << std::setw(8) << " " << std::right << std::setw(8) << " " << std::right
                    ...
                    {
                        stream << std::right << std::setw(6) << std::fixed << std::setprecision(2)
                               << heterogen_atom->GetAtomOccupancy();
                    }
                    else
                    {
                        stream << std::right << std::setw(6) << " ";
                    ...
                    {
                        stream << std::right << std::setw(6) << std::fixed << std::setprecision(2)
                               << heterogen_atom->GetAtomTempretureFactor();
                    }
                    else
                    {
                        stream << std::right << std::setw(6) << " ";
                    ...
                    stream << std::left << std::setw(10) << " " << std::right << std::setw(2)
                           << heterogen_atom->GetAtomElementSymbol() << std::left << std::setw(2)
                           << heterogen_atom->GetAtomCharge() << std::endl;
                }
            }
        }
    }
    else
    {
        ...
        {
            PdbModelCard* model = (*it).second;
            stream << std::left << std::setw(6) << models_->GetRecordName() << std::left << std::setw(4) << " ";
            ...
            stream << std::left << std::setw(6) << "ENDMDL" << std::left << std::setw(74) << " " << std::endl;
        }
    }
}
    ...
    if (models.size() != 0)
    {
             ...
             it1++)
        {
            ...
            for (PdbAtomSection::PdbAtomMap::iterator it2 = atoms.begin(); it2 != atoms.end(); it2++)
            {
                ...
                {
                    for (std::vector<PdbFileSpace::PdbAtomCard*>::iterator thisAltAtom = alternate_atom_cards.begin();
                         thisAltAtom != alternate_atom_cards.end(); thisAltAtom++)
                    {
                        atom = *thisAltAtom;
                        stream << std::left << std::setw(6) << atom_card->GetRecordName();
                        if (atom->GetAtomSerialNumber() != gmml::iNotSet)
                        ...
                        {
                            stream << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                                   << atom->GetAtomOrthogonalCoordinate().GetX() << std::right << std::setw(8)
                                   << std::fixed << std::setprecision(3) << atom->GetAtomOrthogonalCoordinate().GetY()
                                   << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                                   << atom->GetAtomOrthogonalCoordinate().GetZ();
                        }
                        else
                        {
                            stream << std::right << std::setw(8) << " " << std::right << std::setw(8) << " "
            ...
            if (atoms_size != 0)
            {
                ...
                stream << std::left << std::setw(6) << " " << std::right << std::setw(3) << residue_name << std::left
                       << std::setw(1) << " ";
                if (chain_id == gmml::BLANK_SPACE)
                {
                    stream << std::left << std::setw(1) << " ";
                ...
                stream << std::left << std::setw(1) << insertion_code << std::left << std::setw(53) << " " << std::endl;
            }
        }
        PdbModelResidueSet::HeterogenAtomSectionVector heterogen_atom_cards = residue_set->GetHeterogenAtomCards();
        for (PdbModelResidueSet::HeterogenAtomSectionVector::iterator it1 = heterogen_atom_cards.begin();
        ...
        {
            PdbHeterogenAtomSection* heterogen_atom_card = (*it1);
            PdbHeterogenAtomSection::PdbHeterogenAtomCardMap heterogen_atoms =
                heterogen_atom_card->GetHeterogenAtomCards();
            for (PdbHeterogenAtomSection::PdbHeterogenAtomCardMap::iterator it2 = heterogen_atoms.begin();
                 it2 != heterogen_atoms.end(); it2++)
            {
                        ...
                        GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) == false)
                {
                    stream << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                           << heterogen_atom->GetAtomOrthogonalCoordinate().GetX() << std::right << std::setw(8)
                           << std::fixed << std::setprecision(3) << heterogen_atom->GetAtomOrthogonalCoordinate().GetY()
                           << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                           << heterogen_atom->GetAtomOrthogonalCoordinate().GetZ();
                }
                else
...
{
    PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap bonded_atoms =
        connectivities_->GetBondedAtomsSerialNumbers();
    for (PdbFileSpace::PdbConnectSection::BondedAtomsSerialNumbersMap::iterator it = bonded_atoms.begin();
         it != bonded_atoms.end(); it++)
    {
        ...
        if (number_of_bonded_atoms <= MAX_SERIAL_NUMBER_IN_LINE)
        {
            ...
            for (std::vector<int>::iterator it1 = bonded_atoms_serial_number.begin();
            ...
            {
                stream << std::left
                       << std::setw((MAX_SERIAL_NUMBER_IN_LINE - number_of_bonded_atoms) * SERIAL_NUMBER_LENGTH) << " "
                       << std::left << std::setw(49) << " " << std::endl;
            }
            else
            {
                stream << std::left << std::setw(49) << " " << std::endl;
            }
            ...
            for (int i = 1; i <= number_of_lines; i++)
            {
                ...
                if (i != number_of_lines)
                {
                    for (std::vector<int>::iterator it1 =
                             bonded_atoms_serial_number.begin() + (i - 1) * MAX_SERIAL_NUMBER_IN_LINE;
                ...
                else
                {
                    ...
                    if ((MAX_SERIAL_NUMBER_IN_LINE - (number_of_bonded_atoms - (i - 1) * MAX_SERIAL_NUMBER_IN_LINE)) *
...
{
    stream << std::left << std::setw(6) << master_->GetRecordName() << std::right << std::setw(4) << " " << std::right
           << std::setw(5) << master_->GetNumRemark() << std::right << std::setw(5) << 0 << std::right << std::setw(5)
           << master_->GetNumHet() << std::right << std::setw(5) << master_->GetNumHelix() << std::right << std::setw(5)
           << master_->GetNumSheet() << std::right << std::setw(5) << 0 << std::right << std::setw(5)
           << master_->GetNumSite() << std::right << std::setw(5) << master_->GetNumXForm() << std::right
           << std::setw(5) << master_->GetNumCoord() << std::right << std::setw(5) << master_->GetNumTer() << std::right
           << std::setw(5) << master_->GetNumConnect() << std::right << std::setw(5) << master_->GetNumSeq()
           << std::endl;
}
