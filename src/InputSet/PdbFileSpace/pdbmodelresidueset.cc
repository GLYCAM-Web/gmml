    ...
    while (!gmml::Trim(temp).empty())
    {
        ...
        while (line.find("ATOM") != std::string::npos || line.find("ANISOU") != std::string::npos)
        {
            ...
            while (line.find("ATOM") != std::string::npos)
            {
                atom_block
                    << line
                    << std::endl; /// Append all lines of ATOM section to create a block of stream of each atom set
        ...
        while (line.find("HETATM") != std::string::npos || line.find("ANISOU") != std::string::npos)
        {
            heterogen_atom_block << line << std::endl; /// Append all lines of HETATM section to create a block of
                                                       /// stream of each heterogen atom set
            getline(residue_set_block, line);          /// Read next line
            temp = line;
            /// Discard ANISOU section
            while (line.find("ANISOU") != std::string::npos)
        ...
        if (gmml::Trim(temp).empty())
        {
            ...
            PdbHeterogenAtomSection* heterogen_atoms =
                new PdbHeterogenAtomSection(heterogen_atom_block, card_index.str());
...
{
    return atomSectionVector_;
}

PdbModelResidueSet::HeterogenAtomSectionVector PdbModelResidueSet::GetHeterogenAtomCards()
{
    return heterogenAtomsSectionVeector_;
}
...
{
    atomSectionVector_.clear();
    for (PdbAtomSectionVector::iterator it = atoms.begin(); it != atoms.end(); it++)
    {
        atomSectionVector_.push_back(*it);
    }
}

void PdbModelResidueSet::AddAtom(PdbAtomSection* atom)
{
    atomSectionVector_.push_back(atom);
}

void PdbModelResidueSet::SetHeterogenAtoms(PdbModelResidueSet::HeterogenAtomSectionVector heterogen_atoms)
{
    heterogenAtomsSectionVeector_.clear();
    for (HeterogenAtomSectionVector::iterator it = heterogen_atoms.begin(); it != heterogen_atoms.end(); it++)
    {
        heterogenAtomsSectionVeector_.push_back(*it);
    }
}

void PdbModelResidueSet::AddHeterogenAtom(PdbHeterogenAtomSection* heterogen_atom)
{
    heterogenAtomsSectionVeector_.push_back(heterogen_atom);
}
...
{
    out << "----------------- Atoms -------------" << std::endl;
    for (PdbModelResidueSet::PdbAtomSectionVector::iterator it = atomSectionVector_.begin();
         it != atomSectionVector_.end(); it++)
    {
        (*it)->Print(out);
    }
    out << std::endl << "-------------- Heterogen Atoms --------------" << std::endl;
    for (PdbModelResidueSet::HeterogenAtomSectionVector::iterator it = heterogenAtomsSectionVeector_.begin();
         it != heterogenAtomsSectionVeector_.end(); it++)
    {
        (*it)->Print(out);
    }
    out << std::endl;
