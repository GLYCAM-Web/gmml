    ...
    if (models.size() != 0)
    {
             ...
             it1++)
        {
                 ...
                 it2 != ordered_atoms.end(); it2++)
            {
                ...
                sss << residue_name << "_" << chain_id << "_" << atom_sequence_number << "_" << insertion_code << "_"
                    << alternate_location;
        ...
        PdbModelResidueSet::HeterogenAtomSectionVector updated_heterogen_atom_cards =
            PdbModelResidueSet::HeterogenAtomSectionVector();
             ...
             it1 != heterogen_atom_cards.end(); it1++)
        {
            ...
            PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector updated_heterogen_atoms_vector =
                PdbHeterogenAtomSection::PdbHeterogenAtomOrderVector();
                 ...
                 it2 != ordered_heterogen_atoms.end(); it2++)
            {
                ...
                char alternate_location            = gmml::BLANK_SPACE;
                std::stringstream sss;
                sss << residue_name << "_" << chain_id << "_" << heterogen_atom_sequence_number << "_" << insertion_code
                    << "_" << alternate_location;
                std::string key = sss.str();
                if (pdb_residue_map.find(key) == pdb_residue_map.end())
                {
                    serial_number_mapping_[atom->GetAtomSerialNumber()] = serial_number;
...
{
    std::map<std::string, PdbFileSpace::PdbAtomCard*> pdb_atom_map =
        std::map<std::string, PdbFileSpace::PdbAtomCard*>();
...
void PdbFile::UpdateResidueNameWithTheGivenModelNumber(PdbResidue* residue, std::string updated_residue_name,
                                                       int model_number)
{
    std::string target_residue_name        = residue->GetResidueName();
    char target_residue_chain_id           = residue->GetResidueChainId();
...
void PdbFile::InsertResidueBeforeWithTheGivenModelNumber(PdbAtomSection* residue, int model_number)
{
    ...
    if (models.size() != 0)
    {
        ...
        int offset          = 0;
        PdbFileSpace::PdbAtomCard *cAtomInProtein, *nAtomInProtein,
            *caAtomInProtein; // OG Dec 21 Sorry about the warnings.
        for (PdbModelResidueSet::PdbAtomSectionVector::iterator it1 = atom_cards.begin(); it1 != atom_cards.end();
             it1++)
        {
            PdbAtomSection* atom_card         = (*it1);
                 ...
                 it2 != ordered_atoms.end(); it2++)
            {
                    ...
                    residue_sequence_number == atom->GetAtomResidueSequenceNumber())
                {
                    ...
                    while ((fromHere != ordered_atoms.end()) &&
                           ((*fromHere)->GetAtomResidueSequenceNumber() == residue_sequence_number) &&
                           (located == true))
                    {
                    ...
                    if (it2 != ordered_atoms.begin())
                    {
                        ...
                        {
                            int diff =
                                atom->GetAtomResidueSequenceNumber() - previous_atom->GetAtomResidueSequenceNumber();
                            if (diff == 1)
                            {
                                sequence_number++;
                            }
                    ...
                    if (located)
                    {
                        sequence_number =
                            ...
                            1; // OG edit: Single gaps will end up with the same ACE NME resid numbers. Otherwise good.
                        GeometryTopology::CoordinateVector coordinate_set = GeometryTopology::CoordinateVector();
                        GeometryTopology::Coordinate* cCoordProtein =
                            new GeometryTopology::Coordinate(cAtomInProtein->GetAtomOrthogonalCoordinate());
                        GeometryTopology::Coordinate* caCoordProtein =
                            new GeometryTopology::Coordinate(caAtomInProtein->GetAtomOrthogonalCoordinate());
                        GeometryTopology::Coordinate* nCoordProtein =
                            new GeometryTopology::Coordinate(nAtomInProtein->GetAtomOrthogonalCoordinate());
                        // This is a bad idea, but it works. Better to load in the template and align it, but the
                        // support code didn't work and I'm a quitter.
                        ...
                        GeometryTopology::Coordinate oCoordACE =
                            GeometryTopology::get_cartesian_point_from_internal_coords(caCoordProtein, nCoordProtein,
                                                                                       cCoordACE, 120.0, 0.0, 1.23);
                        GeometryTopology::Coordinate ch3CoordACE =
                            GeometryTopology::get_cartesian_point_from_internal_coords(caCoordProtein, nCoordProtein,
                                                                                       cCoordACE, 125.0, 180.0, 1.48);
                        GeometryTopology::Coordinate hh31CoordACE =
                            GeometryTopology::get_cartesian_point_from_internal_coords(oCoordACE, cCoordACE,
                                                                                       ch3CoordACE, 109.0, 180.0, 1.09);
                        GeometryTopology::Coordinate hh32CoordACE =
                            GeometryTopology::get_cartesian_point_from_internal_coords(oCoordACE, cCoordACE,
                                                                                       ch3CoordACE, 109.0, 60.0, 1.09);
                        GeometryTopology::Coordinate hh33CoordACE =
                            GeometryTopology::get_cartesian_point_from_internal_coords(oCoordACE, cCoordACE,
                                                                                       ch3CoordACE, 109.0, -60.0, 1.09);
                        ...
                        for (PdbAtomSection::PdbAtomCardOrderVector::iterator it3 = ordered_atoms_of_residue.begin();
                             it3 != ordered_atoms_of_residue.end(); it3++)
                        {
                            ...
                            PdbFileSpace::PdbAtomCard* new_atom = new PdbFileSpace::PdbAtomCard(
                                serial_number, atom_of_residue->GetAtomName(),
                                atom_of_residue->GetAtomAlternateLocation(), atom_of_residue->GetAtomResidueName(),
                                atom_of_residue->GetAtomChainId(), sequence_number,
                        ...
                        gmml::log(__LINE__, __FILE__, gmml::INF,
                                  "Created ACE residue numbered: " + std::to_string(sequence_number));
                    ...
                    if (!located)
                    {
                        PdbFileSpace::PdbAtomCard* updated_atom = new PdbFileSpace::PdbAtomCard(
                            serial_number, atom->GetAtomName(), atom->GetAtomAlternateLocation(),
                            atom->GetAtomResidueName(), atom->GetAtomChainId(), sequence_number,
                            atom->GetAtomInsertionCode(), atom->GetAtomOrthogonalCoordinate(), atom->GetAtomOccupancy(),
                            atom->GetAtomTempretureFactor(), atom->GetAtomElementSymbol(), atom->GetAtomCharge(),
                            atom->GetAlternateAtomCards());
             ...
             it1 != heterogen_atom_cards.end(); it1++)
        {
                 ...
                 it2 != ordered_heterogen_atoms.end(); it2++)
            {
                ...
                PdbFileSpace::PdbAtomCard* updated_heterogen_atom = new PdbFileSpace::PdbAtomCard(
                    serial_number, heterogen_atom->GetAtomName(), heterogen_atom->GetAtomAlternateLocation(),
                    heterogen_atom->GetAtomResidueName(), heterogen_atom->GetAtomChainId(),
                    heterogen_atom->GetAtomResidueSequenceNumber(), heterogen_atom->GetAtomInsertionCode(),
                    heterogen_atom->GetAtomOrthogonalCoordinate(), heterogen_atom->GetAtomOccupancy(),
                    heterogen_atom->GetAtomTempretureFactor(), heterogen_atom->GetAtomElementSymbol(),
                    heterogen_atom->GetAtomCharge(), heterogen_atom->GetAlternateAtomCards());
...
void PdbFile::InsertResidueAfterWithTheGivenModelNumber(PdbAtomSection* residue, int model_number)
{
    ...
    gmml::log(__LINE__, __FILE__, gmml::INF,
              "InsertResidueAfterWithTheGivenModelNumber() aka NME inserter was called.");
    PdbFileSpace::PdbModelSection::PdbModelCardMap models = models_->GetModels();
    if (models.size() == 0)
    {
        return;
    ...
    for (PdbModelResidueSet::PdbAtomSectionVector::iterator it1 = atom_cards.begin(); it1 != atom_cards.end(); it1++)
    {
        ...
        for (PdbAtomSection::PdbAtomCardOrderVector::iterator it2 = ordered_atoms.begin(); it2 != ordered_atoms.end();
             it2++)
        {
            PdbFileSpace::PdbAtomCard* atom = (*it2);
            ...
            if ((residue_chain_id == atom->GetAtomChainId()) &&
                (residue_sequence_number == atom->GetAtomResidueSequenceNumber()) &&
                (atom->GetAtomResidueName() != "ACE")) // this code is making me commit crimes against humanity.
            {
                std::stringstream ss;
                ss << residue_chain_id << "_" << residue_sequence_number << "_" << atom->GetAtomResidueName();
                ...
                PdbFileSpace::PdbAtomCard* updated_atom = new PdbFileSpace::PdbAtomCard(
                    serial_number, atom->GetAtomName(), atom->GetAtomAlternateLocation(), atom->GetAtomResidueName(),
                    atom->GetAtomChainId(), sequence_number, atom->GetAtomInsertionCode(),
                    atom->GetAtomOrthogonalCoordinate(), atom->GetAtomOccupancy(), atom->GetAtomTempretureFactor(),
                    atom->GetAtomElementSymbol(), atom->GetAtomCharge(), atom->GetAlternateAtomCards());
                updated_atoms[serial_number] = updated_atom;
                updated_atoms_vector.push_back(updated_atom);
                serial_number++;
                if (sequence_number != atom->GetAtomResidueSequenceNumber())
            ...
            else
            {
                if (located)
                {
                    GeometryTopology::CoordinateVector coordinate_set =
                        GeometryTopology::CoordinateVector(); // These are pointers to newly created copies!
                    GeometryTopology::Coordinate* cAtomInProteinCoord =
                        new GeometryTopology::Coordinate(cAtomInProtein->GetAtomOrthogonalCoordinate());
                    GeometryTopology::Coordinate* caAtomInProteinCoord =
                        new GeometryTopology::Coordinate(caAtomInProtein->GetAtomOrthogonalCoordinate());
                    GeometryTopology::Coordinate* oAtomInProteinCoord =
                        new GeometryTopology::Coordinate(oAtomInProtein->GetAtomOrthogonalCoordinate());
                    // This is a bad idea, but it works. Better to load in the template and align it, but the support
                    // code didn't work.
                    GeometryTopology::Coordinate bcoordOfNAtomInNME =
                        GeometryTopology::get_cartesian_point_from_internal_coords(
                            oAtomInProteinCoord, caAtomInProteinCoord, cAtomInProteinCoord, 120.0, 180.0, 1.4);
                    coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfNAtomInNME));
                    GeometryTopology::Coordinate bcoordOfHAtomInNME =
                        GeometryTopology::get_cartesian_point_from_internal_coords(
                            oAtomInProteinCoord, cAtomInProteinCoord, bcoordOfNAtomInNME, 109.0, 180.0, 1.0);
                    coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfHAtomInNME));
                    GeometryTopology::Coordinate bcoordOfCH3AtomInNME =
                        GeometryTopology::get_cartesian_point_from_internal_coords(
                            caAtomInProteinCoord, cAtomInProteinCoord, bcoordOfNAtomInNME, 125.0, 180.0, 1.48);
                    coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfCH3AtomInNME));
                    GeometryTopology::Coordinate bcoordOfHH31AtomInNME =
                        GeometryTopology::get_cartesian_point_from_internal_coords(
                            bcoordOfHAtomInNME, bcoordOfNAtomInNME, bcoordOfCH3AtomInNME, 109.0, 180.0, 1.09);
                    coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfHH31AtomInNME));
                    GeometryTopology::Coordinate bcoordOfHH32AtomInNME =
                        GeometryTopology::get_cartesian_point_from_internal_coords(
                            bcoordOfHAtomInNME, bcoordOfNAtomInNME, bcoordOfCH3AtomInNME, 109.0, 60.0, 1.09);
                    coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfHH32AtomInNME));
                    GeometryTopology::Coordinate bcoordOfHH33AtomInNME =
                        GeometryTopology::get_cartesian_point_from_internal_coords(
                            bcoordOfHAtomInNME, bcoordOfNAtomInNME, bcoordOfCH3AtomInNME, 109.0, -60.0, 1.09);
                         ...
                         it3 != ordered_atoms_of_residue.end(); it3++)
                    {
                        ...
                        int index                                  = distance(ordered_atoms_of_residue.begin(), it3);
                        PdbFileSpace::PdbAtomCard* new_atom        = new PdbFileSpace::PdbAtomCard(
                            serial_number, atom_of_residue->GetAtomName(), atom_of_residue->GetAtomAlternateLocation(),
                    ...
                    gmml::log(__LINE__, __FILE__, gmml::INF,
                              "Created NME residue numbered: " + std::to_string(sequence_number));
                    sequence_number++;
                    // offset++;
                    located = false;
                }
            ...
            if (it2 == --ordered_atoms.end() &&
                located) // Note that the below no longer uses it2, I've made the spaghetti worse.
            {
                GeometryTopology::CoordinateVector coordinate_set =
                    GeometryTopology::CoordinateVector(); // These are pointers to newly created copies!
                GeometryTopology::Coordinate* cAtomInProteinCoord =
                ...
                GeometryTopology::Coordinate* caAtomInProteinCoord =
                    new GeometryTopology::Coordinate(caAtomInProtein->GetAtomOrthogonalCoordinate());
                GeometryTopology::Coordinate* nAtomInProteinCoord =
                    new GeometryTopology::Coordinate(oAtomInProtein->GetAtomOrthogonalCoordinate());
                // This is a bad idea, but it works. Better to load in the template and align it, but the support code
                // didn't work.
                GeometryTopology::Coordinate bcoordOfNAtomInNME =
                    GeometryTopology::get_cartesian_point_from_internal_coords(
                        nAtomInProteinCoord, caAtomInProteinCoord, cAtomInProteinCoord, 120.0, 140.0, 1.4);
                coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfNAtomInNME));
                GeometryTopology::Coordinate bcoordOfHAtomInNME =
                    GeometryTopology::get_cartesian_point_from_internal_coords(
                        caAtomInProteinCoord, cAtomInProteinCoord, bcoordOfNAtomInNME, 109.0, 0.0, 1.0);
                coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfHAtomInNME));
                GeometryTopology::Coordinate bcoordOfCH3AtomInNME =
                    GeometryTopology::get_cartesian_point_from_internal_coords(
                        caAtomInProteinCoord, cAtomInProteinCoord, bcoordOfNAtomInNME, 125.0, 180.0, 1.48);
                ...
                coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfHH31AtomInNME));
                GeometryTopology::Coordinate bcoordOfHH32AtomInNME =
                    GeometryTopology::get_cartesian_point_from_internal_coords(bcoordOfHAtomInNME, bcoordOfNAtomInNME,
                                                                               bcoordOfCH3AtomInNME, 109.0, 60.0, 1.09);
                coordinate_set.push_back(new GeometryTopology::Coordinate(bcoordOfHH32AtomInNME));
                GeometryTopology::Coordinate bcoordOfHH33AtomInNME =
                    GeometryTopology::get_cartesian_point_from_internal_coords(
                        bcoordOfHAtomInNME, bcoordOfNAtomInNME, bcoordOfCH3AtomInNME, 109.0, -60.0, 1.09);
             ...
             it1++)
        {
                 ...
                 it2 != ordered_atoms.end(); it2++)
            {
                ...
                if (located)
                {
                    ...
                    PdbFileSpace::PdbAtomCard* updated_atom = new PdbFileSpace::PdbAtomCard(
                        serial_number, atom->GetAtomName(), atom->GetAtomAlternateLocation(),
                        atom->GetAtomResidueName(), atom->GetAtomChainId(), atom->GetAtomResidueSequenceNumber(),
                        atom->GetAtomInsertionCode(), atom->GetAtomOrthogonalCoordinate(), atom->GetAtomOccupancy(),
                        atom->GetAtomTempretureFactor(), atom->GetAtomElementSymbol(), atom->GetAtomCharge(),
                        atom->GetAlternateAtomCards());
...
void PdbFile::SplitAtomCardOfModelCardWithTheGivenModelNumber(char split_point_chain_id,
                                                              int split_point_sequence_number, int model_number)
{
    PdbFileSpace::PdbModelSection::PdbModelCardMap models = models_->GetModels();
    if (models.size() != 0)
    {
