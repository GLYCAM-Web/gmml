...
void TopologyFile::AddBond(TopologyBond* bond)
{
    ...
    std::stringstream ss;
    ss << residue_names.at(0) << ":" << bonds.at(0) << "-" << residue_names.at(1) << ":" << bonds.at(1) << "_"
       << bond_type;
    bonds_[ss.str()] = bond;
}
...
void TopologyFile::AddAngle(TopologyAngle* angle)
{
    ...
    std::stringstream ss;
    ss << residue_names.at(0) << ":" << angles.at(0) << "-" << residue_names.at(1) << ":" << angles.at(1) << "-"
       << residue_names.at(2) << ":" << angles.at(2) << "_" << angle_type;
...
void TopologyFile::AddDihedral(TopologyDihedral* dihedral)
{
    ...
    ss << residue_names.at(0) << ":" << dihedrals.at(0) << "-" << residue_names.at(1) << ":" << dihedrals.at(1) << "-"
       << residue_names.at(2) << ":" << dihedrals.at(2) << "-" << residue_names.at(3) << ":" << dihedrals.at(3) << "_"
       << dihedral_type;
...
void TopologyFile::ParseSections(std::ifstream& in_stream)
{
    ...
    {
        bond_types_[i] =
            new TopologyFileSpace::TopologyBondType(i, bond_force_constants.at(i), bond_equil_values.at(i));
    }
    // Angle types
    for (int i = 0; i < number_of_angle_types_; i++)
    {
        angle_types_[i] =
            new TopologyFileSpace::TopologyAngleType(i, angle_force_constants.at(i), angle_equil_values.at(i));
    }
    // Dihedral types
    for (int i = 0; i < number_of_dihedral_types_; i++)
    {
        dihedral_types_[i] = new TopologyFileSpace::TopologyDihedralType(
            i, dihedral_force_constants.at(i), dihedral_periodicities.at(i), dihedral_phases.at(i),
    ...
    {
        //        std::cout << "Lennard Jones coefficients section of the file is not written properly. This section
        //        will be ignored!" << std::endl;
        gmml::log(
            __LINE__, __FILE__, gmml::INF,
            "Lennard Jones coefficients section of the file is not written properly. This section will be ignored!");
    }
    else
    {
        for (int i = 0; i < number_of_atoms_; i++)
        {
            ...
            for (int j = 0; j < number_of_atoms_; j++)
            {
                ...
                int index = nonbonded_parm_indexes.at(number_of_types_ * (atom_type_indexes.at(i) - 1) +
                                                      atom_type_indexes.at(j) - 1);
                if (index > 0)
                {
                ...
                if (pairs_.find(atom_type_1 + "-" + atom_type_2) != pairs_.end() ||
                    pairs_.find(atom_type_2 + "-" + atom_type_1) != pairs_.end())
                {}
                else
                {
                    pairs_[atom_type_1 + "-" + atom_type_2] = new TopologyFileSpace::TopologyAtomPair(
                        atom_type_1 + "-" + atom_type_2, coefficient_a, coefficient_b, index);
    ...
    for (int i = 0; i < number_of_bonds_including_hydrogen_; i++)
    {
        ...
        key << bond_atoms_residue_names.at(0) << ":" << bonds.at(0) << "-" << bond_atoms_residue_names.at(1) << ":"
            << bonds.at(1) << "_" << topology_bond->GetBondType();
    ...
    for (int i = 0; i < number_of_angles_including_hydrogen_; i++)
    {
        ...
        std::stringstream key;
        key << angle_atoms_residue_names.at(0) << ":" << angle_atoms.at(0) << "-" << angle_atoms_residue_names.at(1)
            << ":" << angle_atoms.at(1) << "-" << angle_atoms_residue_names.at(2) << ":" << angle_atoms.at(2) << "_"
            << angle->GetAngleType();
    ...
    for (int i = 0; i < number_of_dihedrals_including_hydrogen_; i++)
    {
        ...
        std::stringstream key;
        key << dihedral_atoms_residue_names.at(0) << ":" << dihedral_atoms.at(0) << "-"
            << dihedral_atoms_residue_names.at(1) << ":" << dihedral_atoms.at(1) << "-"
            << dihedral_atoms_residue_names.at(2) << ":" << dihedral_atoms.at(2) << "-"
            << dihedral_atoms_residue_names.at(3) << ":" << dihedral_atoms.at(3) << "_"
            << dihedral->GetDihedralType()->GetIndex() << "_" << (dihedral->GetIsImproper() ? "IY" : "IN") << "_"
            << (dihedral->GetIgnoredGroupInteraction() ? "GY" : "GN");
        ...
        for (int i = starting_atom_index - 1; i < ending_atom_index - 1; i++)
        {
            ...
            for (int j = start_index; j < start_index + number_excluded_atoms.at(i); j++)
            {
                ...
                excluded_atoms.push_back(excluded_atom_residue_name + ":" + atom_names.at(index) + "(" +
                                         gmml::ConvertT<int>(index + 1) + ")");
            }
            atoms.push_back(new TopologyAtom(i + 1, atom_names.at(i), amber_atom_types.at(i), charges.at(i),
                                             atomic_numbers.at(i), masses.at(i), excluded_atoms,
                                             number_excluded_atoms.at(i), radiis.at(i), screens.at(i),
                                             tree_chain_classifications.at(i), residue_name));
        }
        std::stringstream residue_key;
        residue_key << residue_name << "_" << gmml::BLANK_SPACE << "_" << residue_index;
        residues.push_back(new TopologyResidue(residue_name, atoms, residue_index,
                                               starting_atom_index)); // edited by Ayush on 06/30/17 for Solvent
    }
...
void TopologyFile::ParseTitlePartition(std::stringstream& stream)
{
    ...
    while (!line.empty())
    {
        ...
        if (line[0] == '%')
        {
            if (line.find("%FORMAT") != std::string::npos)
            {
                format = line.substr(line.find_first_of('(', 0) + 1,
                                     line.find_last_of(')', line.length()) - line.find_first_of('(', 0) - 1);
            }
        }
        else
        {
...
template<typename T> std::vector<T> TopologyFile::ParsePartition(std::stringstream& stream)
{
    std::string line;
...
template<typename T> std::vector<T> TopologyFile::PartitionLine(std::string line, std::string format)
{
...
{
    out << "%FLAG TITLE" << std::endl << "%FORMAT(20a4)" << std::endl << title_ << std::endl;
}

void TopologyFile::ResolvePointersSection(std::ofstream& out)
{
    out << "%FLAG POINTERS" << std::endl << "%FORMAT(10I8)" << std::endl;
...
{
    out << "%FLAG ATOM_NAME" << std::endl << "%FORMAT(20a4)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
...
{
    out << "%FLAG CHARGE" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    ...
    for (int i = 0; i < number_of_residues_; i++)
    {
        ...
        for (unsigned int j = 0; j < residue->GetAtoms().size(); j++)
        {
            ...
            index++;
            std::stringstream ss;
            ss << std::setw(ITEM_LENGTH) << std::right << std::scientific << std::setprecision(8)
               << atom->GetAtomCharge();
...
{
    out << "%FLAG ATOMIC_NUMBER" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
...
{
    out << "%FLAG MASS" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 5;
...
{
    out << "%FLAG ATOM_TYPE_INDEX" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count                              = 0;
    int count                                    = 0;
    int index                                    = 0;
    const int MAX_IN_LINE                        = 10;
...
{
    out << "%FLAG NUMBER_EXCLUDED_ATOMS" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 10;
...
{
    out << "%FLAG NONBONDED_PARM_INDEX" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int index             = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    ...
    for (int i = 0; i < number_of_atoms_; i++)
    {
        ...
        for (int j = 0; j < number_of_atoms_; j++)
        {
            ...
            {
                int nonbond_index =
                    number_of_types_ * (atom_type_index_map[atom_type_1] - 1) + atom_type_index_map[atom_type_2];
...
{
    out << "%FLAG RESIDUE_LABEL" << std::endl << "%FORMAT(20a4)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 20;
    const int ITEM_LENGTH = 4;
...
{
    out << "%FLAG RESIDUE_POINTER" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
...
{
    out << "%FLAG BOND_FORCE_CONSTANT" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG BOND_EQUIL_VALUE" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
void TopologyFile::ResolveAngleForceConstantSection(std::ofstream& out)
{
    ...
    out << "%FLAG ANGLE_FORCE_CONSTANT" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG ANGLE_EQUIL_VALUE" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
    for (int i = 0; i < number_of_angle_types_; i++)
    {
        ...
        if (angle_type != NULL)
        {
            std::stringstream ss;
            ss << std::setw(ITEM_LENGTH) << std::right << std::scientific << std::setprecision(8)
               << angle_type->GetEquilibriumValue();
            std::string sss = ss.str();
            std::transform(sss.begin(), sss.end(), sss.begin(), ::toupper);
            out << sss;
            count++;
...
{
    out << "%FLAG DIHEDRAL_FORCE_CONSTANT" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
    for (int i = 0; i < number_of_dihedral_types_; i++)
    {
        ...
        if (dihedral_type != NULL)
        {
            std::stringstream ss;
            ss << std::setw(ITEM_LENGTH) << std::right << std::scientific << std::setprecision(8)
               << dihedral_type->GetForceConstant();
            std::string sss = ss.str();
            std::transform(sss.begin(), sss.end(), sss.begin(), ::toupper);
            out << sss;
            count++;
...
{
    out << "%FLAG DIHEDRAL_PERIODICITY" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG DIHEDRAL_PHASE" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG SCEE_SCALE_FACTOR" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG SCNB_SCALE_FACTOR" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG SOLTY" << std::endl << "%FORMAT(5E16.8)" << std::endl << std::endl;
    //
}

void TopologyFile::ResolveLennardJonesACoefSection(std::ofstream& out)
{
    out << "%FLAG LENNARD_JONES_ACOEF" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG LENNARD_JONES_BCOEF" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 5;
    const int ITEM_LENGTH = 16;
...
{
    out << "%FLAG BONDS_INC_HYDROGEN" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
    for (TopologyBondMap::iterator it = bonds_.begin(); it != bonds_.end(); it++)
    {
        ...
        {
            int atom_index_1 = gmml::ConvertString<int>(
                gmml::Split(atom_names.at(0), "()")
                    .at(1)); // +
                             // this->assembly_->GetResidueByIndex(gmml::ConvertString<int>(gmml::Split(residue_names.at(0),
                             // "()").at(1)))->GetStartingAtomIndex() - 1;
            // this->assembly_->GetAtomIndexByName(atom_names.at(0),
            // gmml::ConvertString<int>(gmml::Split(residue_names.at(0), "()").at(1)));
            int atom_index_2 = gmml::ConvertString<int>(
                gmml::Split(atom_names.at(1), "()")
                    .at(1)); // +
                             // this->assembly_->GetResidueByIndex(gmml::ConvertString<int>(gmml::Split(residue_names.at(1),
                             // "()").at(1)))->GetStartingAtomIndex() - 1;
            // this->assembly_->GetAtomIndexByName(atom_names.at(1),
            // gmml::ConvertString<int>(gmml::Split(residue_names.at(1), "()").at(1)));
...
{
    out << "%FLAG BONDS_WITHOUT_HYDROGEN" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
...
{
    out << "%FLAG ANGLES_INC_HYDROGEN" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
    for (TopologyAngleMap::iterator it = angles_.begin(); it != angles_.end(); it++)
    {
        ...
        if (angle->GetIncludingHydrogen())
        {
            ...
            //                    this->assembly_->GetResidueByIndex(gmml::ConvertString<int>(gmml::Split(residue_names.at(2),
            //                    "()").at(1)))->GetStartingAtomIndex() - 1;
            // this->assembly_->GetAtomIndexByName(atom_names.at(2),
            // gmml::ConvertString<int>(gmml::Split(residue_names.at(2), "()").at(1)));
...
{
    out << "%FLAG ANGLES_WITHOUT_HYDROGEN" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
...
{
    out << "%FLAG DIHEDRALS_INC_HYDROGEN" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
    for (TopologyDihedralMap::iterator it = dihedrals_.begin(); it != dihedrals_.end(); it++)
    {
        ...
        if (dihedral->GetIncludingHydrogen())
        {
            ...
            //                    this->assembly_->GetResidueByIndex(gmml::ConvertString<int>(gmml::Split(residue_names.at(3),
            //                    "()").at(1)))->GetStartingAtomIndex() - 1;
            // this->assembly_->GetAtomIndexByName(atom_names.at(3),
            // gmml::ConvertString<int>(gmml::Split(residue_names.at(3), "()").at(1)));
...
{
    out << "%FLAG DIHEDRALS_WITHOUT_HYDROGEN" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    const int MAX_IN_LINE = 10;
    const int ITEM_LENGTH = 8;
...
{
    out << "%FLAG EXCLUDED_ATOMS_LIST" << std::endl << "%FORMAT(10I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 10;
...
{
    out << "%FLAG HBOND_ACOEF" << std::endl << "%FORMAT(5E16.8)" << std::endl << std::endl;
    //
}

void TopologyFile::ResolveHydrogenBondBCoefSection(std::ofstream& out)
{
    out << "%FLAG HBOND_BCOEF" << std::endl << "%FORMAT(5E16.8)" << std::endl << std::endl;
    //
}

void TopologyFile::ResolveHBCutSection(std::ofstream& out)
{
    out << "%FLAG HBCUT" << std::endl << "%FORMAT(5E16.8)" << std::endl << std::endl;
    //
}

void TopologyFile::ResolveAmberAtomTypeSection(std::ofstream& out)
{
    out << "%FLAG AMBER_ATOM_TYPE" << std::endl << "%FORMAT(20a4)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 20;
...
{
    out << "%FLAG TREE_CHAIN_CLASSIFICATION" << std::endl << "%FORMAT(20a4)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 20;
...
{
    out << "%FLAG JOIN_ARRAY" << std::endl << "%FORMAT(10I8)" << std::endl;
    //
    // out << std::endl;
    int total_count       = 0;
    int count             = 0;
...
{
    out << "%FLAG IROTAT" << std::endl << "%FORMAT(10I8)" << std::endl;
    // out << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
...
void TopologyFile::ResolveRadiusSetSection(std::ofstream& out)
{
    if (radius_set_.size() != 0)
    {
        ...
        out << "%FLAG RADIUS_SET" << std::endl << "%FORMAT(1a80)" << std::endl << radius_set << std::endl;
    }
    else
    {
        out << "%FLAG RADIUS_SET" << std::endl << "%FORMAT(1a80)" << std::endl << std::endl;
    }
}

void TopologyFile::ResolveRadiiSection(std::ofstream& out)
{
    out << "%FLAG RADII" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 5;
...
{
    out << "%FLAG SOLVENT_POINTERS" << std::endl << "%FORMAT(3I8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
...
{
    out << "%FLAG SCREEN" << std::endl << "%FORMAT(5E16.8)" << std::endl;
    int total_count       = 0;
    int count             = 0;
    int index             = 0;
    const int MAX_IN_LINE = 5;
