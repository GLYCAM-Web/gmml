...
PdbqtModel::PdbqtModel(std::ifstream& model_block)
{
    ...
    // This is the MODEL line if file contains MODEL at all. If not, then the file doesn't contain MODEL, could be
    // ROOT,BRANCH etc. Should the latter be the case, go back one line since the current line contains useful
    // information. It needs to be read again below.
    getline(model_block, line);
    if (line.find("MODEL") != std::string::npos)
    { // If there is MODEL entry
    ...
    { // If there isn't a MODEL entry, the entire file is one model. Also go up one line
        model_serial_number_ = 1;
        int offset           = -1 * ((int)line.length() +
    ...
    while (getline(model_block, line))
    {
        ...
        { // Exit upon ENDMDL is the normal outcome.
            break;
        }
        ...
        { // If not the keywords above, the file stream has gone one line beyond the model block. Go back one line and
          // exit
            // int offset = -1*((int)line.length() +1);  //Rewind file stream postion by length of current line + 1, to
            // go back to the last line. model_block.seekg(offset, model_block.cur);//Go back one line break;
...
void PdbqtModel::Print(std::ostream& out)
{
    ...
    out << std::endl << "====================== Residue Set =====================" << std::endl;
    model_residue_set_->Print(out);
    out << std::endl << "====================== Torsional DOF =====================" << std::endl;
    out << std::endl;
