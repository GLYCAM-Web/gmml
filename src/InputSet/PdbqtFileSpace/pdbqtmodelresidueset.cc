...
PdbqtModelResidueSet::PdbqtModelResidueSet(std::ifstream& residue_set_block)
{
    ...
    // This vector contains atom cards from root and all downstream branches.They are then concatenated into a single
    // atom card and stored in this object.
    ...
    while (getline(residue_set_block, line))
    {
        ...
        // If not ROOT,BRANCH,ENDROOT, or ENDBRANCH, the code has gone one line beyond the residue set section. So go
        // back one line and exit.
    ...
    for (std::vector<PdbqtFileSpace::PdbqtAtomCard*>::iterator it = all_atom_cards.begin(); it != all_atom_cards.end();
         it++)
    {
