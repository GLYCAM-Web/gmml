...
bool PdbqtFile::ParseCards(std::ifstream& in_stream)
{
    ...
    /*if (line.find("MODEL") != std::string::npos || line.find("COMPND") != std::string::npos || line.find("REMARK") !=
       std::string::npos
            || line.find("ROOT") != std::string::npos || line.find("ATOM") != std::string::npos || line.find("ENDROOT")
       != std::string::npos
    ...
    while (getline(in_stream, line))
    {
        ...
        if (record_name.compare("MODEL") == 0 || record_name.compare("ROOT") == 0 ||
            record_name.compare("BRANCH") == 0 || record_name.compare("ATOM") == 0 ||
            record_name.compare("HETATM") == 0 || record_name.compare("COMPND") == 0 ||
            record_name.compare("REMARK") == 0 || record_name.compare("TORSDOF") == 0)
        {
            int offset =
                -1 * ((int)line.length() +
...
bool PdbqtFile::ParseModelCard(std::ifstream& stream, std::string& line)
{
    ...
    while (getline(stream, line))
    {
        ...
        if (line.find("MODEL") != std::string::npos || line.find("COMPND") != std::string::npos ||
            line.find("REMARK") != std::string::npos || line.find("ROOT") != std::string::npos ||
            line.find("ATOM") != std::string::npos || line.find("ENDROOT") != std::string::npos ||
            line.find("BRANCH") != std::string::npos || line.find("ENDBRANCH") != std::string::npos ||
            line.find("HETATM") != std::string::npos || line.find("TORSDOF") != std::string::npos ||
            line.find("ENDMDL") != std::string::npos)
        {
...
void PdbqtFile::ResolveModelCard(std::ofstream& stream)
{
    ...
    for (PdbqtFileSpace::PdbqtModelCard::PdbqtModelMap::iterator it = models.begin(); it != models.end(); it++)
    {
        ...
        for (PdbqtModel::TorsionalDoFCardVector::iterator it1 = tors_dof_vector.begin(); it1 != tors_dof_vector.end();
             it1++)
        {
...
void PdbqtFile::ResolveBranchCards(std::ofstream& stream, PdbqtModelResidueSet::BranchCardVector branches)
{
    for (PdbqtModelResidueSet::BranchCardVector::iterator it = branches.begin(); it != branches.end(); it++)
    {
        ...
        stream << "ENDBRANCH"
               << " " << std::right << std::setw(3) << branch_card->GetSolidAtomSerialNumber() << " " << std::right
               << std::setw(3) << branch_card->GetRotatbleAtomSerialNumber() << std::endl;
    }
}
    ...
    for (PdbqtAtomCard::PdbqtAtomMap::iterator it = atom_map.begin(); it != atom_map.end(); it++)
    {
                ...
                GeometryTopology::Coordinate(gmml::dNotSet, gmml::dNotSet, gmml::dNotSet)) == false)
        {
            stream << std::right << std::setw(8) << std::fixed << std::setprecision(3)
                   ...
                   << std::setprecision(3) << atom->GetAtomOrthogonalCoordinate().GetY() << std::right << std::setw(8)
                   << std::fixed << std::setprecision(3) << atom->GetAtomOrthogonalCoordinate().GetZ();
        }
        else
        {
