...
CondensedSequence::CondensedSequence(std::string sequence)
{
    try
    {
        ...
        { // Also if MD service is requested
            ...
            if (!MD_eligible)
            {
                this->AddNoteToResponse(new Glycan::Note(Glycan::NoteType::WARNING,
                                                         Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE,
                                                         "This sequence is not eligible for MD."));
            }
        }
        DetectAnomericAnomericLinkages();
        if (this->anomeric_anomeric_linkages_.size() > 1)
        {
            // Find the anomeric-anomeric linkage, if there is one.  Name that linkage '0'.  If there is more than one,
            // punt. Throw exception?std::exit(1)?Add error notice? Somehow stop the code from doing anything else.
    ...
    catch (...)
    {
        ...
        this->SetIsSequenceOkay(false);
        // logss << this->GetResponse().GetServiceType() << " : " << this->GetResponse().GetTags().first << " : " <<
        // this->GetResponse().GetTags().second << std::endl; gmml::log(__LINE__, __FILE__, gmml::ERR, logss.str());
...
//     ss << "\tnode [shape=\"none\" fontname=Helvetica labelfontsize=12 forcelabels=\"true\"; label=\"none\" size=50
//     fixedsize=\"true\" scale=\"true\"];" << std::endl; ss << "\tedge [labelfontsize=12 fontname=Helvetica
//     labeldistance=1.2 labelangle = 320.0];" << std::endl; ss << "\trankdir=RL" << std::endl; ss <<
//     "\tnodesep=\"0.01\" ;" << std::endl;
//
...
//		if( std::find( svg_residue_names.begin(), svg_residue_names.end(), residue_name ) !=
// svg_residue_names.end() ) { 				ss << "[label=\"\" image=\"" << configs.svg_directory_path_ <<
// residue_isomer << residue_file_name << ".svg\" height=\"0.7\"]";
//		}
//		// If the residue is not in the LABELS vector, but is in the BOX shape vector.
//		else if( std::find( box_shape_residue_names.begin(), box_shape_residue_names.end(), residue_name ) !=
// box_shape_residue_names.end() ) {
...
//		else if(  std::find( other_shape_residue_names.begin(), other_shape_residue_names.end(), residue_name )
//!= other_shape_residue_names.end() ||
//                   residue_ring_type == "p" ||
//                   residue_ring_type == "f" ) {
...
//			   ss << "fontsize=\"9\",forcelabels=\"true\"; height=\"0.3\"; labelloc=b; label=\"" <<
// derivatives_label.substr(0,8) << "\n" << derivatives_label.substr(9) << "\"]" << std::endl; 		   } else {
// ss << "fontsize=\"12\",forcelabels=\"true\"; height=\"0.3\"; labelloc=b;  label=\"" << derivatives_label << "\"]" <<
// std::endl;
//		   }
//
...
//
//		// We store the CondensedSequenceResidue.configuration_ as UpperCase, so this will convert it to
// LowerCase. 		std::transform(residue_configuration.begin(), residue_configuration.end(),
// residue_configuration.begin(),
//::tolower);
//
...
bool CondensedSequence::ParseSequenceAndCheckSanity(std::string sequence)
{
    ...
    {
        this->AddNoteToResponse(new Glycan::Note(Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE,
                                                 bad_residue_notice));
        return false;
    }
    ...
    else
    {
        ...
        {
            this->AddNoteToResponse(new Glycan::Note(Glycan::NoteType::ERROR,
                                                     Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, bad_residue_notice));
            return false;
        }
        ...
        else
        {
            ...
            {
                this->AddNoteToResponse(new Glycan::Note(
                    Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, bad_residue_notice));
                return false;
            }

            else
            {
                this->AddNoteToResponse(
                    new Glycan::Note(Glycan::NoteType::COMMENT, Glycan::NoteCat::GLYCOSIDIC, good_residue_notice));
                return true;
            }
        }
    }
...
{
    for (CondensedSequenceResidueVector::iterator res_it = this->residues_.begin(); res_it != this->residues_.end();
         res_it++)
    {
        ...
        // Compare with input values. If does not match for isomer and anomeric configuration.Throw warning.For anomeric
        // positon mismatch throw error.
    }
    return true;
}
...
{
    for (CondensedSequenceResidueVector::iterator res_it = this->condensed_sequence_residue_tree_.begin();
         res_it != this->condensed_sequence_residue_tree_.end(); res_it++)
    {
        ...
        for (std::vector<int>::iterator child_it = child_ids.begin(); child_it != child_ids.end(); child_it++)
        {
            ...
            // If child open valence does not exist as key in map, then it's accessing disallowed positions. If that
            // position is allowed by occupied, that's also an error.
        }
    }
    return true;
}

int CondensedSequence::InsertNodeInCondensedSequenceResidueTree(CondensedSequenceResidue* condensed_residue,
                                                                int parent_node_id, int bond_id)
{
    if (parent_node_id >= (int)condensed_sequence_residue_tree_.size())
    {
        std::stringstream note_str;
        note_str << "Invalid parent index at residue " << condensed_residue->GetName() << ", parent index is "
                 << parent_node_id;
        this->AddNoteToResponse(
            new Glycan::Note(Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, note_str.str()));
    ...
    {
        condensed_sequence_residue_tree_[parent_node_id]->AddChildId(
            condensed_sequence_residue_tree_.size()); // Not only child knows parent, parent also knows child.
    }
    condensed_sequence_residue_tree_.push_back(condensed_residue);
    return condensed_sequence_residue_tree_.size() - 1;
}

int CondensedSequence::InsertNodeInCondensedSequenceGlycam06ResidueTree(
    CondensedSequenceSpace::CondensedSequenceGlycam06Residue* condensed_glycam06_residue, int parent_node_id,
    int bond_id)
{
...
int CondensedSequence::BuildArrayTreeOfCondensedSequenceResidue()
{
    ...
    if (residues_.size() == 0)
    {
        return_value = 1;
        this->AddNoteToResponse(new Glycan::Note(Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE,
                                                 "Condensed sequence does not contain any detectable residues."));
    }
    ...
    int bond_count = 0; // Added by Yao 08/03/2018. Whenever a parent-child relationship is found, there is a new bond.
                        // This int count the index of the new bond to be added, and becomes its label.
            ...
            case gmml::CONDENSED_SEQUENCE_RIGHT_BRACKET:
                {
                    ...
                    if (residue_stack.empty())
                    {
                        return_value = 1;
                        this->AddNoteToResponse(new Glycan::Note(
                            Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, "Invalid sequence"));
                        // throw CondensedSequenceProcessingException("Invalid sequence");
                    }
                    if (current_residue != residues_.rend() && !residue_stack.empty())
                    {
                        residue_stack.push(this->InsertNodeInCondensedSequenceResidueTree(
                            *current_residue, residue_stack.top(), bond_count));
                        bond_count++;
                    }
                    break;
                }
            case gmml::CONDENSED_SEQUENCE_RESIDUE:
                {
                    ...
                    if (current_residue != residues_.rend() && !residue_stack.empty())
                    {
                        int parent = this->InsertNodeInCondensedSequenceResidueTree(*current_residue,
                                                                                    residue_stack.top(), bond_count);
                        residue_stack.pop();
...
{
    // Count the residues on either side of '0'.  If one side has more residues, then that side becomes the "left" side.
    // The 'left' side gets the positive integer labels/indexes. If both sides have the same number of residues, the one
    // with more branches goes on the left. If both sides have the same number of residues and branches, then the
    // lowest-numbered anomeric carbon goes on the left. If all are the same still, then it goes alphabetically or by
    ...
    this->RecursivelyCountNumberofDownstreamResiduesAndBranches(current_parent_residue_index, num_residues_1,
                                                                num_branches_1);
    this->RecursivelyCountNumberofDownstreamResiduesAndBranches(the_other_residue_index, num_residues_2,
                                                                num_branches_2);
    ...
    if (reevaluated_parent_index == the_other_residue_index)
    { // If we need to switch parent to the other residue
        CondensedSequenceResidue* current_parent_residue =
            this->condensed_sequence_residue_tree_[current_parent_residue_index];
...
void CondensedSequence::RecursivelyCountNumberofDownstreamResiduesAndBranches(int parent_residue_index,
                                                                              int& num_residues, int& num_branches)
{
    ...
    // Exclude the anomeric-anomeric path. Remove the other residue (not the current parent) in the ano-ano link from
    // child residues, if it exists.
...
bool CondensedSequence::BuildArrayTreeOfCondensedSequenceGlycam06Residue(CondensedSequenceResidueTree residue_tree)
{
    ...
    for (unsigned int i = 0; i < residue_tree.size(); i++)
    {
        ...
        for (CondensedSequenceResidue::DerivativeMap::iterator it = condensed_residue_derivatives.begin();
             it != condensed_residue_derivatives.end(); ++it)
        {
            int derivative_index        = it->first;
            std::string derivative_name = it->second;
            if (derivative_name != "D")
            { // Deoxy shouldn't be considered as open valence
    ...
    {
        this->InsertNodeInCondensedSequenceGlycam06ResidueTree(
            new CondensedSequenceSpace::CondensedSequenceGlycam06Residue(
                this->GetGlycam06TerminalResidueCodeOfTerminalResidue(terminal)),
    ...
    else if (terminal_residue->GetIsTerminalSugar())
    {
        CondensedSequenceSpace::CondensedSequenceGlycam06Residue* tree_residue =
            new CondensedSequenceSpace::CondensedSequenceGlycam06Residue(
                this->GetGlycam06ResidueCodeOfCondensedResidue(terminal_residue, open_valences[0]));
        this->InsertNodeInCondensedSequenceGlycam06ResidueTree(
            tree_residue, gmml::iNotSet,
            gmml::iNotSet); // Giving fake derivative and bond id. They will be deprecated soon anyway.
    }
    ...
    for (unsigned int i = 1; i < residue_tree.size(); i++)
    {
        ...
        (parent_name.compare("OME") == 0)
            ? oxygen_position = "O"
            : oxygen_position = "O" + gmml::ConvertT<int>(condensed_residue->GetOxygenPosition());
        ...
        try
        {
            ...
            CondensedSequenceSpace::CondensedSequenceGlycam06Residue* tree_residue =
                new CondensedSequenceSpace::CondensedSequenceGlycam06Residue(
                    this->GetGlycam06ResidueCodeOfCondensedResidue(condensed_residue, open_valences[i]),
                    anomeric_carbon, oxygen_position);

            int residue_index = this->InsertNodeInCondensedSequenceGlycam06ResidueTree(
                tree_residue, parent + derivatives[parent], glycam_06_residue_bond_id);
                 ...
                 it != condensed_residue_derivatives.end(); ++it)
            {
                ...
                int derivative_index        = it->first;
                this->InsertNodeInCondensedSequenceGlycam06ResidueTree(
                    this->GetCondensedSequenceDerivativeGlycam06Residue(derivative_name, derivative_index),
                    residue_index, glycam_06_residue_bond_id);
        ...
        catch (std::exception ex)
        {
            ...
            this->AddNoteToResponse(new Glycan::Note(Glycan::NoteType::WARNING,
                                                     Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, notice.str()));

            MD_eligible = false;
            // throw CondensedSequenceProcessingException("Invalid residue in the sequence (" +
            // condensed_residue->GetName().substr(0,3) + ")");
        }
    }
...
std::string CondensedSequence::GetGlycam06ResidueCodeOfCondensedResidue(CondensedSequenceResidue* condensed_residue,
                                                                        std::vector<int> open_valences)
{
    ...
    for (unsigned int i = 0; i < open_valences.size(); i++)
    {
        ...
        {
            std::stringstream notice;
            notice << "Not eligible for MD: " << condensed_residue->GetName()
                   << "unsupported open valence position: <0 or >= 10";
            this->AddNoteToResponse(new Glycan::Note(Glycan::NoteType::WARNING,
                                                     Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, notice.str()));
    ...
    std::string residue_code = this->GetFirstLetterOfGlycam06ResidueCode(open_valences_check) +
                               this->GetSecondLetterOfGlycam06ResidueCode(residue_name, isomer);
...
std::string CondensedSequence::GetFirstLetterOfGlycam06ResidueCode(std::bitset<10> open_valences_check)
{
    ...
    std::string notice = "Not eligible for MD: There is no code in the GLYCAM code set for residues with open valences "
                         "at the given positions.";
    this->AddNoteToResponse(
        new Glycan::Note(Glycan::NoteType::WARNING, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, notice));

    // throw CondensedSequenceProcessingException("There is no code in the GLYCAM code set for residues with open
...
std::string CondensedSequence::GetSecondLetterOfGlycam06ResidueCode(std::string residue_name, std::string isomer)
{
    ...
    std::stringstream notice;
    notice << "Not eligible for MD: " << residue_name
           << "This residue name has no corresponding entry in glycam06 force field.";
    this->AddNoteToResponse(
        new Glycan::Note(Glycan::NoteType::WARNING, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, notice.str()));
...
CondensedSequence::GetCondensedSequenceDerivativeGlycam06Residue(std::string derivative_name, int derivative_index)
{
    // Oxygen names are usually OK, but why does an SO3 have a C atom? Here basicaaly oxygen_name = tail atom name,
    // carbon_name = head atom name. Right now, the carbon_name has to be hard-coded.
    ...
    {
        // D means deoxy.This derivative is not a template, but an action, of removing the oxygen this derivative
        // attaches to. Here I create a false glycam06 residue for this purpose later on.
    ...
    std::stringstream notice;
    notice << "Not eligible for MD: " << derivative_name
           << "This derivative name has no corresponding entry in glycam06 force field.";
    this->AddNoteToResponse(
        new Glycan::Note(Glycan::NoteType::WARNING, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, notice.str()));
    // throw CondensedSequenceProcessingException("There is no derivative in the GLYCAM code set represented by the
    // letter " + derivative_name);
    //  If none of the above, return something:
    //    std::cout << "WARNING: There is no derivative in the GLYCAM code set represented by the letter " <<
    //    derivative_name << "\n";
    return new CondensedSequenceSpace::CondensedSequenceGlycam06Residue("UNK", "UNK", oxygen_name, true);
}

std::string CondensedSequence::BuildLabeledCondensedSequence(CondensedSequence::Reordering_Approach labeling_approach,
                                                             CondensedSequence::Reordering_Approach reordering_approach,
                                                             bool label)
{
    // WARNING: preserve user input option doesn't work well with the labeing function, but okay with reordering
    // function.
    std::string labeled_sequence = "";
    int reevaluated_parent_index = 0; // The default parent in the input sequence is 0
    int branch_depth             = 0;
    std::vector<int> longest_path; // Elements in this vector is the residue ids on the longest path. Will be used or
                                   // ignored based on reordering approach. See below.
    ...
    if (reordering_approach != CondensedSequence::Reordering_Approach::PRESERVE_USER_INPUT &&
        this->anomeric_anomeric_linkages_.size() == 1)
    {
        reevaluated_parent_index = this->ReEvaluateParentIdentityUponAnomericAnomericLinkage();
    }

    if (labeling_approach == CondensedSequence::Reordering_Approach::LONGEST_CHAIN ||
        reordering_approach == CondensedSequence::Reordering_Approach::LONGEST_CHAIN)
    {
    ...
    if (label)
    {
        ...
        RecursivelyLabelCondensedSequence(reevaluated_parent_index, current_residue_label_index,
                                          current_bond_label_index, residue_label_map, bond_label_map,
                                          labeling_approach, longest_path, condensed_residue_derivative_res_label_map,
                                          condensed_residue_derivative_bond_label_map, rearranged_tree_by_labeling,
                                          rearranged_06_tree_by_labeling, condensed_06_index_map);
    }

    this->RecursivelyBuildLabeledCondensedSequence(reevaluated_parent_index, branch_depth, labeled_sequence,
                                                   reordering_approach, residue_label_map, bond_label_map, longest_path,
                                                   label, condensed_residue_derivative_res_label_map,
                                                   condensed_residue_derivative_bond_label_map);
    // Starts with the first condensed residue in the vector, which is the aglycone. Then go recursively down the tree.
    // Now rearrange the residue tree and 06 residue tree by labeling order. In the future user should be able to choose
    // whether to go with labeling order or reordering order.
...
void CondensedSequence::FindLongestPath(std::vector<int>& longest_path)
{
    for (unsigned int i = 0; i < this->condensed_sequence_residue_tree_.size(); i++)
    {
        ...
        {
            CondensedSequenceSpace::CondensedSequenceResidue* current_residue =
                this->condensed_sequence_residue_tree_[i]; // This finds a non reducing end residue;
            std::vector<int> current_path = std::vector<int>();
            current_path.push_back(i); // i is the index of this non reducing end residue

            while (current_residue->GetParentId() != gmml::iNotSet)
            { // While loop stops at a residue that has no parent, which is the aglycone.
                ...
                if (current_residue->GetParentId() == gmml::iNotSet)
                { // If the aglycone is reached.
                    bool new_path_found = false;
                    if (current_path.size() > longest_path.size())
                    { // If a longer path is found, overwrite the current longest path.
                        new_path_found = true;
                    }
                    else if (current_path.size() == longest_path.size())
                    { // If the new path is exactly the same length, choose based on lower branch index at the first
                      // deverging point.
                        CondensedSequenceSpace::CondensedSequenceResidue* branching_residue = NULL;
                        for (int j = current_path.size() - 1; j >= 0; j--)
                        { // reverse iterating the path to find the first branching point.
                        ...
                        {
                            //                            std::cout << "Two paths at equal lengths are found. Can't
                            //                            decide which residue has lower branch index.This is a bug." <<
                            //                            std::endl;
                        }
                        else
                        {
...
void CondensedSequence::RecursivelyLabelCondensedSequence(
    int current_residue_index, int& current_residue_label_index, int& current_bond_label_index,
    ...
    std::map<int, std::map<int, std::string>>& condensed_residue_derivative_bond_label_map,
    CondensedSequenceResidueTree& rearranged_tree_by_labeling,
    CondensedSequenceGlycam06ResidueTree& rearranged_06_tree_by_labeling, std::map<int, int>& condensed_06_index_map)
{
    CondensedSequenceSpace::CondensedSequenceResidue* current_residue =
        this->condensed_sequence_residue_tree_[current_residue_index];
    ...
    CondensedSequenceSpace::CondensedSequenceGlycam06Residue* corresponding_06_residue =
        this->condensed_sequence_glycam06_residue_tree_[corresponding_06_residue_index];
    rearranged_06_tree_by_labeling.push_back(corresponding_06_residue);

    int this_residue_new_06_tree_id = rearranged_06_tree_by_labeling.size() -
                                      1; // When each residue just gets added to new vector, its index is size()-1
    // This section does not sort the derivatives. To do this, comment out this section and uncommment the other section
    // below.
    /*for (std::map<int, std::string>::iterator mapit =  this_residue_derivatives.begin(); mapit !=
    this_residue_derivatives.end(); mapit++){ int nth_derivative = std::distance(this_residue_derivatives.begin(),
    mapit) + 1; CondensedSequenceSpace::CondensedSequenceGlycam06Residue* derivative_06_residue =
    this->condensed_sequence_glycam06_residue_tree_[corresponding_06_residue_index + nth_derivative];
    ...
    if (!(current_residue->GetIsTerminalAglycone() || current_residue->GetIsTerminalSugar()))
    { // Only create bond label for non-terminal/aglycone residue.
        bond_label.insert(0, ";");
    ...
    std::vector<int> child_ids = current_residue->GetChildIds(); // Get the index of child residues. Sort this vector
                                                                 // based on the reordering approach below.
    std::vector<std::pair<int, int>>
        linkage_index_child_pairs; // Pair first is the linkage index, pair second is condensed residue index, or -999
                                   // indicating a derivative.

    if (labeling_approach == CondensedSequence::Reordering_Approach::LOWEST_INDEX)
    {
        // If sequence is to be reordered based on index, rearrange child in ascending order based on parent open valene
        // position.Use a temporary map for sorting.
        std::map<int, int, std::less<int>> openvalence_childid_map = std::map<int, int, std::less<int>>();
        for (std::vector<int>::iterator it = child_ids.begin(); it != child_ids.end(); it++)
        {
            int child_id = *it;
            CondensedSequenceSpace::CondensedSequenceResidue* child_residue =
                this->condensed_sequence_residue_tree_[child_id];
            int open_valence_position                      = child_residue->GetOxygenPosition();
            openvalence_childid_map[open_valence_position] = child_id;
        }

        for (std::map<int, std::string>::iterator mapit = this_residue_derivatives.begin();
             mapit != this_residue_derivatives.end(); mapit++)
        {
            openvalence_childid_map[mapit->first] = -999;
        }

        // child_ids.clear();
        for (std::map<int, int>::iterator map_it = openvalence_childid_map.begin();
             map_it != openvalence_childid_map.end(); map_it++)
        {
    ...
    {
        // If sequence is to be reordered based on longest chain, sort main chain child that's on the predetermined
        // longest path to the front of vector, preserve the order of the rest
        for (std::vector<int>::iterator it = child_ids.begin(); it != child_ids.end(); it++)
        {
            int child_id              = *it;
        ...
        for (std::vector<int>::iterator it = child_ids.begin(); it != child_ids.end(); it++)
        {
            ...
            if (std::find(longest_path.begin(), longest_path.end(), child_id) != longest_path.end())
            {
                ...
                for (std::vector<std::pair<int, int>>::iterator pairs_it = linkage_index_child_pairs.begin();
                     pairs_it != linkage_index_child_pairs.end(); pairs_it++)
                {
    ...
    for (std::vector<std::pair<int, int>>::iterator pair_it = linkage_index_child_pairs.begin();
         pair_it != linkage_index_child_pairs.end(); pair_it++)
    {
        if (pair_it->second != -999)
        { // If not a derivative, if is a sugar
            int child_sugar_index      = pair_it->second;
            int child_06_residue_index = condensed_06_index_map[child_sugar_index];
            CondensedSequenceSpace::CondensedSequenceGlycam06Residue* child_06_residue =
                this->condensed_sequence_glycam06_residue_tree_[child_06_residue_index];

            child_06_residue->SetParentId(this_residue_new_06_tree_id);

            this->RecursivelyLabelCondensedSequence(
                child_sugar_index, current_residue_label_index, current_bond_label_index, residue_label_map,
                bond_label_map, labeling_approach, longest_path, condensed_residue_derivative_res_label_map,
                condensed_residue_derivative_bond_label_map, rearranged_tree_by_labeling,
                rearranged_06_tree_by_labeling, condensed_06_index_map);
        }
        else
        { //-999 means that it is a derivative. In this case, generate labels and rearrange 06 tree here in place
            ...
            derivative_residue_label.insert(0,
                                            std::to_string(current_residue_label_index)); // Add residue index to label.
            ...
            condensed_residue_derivative_res_label_map[current_residue_index][derivative_linkage_index] =
                derivative_residue_label;
            current_residue_label_index++;
            ...
            // Later declare derivative label structure, add this label to struct.
            condensed_residue_derivative_bond_label_map[current_residue_index][derivative_linkage_index] =
                derivative_bond_label;

            // To also sort the derivatives like regular residues, uncomment this section and comment out the section
            // above.
            for (std::map<int, std::string>::iterator mapit = this_residue_derivatives.begin();
                 mapit != this_residue_derivatives.end(); mapit++)
            {
                ...
                {
                    CondensedSequenceSpace::CondensedSequenceGlycam06Residue* derivative_06_residue =
                        this->condensed_sequence_glycam06_residue_tree_[corresponding_06_residue_index +
                                                                        nth_derivative];
                    rearranged_06_tree_by_labeling.push_back(derivative_06_residue);
                    derivative_06_residue->SetParentId(this_residue_new_06_tree_id);
...
void CondensedSequence::RecursivelyBuildLabeledCondensedSequence(
    int current_index, int& branch_depth, std::string& labeled_sequence,
    CondensedSequence::Reordering_Approach reordering_approach, std::map<unsigned int, std::string>& residue_label_map,
    std::map<unsigned int, std::string>& bond_label_map, std::vector<int>& longest_path, bool label,
    std::map<int, std::map<int, std::string>>& condensed_residue_derivative_res_label_map,
    std::map<int, std::map<int, std::string>>& condensed_residue_derivative_bond_label_map)
{
    CondensedSequenceSpace::CondensedSequenceResidue* current_residue =
        this->condensed_sequence_residue_tree_[current_index];

    std::map<int, std::string> this_residue_derivatives = current_residue->GetDerivatives();

    // Then rearrange possible child derivatives together with this 06 parent residue.

    if (current_residue->GetIsTerminalAglycone())
    { // If current residue is aglycone
    ...
    else
    {
        ...
        // Done bond label
        /*if (current_residue->GetIsTerminalSugar()){ //If current residue is terminal sugar (i.e. anomeric-anomeric
        linkage) labeled_sequence.insert(0, boost::algorithm::to_lower_copy(current_residue->GetConfiguration()));
            labeled_sequence.insert(0, current_residue->GetName());
            labeled_sequence.insert(0, current_residue->GetIsomer());
        }*/

        // The residues connected to the aglycone has OxygenPosition set to 1, actually there shouldn't be such a
        // value.So ignore. else if
        // (this->condensed_sequence_residue_tree_[current_residue->GetParentId()]->GetIsTerminalSugar()){
        // }

        if (!current_residue->GetIsTerminalSugar())
        {
            CondensedSequenceSpace::CondensedSequenceResidue* current_residue_parent =
                this->condensed_sequence_residue_tree_[current_residue->GetParentId()];
            // The residues connected to the aglycone has OxygenPosition set to 1, actually there shouldn't be such a
            // value.So ignore.
        ...
        labeled_sequence.insert(0, boost::algorithm::to_lower_copy(
                                       current_residue->GetConfiguration())); // In residue class anomeric configuration
        ...
        {
            labeled_sequence.insert(0, "]");
            for (CondensedSequenceResidue::DerivativeMap::reverse_iterator derivative_rit = derivatives.rbegin();
                 derivative_rit != derivatives.rend(); derivative_rit++)
            {
                // Insert derivative residue label
                labeled_sequence.insert(
                    0, condensed_residue_derivative_res_label_map[current_index][derivative_rit->first]);
                // std::cout << "deri res label: " <<
                // condensed_residue_derivative_res_label_map[current_index][derivative_rit->first] << std::endl;
                labeled_sequence.insert(0, derivative_rit->second);
                // Insert derivative bond label
                labeled_sequence.insert(
                    0, condensed_residue_derivative_bond_label_map[current_index][derivative_rit->first]);
                // std::cout << "deri bond label: " <<
                // condensed_residue_derivative_bond_label_map[current_index][derivative_rit->first] << std::endl;
    ...
    {
        // If sequence is to be reordered based on index, rearrange child in descending order based on parent open
        // valene position.Use a temporary map for sorting.
    ...
    {
        // If sequence is to be reordered based on longest chain, sort main chain child on the longest chain to the end
        // of vector, preserve the order of the rest
        for (std::vector<int>::iterator it = child_ids.begin(); it != child_ids.end(); it++)
        {
            int child_id = *it;
            if (std::find(longest_path.begin(), longest_path.end(), child_id) != longest_path.end())
    ...
    if (child_ids.size() == 0 && branch_depth > 0)
    { // No child means we're at a non-reducing end residue. if at a branch, add left bracket [
    ...
    for (std::vector<int>::iterator it = child_ids.begin(); it != child_ids.end(); it++)
    {
        ...
        this->RecursivelyBuildLabeledCondensedSequence(
            *it, branch_depth, labeled_sequence, reordering_approach, residue_label_map, bond_label_map, longest_path,
            label, condensed_residue_derivative_res_label_map, condensed_residue_derivative_bond_label_map);
    }
}

CondensedSequence::CondensedSequenceRotamersAndGlycosidicAnglesInfo
CondensedSequence::GetCondensedSequenceRotamersAndGlycosidicAnglesInfo(CondensedSequenceResidueTree residue_tree)
{
    CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles =
        CondensedSequenceRotamersAndGlycosidicAnglesInfo();
    // int linkage_index = 0;
    ...
    for (unsigned int i = 0; i < residue_tree.size(); i++)
    {
        ...
        if (parent >= 0)
        {
            ...
            std::vector<std::pair<std::string, std::vector<std::string>>> possible_rotamers =
            ...
            if (ring_letter == 'p')
            {
                if (parent >= 0) // For the first sugar residue, its parent is the aglycon,whose index is 0. So parent >
                                 // 0 prevents addition of 1st sugar-aglycon bond into vector.
                {

                    CondensedSequenceResidue* parent_residue = residue_tree.at(parent);
                    std::string parent_residue_absolute_name = "";
                    // If parent is something like an aglycon, OH, for example, its residue name is too short for
                    // substr(4), which causes segfault. Below I added an if statement that if length of parent residue
                    // name is lower than 5, copy parent residue name. Otherwise, do the regular substring manipulation.
                    ...
                    {
                        parent_residue_absolute_name =
                            parent_residue->GetName().substr(0, 3) + parent_residue->GetName().substr(4);
                    }
                    std::stringstream rotamers_name;
                    rotamers_name << residue->GetIsomer() << residue->GetName() << residue->GetConfiguration()
                                  << residue->GetAnomericCarbon() << "-" << residue->GetOxygenPosition()
                                  << parent_residue->GetIsomer() << parent_residue->GetName()
                                  << parent_residue->GetConfiguration();
                    ...
                    RotamersAndGlycosidicAnglesInfo* info = new RotamersAndGlycosidicAnglesInfo(
                        linkage_index, possible_rotamers, selected_rotamers, enabled_glycosidic_angles);
                    RotamerNameInfoPair pair_info = std::make_pair(rotamers_name.str(), info);

                    rotamers_glycosidic_angles.push_back(pair_info);
                }

                std::vector<std::pair<std::string, std::vector<std::string>>> der_possible_rotamers =
                    std::vector<std::pair<std::string, std::vector<std::string>>>();
                std::vector<std::pair<std::string, std::vector<std::string>>> der_selected_rotamers =
                    std::vector<std::pair<std::string, std::vector<std::string>>>();
                std::vector<std::pair<std::string, double>> der_enabled_glycosidic_angles =
                    std::vector<std::pair<std::string, double>>();
                ...
                for (CondensedSequenceResidue::DerivativeMap::iterator it = derivatives.begin();
                     it != derivatives.end(); it++)
                {
                    int derivative_index        = (*it).first;
                    std::string derivative_name = (*it).second;
                    switch (gmml::ResidueNameIndexLookup(residue_absolute_name).index_)
                    ...
                    RotamersAndGlycosidicAnglesInfo* der_info = new RotamersAndGlycosidicAnglesInfo(
                        linkage_index, der_possible_rotamers, der_selected_rotamers, der_enabled_glycosidic_angles);
...
int CondensedSequence::CountAllPossibleSelectedRotamers(
    CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info)
{
    ...
    {
        std::vector<std::pair<std::string, double>> angles =
            rotamers_glycosidic_angles_info.at(i).second->enabled_glycosidic_angles_;
        std::vector<std::pair<std::string, std::vector<std::string>>> selected_rotamers =
            rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_;
        ...
        if (angles.at(0).second == gmml::dNotSet)
        { // Phi
...
int CondensedSequence::CountAllPossible28LinkagesRotamers(
    CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info)
{
    int count = 1;
    for (unsigned int i = 0; i < rotamers_glycosidic_angles_info.size(); i++)
...
std::vector<std::vector<int>> CondensedSequence::CreateBaseMapAllPossibleSelectedRotamers(
    CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info)
{
    ...
    CondensedSequenceRotamersAndGlycosidicAnglesInfo rotamers_glycosidic_angles_info, IndexNameMap& names)
{
    ...
    std::vector<std::vector<int>> mapping =
        this->CreateBaseMapAllPossibleSelectedRotamers(rotamers_glycosidic_angles_info);
    std::vector<std::vector<std::vector<double>>> phi_psi_omega_vector_map =
        std::vector<std::vector<std::vector<double>>>();
    std::vector<std::vector<std::vector<std::string>>> phi_psi_omega_rt_vector_map =
    ...
    for (unsigned int i = 0; i < mapping.size(); i++)
    {
        ...
        if (rotamers_glycosidic_angles_info.at(i).second->enabled_glycosidic_angles_.at(0).second ==
            gmml::dNotSet) // value not set
        {
            ...
            else if (rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.size() ==
                     0) // no selected rotamer
            {
                phi.push_back(gmml::dNotSet);
                phi_rt.push_back("df");
            }
            else
            {
                ...
                for (unsigned int j = 0; j < rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.size();
                     j++)
                {
                    if (rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.at(j).first.compare("phi") ==
                        0)
                    {
                        ...
                        else
                        {
                            for (unsigned int k = 0;
                                 k <
                                 rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.at(j).second.size();
                                 k++)
                            {
                                if (rotamers_glycosidic_angles_info.at(i)
                                        .second->selected_rotamers_.at(j)
                                        .second.at(k)
                                        .compare("t") == 0)
                                {
                                    phi.push_back(180.0);
                                ...
                                else if (rotamers_glycosidic_angles_info.at(i)
                                             .second->selected_rotamers_.at(j)
                                             .second.at(k)
                                             .compare("g") == 0)
                                {
        ...
        {
            if (rotamers_glycosidic_angles_info.at(i).second->enabled_glycosidic_angles_.at(2).second ==
                gmml::dNotSet) // value not set
            {
                if (rotamers_glycosidic_angles_info.at(i).second->possible_rotamers_.size() ==
                    0) // no possible rotamer, never happens
                {
                ...
                else
                {
                    ...
                    for (unsigned int j = 0;
                         j < rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.size(); j++)
                    {
                        if (rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.at(j).first.compare(
                                "omega") == 0)
                        {
                            if (rotamers_glycosidic_angles_info.at(i).second->selected_rotamers_.at(j).second.size() ==
                                0)
                            {
                                omega.push_back(180.0);
                                omega_rt.push_back("df");
                            ...
                            {
                                for (unsigned int k = 0; k < rotamers_glycosidic_angles_info.at(i)
                                                                 .second->selected_rotamers_.at(j)
                                                                 .second.size();
                                     k++)
                                {
                        ...
                        rotamer_name.find("DNeup5AcB2-8") != std::string::npos)
                    {
                        for (int m = 0; m < 6; m++)
                        {
                            ...
                            for (int n = 0; n < 5; n++)
                            {
                                ...
                                new_combination_rt.push_back("E" + gmml::ConvertT<int>(m) + "/" +
                                                             gmml::ConvertT<int>(n));
                            }
                    ...
                    else if (rotamer_name.find("DNeup5GcA2-8") != std::string::npos)
                    {
                        for (int m = 0; m < 5; m++)
                        {
                            ...
                            for (int n = 0; n < 5; n++)
                            {
                                ...
                                new_combination_rt.push_back("I" + gmml::ConvertT<int>(m) + "/" +
                                                             gmml::ConvertT<int>(n));
                            }
                            phi_psi_omega.push_back(new_combination);
                            phi_psi_omega_rt.push_back(new_combination_rt);
    ...
    for (unsigned int i = 0; i < mapper.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < mapper[i].size(); j++)
        {
            for (unsigned int k = 0; k < mapper[i].at(j).size(); k++)
            {
                if (name_mapper[i].at(j).at(k).compare("df") != 0 && name_mapper[i].at(j).at(k).compare("cu") != 0 &&
