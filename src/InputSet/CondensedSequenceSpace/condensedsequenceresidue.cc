                                                   ...
                                                   CondensedSequenceSpace::CondensedSequence* condensed_sequence)
{
    ...
    if (dash_index == std::string::npos) // If there is no dashes, then this must be a terminal residue, which is either
                                         // an aglycone or a sugar bearing ano-ano linkage.
    {
        ...
        {
            // In this case, see if the 2nd character is ]. If so, ignore this character.3rd becomes isomer, 4th to
            // (n-1)th becomes residue name, last char becomes configuration.
            if (residue_string.substr(1, 1) == "]")
            {
                ...
                else
                {
                    ...
                    condensed_sequence->AddNoteToResponse(new Glycan::Note(
                        Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, error_notice.str()));
                ...
                {
                    std::stringstream error_notice;
                    error_notice << "ERROR at residue " << residue_string
                                 << ": invalid anomeric configuration, must be either a/b/x";
                    condensed_sequence->AddNoteToResponse(new Glycan::Note(
                        Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, error_notice.str()));
            ...
            { // If residue_string does not contain "]",for example 1DFrufb
    ...
    else
    {
        ...
        if (residue_string[residue_string.length() - 1] == '-')
        { // DGlcpa1-ROH, the token before aglycone, DGlcpa1- , length is 8
            minimum_token_length = 8;
        }

        if (residue_string.length() < minimum_token_length)
        { // Expected 9 chars for a standard non-terminal token, for example, DGlcpa1-4

            std::string error_notice = "Truncated residue token detected: ";
            error_notice             += residue_string;
            condensed_sequence->AddNoteToResponse(
                new Glycan::Note(Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, error_notice));
        ...
        else
        {
            ...
            else
            {
                ...
                if (!std::isdigit(residue_string[dash_index - 1]))
                {
                    ...
                    std::stringstream error_notice;
                    error_notice << "ERROR at residue " << residue_string
                                 << ": non-numeric anomeric position, usually 1 for aldose and 2 for ketose";
                    condensed_sequence->AddNoteToResponse(new Glycan::Note(
                        Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, error_notice.str()));
                    // throw CondensedSequenceProcessingException("Invalid residue in sequence");
                }
                ...
                {
                    this->anomeric_carbon_ =
                        gmml::ConvertString<int>(gmml::ConvertT<char>(residue_string[dash_index - 1]));
                }
                ...
                else
                {
                    ...
                    std::stringstream error_notice;
                    error_notice << "WARNING at residue " << residue_string
                                 << ": unresolved open valence position, since not exactly 1 character found after "
                                    "dash.Setting to default value 1.";
                    error_notice << "Ignore this warning if this residue is connected to the aglycone";
                    condensed_sequence->AddNoteToResponse(new Glycan::Note(
                        Glycan::NoteType::WARNING, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE, error_notice.str()));
                ...
                else
                {
                    ...
                    for (std::vector<std::string>::iterator it = derivatives_tokens.begin();
                         it != derivatives_tokens.end(); ++it)
                    {
                        if (!std::isdigit(it->at(0)))
                        {
                            ...
                            std::stringstream error_notice;
                            error_notice << "ERROR at residue " << residue_string << ": Illegal derivative position at "
                                         << *it << " ,derivative postion must be an integer.";
                            condensed_sequence->AddNoteToResponse(
                                new Glycan::Note(Glycan::NoteType::ERROR, Glycan::NoteCat::IMPROPER_CONDENSED_SEQUENCE,
                                                 error_notice.str()));
                            // throw CondensedSequenceProcessingException("Invalid derivative position in sequence");
                        }
                        else
                        {
                            this->derivatives_[gmml::ConvertString<int>(gmml::ConvertT<char>(it->at(0)))] =
                                it->substr(1);
                        }
                    }
                }
