...
void CoordinateFile::Read(std::ifstream& in_file)
{
    ...
    while (!gmml::Trim(line).empty()) /// Read until the end of the file
    {
        ...
        switch (vectorTokens.size())
        {
            ...
            case 3:
                coordinates_.push_back(new GeometryTopology::Coordinate(
                    gmml::ConvertString<double>(vectorTokens.at(0)), gmml::ConvertString<double>(vectorTokens.at(1)),
            ...
            case 6:
                coordinates_.push_back(new GeometryTopology::Coordinate(
                    ...
                    gmml::ConvertString<double>(vectorTokens.at(2))));
                coordinates_.push_back(new GeometryTopology::Coordinate(
                    gmml::ConvertString<double>(vectorTokens.at(3)), gmml::ConvertString<double>(vectorTokens.at(4)),
...
void CoordinateFile::Print(std::ostream& out)
{
    ...
    for (unsigned int i = 0; i < coordinates_.size(); i++)
    {
        out << (i + 1) << ". " << coordinates_.at(i)->GetX() << ", " << coordinates_.at(i)->GetY() << ", "
            << coordinates_.at(i)->GetZ() << std::endl;
    }
}
