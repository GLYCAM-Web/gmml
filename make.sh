...
#on our test files, also it prevents cmake from genning a makefile
#that tells the linter to expect some gmml_wrapped.cxx file (its
#an intermediate file that is made for wrapping into python, if 
#we dont add the skip the tooling will try to apply to the generated
...
if [ "${CLEAN}" == "1" ]; then
	...
	if [ -d "./cmakeBuild" ]; then
		echo "Removing the auto generated cmakeBuild folder"
		rm -rf ./cmakeBuild
	fi
	if [ -d "./lib" ]; then
        echo "Removing the auto generated lib folder"
		rm -rf ./lib
	fi
...
#in that directory.
cmake "${CMAKE_BUILD_TYPE_FLAG}" -S . -B ./cmakeBuild -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DTESTING="${AUTO_TESTIN_TIME}" \
