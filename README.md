...
TL;DR - **Do not just fire off the `updateCmakeFiles.sh` in order to try and fix a compile issue, and if you do fire off the script take note of what has changed and ensure that the changes make sense by git-diffing the file and reading each changed line. Sometimes there can be files that are generated from a bug, which has happened before, and by just calling the `updateCmakeFiles.sh` and adding those generated files/folders to the file-lists then commiting can break compilation and end up breaking `GMML` for everyone and do so in a manner that also introduces extra files to our repo that should never be there. Whenever you run the `updateCmakeFiles.sh` you must run a `git diff`, or something similar, and ensure all changed lines actually make sense. If this ends up becomming an issue we will be forced to remove the workaround in order to increase code stability.**
